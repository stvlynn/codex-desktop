...
export function startAppgenConversationS(yarn: unknown) {
  let {
    className,
    viewMode,
    ...rest
  } = yarn;
  let zephyr = viewMode === "list" ? "text-sm leading-5" : "text-xs leading-[18px]",
    acorn = IntlProvider("truncate font-medium text-token-foreground", zephyr, className);
  return <div className={acorn} {...rest} />;
}
export function startAppgenConversationY(bloom: unknown) {
  let {
    className,
    ...rest
  } = bloom;
  let coral = IntlProvider("text-xs leading-[18px] text-token-text-secondary", className);
  return <div className={coral} {...rest} />;
}
