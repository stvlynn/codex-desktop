...
export function setTransferBrowserTabsToLocalConversationPeers(
  next: TransferBrowserTabsToLocalConversationPeers,
): void {
  peers = next;
}
...
export function transferBrowserTabsToLocalConversation(
  ...
  if (peers == null) {
    throw new Error(
      "transferBrowserTabsToLocalConversation peers are not configured",
    );
  }
  ...
  (peers.Ixo(e, peers.eO, t.tabs),
    peers.Ixo(e, peers.tO, t.tabs),
    peers.Lxo(e, t),
    e.set(peers.zxo, e.value.conversationId, null));
}
