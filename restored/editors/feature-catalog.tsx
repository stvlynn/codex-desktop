...
export function featureCatalogI({
  ...
  if (alpha.useEffect(() => {
    open && (_featureCatalogT.current?.focus(), _featureCatalogT.current?.select());
  }, [focusToken, open]), !open) return null;
  let _featureCatalogN = summary.length > 0 && summary !== "No results";
  return <div data-testid="popcorn-find-bar" className="border-token-border-light bg-token-bg-primary pointer-events-auto absolute end-3 top-3 z-40 flex h-10 items-center gap-2 rounded-xl border px-2 shadow-lg"><div className="border-token-border-light bg-token-bg-primary flex h-8 items-center rounded-lg border"><input ref={_featureCatalogT} value={query} onChange={event => onQueryChange(event.target.value)} onKeyDown={event => {
        ...
        event.key === "Escape" && (event.preventDefault(), _featureCatalogA());
      }} placeholder="Find" aria-label="Find" className="text-token-text-primary placeholder:text-token-text-tertiary h-8 w-[220px] rounded-lg border-0 bg-transparent px-2 text-sm outline-none" /></div><div className={IntlProvider("min-w-[64px] text-right text-xs", _featureCatalogN ? "text-token-text-secondary" : "text-token-text-tertiary")}>{summary}</div><button type="button" aria-label="Previous search result" className="text-token-text-primary hover:bg-token-bg-secondary inline-flex size-8 items-center justify-center rounded-md disabled:cursor-default disabled:opacity-40" disabled={!_featureCatalogN} onClick={onNavigatePrevious}>{<RemoteTextEditSessionLowerC className="size-3.5" />}</button><button type="button" aria-label="Next search result" className="text-token-text-primary hover:bg-token-bg-secondary inline-flex size-8 items-center justify-center rounded-md disabled:cursor-default disabled:opacity-40" disabled={!_featureCatalogN} onClick={_featureCatalogI}>{<RemoteTextEditSessionLowerS className="size-3.5" />}</button><button type="button" aria-label="Close find" className="text-token-text-primary hover:bg-token-bg-secondary inline-flex size-8 items-center justify-center rounded-md" onClick={_featureCatalogA}>{<RemoteTextEditSessionLowerO className="size-4" />}</button></div>;
}
var alpha, bravo;
...
export function featureCatalogN() {
  return delta.map(item => [copper(item.artifact), item.area, item.feature, item.summary]);
}
