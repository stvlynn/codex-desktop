...
function bloom(deltaR4, echoR4, falconR4, gammaR4, harborR4, indigoR4) {
  if (
    ((deltaR4.bodyElements = deltaR4.bodyElements.filter(
  ...
  xenonR21.textHeightPx <= 0 ||
    deltaR4.bodyElements.push(
      acorn(
        xenonR21.element,
        "text",
        gammaR4.contentLeftPx,
        gammaR4.contentBottomPx - xenonR21.textHeightPx,
        gammaR4.contentWidthPx,
        xenonR21.textHeightPx,
        harborR4,
        indigoR4,
        xenonR21.lines,
        {
...
function coral(
  ...
  let eagleR25 = false;
  for (let zincR14 of prismR3(kiteR8, marbleR8, nickelR8, jadeR8))
    indigoR8.idSet.has(zincR14) ||
      (indigoR8.idSet.add(zincR14),
      indigoR8.ids.push(zincR14),
      (eagleR25 = true));
  ...
  for (let [questR20, ridgeR20] of harborR13.entries()) {
    let apexR20 = apexR2(ridgeR20),
      brookR20 = reef(
        ...
        apexR20,
        oliveR13,
        daisyR13.contentWidthPx,
        vaporR13,
      ),
      cliffR20 =
        apexR20 === "table"
          ? (gamma(ridgeR20, daisyR13) ??
            lunarR2(ridgeR20, daisyR13.contentLeftPx))
          : deltaR3(apexR20)
            ? (gamma(ridgeR20, daisyR13) ??
              xenonR2(ridgeR20, daisyR13.contentLeftPx))
            : daisyR13.contentLeftPx,
      duskR20 =
        apexR20 === "text"
          ? nickelR20
          : kiteR3(
...
function $i(jewelR5, unityR5, isleR5 = {}) {
  let lemonR19 = jewelR5.pageSetup?.widthEmu || unityR5.widthEmu || apexR18,
    ...
    quartzR19 =
      isleR5.layoutMode === "pageless"
        ? Math.max(pearlR19, nickelR19)
        : pearlR19,
  ...
  return {
    ...
    key: [
      isleR5.layoutMode ?? "paged",
      lemonR19,
      isleR5.layoutMode === "pageless" ? quartzR19 : marbleR19,
      amberR19.top ?? yellowR19.top,
      amberR19.bottom ?? yellowR19.bottom,
      amberR19.left ?? yellowR19.left,
      amberR19.right ?? yellowR19.right,
      amberR19.header ?? yellowR19.header,
      amberR19.footer ?? yellowR19.footer,
      xenonR19?.type ?? "no-grid-type",
      xenonR19?.linePitchTwips ?? "no-grid-line-pitch",
      xenonR19?.charSpaceTwips ?? "no-grid-char-space",
      zincR19 ?? "no-grid",
      daisy(jewelR5.id) ? "mirror" : "plain",
    ].join(":"),
  };
}
function glide(jadeR7) {
  return (
    jadeR7.startsWithPageBreak === true ||
    jadeR7.breakType === C.SECTION_BREAK_TYPE_NEXT_PAGE ||
    jadeR7.breakType === C.SECTION_BREAK_TYPE_EVEN_PAGE ||
    jadeR7.breakType === C.SECTION_BREAK_TYPE_ODD_PAGE
  );
}
function honey(emberR7) {
  return (emberR7.elements ?? []).some((item) =>
    apexR2(item) === "text"
...
function iris(hazelR11) {
  return !glide(hazelR11) || honey(hazelR11)
    ? false
    : (hazelR11.elements?.length ?? 0) > 0;
}
  ...
  for (let reefR21 of ridgeR3) {
    ...
    let bloomR21 = groveR20[groveR20.length - 1],
      ...
      eagleR21 =
        ...
        bloomR21 !== undefined &&
        glide(reefR21) &&
        coralR21?.key === driftR21.key &&
        !riverR3(coralR21.columns, driftR21.columns) &&
        !reefR21.pageSetup &&
        !reefR21.header &&
        !reefR21.footer &&
        !reefR21.firstHeader &&
        !reefR21.firstFooter &&
        !reefR21.differentFirstPage;
    groveR20.push(
      eagleR21
        ? {
            ...reefR21,
...
function knoll(fernR9, groveR9) {
  if (fernR9) {
    for (let quillR14 of fernR9.split(";"))
      if (quillR14.startsWith(groveR9)) return quillR14.slice(groveR9.length);
  }
}
...
function moss(coralR5) {
  ...
  let umbraR23 = knoll(coralR5.id, duskR19),
    ...
    willowR23 =
      violetR23 !== undefined && Number.isFinite(violetR23) && violetR23 > 0
        ? violetR23
        : undefined,
...
function north(acornR7, bloomR7) {
  ...
  jadeR25 &&
    (jadeR25.formatType && (acornR7.formatType = jadeR25.formatType),
    jadeR25.start === undefined
      ? acornR7.nextValue === undefined && (acornR7.nextValue = 1)
      : (acornR7.nextValue = jadeR25.start));
}
function orbit(echoR6, falconR6) {
  let honeyR23 = falconR6 ?? "decimal";
  return honeyR23 === "upperRoman"
    ? pine(echoR6).toUpperCase()
    : honeyR23 === "lowerRoman"
      ? pine(echoR6).toLowerCase()
      : honeyR23 === "upperLetter"
        ? ridge(echoR6).toUpperCase()
        : honeyR23 === "lowerLetter"
          ? ridge(echoR6).toLowerCase()
          : String(echoR6);
}
function pine(pearlR5) {
  let prismR22 = [
    ...
    reefR22 = "";
  for (let [bloomR14, coralR14] of prismR22)
    for (; quillR22 >= bloomR14; ) {
      reefR22 += coralR14;
      quillR22 -= bloomR14;
    }
  return reefR22;
...
function tide(basaltR4, cedarR4) {
  return (basaltR4.paragraphs ?? []).some((item) =>
    (item.runs ?? []).some((_item) => (_item.text ?? "").includes(cedarR4)),
  ) || (basaltR4.children ?? []).some((item) => tide(item, cedarR4))
    ? true
    : (basaltR4.table?.rows ?? []).some((item) =>
        item.cells.some((_item) =>
          (_item.text ?? "").includes(cedarR4) ||
          (_item.paragraphs ?? []).some((__item) =>
            (__item.runs ?? []).some((___item) =>
              (___item.text ?? "").includes(cedarR4),
            ),
          )
            ? true
            : (_item.elements ?? []).some((__item) => tide(__item, cedarR4)),
        ),
      );
}
function unity(acornR8, bloomR8) {
  let groveR25 = (slateR10) =>
    slateR10
      .replaceAll(marbleR17, bloomR8.currentPageText ?? "")
      .replaceAll(xenonR17, bloomR8.totalPagesText ?? "");
...
function wave(coralR13, driftR13) {
  let flintR20 = Math.max(
    driftR13.contentTopPx,
    driftR13.pageHeightPx - driftR13.margins.footerPx - 18,
  );
  return {
    kind: "text",
    element: {
...
function apex(
  ...
  let wheatR19 =
      storm(cedarR9, "__docxField:PAGE__") ||
      storm(novaR9, "__docxField:PAGE__"),
    yarnR19 = wheatR19
      ? orbit(knollR9.nextValue ?? 1, knollR9.formatType)
      : undefined;
  ...
  let zephyrR19 = unity(cedarR9, {
    ...
    bloomR19 = eagle(
      zephyrR19,
      Math.max(0, riverR9.margins.headerPx),
      riverR9,
      valeR9,
      alphaR10,
    ),
    ...
    driftR19 = eagle(
      acornR19,
      Math.max(
        riverR9.contentTopPx,
        riverR9.pageHeightPx -
          riverR9.margins.footerPx -
          coralR19.totalHeightPx,
      ),
      riverR9,
      valeR9,
      alphaR10,
  ...
  return (
    zephyrR9 &&
      !wheatR19 &&
      knollR9.nextValue !== undefined &&
      ((frostR19 = wave(orbit(knollR9.nextValue, knollR9.formatType), riverR9)),
      (knollR9.nextValue += 1)),
    {
      sectionId: gammaR9,
