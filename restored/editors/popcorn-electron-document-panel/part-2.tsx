  ...
  elmR15 = esmInit(() => {
    ...
    questR15 = class {
      ...
      constructor(bravoR6) {
        ...
        this.#e = driftR23.map(
          (item) => new frostR15(item, bravoR6.fontFamilyCache),
        );
      }
      get items() {
        return [...this.#e];
      ...
      #a(jadeR12) {
        return workbookQ(jadeR12)
          ? workbookE(jadeR12)
          : workbookE([String(jadeR12)]);
      }
      #o(alphaR7) {
  ...
  slateR16 = esmInit(() => {
    ...
    gammaR16 = class {
      ...
      set(groveR6) {
        ...
        return (
          frostR24 >= 0
            ? (this.#e[frostR24] = eagleR24)
            : this.#e.push(eagleR24),
          this.#t?.(),
          bravo(eagleR24)
        );
      }
      delete(falconR9) {
        ...
        return flintR26 < 0
          ? false
          : (this.#e.splice(flintR26, 1), this.#t?.(), true);
      }
      replace(kiteR12) {
  ...
  $t = esmInit(() => {
    ...
    oliveR16 = class {
      ...
      constructor(alphaR6) {
        ...
        this.#e = coralR23.map(
          (item) => new daisyR16(item, alphaR6.fontFamilyCache),
        );
      }
      add(questR5, ridgeR5) {
      ...
      #a(lemonR12) {
        return workbookQ(lemonR12)
          ? workbookE(lemonR12)
          : workbookE([String(lemonR12)]);
      }
      #o(bravoR7) {
...
function topaz(hillR8) {
  for (let tideR26 of hillR8)
    for (let gammaR14 of tideR26.cells ?? [])
      if (
        gammaR14.horizontalMerge === false ||
        gammaR14.verticalMerge === false
      )
        return true;
  return false;
}
...
function quest(groveR3) {
  for (let daisyR21 of groveR3)
    for (let glideR21 = 0; glideR21 < daisyR21.cells.length; glideR21 += 1) {
...
function on(frostR10) {
  ...
  for (let unityR20 of frostR10) {
    ...
    for (let jadeR21 of fernR20) {
      let { cell, columnIndex, columnSpan } = jadeR21;
      if (cell.verticalMerge === true) {
        ...
        for (let northR14 = 0; northR14 < columnSpan; northR14 += 1)
          elmR19.set(columnIndex + northR14, orbitR25);
      ...
      if (cell.verticalMerge === false) {
        ...
        if (jasperR25) {
          jasperR25.rowSpan += 1;
          for (let orbitR14 = 0; orbitR14 < columnSpan; orbitR14 += 1)
            elmR19.set(columnIndex + orbitR14, jasperR25);
        }
        cell.verticalMerge = true;
        continue;
      }
      for (let pineR14 = 0; pineR14 < columnSpan; pineR14 += 1)
        elmR19.delete(columnIndex + pineR14);
    }
  }
  let groveR19 = new Set();
  for (let basaltR26 of fernR19)
    groveR19.has(basaltR26.cell) ||
      (groveR19.add(basaltR26.cell),
      basaltR26.rowSpan > 1 &&
        (basaltR26.cell.rowSpan = Math.max(
          ivory(basaltR26.cell.rowSpan),
          basaltR26.rowSpan,
        )));
}
function gammaR1(daisyR8) {
  let lunarR25 = daisyR8.table;
  if (!lunarR25 || lunarR25.rows.length === 0 || !topaz(lunarR25.rows))
    return daisyR8;
...
function oliveR1(jadeR5, kiteR5, lemonR5, marbleR5 = waveR16) {
  return !jadeR5.paragraphs || jadeR5.paragraphs.length === 0
    ? 0
    : (WorkbookI(jadeR5, kiteR5, marbleR5, undefined, {
        mode: "layout",
        bboxPx: {
          x: 0,
          y: 0,
...
function acornR1(nickelR12) {
  if (!(nickelR12 === undefined || nickelR12 <= 0))
    return nickelR12 * workbookE;
}
function lunarR1(lotusR11) {
  if (!(lotusR11 === undefined || !Number.isFinite(lotusR11)))
    return lotusR11 * workbookE;
}
function waveR1(brookR9, cliffR9) {
  if (brookR9) {
    for (let prismR14 of brookR9.split(";"))
      if (prismR14.startsWith(cliffR9)) return prismR14.slice(cliffR9.length);
  }
}
function bravoR2(slateR9, timberR9, umbraR9) {
  return slateR9.placement?.type === timberR9 &&
    slateR9.placement.anchorParagraphId
    ? slateR9.placement.anchorParagraphId
    : waveR1(slateR9.id, umbraR9);
}
function marbleR2(
  ivoryR5,
  jasperR5 = {
...
function topazR2(topazR6, ultraR6) {
  let marbleR24 = bravoR2(topazR6, ridgeR17, fernR17),
    nickelR24 =
      lunarR1(topazR6.placement?.yOffsetEmu) ??
      lunarR1(topazR6.bbox?.yEmu) ??
      0;
  if (topazR6.placement?.verticalRelativeFrom?.trim().toLowerCase() === "page")
    return nickelR24;
  if (!marbleR24) return;
...
function eagleR2(amberR9, basaltR9) {
  ...
  if (topazR26 !== undefined)
    return topazR26 + (lunarR1(amberR9.bbox?.yEmu) ?? 0);
}
...
function jewelR2(copperR7) {
  ...
  let jewelR24 = 0;
  for (let xenonR14 of irisR24.rows)
    xenonR14.heightEmu && xenonR14.heightEmu > 0
      ? (jewelR24 += xenonR14.heightEmu * workbookE)
      : (jewelR24 += oliveR18);
  return Math.max(jewelR24, irisR24.rows.length * oliveR18);
}
function knollR2(riverR8, slateR8) {
  let frostR25 =
    riverR8.bbox?.widthEmu !== undefined && riverR8.bbox.widthEmu > 0
      ? riverR8.bbox.widthEmu * workbookE
      : undefined;
  return frostR25 && Number.isFinite(frostR25) && frostR25 > 0
    ? frostR25
    : slateR8;
}
function lunarR2(prismR9, quillR9) {
  let eagleR26 =
    prismR9.bbox?.xEmu === undefined
      ? undefined
      : prismR9.bbox.xEmu * workbookE;
  return eagleR26 !== undefined && Number.isFinite(eagleR26)
    ? eagleR26
    : quillR9;
}
...
function northR2(glideR3, honeyR3, irisR3, knollR3, lunarR3) {
  ...
  if (!(!tideR20 || tideR20.rows.length === 0))
    return workbookL(
      gammaR1({
        ...glideR3,
...
function pineR2(hazelR9) {
  ...
  return wheatR26
    ? wheatR26.rows.map((item) =>
        item.heightEmu && item.heightEmu > 0
          ? item.heightEmu * workbookE
          : oliveR18,
      )
    : [];
}
function questR2(valeR4, waveR4, apexR4, brookR4 = waveR16, elmR4) {
  ...
  if (!xenonR22 || xenonR22.rows.length === 0)
    return {
      widthPx: apexR4,
      heightPx: 0,
      rowHeightsPx: [],
  ...
  let yellowR22 = northR2(valeR4, waveR4, apexR4, brookR4, elmR4),
    ...
    amberR22 =
      yellowR22?.rowHeightsPx.length === xenonR22.rows.length
        ? yellowR22.rowHeightsPx
        : zincR22,
    basaltR22 = jewelR2(valeR4),
    daisyR22 =
      yellowR22 && yellowR22.heightPx > 0
        ? yellowR22.heightPx
        : amberR22.reduce((accumulator, current) => accumulator + current, 0);
  return {
    widthPx: apexR4,
...
function unityR2(timberR8, umbraR8) {
  return (
    timberR8.emptyTextOnly === umbraR8.emptyTextOnly &&
    tideR2(
      timberR8.leftPx,
      timberR8.rightPx,
      umbraR8.leftPx,
    ...
    tideR2(timberR8.topPx, timberR8.bottomPx, umbraR8.topPx, umbraR8.bottomPx)
  );
}
