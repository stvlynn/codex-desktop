...
function waveR2(tideR3, valeR3) {
  ...
  for (; echoR21; ) {
    ...
    for (let ridgeR22 = tideR3.length - 1; ridgeR22 >= 0; --ridgeR22) {
      ...
      !reefR23 ||
        !unityR2(reefR23, deltaR21) ||
        ((deltaR21.leftPx = Math.min(deltaR21.leftPx, reefR23.leftPx)),
        (deltaR21.rightPx = Math.max(deltaR21.rightPx, reefR23.rightPx)),
        (deltaR21.topPx = Math.min(deltaR21.topPx, reefR23.topPx)),
        (deltaR21.bottomPx = Math.max(deltaR21.bottomPx, reefR23.bottomPx)),
        tideR3.splice(ridgeR22, 1),
        (echoR21 = true));
    }
  }
  tideR3.push(deltaR21);
  tideR3.sort((marbleR10, nickelR10) =>
    Math.abs(marbleR10.topPx - nickelR10.topPx) > 0.01
      ? marbleR10.topPx - nickelR10.topPx
      : marbleR10.leftPx - nickelR10.leftPx,
  );
}
function $n(
  jewelR13,
  ...
  for (;;) {
    let timberR21 = jewelR13.find(
      (item) =>
        valeR2(item, coralR3) &&
        tideR2(knollR13, bloomR20, item.leftPx, item.rightPx) &&
        coralR20 >= item.topPx - 0.01 &&
        coralR20 < item.bottomPx - 0.01,
    );
    if (!timberR21) {
      let pineR22 = jewelR13
          .filter(
            (item) =>
              valeR2(item, coralR3) &&
              tideR2(knollR13, bloomR20, item.leftPx, item.rightPx) &&
              item.topPx > coralR20 + 0.01,
          )
          .sort((willowR13, xenonR13) => willowR13.topPx - xenonR13.topPx)[0],
        questR22 = Math.min(acornR3, pineR22?.topPx ?? acornR3);
      if (!pineR22 || questR22 - coralR20 >= driftR20 - 0.01)
        return {
          currentY: coralR20,
...
function apexR2(brookR3) {
  return brookR3.type === m.ELEMENT_TYPE_TABLE || brookR3.table
    ? "table"
    : brookR3.type === m.ELEMENT_TYPE_CHART ||
        brookR3.type === m.ELEMENT_TYPE_CHART_REFERENCE ||
        brookR3.chartReference
      ? "chart"
      : brookR3.type === m.ELEMENT_TYPE_IMAGE ||
          brookR3.type === m.ELEMENT_TYPE_IMAGE_REFERENCE ||
          brookR3.imageReference ||
          brookR3.image
        ? "image"
        : brookR3.type === m.ELEMENT_TYPE_SHAPE || brookR3.shape
          ? "shape"
          : brookR3.type === m.ELEMENT_TYPE_GROUP ||
              (brookR3.children?.length ?? 0) > 0
            ? "group"
            : "text";
}
...
function elmR2(pineR9, questR9 = false) {
  ...
  return ridgeR13.length === 0
    ? [questR9]
    : ((ridgeR13[0] = questR9), ridgeR13);
}
function fernR2(ridgeR9, stormR9) {
  let stormR13 = ridgeR9.columns.widths.map(() => undefined);
  return stormR13.length === 0
    ? [stormR9]
    : ((stormR13[0] = stormR9), stormR13);
}
  ...
  return {
    ...
    documentGridLinePitchPx:
      settings.documentGridLinePitchTwips === undefined
        ? undefined
        : honeyR2(settings.documentGridLinePitchTwips),
  };
}
...
function bravoR3({
  ...
  return (
    currentParagraphUsesContextualSpacing &&
    alphaR3(previousParagraphStyleId, currentParagraphStyleId)
  );
}
function copperR3(emberR10, flintR10, garnetR10) {
  for (let reefR14 of emberR10.paragraphs ?? [])
    reefR14.id &&
      !garnetR10.has(reefR14.id) &&
      garnetR10.set(reefR14.id, flintR10);
}
...
function harborR3(pearlR11) {
  return (
    pearlR11.placement?.type === riverR20 &&
    pearlR11.placement.wrap?.type === cedarR20
  );
}
function indigoR3(quartzR11) {
  return (
    quartzR11.placement?.type === riverR20 &&
    quartzR11.placement.wrap?.type === novaR20
  );
}
function _r(jewelR8) {
...
function kiteR3(honeyR4, irisR4, jewelR4, knollR4, lunarR4 = 0) {
  if (falconR3(honeyR4.kind)) {
    ...
    if (jadeR3(honeyR4.element))
      return irisR4 + workbookJ(honeyR4.element).topPx;
    if (frostR2(honeyR4.element) && knollR4 !== undefined)
      return knollR4 + (honeyR4.element.bbox?.yEmu ?? 0) * workbookE;
    let lunarR22 = topazR2(honeyR4.element, jewelR4);
    if (lunarR22 !== undefined)
      return marbleR3(honeyR4.element) ? lunarR4 + lunarR22 : lunarR22;
...
function marbleR3(reefR9) {
  return (
    reefR9.placement?.type === riverR20 &&
    reefR9.placement.verticalRelativeFrom?.trim().toLowerCase() === "page"
  );
}
...
function onyxR3(harborR9, indigoR9, jadeR9, kiteR9) {
  return falconR3(harborR9.kind) &&
    (gammaR3(harborR9.element) ||
      indigoR3(harborR9.element) ||
      harborR3(harborR9.element))
    ? indigoR9
    : Math.max(indigoR9, jadeR9 + kiteR9 + quartzR3(harborR9.element));
}
...
function riverR3(fernR5, groveR5) {
  ...
  for (let indigoR25 = 0; indigoR25 < fernR5.widths.length; indigoR25 += 1)
    if (
      Math.abs(
        (fernR5.widths[indigoR25] ?? 0) - (groveR5.widths[indigoR25] ?? 0),
      ) > 0.5 ||
      Math.abs(
        (fernR5.xPositions[indigoR25] ?? 0) -
          (groveR5.xPositions[indigoR25] ?? 0),
      ) > 0.5
    )
      return true;
  return false;
...
function slateR3(eagleR9) {
  return eagleR9.sections.length > 0
    ? eagleR9.sections.flatMap((item) => item.elements ?? [])
    : (eagleR9.elements ?? []);
}
function timberR3(glideR6, honeyR6) {
  ...
  if (willowR24)
    for (let pearlR26 of willowR24.rows)
      for (let mossR26 of pearlR26.cells) {
        for (let brookR14 of mossR26.paragraphs ?? []) honeyR6(brookR14);
        for (let stormR14 of mossR26.elements ?? [])
          timberR3(stormR14, honeyR6);
      }
}
...
function xenonR3(prismR4, quillR4, sageR4) {
  let bravoR22 = prismR4?.levels?.find(
    (yellowR13) => (yellowR13.level ?? 0) === quillR4,
  );
  if (!bravoR22) return;
...
function yellowR3(flintR7, garnetR7, hazelR7) {
  ...
  let isleR24 = garnetR7?.levels?.find(
    (zincR13) => (zincR13.level ?? 0) === hazelR7,
  )?.startAt;
...
function zincR3(yarnR5) {
  let echoR19 = new Map(),
    ...
    jadeR19 = new Map(),
    kiteR19 = new Set();
  for (let zephyrR14 of yarnR5.textStyles ?? [])
    zephyrR14.id && gammaR19.set(zephyrR14.id, zephyrR14.basedOn);
  for (let tideR23 of yarnR5.numberingDefinitions ?? []) {
    tideR23?.numId && harborR19.set(tideR23.numId, tideR23);
    for (let cedarR25 of tideR23?.levels ?? [])
      cedarR25?.paragraphStyleId &&
        tideR23?.numId &&
        falconR19.set(cedarR25.paragraphStyleId, {
          numId: tideR23.numId,
          level: Math.max(0, cedarR25.level ?? 0),
        });
  }
  for (let indigoR14 of yarnR5.paragraphNumberings ?? [])
    indigoR14?.paragraphId &&
      indigoR14.numId &&
      echoR19.set(indigoR14.paragraphId, indigoR14);
  for (let kiteR21 of slateR3(yarnR5))
    timberR3(kiteR21, (riverR4) => {
      if (!riverR4.id || kiteR19.has(riverR4.id)) return;
      kiteR19.add(riverR4.id);
      let emberR21 =
        echoR19.get(riverR4.id) ??
        umbraR3(riverR4.styleId, falconR19, gammaR19);
      ...
      for (
        let knollR14 = flintR21 + 1;
        knollR14 < ivoryR21.length;
        knollR14 += 1
      )
        ivoryR21[knollR14] = undefined;
      ivoryR21[flintR21] === undefined &&
        (ivoryR21[flintR21] = yellowR3(
          riverR4.paragraphStyle,
          garnetR21,
          flintR21,
        ));
...
function basaltR3(prismR6, quillR6, reefR6) {
  ...
  if (lemonR24)
    return (
      prismR6?.autoNumberType &&
        (prismR6.marginLeft === undefined && (lemonR24.marginLeft = undefined),
        prismR6.indent === undefined && (lemonR24.indent = undefined)),
      reefR6 !== undefined && (lemonR24.autoNumberStartAt = reefR6),
      lemonR24
    );
}
    ...
    paragraphs: kiteR4.map((item) => {
      ...
      if (!frostR18) {
        ...
        return (
          slateR23 &&
            eagleR18 !== undefined &&
            slateR23.autoNumberType &&
            (slateR23.autoNumberStartAt = eagleR18),
          slateR23 === item.paragraphStyle
            ? item
            : ((quillR18 = true),
              {
                ...item,
      ...
      let glideR18 = emberR3(
          item.textStyle,
          frostR18.textStyle,
          reefR4.preserveDirectTextStyle === true,
        ),
        honeyR18 = frostR18.contextualSpacing ? WorkbookR(glideR18) : glideR18,
        irisR18 = basaltR3(
          item.paragraphStyle,
          frostR18.paragraphStyle,
          undefined,
        ),
        jewelR18 = workbookLt(driftR18?.paragraphStyle, irisR18);
      jewelR18 &&
        eagleR18 !== undefined &&
        jewelR18.autoNumberType &&
        (jewelR18.autoNumberStartAt = eagleR18);
      let knollR18 = flintR3(
          item.runs,
          reefR4.preserveDirectTextStyle === true
            ? glideR18
            : frostR18.textStyle,
      ...
      return (
        (orbitR18.textStyle !== item.textStyle ||
          orbitR18.paragraphStyle !== item.paragraphStyle ||
          orbitR18.runs !== item.runs ||
          orbitR18.spaceBefore !== item.spaceBefore ||
          orbitR18.spaceAfter !== item.spaceAfter) &&
          (quillR18 = true),
        orbitR18
      );
    }),
...
function emberR3(zephyrR6, acornR6, bloomR6 = false) {
  if (!bloomR6) {
    let pearlR25 = {};
    return (
      zephyrR6?.alignment !== undefined &&
        (pearlR25.alignment = zephyrR6.alignment),
      zephyrR6?.scheme !== undefined && (pearlR25.scheme = zephyrR6.scheme),
      Object.keys(pearlR25).length === 0
        ? acornR6
        : workbookS(pearlR25, acornR6)
    );
  }
...
function hazelR3(zincR10, amberR10, basaltR10) {
  return garnetR3(
    zincR10.paragraphs && zincR10.paragraphs.length > 0
      ? cedarR3(zincR10, amberR10, basaltR10)
      : zincR10,
    amberR10,
    basaltR10,
  );
...
function mintR3(jadeR4, lemonR4, marbleR4, nickelR4) {
  ...
  for (let lotusR22 of jadeR4.footnotes ?? []) {
    ...
    let glideR22 = jasperR3(
      [kelpR3(`docx-footnote-${lotusR22.id}`, lotusR22.paragraphs ?? [])],
      lemonR4,
      marbleR4,
      nickelR4,
    )[0];
...
function oliveR3(zincR6, amberR6) {
  ...
  for (let reefR25 of zincR6.segments) {
    ...
    if (jadeR26)
      for (let mossR14 of jadeR26)
        groveR23.has(mossR14) || (groveR23.add(mossR14), fernR23.push(mossR14));
  }
  return fernR23;
}
function prismR3(vaporR8, wheatR8, yarnR8, zephyrR8) {
  let duskR25 = [],
    elmR25 = new Set();
  for (let riverR14 = wheatR8; riverR14 < yarnR8; riverR14 += 1)
    for (let acornR14 of oliveR3(vaporR8[riverR14], zephyrR8))
      elmR25.has(acornR14) || (elmR25.add(acornR14), duskR25.push(acornR14));
  return duskR25;
}
...
function topazR3(lunarR9, mossR9, northR9, orbitR9) {
  return lunarR9.ids.length === 0
    ? 0
    : sageR3(mossR9, lunarR9.ids, northR9.contentWidthPx, orbitR9)
        .reservedHeightPx;
}
function ultraR3(mossR11, northR11, orbitR11, pineR11) {
  return (
    orbitR11.contentBottomPx - topazR3(mossR11, northR11, orbitR11, pineR11)
  );
}
