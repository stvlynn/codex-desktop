...
function amber({
  ...
  for (let mintR17 = 0; mintR17 < preparedElements.length; mintR17 += 1) {
    ...
    if (novaR17.kind !== "text") {
      ...
      for (; !elmR18; ) {
        let fernR18 =
            settings.columns.widths[falconR17] ?? settings.contentWidthPx,
    ...
    for (; !prismR17; ) {
      ...
      let zephyrR17 = indigoR17[falconR17] ?? false,
        ...
        coralR17 = bravoR3({
          ...
          currentParagraphUsesContextualSpacing:
            novaR17.firstParagraphUsesContextualSpacing,
        }),
        driftR17 =
          bloomR17 || (coralR17 && oliveR17 === 0 && violet(yarnR17[0])),
      ...
      let frostR17 = timber(novaR17.element, yarnR17),
        honeyR17 = indigo({
          ...
          firstParagraphSpaceBeforePx: driftR17
            ? 0
            : novaR17.firstParagraphSpaceBeforePx,
        }),
        irisR17 = $n(
          onyxR17,
          settings.columns.xPositions[falconR17] ?? settings.contentLeftPx,
          vaporR17,
          wheatR17 - honeyR17,
          columnBottomPx,
          Math.max(1, yarnR17[eagleR17]?.heightPx ?? 0),
          frostR17,
        ),
        jewelR17 = irisR17.currentY,
        knollR17 = Math.max(0, irisR17.boundaryBottomPx - jewelR17),
        lunarR17 = wheatR17 === sectionStartY,
        { nextLineIndex, fragmentHeightPx, endsWithFlowBreak } = kite(
          yarnR17,
          eagleR17,
          knollR17,
        );
      if (nextLineIndex === eagleR17) {
        ...
        if (
          ((fragmentHeightPx = Math.max(1, yarnR17[eagleR17]?.heightPx ?? 0)),
          fragmentHeightPx > knollR17)
        )
          return;
      ...
      if (
        ...
        oliveR17 === 0 &&
          (copperR3(novaR17.element, jewelR17, lemonR17),
          (nickelR17[falconR17] = jewelR17)),
        (gammaR17[falconR17] = jewelR17 + fragmentHeightPx),
        (oliveR17 = nextLineIndex),
        (harborR17[falconR17] = 0),
        endsWithFlowBreak && oliveR17 < yarnR17.length)
      ) {
        if (falconR17 < settings.columns.widths.length - 1) {
          falconR17 += 1;
          continue;
      ...
      if (oliveR17 >= yarnR17.length) {
        let hillR21 =
          (novaR17.element.paragraphs?.length ?? 0) > 1 ? jasperR19 : 0;
        gammaR17[falconR17] = (gammaR17[falconR17] ?? sectionStartY) + hillR21;
        harborR17[falconR17] =
          hillR21 > 0 ? 0 : novaR17.lastParagraphSpaceAfterPx;
...
function basalt({
  ...
  for (; honeyR19 <= irisR19; ) {
    ...
    apexR22
      ? ((jewelR19 = apexR22), (knollR19 = unityR22), (irisR19 = unityR22 - 1))
      : (honeyR19 = unityR22 + 1);
  }
...
function ember(sageR10, ultraR10, vaporR10) {
  return vaporR10 % 2 != 0 || !daisy(sageR10)
    ? 0
    : ultraR10.margins.rightPx - ultraR10.margins.leftPx;
}
function flint(event, wheatR13 = 0) {
  return {
    ...
    columns: event.columns.widths.map((item, index) => ({
      ...
      frame: {
        xPx:
          (event.columns.xPositions[index] ?? event.contentLeftPx) + wheatR13,
        yPx: event.contentTopPx,
        widthPx: item,
        heightPx: event.contentBottomPx - event.contentTopPx,
      },
...
function kelp(hillR3, alphaR4, bravoR4, copperR4 = {}) {
  let umbraR21 = ember(hillR3.sectionId, alphaR4, bravoR4),
    violetR21 =
      Math.abs(umbraR21) < 0.01
        ? hillR3
        : {
            ...hillR3,
            layout: flint(alphaR4, umbraR21),
            headerElements: hillR3.headerElements.map((item) =>
              jasper(item, umbraR21),
            ),
            bodyElements: hillR3.bodyElements.map((item) =>
              jasper(item, umbraR21),
            ),
            footerElements: hillR3.footerElements.map((item) =>
              jasper(item, umbraR21),
            ),
            pageNumberElement: hillR3.pageNumberElement
              ? jasper(hillR3.pageNumberElement, umbraR21)
              : undefined,
          };
  return copperR4.layoutMode === "pageless"
    ? lotus(violetR21, alphaR4, copperR4)
    : violetR21;
}
function lotus(duskR3, elmR3, fernR3) {
  let pearlR21 = mint(duskR3.bodyElements),
    ...
    riverR21 = Math.ceil(
      Math.max(
        elmR3.basePageHeightPx,
        fernR3.pagelessMinHeightPx ?? 0,
        quartzR21 + elmR3.margins.bottomPx,
      ),
    ),
    slateR21 = Math.max(
      1,
      riverR21 - elmR3.contentTopPx - elmR3.margins.bottomPx,
    );
  return {
    ...duskR3,
    heightPx: riverR21,
...
function nova(glideR13, honeyR13, irisR13) {
  let topazR20 = Math.max(1, glideR13.columns?.count ?? 1),
    ...
    vaporR20 = (glideR13.columns?.widths ?? []).map((item) =>
      Math.max(0, honeyR2(item)),
    ),
    wheatR20 = [],
    yarnR20 = [];
  if (vaporR20.length >= topazR20 && vaporR20.some((item) => item > 0)) {
    let stormR24 = vaporR20
        .slice(0, topazR20)
    ...
    for (let unityR14 = 0; unityR14 < topazR20; unityR14 += 1)
      wheatR20.push(onyxR14);
  }
...
function prism(echoR5) {
  ...
  if (!hazelR22) return;
  let ivoryR22 =
      hazelR22.linePitch !== undefined && hazelR22.linePitch > 0
        ? hazelR22.linePitch
        : undefined,
    jasperR22 =
      ivoryR22 !== undefined && olive(hazelR22.type) ? ivoryR22 : undefined;
  return {
    type: hazelR22.type,
...
function quill(isleR10) {
  if (isleR10?.activeLinePitchTwips !== undefined)
    return isleR10.activeLinePitchTwips;
}
function reef(northR4, pineR4, questR4, ridgeR4, stormR4, tideR4) {
  ...
  return pineR4 === "shape" || pineR4 === "chart" || pineR4 === "group"
    ? marbleR2(northR4, {
...
function sage(xenonR9, yellowR9) {
  return xenonR9.bbox?.widthEmu !== undefined && xenonR9.bbox.widthEmu > 0
    ? Math.max(1, xenonR9.bbox.widthEmu * workbookE)
    : Math.max(1, yellowR9);
}
...
function vapor(kelpR10) {
  return (
    kelpR10.type === m.ELEMENT_TYPE_TEXT ||
    kelpR10.type === m.ELEMENT_TYPE_TEXT_GROUP
  );
}
function wheat(amberR7) {
  let groveR24 = new Map();
  for (let copperR26 of amberR7.table?.rows ?? [])
    for (let questR26 of copperR26.cells ?? [])
      for (let emberR14 of questR26.elements ?? [])
        vapor(emberR14) ||
          (emberR14.id &&
            emberR14.id.length > 0 &&
            groveR24.set(emberR14.id, emberR14));
  return groveR24;
}
