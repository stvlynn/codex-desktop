  ...
  pearlR14 = esmInit(() => {
    ...
    orbitR13 = class {
      ...
      add(cliffR3) {
        ...
        cliffR3.config?.chartLine === undefined &&
          (nickelR21.chartLine.visible = false);
        let onyxR21 = nickelR21.chart;
        return (
          onyxR21 &&
            !onyxR21.chartSpaceLine.isSet &&
            (onyxR21.chartSpaceLine.visible = false),
          (nickelR21.placement = unityR1(cliffR3.style, marbleR21, {
  ...
  tideR14 = esmInit(() => {
    ...
    irisR14 = class PopcornElectronDocumentPanelClass1 {
      ...
      constructor(driftR3) {
        ...
        for (let waveR14 of remainingElements)
          this.#y(waveR14, lunarR20, mossR20, northR20, orbitR20);
      }
      get text() {
        return this.#i.text;
      }
      get textElements() {
        return this.#n
          ...
          .filter((item) => item instanceof zephyrR22);
      }
      getElementById(apexR11) {
        if (apexR11) return this.#r.get(apexR11);
      }
      removeElementById(flintR9) {
        return !flintR9 || flintR9 === this.#i.id || !this.#r.get(flintR9)
          ? false
          : (this.#_(flintR9), true);
      }
      resolveTextBlock(umbraR5) {
        ...
        if (honeyR22) {
          ...
          return northR25 instanceof workbookL
            ? (northR25.getCellById(honeyR22.cellId)?.text ?? null)
            : null;
        }
        let irisR22 = this.getElementById(umbraR5);
        return irisR22 && (irisR22 instanceof zephyrR22 || "text" in irisR22)
          ? irisR22.text
          : null;
      }
      set text(acornR12) {
      ...
      startParagraphBlock(xenonR7 = false) {
        if (!xenonR7 && this.#n.length === 1 && this.#n[0] === this.#i.id)
          return this.#a;
      ...
      addGroup(alphaR8) {
        let ivoryR19 = stormR1(
          ...
          kelpR19 = new _c(this.#e, {
            ...
            bbox: {
              xEmu:
                ivoryR19.left === undefined
                  ? undefined
                  : workbookP(ivoryR19.left),
              yEmu:
                ivoryR19.top === undefined
                  ? undefined
                  : workbookP(ivoryR19.top),
              widthEmu:
                ivoryR19.width === undefined
                  ? undefined
                  : workbookP(ivoryR19.width),
              heightEmu:
                ivoryR19.height === undefined
                  ? undefined
                  : workbookP(ivoryR19.height),
            },
          });
        return (
          (kelpR19.placement = unityR1(alphaR8.style, ivoryR19, {
      ...
      toProto() {
        return this.#n
          .map((item) => this.#r.get(item))
          .filter((item) => !!item)
          .map((item) => (item instanceof zephyrR22, item.toProto()));
      }
      #d() {
        return {
          ...
          getResolvedParagraphTextStyle: (driftR7, eagleR7) => {
            let kiteR25 = this.#m().resolveParagraphTextStyle(
              driftR7.toProto(),
              eagleR7?.toProto(),
            );
            return kiteR25 ? new __workbookT(kiteR25) : undefined;
          },
          getResolvedParagraphStyle: (onyxR12) =>
            this.#m().resolveParagraphStyle(onyxR12.toProto()),
          getResolvedRunTextStyle: (kiteR7, lemonR7, marbleR7) => {
            let pineR24 = this.#m().resolveRunTextStyle(
              kiteR7.toProto(),
              lemonR7.toProto(),
              marbleR7?.toProto(),
            );
      ...
      #v(indigoR5) {
        let mintR22 = indigoR5.findIndex(
          (item) =>
            item.type === m.ELEMENT_TYPE_TEXT ||
            item.type === m.ELEMENT_TYPE_TEXT_GROUP,
        );
        return mintR22 >= 0
          ? {
              bodyElement: indigoR5[mintR22],
              remainingElements: indigoR5.filter(
                (item, index) => index !== mintR22,
              ),
            }
          : {
      ...
      #y(falconR7, quartzR7, basaltR7, mintR7, zephyrR7) {
        if (
          falconR7.type === m.ELEMENT_TYPE_TEXT ||
          falconR7.type === m.ELEMENT_TYPE_TEXT_GROUP
        ) {
        ...
        if (
          falconR7.type === m.ELEMENT_TYPE_CHART ||
          falconR7.type === m.ELEMENT_TYPE_CHART_REFERENCE ||
          falconR7.chartReference
        ) {
          zephyrR7.add({
            proto: falconR7,
          });
        ...
        if (
          falconR7.type === m.ELEMENT_TYPE_IMAGE ||
          falconR7.type === m.ELEMENT_TYPE_IMAGE_REFERENCE ||
          falconR7.imageReference
        ) {
          basaltR7.add({
            proto: falconR7,
          });
        ...
        if (
          falconR7.type === m.ELEMENT_TYPE_SHAPE ||
          falconR7.shape !== undefined
        ) {
          quartzR7.add({
            proto: falconR7,
          });
        ...
        (falconR7.type === m.ELEMENT_TYPE_GROUP ||
          (falconR7.children?.length ?? 0) > 0) &&
          this.#g(new _c(this.#e, falconR7));
      }
    };
  });
...
function indigoR2(ivoryR8) {
  ...
  for (let fernR13 of pineR25) {
    ...
    for (let glideR14 of cedarR14)
      if ((glideR14.text ?? "").length > 0) return true;
  }
  return false;
}
function jadeR2(wheatR7) {
  for (let stormR25 of wheatR7)
    if (
      stormR25 &&
      (!(
        stormR25.type === m.ELEMENT_TYPE_TEXT ||
        stormR25.type === m.ELEMENT_TYPE_TEXT_GROUP
      ) ||
        indigoR2(stormR25))
    )
      return true;
