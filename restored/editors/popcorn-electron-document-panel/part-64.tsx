...
function OliveR2({
  ...
  let mossR17 = reefR15.useRef(null),
    ...
    pineR17 = reefR15.useMemo(() => jasperR2(pageObjectTargets, selectedObjectId), [pageObjectTargets, selectedObjectId]),
    questR17 = reefR15.useMemo(() => lotusR2(pageTextLayoutBlocks, selectedTextBlockId), [pageTextLayoutBlocks, selectedTextBlockId]);
  return reefR15.useEffect(() => {
    let gammaR23 = mossR17.current;
    if (!(!gammaR23 || !enableWorker) && controller.attachPageCanvas({
      surfaceId,
  ...
  }, [controller, orbitR17, enableWorker, rect.pageIndex, surfaceId]), reefR15.useEffect(() => {
    let stormR21 = mossR17.current;
    if (stormR21) {
      if (stormR21.style.width = `${rect.width}px`, stormR21.style.height = `${rect.height}px`, !enableWorker) {
  ...
  }, [controller, orbitR17, enableWorker, rect.height, rect.width, surfaceId]), reefR15.useEffect(() => {
    ...
    lotusR21 && (lotusR21.setTransform(orbitR17, 0, 0, orbitR17, 0, 0), lotusR21.clearRect(0, 0, rect.width, rect.height), novaR2(lotusR21, pineR17), mintR2(lotusR21, questR17), lotusR21.save(), lotusR21.translate(-rect.left, -rect.top), textEditSession.drawOverlay(lotusR21), lotusR21.restore());
  }, [orbitR17, overlayVersion, rect.height, rect.left, rect.top, rect.width, pineR17, questR17, textEditSession]), <div className="absolute" style={{
    left: rect.left,
    top: rect.top,
    width: rect.width,
    height: rect.height
  }} data-testid={`popcorn-document-page-${rect.pageIndex}`}><div className="absolute inset-0 rounded-sm bg-white shadow-[0_8px_32px_rgba(15,23,42,0.12)] ring-1 ring-black/8" /><canvas ref={mossR17} className="absolute inset-0" data-testid={`popcorn-document-page-canvas-${rect.pageIndex}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onClick={onClick} onDoubleClick={onDoubleClick} /><canvas ref={northR17} className="pointer-events-none absolute inset-0" data-testid={`popcorn-document-page-overlay-${rect.pageIndex}`} /></div>;
}
var reefR15,
  sageR15,
  ultraR15 = esmInit(() => {
...
function PrismR2({
  ...
  let hazelR16 = vaporR15.useRef(null),
    ...
    ultraR16 = $c(ivoryR16),
    vaporR16 = vaporR15.useSyncExternalStore(nickelR13 => controller.subscribeRenderedPages(nickelR13), () => controller.getRenderedPages(), () => controller.getRenderedPages()),
    wheatR16 = vaporR15.useMemo(() => new Map(vaporR16.map(item => [item.pageIndex, item])), [vaporR16]),
    yarnR16 = vaporR15.useMemo(() => {
      ...
      for (let orbitR26 of controller.getObjectTargets()) {
        ...
        daisyR14 ? daisyR14.push(orbitR26) : oliveR25.set(orbitR26.pageIndex, [orbitR26]);
      }
      return oliveR25;
    }, [controller, snapshot.documentVersion, snapshot.pageLayouts, vaporR16]),
    zephyrR16 = vaporR15.useMemo(() => {
      let isleR20 = snapshot.pageLayouts.reduce((accumulator, current) => Math.max(accumulator, Math.round(current.width * snapshot.zoom)), 0),
    ...
    bloomR16 = vaporR15.useMemo(() => {
      let duskR13 = zephyrR16.reduce((accumulator, current) => Math.max(accumulator, current.width), 0);
      return Math.max(ultraR16.width, duskR13 + 80);
    }, [ultraR16.width, zephyrR16]),
    coralR16 = vaporR15.useMemo(() => zephyrR16.map(item => ({
      key: `document-page-${item.pageIndex}`,
      index: item.pageIndex,
      top: item.top,
      height: item.height
    })), [zephyrR16]),
    driftR16 = vaporR15.useMemo(() => daisyR2(coralR16, {
      scrollTop: sageR16,
    ...
    eagleR16 = vaporR15.useMemo(() => {
      ...
      return driftR16.map(item => pineR26.get(item.index)).filter(item => !!item);
    }, [zephyrR16, driftR16]),
    frostR16 = vaporR15.useMemo(() => emberR2(coralR16) + 32 + Math.max(0, bottomScrollReservePx), [bottomScrollReservePx, coralR16]),
    ...
    honeyR16 = vaporR15.useMemo(() => new RemoteTextEditSessionZoomControl({
      onPointerDown: (nickelR5, onyxR5) => {
        let oliveR22 = vaporR2(nickelR5, onyxR5?.blockId ?? null, kelpR16.current, lotusR16.current, mintR16.current);
        oliveR22 && (kelpR16.current = oliveR22.pageIndex, controller.textPointerDown(oliveR22.pageIndex, oliveR22.pagePoint, {
          shiftKey: onyxR5?.shiftKey
        }));
      },
      onPointerMove: zincR9 => {
        let reefR26 = vaporR2(zincR9, null, kelpR16.current, lotusR16.current, mintR16.current);
      ...
      onSelectWordAtPoint: (oliveR5, prismR5) => {
        let kiteR23 = vaporR2(oliveR5, prismR5?.blockId ?? null, kelpR16.current, lotusR16.current, mintR16.current);
        kiteR23 && (kelpR16.current = kiteR23.pageIndex, controller.textSelectWordAtPoint(kiteR23.pageIndex, kiteR23.pagePoint));
      },
      onSelectParagraphAtPoint: (garnetR5, hazelR5) => {
        let deltaR23 = vaporR2(garnetR5, hazelR5?.blockId ?? null, kelpR16.current, lotusR16.current, mintR16.current);
        deltaR23 && (kelpR16.current = deltaR23.pageIndex, controller.textSelectParagraphAtPoint(deltaR23.pageIndex, deltaR23.pagePoint));
      },
  ...
  vaporR15.useEffect(() => (honeyR16.attachContainer(ivoryR16.current), () => {
    honeyR16.dispose();
  }), [honeyR16]);
  vaporR15.useEffect(() => {
  ...
  vaporR15.useEffect(() => {
    prismR16.current !== snapshot.pageIndex && (prismR16.current = snapshot.pageIndex, quillR2(zephyrR16, sageR16, ultraR16.height) !== snapshot.pageIndex && (novaR16.current = snapshot.pageIndex));
  }, [ultraR16.height, zephyrR16, sageR16, snapshot.pageIndex]);
  vaporR15.useEffect(() => {
    let bloomR25 = quillR2(zephyrR16, sageR16, ultraR16.height);
    novaR16.current !== snapshot.pageIndex && (bloomR25 == null || bloomR25 === snapshot.pageIndex || controller.setPageIndex(bloomR25, {
      origin: "scroll"
    }));
  ...
  vaporR15.useEffect(() => {
    ...
    if (novaR16.current = snapshot.pageIndex, typeof ivoryR16.current.scrollTo == "function") {
  ...
  let irisR16 = vaporR15.useCallback(() => {
    ...
    jewelR16 = vaporR15.useCallback((knollR7, valeR7) => event => {
      ...
      if (honeyR16.tryPointerDown(flintR19, {
    ...
    knollR16 = vaporR15.useCallback((frostR4, glideR4) => event => {
      ...
      if (!jasperR16.current) {
        ...
        let vaporR23 = nickelR22 ? kelpR2(glideR4?.textLayoutBlocks ?? [], nickelR22) : null;
        event.currentTarget.style.cursor = vaporR23 ? "text" : "";
        return;
      }
      honeyR16.pointerMove(marbleR22);
    }, [yarnR16, honeyR16]),
    mossR16 = vaporR15.useCallback(oliveR6 => event => {
      if (jasperR16.current) {
    ...
    northR16 = vaporR15.useCallback(yellowR8 => event => {
      if (event.detail < 3) return;
      let tideR25 = sageR2(event, yellowR8);
      honeyR16.selectParagraphAtPoint(tideR25) && (kelpR16.current = yellowR8.pageIndex, event.preventDefault());
    }, [honeyR16]),
    orbitR16 = vaporR15.useCallback(knollR8 => event => {
      let onyxR26 = sageR2(event, knollR8);
      (honeyR16.selectWordAtPoint(onyxR26) || honeyR16.tryPointerDown(onyxR26)) && (kelpR16.current = knollR8.pageIndex, event.preventDefault());
    }, [honeyR16]);
  return <div ref={ivoryR16} className="relative h-full min-h-0 overflow-auto bg-token-bg-tertiary" tabIndex={0} data-testid="popcorn-document-stage" onKeyDown={event => {
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") {
      event.shiftKey ? controller.redo() : controller.undo();
      event.preventDefault();
      return;
    ...
    if ((event.key === "Backspace" || event.key === "Delete") && !honeyR16.isActive() && snapshot.selectedObjectId) {
    ...
    event.key === "Enter" && !honeyR16.isActive() && snapshot.selectedTextBlockId && controller.textActivateBlockEnd(snapshot.selectedTextBlockId) && event.preventDefault();
  }} onScroll={irisR16}><div className="relative min-w-full" style={{
      height: Math.max(frostR16, ultraR16.height),
      width: Math.max(bloomR16, ultraR16.width)
    }}>{eagleR16.map(item => {
        let xenonR20 = wheatR16.get(item.pageIndex);
        return <OliveR2 key={item.pageIndex} controller={controller} surfaceId={`document-page-${item.pageIndex}`} rect={item} pageTextLayoutBlocks={xenonR20?.textLayoutBlocks ?? []} pageObjectTargets={yarnR16.get(item.pageIndex) ?? []} selectedTextBlockId={snapshot.selectedTextBlockId} selectedObjectId={snapshot.selectedObjectId} textEditSession={honeyR16} overlayVersion={quillR16} enableWorker={enableWorker} onPointerDown={jewelR16(item, xenonR20)} onPointerMove={knollR16(item, xenonR20)} onPointerUp={mossR16(item)} onClick={northR16(item)} onDoubleClick={orbitR16(item)} />;
      })}</div></div>;
}
function quillR2(garnetR6, ivoryR6, jasperR6) {
  ...
  for (let stormR26 of garnetR6) {
    ...
    hillR13 < falconR24 && (falconR24 = hillR13, echoR24 = stormR26.pageIndex);
  }
  return echoR24;
}
...
function ultraR2(oliveR8, prismR8) {
  ...
  return unityR25 < 0 || valeR25 < 0 || unityR25 > prismR8.width || valeR25 > prismR8.height ? null : {
...
function $l(duskR4, falconR5) {
  let tideR18 = falconR5.scale,
    unityR18 = kiteR13 => kiteR13 === undefined ? undefined : kiteR13 * tideR18;
  return {
    ...duskR4,
    hitBox: {
    ...
    layout: {
      ...
      lines: duskR4.layout.lines.map(item => ({
        ...
        segments: item.segments.map(_item => ({
          ...
          advance: Float32Array.from(_item.advance, ultraR13 => ultraR13 * tideR18)
        }))
      }))
    }
  };
