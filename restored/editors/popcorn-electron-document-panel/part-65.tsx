...
function WheatR2({
  ...
  let zincR17 = RemoteTextEditSessionW(
    controller,
    acornR15.useCallback(() => cedarR2(), []),
  );
  if (!zincR17) return null;
  let amberR17 = acornR15.useSyncExternalStore(
      (kelpR13) => zincR17.subscribe(kelpR13),
      () => zincR17.getSnapshot(),
      () => zincR17.getSnapshot(),
    ),
    ...
    emberR17 = $c(daisyR17),
    flintR17 =
      emberR17.width > 0 && amberR17.pageLayouts.length > 0
        ? yarnR2({
  ...
  acornR15.useEffect(() => {
    !basaltR17 ||
      !garnetR17 ||
      Math.abs(amberR17.zoom - flintR17) < driftR15 ||
      zincR17.setZoom(flintR17);
  }, [garnetR17, zincR17, flintR17, basaltR17, amberR17.zoom]);
  let kelpR17 = amberR17.textEditState?.selection
      ? `${amberR17.textEditState.selection.selectedCharacterCount} chars selected`
      : amberR17.textEditState?.activeBlockId
        ? "Editing text"
        : amberR17.selectedObjectId
          ? `${amberR17.selectedObjectKind ?? "object"} selected`
          : amberR17.selectedTextBlockId
            ? "Text block selected"
            : "",
  ...
  return (
    ...
    }, [artifactSearchEnabled, zincR17, amberR17.findOpen]),
    (
      <SlateR2
        artifactLabel="Document"
        title={title ?? amberR17.title}
        className={className}
        header={
          <RemoteTextEditSessionChrome
            title={title ?? amberR17.title}
            ...
            centeredContent={
              (enablePageNavigation ?? theme === "codex") ? (
                <PopcornPageNumberNavigation
                  currentIndex={amberR17.pageIndex}
                  totalCount={amberR17.pageCount}
                  itemLabel="page"
                  onChangeIndex={(amberR13) => zincR17.setPageIndex(amberR13)}
            ...
            actions={
              <>
                <button
                  ...
                  onClick={() => zincR17.undo()}
                  disabled={!amberR17.canUndo}
                >
                  {"Undo"}
                </button>
                <button
                  type="button"
                  className="rounded-md border px-2 py-1 text-xs disabled:opacity-50"
                  onClick={() => zincR17.redo()}
                  disabled={!amberR17.canRedo}
                >
                  {"Redo"}
                </button>
                <button
                  ...
                  onClick={() =>
                    isEditing
                      ? zincR17.appendParagraph("New paragraph")
                      : undefined
                  }
                  disabled={!isEditing}
                >
            ...
            icon={
              <RemoteTextEditSessionSurfaceKind kind="document">
                <RemoteTextEditSessionU className="size-4" />
              </RemoteTextEditSessionSurfaceKind>
            }
          />
        }
      >
        <div className="relative h-full min-h-0">
          {artifactSearchEnabled ? (
            <PopcornFindBar
              open={amberR17.findOpen}
              query={amberR17.findQuery}
              summary={amberR17.findSummary}
              focusToken={amberR17.findFocusToken}
              onQueryChange={(basaltR13) => zincR17.setFindQuery(basaltR13)}
              onNavigatePrevious={() => zincR17.goToPreviousFindResult()}
              onNavigateNext={() => zincR17.goToNextFindResult()}
              onClose={() => zincR17.closeFind()}
            />
          ) : null}
          {
            <PrismR2
              ...
              artifactSearchEnabled={artifactSearchEnabled}
              bottomScrollReservePx={bottomScrollReservePx}
              scrollContainerRef={daisyR17}
            />
          }
        </div>
      </SlateR2>
...
function yarnR2({ pageLayouts, viewportWidth }) {
  let novaR26 = pageLayouts.reduce(
...
function acornR2({ pageIndex, pageLayouts, viewportHeight, zoom }) {
  let timberR24 = 32;
  for (let violetR26 of pageLayouts) {
    let bloomR26 = Math.max(1, Math.round(violetR26.height * zoom));
    if (violetR26.pageIndex === pageIndex)
      return Math.max(0, timberR24 + bloomR26 / 2 - viewportHeight / 2);
...
export function PopcornElectronDocumentPanel({
  ...
  let indigoR18 = glideR15.useRef(documentProtoVersion),
    ...
    kiteR18 = RemoteTextEditSessionW(
      undefined,
      () =>
        new hazelR15({
  ...
  return (
    glideR15.useEffect(() => {
      kiteR18 &&
        documentProtoVersion !== indigoR18.current &&
        ((indigoR18.current = documentProtoVersion),
        kiteR18.replaceDocument(coralR2(initialDocumentProto)));
    }, [kiteR18, documentProtoVersion, initialDocumentProto]),
    glideR15.useEffect(() => {
      !kiteR18 ||
        elmR12 == null ||
        (elmR12.requestId !== jadeR18.current &&
          ((jadeR18.current = elmR12.requestId),
          kiteR18.setPageIndex(elmR12.pageNumber - 1, {
            origin: "navigation",
          })));
    }, [kiteR18, elmR12]),
    kiteR18 ? (
      ...
      >
        {
          <WheatR2
            ...
            drawingAnnotationsEnabled={drawingAnnotationsEnabled}
            enablePageNavigation={enablePageNavigation}
            artifactSearchEnabled={artifactSearchEnabled}
            commentThreadsEnabled={commentThreadsEnabled}
          />
        }
      </section>
    ) : null
