...
function $r(riverR7, slateR7, timberR7 = waveR16, umbraR7) {
  ...
  let brookR24 = riverR7.kind === "text" ? drift(riverR7.element, slateR7, timberR7, riverR7.defaultTabStopTwips, umbraR7) : [];
...
function vaporR3(glideR7, honeyR7, irisR7, jewelR7 = waveR16, lunarR7) {
  ...
  let slateR25 = reef(glideR7.element, glideR7.kind, honeyR7, irisR7, jewelR7, lunarR7);
...
function echo(tideR5, valeR5, waveR5) {
  ...
  return oliveR23 === "center" ? valeR5 + Math.max(0, (waveR5 - prismR23) / 2) : oliveR23 === "right" ? valeR5 + Math.max(0, waveR5 - prismR23) : valeR5;
}
...
function gamma(zephyrR13, acornR13) {
  ...
  let yellowR20 = zephyrR13.placement.horizontalRelativeFrom?.trim().toLowerCase(),
    zincR20 = zephyrR13.bbox?.widthEmu !== undefined && zephyrR13.bbox.widthEmu > 0 ? zephyrR13.bbox.widthEmu * workbookE : 0;
  if (zephyrR13.placement.xOffsetEmu !== undefined && Number.isFinite(zephyrR13.placement.xOffsetEmu)) {
  ...
  return amberR20 === "center" ? daisyR20 + Math.max(0, (emberR20 - zincR20) / 2) : amberR20 === "right" ? daisyR20 + Math.max(0, emberR20 - zincR20) : daisyR20;
}
function harbor(acornR10) {
  return acornR10.firstParagraphSpaceBeforePx > 0 || acornR10.lastParagraphSpaceAfterPx > 0;
}
function indigo({
  lineIndex,
  ...
  return lineIndex === 0 ? workbookJ(trailingSpaceAfterPx, firstParagraphSpaceBeforePx) : 0;
}
function jade(stormR10) {
  return stormR10 ? stormR10.segments.some(item => (item.text ?? "").trim().length > 0) : false;
}
function kite(slateR4, timberR4, umbraR4) {
  let mintR21 = timberR5 => {
      ...
      for (; ultraR22 < slateR4.length;) {
        ...
        if (vaporR22 += copperR25, ultraR22 += 1, timberR5 && alphaR25?.flowBreakAfter) {
          wheatR22 = true;
          break;
        }
      }
...
function ui({
  ...
  if (preparedFootnotes.idsByReferenceRunId.size === 0) return kite(lines, startIndex, Math.max(0, boundaryBottomPx - currentY));
  ...
  for (; valeR19 < lines.length;) {
    ...
    for (let frostR14 of oliveR3(onyxR22, preparedFootnotes)) quartzR22.has(frostR14) || (pearlR22.push(frostR14), quartzR22.add(frostR14));
    let slateR22 = pearlR22.length > 0 ? sageR3(preparedFootnotes, pearlR22, footnoteWidthPx, theme).reservedHeightPx : 0,
    ...
    if (waveR19 += violetR22, valeR19 += 1, brookR19 = pearlR22, cliffR19 = quartzR22, onyxR22?.flowBreakAfter) {
      apexR19 = true;
      break;
    }
  }
...
function lemon(onyxR10) {
  return typeof onyxR10 == "number" && Number.isFinite(onyxR10) && onyxR10 > 1 ? Math.floor(onyxR10) : 1;
}
function nickel(harborR10, indigoR10, jadeR10) {
  let elmR13 = 0;
  for (let driftR14 = indigoR10; driftR14 < jadeR10; driftR14 += 1) elmR13 += Math.max(1, harborR10[driftR14] ?? 0);
  return elmR13;
}
function onyx(irisR6, jewelR6, knollR6) {
  ...
  for (let vaporR25 = jewelR6; vaporR25 < knollR6; vaporR25 += 1) {
    ...
    if (quartzR26) for (let questR13 of quartzR26.cells) {
      let yellowR14 = lemon(questR13.rowSpan);
      if (Math.min(irisR6.length, vaporR25 + yellowR14) > knollR6) return false;
    }
  }
  return true;
}
function pearl(quillR8, reefR8, topazR8) {
  ...
  for (let sageR14 = reefR8 + apexR25; sageR14 <= quillR8.length; sageR14 += 1) if (onyx(quillR8, reefR8, sageR14)) return sageR14 - reefR8;
  return apexR25;
}
function quartz(elmR7) {
  let mintR25 = elmR7.table;
  if (!mintR25 || mintR25.rows.length <= 1 || mintR25.properties?.firstRow !== true) return 0;
...
function river(cedarR6, daisyR6, emberR6, flintR6) {
  ...
  let bravoR24 = daisyR6 > 0 && flintR6 > 0 ? alphaR24.rows.slice(0, flintR6) : [],
...
function _i({
  ...
  if (!amberR18 || amberR18.rows.length === 0 || rowOffset >= amberR18.rows.length) return {
    rowCount: 0,
    widthPx,
    heightPx: 0
  };
  let basaltR18 = amberR18.rows,
    cedarR18 = copper(preparedElement, ctx, widthPx, theme, documentGridLinePitchTwips),
    ...
    hazelR18 = rowOffset === 0 && flintR18 > 0 && amberR18.rows.length > flintR18 ? flintR18 + 1 : 1,
    ivoryR18 = Math.min(hazelR18, amberR18.rows.length - rowOffset);
  if (emberR18) {
    let yarnR22 = amberR18.rows.length - rowOffset,
      acornR22 = garnetR18 + nickel(cedarR18.rowHeightsPx, rowOffset, amberR18.rows.length);
    if (!isFreshPageSlot && acornR22 > availableHeight + topazR19) return {
      rowCount: 0,
  ...
  let jasperR18 = garnetR18,
    kelpR18 = 0,
    lotusR18 = 0;
  for (let daisyR25 = rowOffset; daisyR25 < amberR18.rows.length; daisyR25 += 1) {
    ...
    xenonR26 >= ivoryR18 && onyx(basaltR18, rowOffset, daisyR25 + 1) && (kelpR18 = xenonR26, lotusR18 = jasperR18);
  }
  if (kelpR18 === 0) {
    ...
    lotusR18 = garnetR18 + nickel(cedarR18.rowHeightsPx, rowOffset, rowOffset + kelpR18);
  }
...
function slate(isleR7) {
  ...
  return sageR25 ? (sageR25.runs ?? []).every(item => (item.text ?? "").trim().length === 0) : false;
}
function timber(basaltR11, daisyR11) {
  return slate(basaltR11) ? daisyR11.length > 0 && daisyR11.every(item => !jade(item)) : false;
}
function umbra(irisR9) {
  let unityR26 = irisR9.paragraphs ?? [];
  return unityR26.length === 1 && unityR26[0]?.docxSectionBreakCarrier === true && slate(irisR9);
}
function violet(brookR7) {
  return brookR7 ? brookR7.segments.length === 0 && brookR7.widthPx === 0 && brookR7.baselineOffsetPx === 0 && brookR7.maxAscentPx === 0 && brookR7.maxDescentPx === 0 : false;
}
function willow({
  lineIndex,
  lines,
  ...
  return lineIndex !== 0 || firstParagraphSpaceBeforePx <= 0 || !violet(lines[0]) ? false : previousElementWasEmptyParagraph && previousEmptyParagraphHasSpacing;
}
function yellow(northR8, orbitR8) {
  let echoR26 = northR8[orbitR8];
  return echoR26 ? falconR3(echoR26.kind) ? !(gammaR3(echoR26.element) || harborR3(echoR26.element) || indigoR3(echoR26.element) && !frostR2(echoR26.element)) : true : false;
}
function zinc(lemonR8, willowR8, hazelR8, sageR8, eagleR8, pineR8 = waveR16, elmR8) {
  let lotusR19 = 0;
  for (let ultraR19 = hazelR8 + 1; ultraR19 < willowR8.length; ultraR19 += 1) {
    ...
    if (lunarR19.kind !== "text") return lotusR19 + vaporR3(lunarR19, sageR8, eagleR8, pineR8, elmR8).heightPx;
    let mossR19 = $r(lunarR19, eagleR8, pineR8, elmR8),
      ...
      orbitR19 = bravoR3({
        ...
        currentParagraphUsesContextualSpacing: lunarR19.firstParagraphUsesContextualSpacing
      });
    if (!northR19 && lunarR19.firstParagraphSpaceBeforePx <= 0 && lotusR19 <= 0) return;
    let questR19 = willow({
      lineIndex: 0,
      lines: mossR19,
    ...
    let ridgeR19 = mossR19.slice(questR19).reduce((accumulator, current) => accumulator + Math.max(1, current.heightPx), 0);
    if (northR19) {
      lotusR19 += ridgeR19;
      continue;
    }
