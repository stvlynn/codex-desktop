...
function alpha(flint) {
  ...
  if (!garnet.success) throw Error("Notebook must be a JSON object with a cells array");
  return {
  ...
  switch (hazel.cell_type) {
    case "code":
      return {
        ...
        id: jasper,
        outputs: (hazel.outputs ?? []).flatMap((item, index) => copper(item, index, indigo(hazel.metadata, index))),
        source: ivory,
        title: kelp
      };
    case "markdown":
...
function delta(prism, quill, reef) {
  ...
  if (ultra != null && ultra.trim().length > 0) return [{
    html: ultra,
    type: "html"
  }];
  let vapor = kite(sage.data["text/markdown"]);
  if (vapor != null && vapor.trim().length > 0) return [{
    markdown: vapor,
    type: "markdown"
  }];
  ...
  let yarn = sage.data["application/json"] ?? sage.data["application/vnd.vega.v5+json"];
  return yarn == null ? [] : [{
    summaryMarkdown: reef,
...
function harbor(iris, jewel) {
  for (let knoll of jade(iris)) for (let lunar of jewel) {
...
function jade(tide) {
  return tide == null ? [] : [...lemon.flatMap(item => {
    let unity = onyx.safeParse(tide[item]);
    return unity.success ? [unity.data] : [];
  }), tide];
}
function kite(vale) {
  return typeof vale == "string" ? vale : Array.isArray(vale) && vale.every(item => typeof item == "string") ? vale.join("") : null;
}
var lemon,
  marble,
  nickel,
  ...
  timber = esmInit(() => {
    ...
    nickel = ["codeDescriptionMarkdown", "code_description_markdown", "descriptionMarkdown", "description_markdown", "description"];
    onyx = ensureAccountPlanQueryInit(coalesceTruthy(), siteAnalyticsPath());
    pearl = siteAnalyticsEventsPath([coalesceTruthy(), deferredNavigationFT(coalesceTruthy())]).optional();
    quartz = onyx;
    river = deferredUiXT({
...
var violet,
  willow = esmInit(() => {
    violet = ["default-src 'none'", "base-uri 'none'", "connect-src 'none'", "font-src data:", "form-action 'none'", "frame-src 'none'", "img-src data: blob:", "media-src data: blob:", "object-src 'none'", "script-src 'none'", "style-src 'unsafe-inline'"].join("; ");
  });
export function NotebookPreviewPanel(joint: unknown) {
  ...
  let motor = latch,
    nut = motor.status === "ready" ? motor.document.title ?? daisy(title) : daisy(title);
  let piston = nut,
    rivet = motor.status === "ready" ? keystone.formatMessage({
  ...
  let screw = rivet,
    ...
    valve = motor.status === "ready" ? <>
        ...
        children: [<AppInitialJa className="icon-2xs" />, <span className="hidden md:inline">
  ...
  let axle = <NotebookPreviewOpenActions {...{
    hostId,
    path
  }} />;
  let bracket = <div className="flex min-w-0 flex-wrap items-center justify-end gap-1 overflow-hidden">
      {valve}
      {axle}
      {headerRightContent}
    </div>;
  let clamp = <IsLocalOrNullValue {...{
    artifactType: torque,
    centerContent: null,
    rightContent: bracket,
    title: piston
  }} />;
  let drill = motor.status === "ready" ? <NotebookPreviewPanelHelper15 {...{
...
function NotebookPreviewPanelHelper13() {
  return <span className="bg-token-main-surface-secondary/30 inline-flex h-7 shrink-0 items-center rounded-full border border-token-border-light px-2 text-xs font-medium text-token-text-tertiary">
      {<MemoizedFormattedMessage {...{
...
function NotebookPreviewPanelHelper14(apex) {
  ...
  return <button aria-disabled={true} className="inline-flex h-7 shrink-0 cursor-default items-center gap-1 rounded-md px-2 text-xs font-medium text-token-text-tertiary/70" disabled={true} title={label} type="button">
      {children}
    </button>;
}
function NotebookPreviewPanelHelper15(brook) {
  ...
  if (document.cells.length === 0) {
    let dusk;
    return <div className="flex min-h-0 flex-1 items-center justify-center px-6 text-center text-sm text-token-text-tertiary">
        {<MemoizedFormattedMessage {...{
  ...
  {
    let elm;
    elm = (fern, grove) => <_e key={fern.id ?? grove} {...{
      cell: fern,
      cellNumber: grove + 1,
  ...
  return <div className="min-h-0 flex-1 overflow-auto bg-token-side-bar-background px-4 py-4 sm:px-6 sm:py-5">
...
function _e(hill) {
  let {
    ...
    isle = <summary className="flex cursor-interaction list-none items-center justify-between gap-3 border-b border-token-border-light px-4 py-2 [&::-webkit-details-marker]:hidden">
  ...
  let juniper = <NotebookPreviewPanelHelper17 {...{
    cell
  }} />;
  return <details className="group/notebook-cell overflow-hidden rounded-lg border border-token-border-light bg-token-main-surface-primary" open={true}>
      {isle}
      {juniper}
    </details>;
}
function NotebookPreviewPanelHelper16(lagoon) {
  ...
  let quiet = <div className="min-w-0 truncate text-sm font-medium text-token-text-primary" title={oak}>
      {oak}
    </div>;
  let rain = <span className="shrink-0 text-xs text-token-text-tertiary">
      {<MemoizedFormattedMessage {...{
  ...
  let seed = <div className="flex min-w-0 items-center gap-2">
      {petal}
      {quiet}
      {rain}
    </div>;
  let trail = cell.cellType === "code" && cell.executionCount != null ? <span className="tabular-nums">
        {<MemoizedFormattedMessage {...{
  ...
  let urn = cell.cellType === "code" ? <span aria-hidden={true} className="pointer-events-none inline-flex opacity-0 transition-opacity duration-basic group-focus-within/notebook-cell:opacity-60 group-hover/notebook-cell:opacity-60" title={meadow.formatMessage({
  ...
  let vine = <div className="flex shrink-0 items-center gap-2 text-xs font-medium text-token-text-tertiary">
      {trail}
      {urn}
    </div>;
  return <>
...
function NotebookPreviewPanelHelper17(wind) {
  ...
  if (cell.cellType === "markdown") {
    ...
    return <div className="px-4 py-3">
        {cell.source.trim().length === 0 ? <NotebookPreviewPanelHelper19 {...{
  ...
  if (cell.cellType === "code") {
    ...
    let ever = dew.length > 0,
      field = canyon.length > 0 ? <AreVisualizationFeatureGatesEnabled {...{
        allowBasicHtml: true,
        className: "text-size-chat",
        children: canyon
      }} /> : null;
    let grain = ever ? canyon.length > 0 ? <NotebookPreviewPanelHelper18 {...{
      code: cell.source
    }} /> : <IsIndeterminate {...{
      content: cell.source,
      language: "python",
    ...
    let haven = <div className="px-4 py-3">
        {field}
        {grain}
      </div>;
    let ink = cell.outputs.length > 0 ? <div className="bg-token-main-surface-secondary/15 border-t border-token-border-light px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
...
function be(kernel, leaf) {
  return <NotebookPreviewPanelHelper20 key={leaf} {...{
    output: kernel
  }} />;
}
function NotebookPreviewPanelHelper18(maple) {
  let {
    ...
    nimbus = <summary className="flex cursor-interaction list-none items-center gap-2 rounded-md py-1 text-left text-xs font-medium text-token-text-tertiary transition-colors hover:text-token-text-primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-token-text-tertiary [&::-webkit-details-marker]:hidden">
        {<AppIconSft {...{
  ...
  let opal = <MemoizedFormattedMessage {...{
    id: "notebookPreview.pythonCodeTitle",
    defaultMessage: "Python",
    description: "Code snippet title for a Python notebook cell"
  }} />;
  return <details className="group/code mt-3 border-t border-token-border-light pt-2">
      {nimbus}
      <div className="mt-2">
        {<IsIndeterminate {...{
        content: code,
...
function NotebookPreviewPanelHelper19(plume) {
  ...
  return <div className="rounded-md border border-token-border-light px-3 py-2 text-sm text-token-text-tertiary">
      {children}
    </div>;
}
function NotebookPreviewPanelHelper20(quillow) {
  ...
  switch (output.type) {
    case "image":
      {
        ...
        let thorn;
        return <div className="overflow-auto rounded-md bg-token-main-surface-primary/40 p-2">
          <img alt={silk} className="max-h-[640px] max-w-full" src={output.dataUrl} />
        </div>;
      }
    case "html":
      {
        ...
        let wisp = <iframe className="h-72 w-full rounded-md bg-token-main-surface-primary" sandbox="" srcDoc={upland} title={vista} />;
        let yonder = <NotebookPreviewPanelHelper22 {...{
          className: "mt-2",
    ...
    case "markdown":
      {
        let anvil;
        return <div className="rounded-md bg-token-main-surface-primary/40 px-3 py-2">
          {<AreVisualizationFeatureGatesEnabled {...{
            allowBasicHtml: true,
            className: "text-size-chat",
    ...
    case "json":
      {
        let beacon;
        return <NotebookPreviewPanelHelper21 {...{
          language: "json",
          rawText: output.text,
    ...
    case "error":
      {
        let crag = output.summaryMarkdown == null ? <div className="text-sm font-medium text-token-charts-red">
            {output.evalue.length > 0 ? <MemoizedFormattedMessage {...{
          ...
          </div> : <AreVisualizationFeatureGatesEnabled {...{
          allowBasicHtml: true,
          className: "text-size-chat",
          children: output.summaryMarkdown
        }} />;
        let dome = zinc(output).trim().length > 0 ? <NotebookPreviewPanelHelper22 {...{
          className: "mt-2",
          children: zinc(output)
        }} /> : null;
        let eddy;
        return <div className="rounded-md border border-token-charts-red/30 bg-token-charts-red/5 p-3">
          {crag}
          {dome}
        </div>;
      }
...
function NotebookPreviewPanelHelper21(glen) {
  ...
  if (summaryMarkdown != null) {
    let hearth = <AreVisualizationFeatureGatesEnabled {...{
      allowBasicHtml: true,
      className: "text-size-chat",
      children: summaryMarkdown
    }} />;
    let inlet = <NotebookPreviewPanelHelper22 {...{
      className: "mt-2",
      children: rawText
    }} />;
    let jetty;
    return <div className="rounded-md bg-token-main-surface-primary/40 p-3">
        {hearth}
        {inlet}
      </div>;
  }
  ...
  return <NotebookPreviewPanelHelper23 {...{
    children: rawText
  }} />;
}
function NotebookPreviewPanelHelper22(ledge) {
  let {
    ...
    mire = <summary className="cursor-interaction text-xs font-medium text-token-text-tertiary marker:text-token-text-tertiary">
        {<MemoizedFormattedMessage {...{
  ...
  let nook = <NotebookPreviewPanelHelper23 {...{
    className: "mt-2",
    children
  }} />;
...
function NotebookPreviewPanelHelper23(oxbow) {
  let {
    ...
    pond = IntlProvider("overflow-auto rounded-md bg-token-text-code-block-background/20 p-3 font-mono text-xs whitespace-pre-wrap text-token-text-primary", className);
...
function yellow(rapids) {
  ...
  for (let updraft = 0; updraft < spur.length; updraft += 1) tor[updraft] = spur.charCodeAt(updraft);
  return tor;
}
function zinc(verge) {
  let weir = `${verge.ename}: ${verge.evalue}`.trim();
  return verge.traceback.trim().length === 0 ? weir : `${weir}\n${verge.traceback}`;
}
function amber(yard, anchor, bolt) {
  ...
  switch (anchor.cellType) {
    case "markdown":
      return basalt(anchor.source) ?? yard.formatMessage({
    ...
    case "code":
      {
        ...
        return disc.length > 0 ? cedar(disc) : yard.formatMessage({
...
function basalt(edge) {
  let forge = edge.split(/\r?\n/).map(item => item.trim()).find(item => /^#{1,6}\s+/.test(item));
