...
function Component_n(copperPrime) {
    ...
    quartzPrime;
  {
    ...
    let violetPrime = umbraPrime,
      willowPrime = notifications.length > 1 && (isExpanded || nativePresentationOwnsMotion) ? activityStackPresentation.viewportRect.height : activityStackPresentation.slots[0]?.presentationRect.height ?? 0,
      xenonPrime = isVisiblyExpanded ? willowPrime : AppInitialUgt(notifications.length, activityStackPresentation.slots[0]?.presentationRect.height);
    let yellowPrime = xenonPrime,
      zincPrime = isExpanded && !nativeMaterialAttached && compositionSurfaceId == null && activityStackPresentation.contentHeight > activityStackPresentation.viewportRect.height,
    ...
    onyxPrime = activityCopies?.[0]?.notificationListAriaLabel ?? echoPrime.formatMessage(isle.notificationList);
    let emberPrime = compositionSurfaceId == null && nativePresentationOwnsMotion && "overflow-hidden";
    pearlPrime = IntlProvider("relative w-[345px] max-w-full shrink-0", emberPrime);
    quartzPrime = "notification-stack-visible-layout";
    gammaPrime = "list";
    harborPrime = {
      height: yellowPrime
    };
    indigoPrime = isExpanded && onActivityStackScroll != null ? event => {
    ...
    nickelPrime = IntlProvider("absolute", flintPrime, garnetPrime, hazelPrime, ivoryPrime, jasperPrime, kelpPrime);
    let lotusPrime = zincPrime ? "inset-x-6" : "inset-x-0",
      mintPrime = isTrayAboveMascot ? zincPrime && !daisyPrime ? "bottom-6" : "bottom-0" : zincPrime && !basaltPrime ? "top-6" : "top-0";
    ...
    marblePrime = activityStackPresentation.slots.map(item => {
      ...
      if (slotId === "activity-slot-8" && !nativePresentationOwnsMotion) return null;
      ...
      let olivePrime = activityCopies?.[notifications.indexOf(novaPrime)],
        prismPrime = novaPrime === notifications[0] && (!nativePresentationOwnsMotion || (isExpanded ? slotId !== "activity-slot-8" : slotId === "activity-slot-8" || !violetPrime)),
        quillPrime = !isExpanded && !prismPrime;
      if (compositionSurfaceId != null && compositionSurfaceId !== slotId) return null;
      let reefPrime = nativePresentationOwnsMotion && compositionSurfaceId == null;
      return <ensureIntlFormattersInit.div key={`${slotId}:${itemId}`} layout={!nativePresentationOwnsMotion} layoutDependency={isExpanded} aria-hidden={reefPrime || quillPrime ? "true" : undefined} className={IntlProvider("absolute", quillPrime && "pointer-events-none", reefPrime && "invisible")} inert={reefPrime || quillPrime || undefined} style={{
        left: presentationRect.left - activityStackPresentation.viewportRect.left,
        top: presentationRect.top - activityStackPresentation.viewportRect.top,
        width: presentationRect.width,
        zIndex
      }} transition={stackTransition} onUpdate={onAnimationFrame}>{<Knoll canExpandActivityStack={!isExpanded && prismPrime && notifications.length > 1} copy={olivePrime} isStackedBehindPrimary={quillPrime} isNativeMeasurementPlaceholder={reefPrime} isNotificationStackCollapsed={isNotificationStackCollapsed} isPointerSurfaceHovered={pointerSurfaceId === slotId} nativeMaterialAttached={nativeMaterialAttached} nativePresentationOwnsMotion={nativePresentationOwnsMotion} nativeSurfaceId={slotId} notification={novaPrime} notificationFollowUp={notificationFollowUp} publishesFollowUpVisibility={publishesFollowUpVisibility} isExpanded={onNotificationExpansionChange == null ? undefined : expandedNotificationIds.includes(novaPrime.id)} onAnimationFrame={onAnimationFrame} role={prismPrime ? "primary" : "secondary"} onActivateNotification={deltaPrime} onDismissNotification={onDismissNotification} onOpenNotificationActions={onOpenNotificationActions} onExpansionChange={onNotificationExpansionChange} onRunNotificationControl={onRunNotificationControl} onRunNotificationAction={onRunNotificationAction} onSubmitQuestionOption={onSubmitQuestionOption} />}</ensureIntlFormattersInit.div>;
    });
  }
  let riverPrime = <div className={falconPrime} data-avatar-overlay-backing-canvas={jadePrime} data-avatar-overlay-size={kitePrime} style={lemonPrime}>{marblePrime}</div>;
  let slatePrime = <div className={nickelPrime}>{riverPrime}</div>;
  return <div aria-label={onyxPrime} className={pearlPrime} data-avatar-overlay-size={quartzPrime} role={gammaPrime} style={harborPrime} onWheel={indigoPrime}>{slatePrime}</div>;
}
...
function Iris({
  ...
  let wheatPrime = CodexPluginActionType(appScopeAtom),
    ...
    lunarPrime = dusk.useEffectEvent(() => {
      acornPrime.current && (acornPrime.current = false, onActiveChange?.(false));
    }),
    mossPrime = () => {
      onAnimationFrame != null && window.requestAnimationFrame(onAnimationFrame);
    },
    ...
    orbitPrime = async tidePrime => {
      if (!(tidePrime.length === 0 || bloomPrime || onSubmitQuickChat == null)) {
        coralPrime(true);
        honeyPrime(null);
        try {
    ...
    pinePrime = quickChatDictation.supportState,
    {
      ...
      onTranscriptInsert: unityPrime => {
        ...
        valePrime !== frostPrime && (northPrime(valePrime), zephyrPrime.current?.focus());
      },
      ...
      onStartError: brookPrime => {
        wheatPrime.get(toastAtom).danger(AppInitialTct(yarnPrime, "start", brookPrime).message);
        appActionSidebarProjectRefSchema.error("[AvatarOverlay] unable to start dictation", {
          safe: {},
          sensitive: {
            error: brookPrime
          }
      ...
      onTranscribeError: cliffPrime => {
        wheatPrime.get(toastAtom).danger(AppInitialTct(yarnPrime, "transcription", cliffPrime).message);
        appActionSidebarProjectRefSchema.error("[AvatarOverlay] dictation failed", {
          safe: {},
          sensitive: {
            error: cliffPrime
          }
      ...
      onUnsupported: () => {
        wheatPrime.get(toastAtom).danger(yarnPrime.formatMessage({
    ...
    questPrime = pinePrime !== false && (irisPrime.length === 0 || isDictating || isTranscribing || canRetryDictation);
  dusk.useLayoutEffect(() => (onAnimationFrame?.(), () => {
    onAnimationFrame != null && window.requestAnimationFrame(onAnimationFrame);
  }), [onAnimationFrame]);
  dusk.useEffect(() => () => {
    lunarPrime();
  }, []);
  writeScrollTop("avatar-overlay-keyboard-interaction-ready", () => {
  ...
  return <Falcon emptyWidth={jewelPrime} nativeMaterialAttached={nativeMaterialAttached} nativePresentationOwnsMotion={nativePresentationOwnsMotion} width={knollPrime} onFocusCapture={() => {
    acornPrime.current = true;
    onActiveChange?.(true);
  }} onBlurCapture={event => {
    let duskPrime = event.relatedTarget;
    duskPrime instanceof Node && event.currentTarget.contains(duskPrime) || (acornPrime.current = false, onActiveChange?.(false));
  }} onPointerDown={() => {
    acornPrime.current = true;
    onActiveChange?.(true);
  }} onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave} onSubmit={elmPrime => {
    ridgePrime(elmPrime);
  }} onUpdate={onAnimationFrame}><input ref={zephyrPrime} aria-label={yarnPrime.formatMessage(isle.quickChat)} className={IntlProvider("min-w-0 flex-1 bg-transparent text-[14px] leading-[18px] font-medium outline-none", nativeMaterialAttached ? "text-token-foreground placeholder:text-token-foreground/35" : "text-token-foreground placeholder:text-token-text-secondary")} data-avatar-overlay-composition-autofocus="true" placeholder={hasNotifications ? yarnPrime.formatMessage(isle.startNewTaskPlaceholder) : yarnPrime.formatMessage(isle.askPlaceholder)} value={frostPrime} onChange={event => {
      northPrime(event.currentTarget.value);
    }} /><ensureIntlFormattersInit.div className="shrink-0" layout="position" transition={{
      duration: 0.18,
      ease: [0.16, 1, 0.3, 1]
    }}>{questPrime ? <AppInitialLt className="size-7 shrink-0 !bg-[#636363] !text-white enabled:hover:!bg-[#636363] disabled:opacity-100 electron:!rounded-full" color="secondary" idleIcon={<NeedsApps className="icon-sm" />} isDictating={isDictating} isVisible={true} isTranscribing={isTranscribing} canRetryDictation={canRetryDictation} disabled={pinePrime !== true || bloomPrime || isTranscribing} retryDictation={retryDictation} shortcutLabel={null} size="icon" startDictation={startDictation} stopDictation={stopDictation} /> : <ReadLoginRouteQuerySnapshot aria-label={yarnPrime.formatMessage(isle.sendQuickChat)} className="size-7 shrink-0 !bg-[#636363] !text-white enabled:hover:!bg-[#636363] disabled:opacity-100 electron:!rounded-full" color="secondary" disabled={irisPrime.length === 0 || bloomPrime} loading={bloomPrime} size="icon" type="submit">{bloomPrime ? null : <AppIconPlt className="icon-sm" />}</ReadLoginRouteQuerySnapshot>}</ensureIntlFormattersInit.div>{glidePrime == null ? null : <span className="sr-only" role="alert">{glidePrime}</span>}</Falcon>;
}
function Jewel(fernPrime) {
  let {
    ...
    juniperPrime = event => {
      ...
      islePrime.current !== birchPrime && (islePrime.current = birchPrime, onDraftChange(event.currentTarget.value, avatarOverlayPillDismissButtonA(event.currentTarget)));
    };
  ...
  let oakPrime = isNativeMeasurementPlaceholder ? undefined : {
    height: 0,
    marginBottom: 0,
    opacity: 0,
    y: prefersReducedMotion ? 0 : -2
  };
  let petalPrime = isNativeMeasurementPlaceholder || keepsNativeBackingMounted ? false : {
    height: 0,
    marginBottom: 0,
    opacity: 0,
    y: prefersReducedMotion ? 0 : -2
  ...
  let quietPrime = hillPrime ? true : undefined,
    rainPrime = isNativeMeasurementPlaceholder || prefersReducedMotion ? {
      duration: 0
    } : {
      height: {
        duration: 0.22,
  ...
  let yarrowPrime = <textarea ref={inputRef} aria-label={trailPrime} autoFocus={autoFocus} className="[field-sizing:content] max-h-20 min-w-0 resize-none overflow-y-auto bg-transparent py-[4.5px] text-[13px] leading-4 text-token-foreground outline-none placeholder:text-token-text-secondary" data-avatar-overlay-composition-autofocus={urnPrime} placeholder={vinePrime} rows={1} value={draft} onChange={lagoonPrime} onSelect={lagoonPrime} onKeyDown={windPrime} />;
  let azurePrime = hasSubmissionError ? <div className="truncate pb-1 text-[11px] leading-3 text-token-error-foreground" role="alert">{<MemoizedFormattedMessage id="avatarOverlay.notificationFollowUpError" defaultMessage="Unable to send reply" description="Compact error shown when a floating avatar notification follow-up fails" />}</div> : null;
  return <ensureIntlFormattersInit.form aria-hidden={meadowPrime} animate={nestPrime} className="no-drag electron-dark:bg-token-side-bar-background mx-3.5 grid min-h-[26px] grid-cols-1 items-center overflow-hidden rounded-[12.5px] border-[0.5px] border-token-input-border bg-token-main-surface-primary px-2.5" exit={oakPrime} initial={petalPrime} inert={quietPrime} transition={rainPrime} onSubmit={seedPrime} onUpdate={onAnimationFrame}>{yarrowPrime}{azurePrime}</ensureIntlFormattersInit.form>;
}
