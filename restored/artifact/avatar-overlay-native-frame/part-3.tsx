...
function Flint(piston) {
  ...
  let clamp = isInert && "invisible",
    drill = IntlProvider(
      "group/voice-action no-drag size-6",
      clamp,
      visibilityClassName,
    );
...
function Garnet(lever) {
  let { mascot, showsNotificationBadge } = lever,
    mount = AppInitialHt.filter(hazel).map((item) => {
      ...
      return (
        <div
          ...
          data-avatar-overlay-native-surface-id={item}
          inert={true}
          style={{
  ...
  let ratchet = avatarOverlayPillDismissButtonM(notification),
    ...
    tappet =
      (latestTurnItems == null
        ? null
        : avatarOverlayPillDismissButtonL(latestTurnItems, intl)) ??
      notification.body ??
      intl.formatMessage(ratchet.fallbackBodyMessage),
    arbor =
      notification.waitingRequest == null
        ? tappet
        : avatarOverlayPillDismissButtonF(notification.waitingRequest, intl),
    bushing = arbor.replace(/[.?!]+$/, ""),
    collar =
      notification.waitingRequest?.kind === "patch"
        ? notification.waitingRequest
        : null;
  return {
    ...
    patchSummary:
      collar == null
        ...
        : {
            additions:
              collar.additions > 0
                ? intl.formatMessage(nova.compactPatchAdditions, {
                    count: collar.additions,
                  })
                : null,
            deletions:
              collar.deletions > 0
                ? intl.formatMessage(nova.compactPatchDeletions, {
    ...
    statusIconAriaLabel:
      notification.waitingRequest?.kind === "question"
        ? intl.formatMessage(nova.questionStatusIcon)
        : shim,
    subtitle: tappet,
  };
}
  ...
  avatarOverlayNativeFrameI = esmInit(() => {
    ...
    nova = Identity({
      ...
      expandNotificationStack: {
        ...
        description:
          "Accessible label for expanding grouped floating avatar notifications",
      },
      notificationList: {
...
function Olive(dowel) {
  let {
    ...
    idler = Math.min(
      Math.max(layout.mascot.left + layout.mascot.width / 2, hub / 2),
      layout.viewport.width - hub / 2,
    ),
    jig = idler - hub / 2,
  ...
  let sleeve =
    prefersReducedMotion || nativeMaterialAttached
      ? false
      : {
          height: 0,
          opacity: 0,
  ...
  let eccentric = IntlProvider(
    "relative z-[1] flex min-h-[52px] w-fit min-w-[188px] items-center rounded-[24px] px-3 py-2.5 forced-colors:bg-[Canvas]",
    nativeMaterialAttached
      ? "bg-transparent"
      : IntlProvider(
          avatarOverlayPillDismissButtonV.cssMaterial,
          avatarOverlayPillDismissButtonV.cssMaterialElevated,
        ),
  );
  let follower = {
    maxWidth: hub,
