...
function quartz({ mascot, orbScale = 1 }) {
  return {
    left: mascot.left + mascot.width / 2,
    top: mascot.top + (mascot.height * (1 + orbScale)) / 2,
  };
}
function river({ mascot, viewport }) {
  let jetty = mascot.left + mascot.width / 2;
  return Math.max(
    0,
    2 * Math.min(jetty - willow, viewport.width - jetty - willow),
  );
}
export function avatarOverlayNativeFrameA({
  ...
  return caption == null || status == null
    ? 0
    : Math.max(0, status.top + status.height + violet - caption.top);
}
function slate({ mascot, showsNotificationBadge, surfaceId }) {
...
function Yellow(quarry) {
  ...
  return (
    <div
      ...
      data-avatar-overlay-native-surface-id={nativeSurfaceId}
      inert={inert}
      style={style}
    >
      {children}
...
function Cedar(tor) {
  ...
  yard
    ? (forge = lotus.starting)
    : cog && onOpenThread != null
      ? (forge = lotus.openThread)
      : weir && (forge = lotus.stop);
  let gear = disc ? lotus.stop : forge,
    hinge =
      (weir && !isRealtimeVoiceSurfaceVisible) || areControlsVisible
  ...
  let iron = hinge,
    joint =
      !nativeMaterialAttached && nativeSurfaceId == null && disc ? (
        <div
  ...
  let keystone =
    disc && (nativeSurfaceId == null || nativeSurfaceId === "voice-output") ? (
      <Flint
        key="voice-output"
        ariaLabel={verge.formatMessage(
          isMuted ? lotus.unmuteOutput : lotus.muteOutput,
        )}
        disabled={!cog}
        isAnimated={!nativeMaterialAttached}
        isInert={isInert}
        isPressed={isMuted}
        ...
        visibilityClassName={iron}
        onClick={onToggleMute}
      >
        {isMuted ? (
          <ScrollingOrbBackdrop className="icon-xs" />
        ) : (
          <SoundOn className="icon-xs" />
        )}
      </Flint>
    ) : null;
  let latch =
    disc &&
    (nativeSurfaceId == null || nativeSurfaceId === "voice-microphone") ? (
      <Flint
        key="voice-microphone"
        ariaLabel={verge.formatMessage(
          isMicrophoneMuted ? lotus.unmuteMicrophone : lotus.muteMicrophone,
        )}
        disabled={!cog}
        isAnimated={!nativeMaterialAttached}
        isInert={isInert}
        ...
        tooltipContent={
          <MemoizedFormattedMessage
            {...(isMicrophoneMuted
              ? lotus.unmuteMicrophone
              : lotus.muteMicrophone)}
          />
        }
        visibilityClassName={iron}
        onClick={updraft}
  ...
  let nut =
    nativeSurfaceId == null || nativeSurfaceId === "voice-controls" ? (
      <Flint
        ariaLabel={verge.formatMessage(gear)}
        disabled={yard || anchor}
        isInert={isInert}
        nativeMaterialAttached={nativeMaterialAttached}
        ...
        tooltipContent={<MemoizedFormattedMessage {...gear} />}
        visibilityClassName={iron}
        onClick={() => {
          if (disc) {
            onStop?.();
      ...
      >
        {disc ||
        ((isRealtimeVoiceSurfaceVisible || onOpenThread == null) &&
          (cog || anchor)) ? (
