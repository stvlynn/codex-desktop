...
/** Wave GA unresolved companion (jsx-collision:ensureComposerEsm_Hlt_Init@composer/composer-esm-inits.ts) */
...
function copper(quiet) {
  let {
    ...
    rain = nativePresentationOwnsMotion
      ? undefined
      : {
          opacity: 1,
        };
  let seed = nativePresentationOwnsMotion
    ? false
    : {
        opacity: 0,
      };
  ...
  let urn = harbor(
    nativeMaterialAttached,
    IntlProvider(
      avatarOverlayPillDismissButtonV.activityPillMaterial,
      className,
    ),
  );
  let vine = nativeSurfaceId ?? at(role);
  let wind = (
    <Gamma className={urn} cornerRadius={27} nativeSurfaceId={vine}>
      {children}
    </Gamma>
  );
  return (
...
function Delta(yarrow) {
  ...
  return (
    <Gamma
      {...rest}
      className={birch}
      cornerRadius={azure.cornerRadius}
...
function Echo(canyon) {
  let { nativeMaterialAttached, notificationCount } = canyon,
...
function Falcon(opal) {
  let {
    ...
    plume = nativePresentationOwnsMotion
      ? undefined
      : nativeMaterialAttached
        ? {
            opacity: 1,
  ...
  let quillow = nativePresentationOwnsMotion
    ? undefined
    : {
        opacity: 0,
      };
  let root =
    nativeMaterialAttached || nativePresentationOwnsMotion
      ? false
      : {
          opacity: 0,
          width: emptyWidth,
...
function Gamma(wisp) {
  let { children, className, cornerRadius, nativeSurfaceId, ...rest } = wisp;
  return (
    <div
      {...rest}
      className={className}
      data-avatar-overlay-native-corner-radius={cornerRadius}
...
function harbor(yonder, zenith) {
  return IntlProvider(
    ...
    yonder ? "bg-transparent" : avatarOverlayPillDismissButtonV.cssMaterial,
    !yonder && avatarOverlayPillDismissButtonV.cssMaterialElevated,
  );
}
var indigo,
  jade,
...
function AvatarOverlayNativeFrameS(anvil) {
  ...
  let glen = isNativeMeasurementPlaceholder || undefined,
    hearth = isNativeMeasurementPlaceholder ? (
      text
    ) : (
