...
function Pine(riverSecond) {
  let { copy, files, isExpanded, summary } = riverSecond;
  if (copy == null) return null;
  if (!isExpanded) {
    let yellowSecond = <span>{copy.fileCount}</span>;
    let zincSecond =
      copy.additions == null ? null : (
        <span className="ml-1.5 text-token-charts-green">{copy.additions}</span>
      );
    let amberSecond =
      copy.deletions == null ? null : (
        <span className="ml-1.5 text-token-error-foreground">
          {copy.deletions}
        </span>
      );
    let basaltSecond = (
      <BulletSeparator className="mx-1.5 text-token-text-tertiary" />
    );
  ...
  let slateSecond = (
    <span className="text-[11px] leading-4">{copy.fileCount}</span>
  );
  let timberSecond =
    copy.additions == null ? null : (
  ...
  let xenonSecond = (
    <div className="mt-0.5 min-w-0 space-y-0.5">{willowSecond}</div>
  );
  return (
    <div
      data-avatar-overlay-compact-waiting-summary-text="true"
...
function quest(emberSecond) {
  return (
    <div key={emberSecond} className="leading-4 break-words">
      {emberSecond}
    </div>
  );
}
function Ridge(flintSecond) {
  let { actions, nativeMaterialAttached, onRunNotificationAction } =
    flintSecond;
  return (
    <div className="no-drag mt-1.5 flex min-w-0 flex-wrap items-center justify-end gap-2 overflow-visible pb-[3px]">
      {[...actions.filter(unity), ...actions.filter(tide)].map((item) => (
...
function wave(jasperSecond) {
  return (
    jasperSecond.clientWidth > 0 &&
    jasperSecond.scrollWidth > jasperSecond.clientWidth + grove
  );
}
...
function brook(lotusSecond) {
  ...
  let novaSecond = lotusSecond.mascot,
    ...
    quillSecond = prismSecond
      ? mintSecond.top + mintSecond.height - lagoon
      : novaSecond.top + novaSecond.height - lagoon,
    reefSecond = prismSecond
      ? novaSecond.top + lagoon
      : mintSecond.top + lagoon;
  return {
    ...
    width:
      Math.max(
        mintSecond.left + mintSecond.width,
        novaSecond.left + novaSecond.width,
      ) -
      oliveSecond +
      lagoon,
  };
...
export const avatarOverlayNativeFrameN = esmInit(() => {
  ...
  isle = Identity({
    ...
    stopNotification: {
      ...
      description:
        "Accessible label for stopping a running task from the floating avatar notification tray",
    },
    quickChat: {
    ...
    startNewTaskPlaceholder: {
      ...
      description:
        "Placeholder for Quick Chat when the floating pet has activity",
    },
    quickChatError: {
  ...
  lagoon = 8;
  meadow =
    "relative size-[var(--activity-pill-control-size)] shrink-0 !rounded-full !border-0 bg-transparent !p-0 before:pointer-events-none before:absolute before:inset-0 before:rounded-full before:content-['']";
  nest = {
    duration: 0.22,
    ease: [0.4, 0, 0.6, 1],
  };
