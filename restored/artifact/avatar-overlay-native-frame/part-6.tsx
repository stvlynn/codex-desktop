...
function Knoll(dewPrime) {
  ...
  let inkPrime = havenPrime,
    ...
    [thornPrime, uplandPrime] = dusk.useState(
      notificationFollowUp?.draft ?? "",
    ),
    [vistaPrime, wispPrime] = dusk.useState(false),
    [yonderPrime, zenithPrime] = dusk.useState(null),
    anvilPrime =
      isPointerSurfaceHovered || (!nativePresentationOwnsMotion && vistaPrime),
    ...
    fjordPrime =
      ...
      canExpandActivityStack &&
      onActivateNotification != null,
    glenPrime =
      fjordPrime ||
      (notification.kind !== "activity" &&
        (onActivateNotification != null || notification.action != null)),
    hearthPrime =
      notification.controlTarget != null && onRunNotificationControl != null,
    ...
    nookPrime =
      hearthPrime &&
      rootPrime !== undefined &&
      rootPrime === notification.turnKey,
    oxbowPrime =
      notificationFollowUp?.notificationId === notification.id &&
      notificationFollowUp.turnKey === notification.turnKey
        ? notificationFollowUp
        : null,
    pondPrime = notificationFollowUp !== undefined,
    be =
      nativePresentationOwnsMotion && !isNativeMeasurementPlaceholder
        ? thornPrime
        : (oxbowPrime?.draft ?? thornPrime),
    ...
    spurPrime =
      inletPrime &&
      (quarryPrime || (!isNotificationStackCollapsed && anvilPrime)),
    torPrime =
      rapidsPrime &&
      (quarryPrime || (!isNotificationStackCollapsed && anvilPrime)),
    ...
    discPrime =
      ...
      inkPrime.subtitle.length > 40,
    edgePrime = (helixPrime) => {
      if (
        (eddyPrime.current?.(),
        (eddyPrime.current = undefined),
        helixPrime == null || kernelPrime == null)
      )
        return;
      let impellerPrime = () => {
        let journalPrime = Array.from(
            helixPrime.querySelectorAll(
              '[data-avatar-overlay-overflow-content="true"]',
            ),
          ).some(wave),
          kingpinPrime = helixPrime.querySelector(
            '[data-avatar-overlay-waiting-request-body="true"]',
          ),
          landPrime =
            kingpinPrime != null &&
            kingpinPrime.clientHeight > 0 &&
            kingpinPrime.scrollHeight > kingpinPrime.clientHeight + grove;
        plumePrime((meshPrime) =>
          quillowPrime.current && meshPrime
            ? meshPrime
            : journalPrime || landPrime,
        );
      };
      impellerPrime();
  ...
  let bracketPrime = (event) => {
    ...
    !knobPrime ||
      (padPrime instanceof HTMLElement &&
        (padPrime.isContentEditable ||
          padPrime.closest(
  ...
  let enginePrime = (event) => {
    ...
    if (
      quillshaftPrime instanceof Node &&
      cragPrime.current?.contains(quillshaftPrime)
    ) {
      zenithPrime("reply");
      return;
    }
    if (
      quillshaftPrime instanceof Node &&
      domePrime.current?.contains(quillshaftPrime)
    ) {
      zenithPrime("stop");
      return;
    }
  ...
  let framePrime = glenPrime || knobPrime ? "button" : undefined,
    gasketPrime = fjordPrime
      ? inkPrime.expandNotificationStackAriaLabel
      : glenPrime
        ? inkPrime.actionAriaLabel
        : knobPrime
          ? fieldPrime.formatMessage(
  ...
  let _t = notification.kind !== "activity" && "font-bold",
    ...
    ratchetPrime =
      !discPrime &&
      (kernelPrime != null && nimbusPrime
        ? "break-words whitespace-pre-wrap"
        : "truncate"),
    _avatarOverlayNativeFrameC =
      notification.kind === "activity" && "text-token-text-secondary",
    shimPrime = notification.kind !== "activity" && "text-token-foreground",
    tappetPrime = IntlProvider(
      "text-[13px] leading-[17px]",
      _t,
      _avatarOverlayNativeFrameS,
      platenPrime,
      ratchetPrime,
      _avatarOverlayNativeFrameC,
      shimPrime,
    );
  let arborPrime = discPrime && (
    <>
      <span
        aria-hidden={true}
      ...
      <span
        className={IntlProvider(
          ...
          notification.isLoading && "inline loading-shimmer-pure-text",
        )}
      >
        {inkPrime.subtitle}
      </span>
  ...
  let bushingPrime = (
    <div className={tappetPrime} data-avatar-overlay-overflow-content="true">
      {notification.title}
      {arborPrime}
    </div>
  );
  let _avatarOverlayNativeFrameA =
    kernelPrime == null
      ? undefined
      : {
  ...
  let collarPrime = (notification.kind === "activity" || discPrime) && "hidden",
    ...
    flangePrime =
      kernelPrime != null &&
      (nimbusPrime ? "overflow-y-auto overscroll-contain" : "overflow-clip"),
    gibPrime = IntlProvider(
      "text-[13px] leading-[17px]",
      collarPrime,
  ...
  let _avatarOverlayNativeFrameO = kernelPrime == null && !discPrime && (
    <span
      className={IntlProvider(
  ...
  let jigPrime = kernelPrime != null && (
    <Lunar
      isExpanded={nimbusPrime}
      nativeMaterialAttached={nativeMaterialAttached}
      patchSummaryCopy={inkPrime.patchSummary}
      request={kernelPrime}
      onRunNotificationAction={(rollerPrime) => {
  ...
  let keeperPrime = (
    <ensureIntlFormattersInit.div
      animate={_avatarOverlayNativeFrameA}
      className={gibPrime}
      initial={false}
      data-avatar-overlay-overflow-content={hubPrime}
      data-avatar-overlay-waiting-request-body="true"
      transition={idlerPrime}
    >
      {_avatarOverlayNativeFrameO}
      {jigPrime}
    </ensureIntlFormattersInit.div>
  );
  let lugPrime = (
    <ensureIntlFormattersInit.div
      ref={forgePrime}
      role={framePrime}
      aria-label={gasketPrime}
      className={knurlPrime}
      tabIndex={leverPrime}
      onClick={mountPrime}
      onKeyDown={nozzlePrime}
    >
      {bushingPrime}
      {keeperPrime}
    </ensureIntlFormattersInit.div>
  ...
  let mandrelPrime = inkPrime.statusIconAriaLabel,
    ...
    orificePrime =
      jadeitePrime.iconType === "check-circle" ? "success" : undefined,
    pinPrime =
      (anchorPrime ||
        (nativeMaterialAttached &&
          nativePresentationOwnsMotion &&
          jadeitePrime.iconType === "check-circle")) &&
      "opacity-0",
    racePrime = IntlProvider(
      "relative flex h-8 w-[var(--activity-pill-status-width)] items-center justify-center",
      pinPrime,
    );
  let sleevePrime =
      kernelPrime?.kind === "question" ? (
        <AppInitialXct className={jadeitePrime.iconClassName} />
      ) : (
        apex(jadeitePrime)
      ),
  ...
  let bossPrime = (
    <span
      ...
      data-avatar-overlay-status={nipplePrime}
      data-avatar-overlay-trailing-accessory={orificePrime}
    >
      {trunnionPrime}
    </span>
  );
  let camPrime = rapidsPrime ? (
    <div
      className={IntlProvider(
    ...
    >
      {
        <ReadLoginRouteQuerySnapshot
          ref={cragPrime}
          aria-label={fieldPrime.formatMessage(isle.replyNotification, {
            title: notification.title,
          })}
          aria-pressed={quarryPrime}
          data-native-hovered={boltPrime || undefined}
          className={IntlProvider(
            meadow,
            avatarOverlayPillDismissButtonV.replyStopControl,
            nativeMaterialAttached
              ? "enabled:hover:!bg-transparent"
              : avatarOverlayPillDismissButtonV.cssControl,
          )}
          color="ghost"
          size="icon"
          title={fieldPrime.formatMessage(isle.replyNotification, {
            title: notification.title,
          })}
          onClick={() => {
            if (quarryPrime) {
        ...
        >
          <Onyx className="relative size-7" />
        </ReadLoginRouteQuerySnapshot>
      }
      {_e ? null : (
        <ReadLoginRouteQuerySnapshot
          ref={domePrime}
          aria-label={fieldPrime.formatMessage(isle.stopNotification, {
            title: notification.title,
          })}
          aria-hidden={!spurPrime || undefined}
          data-native-hovered={cogPrime || undefined}
          className={IntlProvider(
            meadow,
            !spurPrime && "pointer-events-none opacity-0",
            avatarOverlayPillDismissButtonV.replyStopControl,
            nativeMaterialAttached
              ? "enabled:hover:!bg-transparent"
              : avatarOverlayPillDismissButtonV.cssControl,
          ...
          color="ghost"
          size="icon"
          tabIndex={spurPrime ? undefined : -1}
          title={fieldPrime.formatMessage(isle.stopNotification, {
            title: notification.title,
          })}
          onClick={() => {
            uplandPrime("");
        ...
        >
          <AddContextSquareIcon className="relative size-6 rotate-[-1deg]" />
        </ReadLoginRouteQuerySnapshot>
      )}
    </div>
  ) : null;
  let detentPrime = mirePrime ? (
    <div className="absolute top-3 right-[var(--activity-pill-control-right-inset)] z-20 flex h-8 items-center">
      {
        <ReadLoginRouteQuerySnapshot
          aria-expanded={nimbusPrime}
          aria-label={
            nimbusPrime
              ? inkPrime.collapseNotificationAriaLabel
              : inkPrime.expandNotificationAriaLabel
          }
          className={IntlProvider(
          ...
          onClick={() => {
            ...
            onExpansionChange == null
              ? maplePrime(thrustPrime)
              : onExpansionChange(notification.id, thrustPrime);
          }}
        >
  ...
  let eccentricPrime =
    quarryPrime || yardPrime ? (
      <Jewel
        autoFocus={nookPrime}
        draft={be}
        hasSubmissionError={updraftPrime}
        inputRef={beaconPrime}
        isNativeMeasurementPlaceholder={isNativeMeasurementPlaceholder}
        isSubmitting={vergePrime}
        isVisible={quarryPrime}
        keepsNativeBackingMounted={yardPrime}
        notificationTitle={notification.title}
        prefersReducedMotion={!!grainPrime}
        onAnimationFrame={onAnimationFrame}
        onClose={rivetPrime}
        onDraftChange={(yokePrime, bafflePrime) => {
  ...
  let followerPrime = (
    <Identity onExitComplete={onAnimationFrame}>{eccentricPrime}</Identity>
  );
  let guidePrime =
    jettyPrime && !quarryPrime ? (
      <div
        aria-hidden={
          nativePresentationOwnsMotion && !anvilPrime ? true : undefined
        }
        className={IntlProvider(
      ...
      >
        {
          <AvatarOverlayPillDismissButtonT
            ariaLabel={inkPrime.dismissNotificationAriaLabel}
            onClick={() => {
  ...
  return (
    <ScrewPrime
      ...
      nativeSurfaceId={nativeSurfaceId}
      role={everPrime}
      onUpdate={onAnimationFrame}
      onContextMenu={bracketPrime}
      onFocusCapture={clampPrime}
      onPointerEnter={drillPrime}
      onPointerLeave={at}
      onPointerMove={enginePrime}
    >
      {lugPrime}
      {bossPrime}
      {camPrime}
