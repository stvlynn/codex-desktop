...
export function setBuildPendingWorktreeForkEntriesPeers(next: BuildPendingWorktreeForkEntriesPeers): void {
  peers = next;
}
 ...
 */
export function buildPendingWorktreeForkEntries(e: unknown, t: unknown, n: unknown) {
  if (peers == null) {
    throw new Error("buildPendingWorktreeForkEntries peers are not configured");
  }

  let r = new Map();
  for (let t of e.get(peers.gnr, n)) r.set(t.conversationId, {
    conversationId: t.conversationId,
    forkedFromId: t.forkedFromId ?? null,
    title: t.title
  });
  for (let t of e.get(peers.aD, n)) r.set(t, {
    conversationId: t,
    forkedFromId: e.get(peers.Hnr, t),
    title: e.get(peers._D, t)
  });
  for (let t of peers.R_(e.get, `pending_worktrees`) ?? []) t.hostId !== n || t.launchMode !== `fork-conversation` || t.initialThreadTitle == null || r.set(t.id, {
    conversationId: t.id,
    forkedFromId: t.sourceConversationId,
    title: t.initialThreadTitle
  });
  return peers.hSo(r.get(t) ?? {
    conversationId: t,
    forkedFromId: e.get(peers.Hnr, t),
    title: e.get(peers._D, t)
  }, r.values());
