...
export function setBindDeferredWorktreeLgtPeers(next: BindDeferredWorktreeLgtPeers): void {
  peers = next;
}
...
export function bindDeferredWorktreeLgt() {
  ...
  return peers.e(() => {
    peers.Sl(), Mme = `Automation schedule has no future runs. Choose a schedule with an occurrence after the current time.`, Nme = `Automation does not exist in the app and could not be updated. It may have been deleted manually by the user.`, Pme = peers.pl([`ACTIVE`, `PAUSED`, `DELETED`]), Fme = peers.pl([`cron`, `heartbeat`]), Ime = peers.pl([`worktree`, `local`]), Lme = peers.dl(`type`, [peers.sl({
      type: peers.ml(`project`),
      project_id: peers.X()
    }), peers.sl({
      type: peers.ml(`projectless`)
    })]), Rme = peers.pl([`failed_runs_only`]), zme = peers.pl([`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`]), peers.sl({
      ...
      created_at: peers.rl(),
      updated_at: peers.rl()
    }), Bme = `worktree`, Vme = `gpt-5.3-codex`;
  });
}
