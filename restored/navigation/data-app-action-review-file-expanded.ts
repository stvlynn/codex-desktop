...
export function setBindDataAppActionReviewFileExpandedPeers(next: BindDataAppActionReviewFileExpandedPeers): void {
  peers = next;
}
...
export function bindDataAppActionReviewFileExpanded() {
  if (peers == null) {
    throw new Error("bindDataAppActionReviewFileExpanded peers are not configured");
  }
  ...
  return peers.e(() => {
    Ff = {
      ...
      sidebarProjectList: e => `[${peers.Ff.sidebarProjectListId}="${CSS.escape(peers.e)}"]`,
    ...
    }, zXe = [peers.If.sidebarSection, peers.If.sidebarProjectRow, peers.If.sidebarThreadRow].join(`,`), Lf = {
      reviewFile: e => ({
