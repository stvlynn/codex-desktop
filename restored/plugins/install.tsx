...
export function bindBindInstall() {
  ...
  return peers.e(() => {
    peers.Au(), peers.Sl(), peers.yEt(), OEt = peers.pl([`session`, `always`]), kEt = peers.ul([peers.OEt, peers.ol(peers.OEt).min(1).max(2)]).optional(), AEt = peers.ml(`always`).optional(), jEt = peers.sl({
      ...
      persist: peers.kEt
    }).passthrough(), zEt = peers.dl(`codex_approval_kind`, [peers.LEt, peers.REt]), BEt = peers.pl([`low`, `high`]), VEt = peers.sl({
