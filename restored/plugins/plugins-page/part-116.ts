...
export function pluginsPageT__splitRest3(__splitParams: any) {
  return <div className={IntlProvider("flex h-full min-h-0 flex-col", headerPlacement === "inline" && "main-surface")}>{!nestFourth && petalFourth === "plugins" ? <Alpha key={harborFourth.key} {...{
      directoryEntrypoint: indigoFourth,
      errorMessage: _errorMessage,
      isPageLoading: motorFourth,
      isPluginDataLoading: ___isLoading,
      pageName: oxbowFourth.pageName,
      routeKey: harborFourth.key
    }} /> : null}{headerPlacement === "none" ? null : <AppIconAZ.Header className="flex shrink-0 px-panel" inline={headerPlacement === "inline"}>{nestFourth ? <Dome {...{
        mode: "manage",
        canCreatePlugin: lunarFourth != null,
        canCreateSkill: skillCreatorPath != null,
        hostSelector: rivetFourth,
        isRefreshDisabled: pistonFourth,
        isRefreshingPage: jasperFourth,
        searchInput: gammaFourth ? <div className="w-32 min-w-0 sm:w-40">{<DeferredWm id="plugins-page-manage-search" inputRef={riverFourth} className="shadow-none" label={spurFourth.searchLabel} onSearchQueryChange={ivoryFourth} placeholder={spurFourth.searchPlaceholder} searchQuery={hazelFourth} />}</div> : undefined,
        marketplaceUpgradeAction: mireFourth === "marketplace" ? {
          disabled: fernFourth,
        ...
        onCreatePlugin: screwFourth,
        onCreateSkill: torqueFourth,
        onExitManageMode: manageOnly ? undefined : leverFourth,
        onRefreshPage: () => {
          clampFourth();
        },
        recordSkillAction: axleFourth ?? undefined
      }} /> : <Dome {...{
        mode: "browse",
        canCreatePlugin: lunarFourth != null,
        canCreateSkill: skillCreatorPath != null,
        currentBrowseTab: petalFourth ?? "plugins",
        hostSelector: rivetFourth,
        isRefreshDisabled: pistonFourth,
        isRefreshingPage: jasperFourth,
        needsSkillsRefresh: kelpFourth,
        onCreatePlugin: screwFourth,
        onCreateSkill: torqueFourth,
        onAddMarketplace: () => {
          driftFourth(true);
        ...
        recordSkillAction: axleFourth ?? undefined
      }} />}</AppIconAZ.Header>}<div className="flex min-h-0 flex-1 flex-col text-base select-none">{!yellowFourth && !(nestFourth && mireFourth === "mcps") ? <Jacket {...{}} /> : <><div className="flex-1 overflow-hidden">{<PluginsPageC {...{
            className: IntlProvider("flex h-full w-full flex-1 flex-col", nestFourth ? "gap-0" : "gap-8"),
            children: nestFourth ? <OpenInBrowser {...{
              embedded: true,
              fullWidth: manageOnly,
              className: IntlProvider(headerPlacement !== "none" && "h-full overflow-y-auto p-panel [scrollbar-gutter:stable]"),
              children: [<div className="flex flex-wrap items-center justify-between gap-3">{<Collar {...{
                  currentTab: mireFourth,
                  showNonMcpTabs: yellowFourth,
                  tabCounts: knobFourth,
                  onSelectTab: jacketFourth
                }} />}{gammaFourth ? null : <div className="w-56 min-w-0">{<DeferredWm id="plugins-page-manage-search" inputRef={riverFourth} className="shadow-none" label={spurFourth.searchLabel} onSearchQueryChange={ivoryFourth} placeholder={spurFourth.searchPlaceholder} searchQuery={hazelFourth} />}</div>}</div>, <div className="flex min-h-0 flex-1 flex-col">{mireFourth === "plugins" ? <As {...{
                  cardContentGap: "default",
                  errorMessage: _errorMessage,
                  hostId: xenonFourth,
                  getCopyLinkUrl: gearFourth,
                  ...
                  onCopyShareLink: gammaFifth => {
                    ironFourth(gammaFifth);
                  },
                  onOpenPluginDetails: nozzleFourth,
                  onShareInstalledPlugin: keystoneFourth,
                  onTryInChat: orbitFourth ? harborFifth => {
                    bracketFourth(harborFifth);
                  } : undefined,
                  plugins: eddyFourth,
                  unavailablePluginIds: glenFourth,
                  onInstallPlugin: async indigoFifth => {
                    installPlugin(indigoFifth, {
                  ...
                  emptyStateTitle: <MemoizedFormattedMessage id="skills.appsPage.empty.plugins" defaultMessage="No plugins found" description="Empty state title when there are no plugins on the Plugins tab" />
                }} /> : mireFourth === "skills" ? <Hinge {...{
                  gap: "compact",
                  hostId: xenonFourth,
                  installedStateAction: "toggle",
                  ...
                  skillEntries: hearthFourth,
                  useCompactEmptyState: true,
                  onSkillsUpdated: markSkillsUpdated
                }} /> : mireFourth === "mcps" ? null : mireFourth === "marketplace" ? <Grain {...{
                  errorMessage: _errorMessage,
                  isLoading: ___isLoading,
                  isRetrying: jasperFourth || _isFetching,
                  marketplaceLoadErrors,
                  configuredMarketplaceNames: _r,
                  configuredGitMarketplaceNames: duskFourth,
                  isUpgradingMarketplaces,
                  marketplaces: torFourth,
                  onUpgradeMarketplace: jadeFifth => {
                    arborFourth(jadeFifth);
                  },
                  onRemoveMarketplace: bloomFourth,
                  onRetry: clampFourth,
                  pendingUpgradeMarketplaceName,
                  pendingRemoveMarketplaceName
                }} /> : _____data == null ? <Vine {...{
                  errorMessage: apexFourth,
                  isLoading: _______isLoading,
                  isRetrying: isHardRefetchingAppsList,
                  onRetry: forgeFourth
                }} /> : <div className="flex flex-col gap-6">{apexFourth == null ? null : <Urn {...{
                    errorMessage: apexFourth,
                    isRetrying: isHardRefetchingAppsList,
                    onRetry: forgeFourth
                  }} />}{<Wind {...{
                    apps: inletFourth,
                    emptyStateTitle: <MemoizedFormattedMessage id="skills.appsPage.empty.installedApps" defaultMessage="No installed apps" description="Empty state title when there are no installed apps on the Skills & Apps page" />,
                    installedStateAction: "toggle",
                    isSingleColumn: true,
                    useCompactEmptyState: true,
                    isAppConnectPending,
                    updatingAppId,
                    onInstallApp: async kiteFifth => {
                      cedarFourth(null);
                      await handleConnectApp(kiteFifth);
                    },
                    onOpenAppTools: lemonFifth => {
                      sageFourth(lemonFifth.id);
                    },
                    onOpenAppUrl: handleOpenAppUrl,
                    onSetAppEnabled: setAppEnabled
                  }} />}</div>}<div key={xenonFourth} hidden={mireFourth !== "mcps"} className="flex min-h-0 flex-1 flex-col">{<Isle {...{
                    cwd: valeFourth,
                    hostId: xenonFourth,
                    searchQuery: hazelFourth,
                    useActiveWorkspaceRoot: waveFourth
                  }} />}</div></div>]
            }} /> : <SearchableDetailPageLayout contentClassName="!pt-6" headerVariant="inset" scrollContainerRef={mountFourth} subtitle={petalFourth === "skills" ? <MemoizedFormattedMessage id="skills.appsPage.browseIntro.skillsSubtitle" defaultMessage="Extend ChatGPT with task-specific skills" description="Subtitle shown at the top of the skills browse page" /> : <MemoizedFormattedMessage id="skills.appsPage.browseIntro.subtitle" defaultMessage="Work with ChatGPT across your favorite tools" description="Subtitle shown at the top of the unified plugins browse page" />} title={petalFourth === "skills" ? <MemoizedFormattedMessage id="skills.appsPage.browseIntro.skillsTitle" defaultMessage="Skills" description="Title shown at the top of the skills browse page" /> : <MemoizedFormattedMessage id="skills.appsPage.browseIntro.title" defaultMessage="Plugins" description="Title shown at the top of the unified plugins browse page" />} search={{
              autoFocus: petalFourth === "plugins",
              id: "plugins-page-search",
              inputRef: riverFourth,
              label: birchFourth.searchLabel,
              onKeyDown: insertFourth,
              onSearchQueryChange: gasketFourth,
              placeholder: birchFourth.searchPlaceholder,
              searchQuery: daisyFourth
            }}><div className={IntlProvider("flex min-h-0 flex-col gap-8", !everFourth && "flex-1")}>{petalFourth === "plugins" && !everFourth ? <Gasket {...{
                  ...collarFourth,
                  connectedPlugins: kernelFourth,
                  importedConnectorsContent: bushingFourth,
                  isWorkspaceAccount: juniperFourth,
                  selectedCategoryId: vistaFourth,
                  tab: lagoonFourth,
                  workspaceName: yarrowFourth,
                  onSelectCategory: wheatFourth,
                  onSelectTab: engineFourth
                }} /> : nimbusFourth ? <Engine {...{
                  ...collarFourth,
                  canRequestPluginSuggestion: nutFourth,
                  category: "",
                  initialOffset: $r.offset,
                  limit: $r.limit,
                  searchTerm: daisyFourth,
                  onLoadedOffsetChange: marbleFifth => {
                    frameFourth({
                      limit: $r.limit,
                      offset: marbleFifth
                    }, true);
                  },
                  onRequestPlugin: drillFourth
                }} /> : null}{petalFourth === "skills" && (!everFourth || beaconFourth > 0) ? <Gear {...{
                  canInstallRecommendedSkills,
                  errorMessage,
                  hostId: xenonFourth,
                  highlightedInstalledSkillPath: domeFourth?.startsWith("installed-skill:") ? domeFourth.slice(16) : null,
                  highlightedRecommendedSkillId: domeFourth?.startsWith("recommended-skill:") ? domeFourth.slice(18) : null,
                  installedSkillGroups: _i,
                  installedStateAction: "check",
                  installedSection: discFourth,
                  installedRecommendedSkillIds,
                  installedSkills: jadeiteFourth,
                  installedSkillsForRecommendedActions: standaloneInstalledSkills,
                  installingSkillId,
                  isLoadingInstalledSkills: _isLoading || updraftFourth,
                  isLoadingRecommendedSkills: __isLoading || updraftFourth,
                  onInstallRecommendedSkill: installRecommendedSkill,
                  onInstalledRecommendedSkillUpdated: () => {
                    clearInstalledRecommendedSkills();
                    markSkillsUpdated();
                  },
                  onSelectSection: zephyrFourth,
                  onSkillsUpdated: markSkillsUpdated,
                  recommendedRepoRoot: repoRoot,
                  recommendedSection: edgeFourth,
                  recommendedSkills: inkFourth,
                  roots: workspaceRoots,
                  selectedSectionId: boltFourth,
                  searchResultsTitle: everFourth ? <MemoizedFormattedMessage id="skills.appsPage.skillsFilter.all" defaultMessage="Skills" description="Label for the Skills option in the browse filter dropdown on the Skills & Apps page" /> : undefined
                }} /> : null}{everFourth && petalFourth === "skills" && beaconFourth === 0 ? <div className="text-base text-token-text-secondary">{<MemoizedFormattedMessage id="skills.appsPage.empty.skills" defaultMessage="No skills found" description="Empty state title when no skills match filters on the Skills & Apps page" />}</div> : null}</div></SearchableDetailPageLayout>
          }} />}{<Kite {...{
            open: coralFourth,
            onAddMarketplace: unityFourth,
            onOpenChange: driftFourth
          }} />}{eagleFourth == null ? null : <Root {...{
            initialSuggestion: eagleFourth,
            onClose: () => {
              frostFourth(null);
            }
          }} />}{<Jadeite {...{
            marketplace: acornFourth,
            isRemoving: acornFourth != null && pendingRemoveMarketplaceName === acornFourth.name,
            onOpenChange: nickelFifth => {
              nickelFifth || bloomFourth(null);
            },
            onRemove: onyxFifth => {
              shimFourth(onyxFifth);
            }
          }} />}{<Skills3 app={isleFourth} isCompletingConnection={hillFourth} onOpenChange={pearlFifth => {
            pearlFifth || (clearConnectingApp(), cedarFourth(null));
          }} onOAuthStarted={handleAppConnectOAuthStarted} onConnected={async () => {
            cedarFourth(null);
            try {
              await AppInitialT({
                hostId: xenonFourth,
                queryClient: quartzFourth,
                refreshAppsList: hardRefetchAppsList
              });
            } catch {}
          }} />}{<ManagePluginDetailPage app={brookFourth} errorMessage={cliffFourth.error?.message ?? null} isLoading={cliffFourth.isLoading} onOpenChange={quartzFifth => {
            quartzFifth || sageFourth(null);
          }} onOpenAppUrl={handleOpenAppUrl} onSetAppEnabled={setAppEnabled} onTryInChat={riverFifth => {
            let slateFifth = `[@${riverFifth.name}](${ensureZodRuntime(buildAppUri(riverFifth.id))})`;
            pearlFourth({
              activeProject: valveFourth,
              prefillPrompt: slateFifth,
              startInSidebar: true
            });
          }} tools={cliffFourth.data ?? []} updatingAppId={updatingAppId} />}</div></>}</div></div>;
  return undefined as any;
}
function engine(timberFifth) {
  let {
    canRequestPluginSuggestion,
    category,
    hostId,
    initialOffset,
    limit,
    onLoadedOffsetChange,
    onRequestPlugin,
    searchTerm,
    ...rest
  } = timberFifth;
  ...
  let {
      ...
      hasNextPage,
      isFetchingNextPage,
      isLoading: violetFifth,
      loadedOffset
    } = frost(umbraFifth),
    willowFifth = !violetFifth && error == null && data?.length === 0 ? canRequestPluginSuggestion ? <Zenith {...{
      onRequestPlugin
    }} /> : <div className="text-base text-token-text-secondary">{<MemoizedFormattedMessage id="skills.appsPage.empty.plugins" defaultMessage="No plugins found" description="Empty state title when there are no plugins on the Plugins tab" />}</div> : <Jetty {...{
      ...rest,
      errorMessage: error,
      hostId,
      isLoading: violetFifth,
      sections: (data ?? []).map(frame)
    }} />;
  let xenonFifth = willowFifth,
    yellowFifth = () => {
      fetchNextPage().then(value => {
        value != null && onLoadedOffsetChange(value);
      });
    };
  let zincFifth = <InfiniteScrollSentinel key={loadedOffset} hasNextPage={hasNextPage} isFetchingNextPage={isFetchingNextPage} onLoadNextPage={yellowFifth} />;
  return <>{xenonFifth}{zincFifth}</>;
}
function frame(amberFifth) {
  let {
    id,
    plugins,
    title
  ...
  return {
    ...
    visibleItemLimit: plugins.length
  };
}
function gasket(basaltFifth) {
  let {
    connectedPlugins,
    hostId,
    importedConnectorsContent,
    isWorkspaceAccount,
    onSelectCategory,
    onSelectTab,
    selectedCategoryId,
  ...
  let daisyFifth = yarn(cedarFifth),
    emberFifth;
  {
    let flintFifth = jasperFifth => {
      let {
        id
      } = jasperFifth;
      return id === selectedCategoryId;
    };
    let garnetFifth = daisyFifth.find(flintFifth),
      hazelFifth = garnetFifth == null ? daisyFifth : [garnetFifth],
      ivoryFifth;
    ivoryFifth = daisyFifth.map(insert);
    emberFifth = <Jetty {...{
      ...rest,
      contentAfterConnected: <>{<Lever {...{
          filterSections: ivoryFifth,
          isWorkspaceAccount,
          selectedSectionId: garnetFifth?.id ?? null,
          selectedTab: tab,
          workspaceName,
          onSelectSection: onSelectCategory,
          onSelectTab
        }} />}{garnetFifth == null && tab === "openai" ? importedConnectorsContent : null}</>,
      hostId,
      sections: [...AppInitialW({
        connectedPlugins,
        plugins: []
      }), ...hazelFifth.map(handle)]
    }} />;
  }
  return emberFifth;
}
function handle(kelpFifth) {
  let {
    id,
    plugins,
    title,
...
function jacket() {
  return <div className="flex min-h-0 flex-1 items-center justify-center">{<Dismiss {...{
      title: <MemoizedFormattedMessage id="skills.appsPage.pluginsUnsupportedHost.title" defaultMessage="Plugins are not available for this host" description="Title shown when the selected host does not support plugins" />,
      description: <MemoizedFormattedMessage id="skills.appsPage.pluginsUnsupportedHost.description" defaultMessage="Choose another host to browse and manage plugins" description="Description shown when the selected host does not support plugins" />
    }} />}</div>;
}
function knurl(mintFifth) {
  let {
    ...
    novaFifth = useIntl(),
    oliveFifth = novaFifth.formatMessage({
  ...
  let prismFifth = oliveFifth,
    quillFifth = connectedRemoteConnections.find(item => item.hostId === selectedHostId) ?? null;
  let reefFifth = quillFifth,
    sageFifth = reefFifth?.displayName ?? prismFifth,
    topazFifth = reefFifth == null ? <AppIconKS className="icon-xs shrink-0 text-token-text-secondary" /> : <DeferredConversationSh {...{
      className: "icon-xs shrink-0",
      hostId: reefFifth.hostId
    }} />;
  let ultraFifth = <span className="truncate text-left">{sageFifth}</span>;
  let vaporFifth = <AppIconSft className="icon-2xs shrink-0 text-token-input-placeholder-foreground" />;
  let wheatFifth = <ReadLoginRouteQuerySnapshot {...{
    color: "secondary",
    size: "toolbar",
    className: "hidden max-w-56 justify-start gap-1.5 md:inline-flex",
    children: [topazFifth, ultraFifth, vaporFifth]
  }} />;
  let yarnFifth = <DropdownMenu.Title><MemoizedFormattedMessage id="skills.appsPage.hostDropdown.title" defaultMessage="Host" description="Title for the host dropdown on the Skills & Apps page" /></DropdownMenu.Title>;
  let zephyrFifth = selectedHostId === "local" ? AppIconZlt : undefined,
    acornFifth = () => {
      onSelectHost(LOCAL_HOST_ID);
    };
  let bloomFifth = <AppIconKS className="icon-xs shrink-0 text-token-text-secondary" />;
  let coralFifth = <span className="flex min-w-0 items-center gap-2">{bloomFifth}<span className="truncate">{prismFifth}</span></span>;
  let driftFifth = <DropdownMenu.Item RightIcon={zephyrFifth} onSelect={acornFifth}>{coralFifth}</DropdownMenu.Item>;
  let eagleFifth;
  {
    let glideFifth;
    glideFifth = honeyFifth => <DropdownMenu.Item key={honeyFifth.hostId} RightIcon={honeyFifth.hostId === selectedHostId ? AppIconZlt : undefined} onSelect={() => {
      onSelectHost(honeyFifth.hostId);
    }}><span className="flex min-w-0 items-center gap-2">{<DeferredConversationSh {...{
          className: "icon-xs shrink-0",
          hostId: honeyFifth.hostId
        }} />}<span className="truncate">{honeyFifth.displayName}</span></span></DropdownMenu.Item>;
    eagleFifth = connectedRemoteConnections.map(glideFifth);
  }
  let frostFifth = <DropdownMenu.Section className="max-h-40 overflow-y-auto">{eagleFifth}</DropdownMenu.Section>;
  return <DropdownMenuPopover align="end" contentMaxHeight="list" contentWidth="menuBounded" triggerButton={wheatFifth}>{yarnFifth}{driftFifth}{frostFifth}</DropdownMenuPopover>;
}
function lever(irisFifth) {
  let {
      filterSections,
      isWorkspaceAccount,
      selectedSectionId,
    ...
    jewelFifth = useIntl(),
    knollFifth = nozzle(jewelFifth, workspaceName, isWorkspaceAccount);
  let lunarFifth = knollFifth,
    mossFifth = jewelFifth.formatMessage({
  ...
  let northFifth = questFifth => {
    questFifth !== selectedTab && onSelectTab(questFifth);
  };
  let orbitFifth = <AppInitialPl ariaLabel={mossFifth} options={lunarFifth} selectedId={selectedTab} size="toolbar" onSelect={northFifth} />;
  let pineFifth = <Platen {...{
    sections: filterSections,
    selectedSectionId,
    onSelectSection
  }} />;
  return <div className="flex min-w-0 items-center justify-between gap-2 px-2">{orbitFifth}{pineFifth}</div>;
}
function mount(ridgeFifth, stormFifth) {
  return ridgeFifth !== "workspace" || stormFifth;
}
function nozzle(tideFifth, unityFifth, valeFifth) {
  return [{
    id: "openai",
    label: tideFifth.formatMessage({
    ...
    id: "workspace",
    label: unityFifth ?? tideFifth.formatMessage({
  ...
  }].filter(item => mount(item.id, valeFifth));
}
