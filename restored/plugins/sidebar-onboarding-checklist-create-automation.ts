...
// Materialized via extractFn(internal `hsc`) / export `cp`.
...
/** Wire bindSidebarOnboardingChecklistCreateAutomation peers once companions land. */
export function setBindSidebarOnboardingChecklistCreateAutomationPeers(next: BindSidebarOnboardingChecklistCreateAutomationPeers): void {
  peers = next;
}
...
export function bindSidebarOnboardingChecklistCreateAutomation() {
  if (peers == null) {
    throw new Error("bindSidebarOnboardingChecklistCreateAutomation peers are not configured");
  }
