...
export function pluginDetailPageZ(zinc3: unknown) {
  let amber3 = zinc3?.marketplaces;
  return Hazel(amber3) ? new Set(Object.keys(amber3)) : new Set();
}
export function pluginDetailPageU(basalt3: unknown, cedar3: unknown) {
  return !basalt3.isBuiltIn && cedar3.has(basalt3.name);
}
