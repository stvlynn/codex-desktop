...
function Slate(petal2) {
  let quiet2 = petal2.replace(/[\\/]+$/, "").split(/[\\/]/).filter(Boolean);
  if (quiet2.length === 0) return null;
  let rain2 = quiet2[quiet2.length - 1];
  return rain2.toLowerCase() === "skill.md" && quiet2.length > 1 ? quiet2[quiet2.length - 2] : rain2.toLowerCase().endsWith(".md") ? rain2.slice(0, -3) : rain2;
}
