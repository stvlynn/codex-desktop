...
export function pluginDetailPageEt(nickel3: unknown) {
  return [...nickel3.filter(item => {
    let onyx3 = Zinc(item.name),
      pearl3 = Zinc(titleCaseColonSegments(item.name));
    return xenon.has(onyx3) || xenon.has(pearl3);
  })].sort((quartz3, river3) => (_o.get(Zinc(titleCaseColonSegments(quartz3.name))) ?? 9007199254740991) - (_o.get(Zinc(titleCaseColonSegments(river3.name))) ?? 9007199254740991));
}
export function pluginDetailPageY({
  apps,
  query
}: unknown) {
  return apps.filter(item => Flint(query, [item.name, item.description ?? ""]));
}
