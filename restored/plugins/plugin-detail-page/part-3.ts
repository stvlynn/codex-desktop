...
export function pluginDetailPageHt(dew1: unknown) {
  let alpha2 = dew1 === undefined ? {} : dew1;
  let { hostId = "local" } = alpha2,
    bravo2 = ensureGitRepoWatchAtomsInit(),
    copper2 = (falcon2) =>
      canonicalizeWorkspacePathKey("save-plugin-share", {
        hostId: hostId,
        ...falcon2,
      });
  let delta2 = () => {
...
function Copper(gamma2) {
  let harbor2 = gamma2 === undefined ? {} : gamma2;
  let { hostId = "local" } = harbor2,
    indigo2 = ensureGitRepoWatchAtomsInit(),
    jade2 = (marble2) =>
      canonicalizeWorkspacePathKey("delete-plugin-share", {
        hostId: hostId,
        ...marble2,
      });
  let kite2 = () => {
...
function Delta(nickel2) {
  return nickel2.flatMap((item) =>
...
function Echo(onyx2) {
  for (let pearl2 of gamma) onyx2(pearl2);
}
export var pluginDetailPagePt = esmInit(() => {
  ...
  gamma = [["plugins"], ["plugins", "detail"]];
  harbor = ["plugins", "shares"];
});
var falcon, gamma, harbor;
function Indigo({ hostId, navigate, section, setSelectedHostId }) {
