...
export function pluginDetailPageJ({
  pluginResultKeys,
  skillResultKeys,
  tab
}: unknown) {
...
function Garnet(marble3) {
  return River(marble3.filter(item => parseSkillsManagePath(item.path)?.pluginId == null));
}
export function pluginDetailPageTt({
  recommendedSkills,
  skills
}: unknown) {
  return recommendedSkills == null ? [] : pluginDetailPageDollar({
    installedSkills: Garnet(skills.filter(item => item.scope === "user")),
    recommendedSkills
  });
}
