...
export function pluginDetailPageW(daisy3: unknown, ember3: unknown) {
  return !daisy3.isBuiltIn && ember3.has(daisy3.name);
}
export function _pluginDetailPageZ({
  ...
  return marketplaces.filter(item => pluginDetailPageU(item, configuredMarketplaceNames) ? Flint(query, [pluginDetailPageV(item), item.name, item.path ?? ""]) : false);
}
function Hazel(flint3) {
  return typeof flint3 == "object" && !!flint3 && !Array.isArray(flint3);
}
