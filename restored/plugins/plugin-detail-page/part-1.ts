...
export async function pluginDetailPageDt({ hostId, ...rest }: unknown) {
  let { plugin } = await canonicalizeWorkspacePathKey("read-plugin", {
    hostId: hostId ?? "local",
    ...rest,
  });
  return {
...
export function pluginDetailPageMt(isle1: unknown) {
  ...
  let { enabled = true, hostId = "local" } = juniper1,
    lagoon1 = {
      hostId: hostId,
    };
  ...
  oak1 = async () =>
    (
      await canonicalizeWorkspacePathKey("list-plugin-shares", {
        hostId: hostId,
      })
    ).data;
