...
export function pluginDetailPageX({
  skills,
  query
}: unknown) {
  return skills.filter(({
    skill
  }) => Flint(query, [skill.name, getPluginDisplayName(skill), getPluginShortDescription(skill)]));
}
export function _pluginDetailPageQ({
  skills,
  query
}: unknown) {
  return skills.filter(item => Flint(query, [item.name, item.description, item.shortDescription ?? ""]));
}
