...
export function _pluginDetailPageB(azure4: unknown) {
  let {
      disableMenuLabel,
      enableButtonLabel,
    ...
    [birch4, canyon4] = jewel.useState(false),
    dew4 = onUninstall != null || isUninstallDisabled && uninstallMenuLabel != null,
    alpha5 = menuItems != null || dew4 || isEnabled && onDisable != null,
    bravo5 = alpha5 ? <div className={birch4 ? "visible opacity-100" : "invisible opacity-0 group-hover:visible group-hover:opacity-100"}>{<DropdownMenuPopover {...{
        align: "end",
        contentWidth: "icon",
        open: birch4,
        onOpenChange: canyon4,
        triggerButton: <CoerceLocalFilesystemPath {...{
          label: menuLabel ?? enabledStatusAriaLabel,
          className: IntlProvider("!cursor-interaction px-1", birch4 ? "opacity-100" : "opacity-50 hover:opacity-100 focus-visible:opacity-100"),
          iconClassName: "icon-sm text-token-description-foreground",
          size: "icon",
          uniform: false
        }} />,
        children: [menuItems, isEnabled && onDisable ? <DropdownMenu.Item {...{
          LeftIcon: chatgpt2,
          leftIconClassName: "icon-xs",
          onSelect: onDisable,
          children: disableMenuLabel
        }} /> : null, dew4 ? <DropdownMenu.Item {...{
          LeftIcon: AppIconBO,
          leftIconClassName: "icon-xs",
          disabled: isUninstalling || isUninstallDisabled,
          onSelect: onUninstall,
          children: uninstallMenuLabel
        }} /> : null]
      }} />}</div> : null;
  let copper5 = !isEnabled && onEnable ? <ReadLoginRouteQuerySnapshot {...{
    color: "outline",
    size: "toolbar",
  ...
  }} /> : isEnabled ? <AriaIconBadge ariaLabel={enabledStatusAriaLabel} icon={<AppIconZlt {...{
    className: "icon-sm opacity-60"
  }} />} /> : <span aria-hidden={true} className="h-7 w-7 shrink-0" />;
  return <div className="flex items-center gap-2">{bravo5}{copper5}</div>;
}
export var _pluginDetailPageX = esmInit(() => {
