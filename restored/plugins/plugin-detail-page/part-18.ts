...
export function pluginDetailPageI(frost3: unknown, glide3: unknown) {
  let honey3 = {
    hostId: glide3
  };
  let {
      ...
      isLoading,
      refresh
    } = useRecommendedSkills(honey3),
    iris3 = data?.repoRoot ?? frost3,
    jewel3 = data?.skills ?? [];
  return {
    errorMessage,
    isLoading,
...
export function pluginDetailPageP(knoll3: unknown) {
  let {
      forceReloadSkills,
      hostId,
      onInstalled
    } = knoll3,
    lunar3 = CodexPluginActionType(appScopeAtom),
    moss3 = ensureGitRepoWatchAtomsInit(),
    [, north3] = PLEASE_IMPLEMENT_THIS_PLAN_HEADER("skills_refresh_nonce"),
    orbit3 = readCodexHomeFromQuery(hostId) != null,
    pine3 = useRecommendedSkillStatsigOverrides(),
    [quest3, ridge3] = lotus.useState(null),
    storm3 = Symbol();
  let tide3 = lotus.useRef(storm3),
    unity3 = {};
  let [vale3, wave3] = lotus.useState(unity3),
    apex3 = applyU0000U001FU007F2("install-recommended-skill"),
    brook3 = () => {
      tide3.current = Symbol();
      ridge3(null);
      wave3(fern3 => ({
        ...fern3,
        [hostId]: []
  ...
  let cliff3 = (grove3, hill3) => {
    if (!orbit3) return;
    let isle3 = tide3.current;
    ridge3(grove3.id);
    apex3.mutate({
      hostId,
      skillId: grove3.id,
      repoPath: grove3.repoPath,
      installRoot: hill3,
      skillStatsigOverride: getRecommendedSkillStatsigOverride(pine3, grove3.id)
    }, {
      onSuccess: (juniper3, lagoon3) => {
        !juniper3.success || tide3.current !== isle3 || (wave3(meadow3 => {
          let nest3 = meadow3[lagoon3.hostId];
          return nest3?.includes(lagoon3.skillId) ? meadow3 : {
            ...meadow3,
            [lagoon3.hostId]: [...(nest3 ?? []), lagoon3.skillId]
          };
        }), moss3(buildVscodeQueryKey("recommended-skills")), Jasper({
          forceReload: forceReloadSkills,
          invalidateQueriesAndBroadcast: moss3,
          setSkillsRefreshNonce: north3
        }), onInstalled?.(), lunar3.get(toastAtom).success(<MemoizedFormattedMessage {...{
          ...
          values: {
            skillName: <span className="font-medium">{grove3.name}</span>
          }
        }} />));
      },
      onSettled: () => {
        tide3.current === isle3 && ridge3(oak3 => oak3 === grove3.id ? null : oak3);
      }
    });
  };
  let dusk3 = vale3[hostId],
    elm3 = new Set(dusk3);
  return {
    clearInstalledRecommendedSkills: brook3,
    installRecommendedSkill: cliff3,
    installedRecommendedSkillIds: elm3,
    installingSkillId: quest3
  };
}
function Jasper({
  forceReload,
  ...
  invalidateQueriesAndBroadcast(isPluginInstallModalOpenInLocationState);
  forceReload();
  setSkillsRefreshNonce(petal3 => (petal3 ?? 0) + 1);
}
