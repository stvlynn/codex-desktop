...
function UseInfiniteListQuery(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
...
export function _pluginDetailPageV({
  cardIcon,
  cardIconContainer,
  ...
  let [echo5, falcon5] = lunar.useState(false),
    gamma5 = lunar.useRef(false),
    harbor5 = () => {
      if (gamma5.current) {
        gamma5.current = false;
        return;
      }
      falcon5(true);
    };
  return <><StackedCardHeader className={IntlProvider("justify-center border-none", cardActions != null && "group")} contentClassName={IntlProvider(isContentDimmed && "opacity-60")} icon={cardIcon} iconContainer={cardIconContainer} iconContainerVariant={cardIconContainerVariant} title={cardTitle} description={cardDescription} descriptionClassName="line-clamp-1" descriptionPlacement={cardDescriptionPlacement} badges={cardBadges} isHighlighted={isHighlighted} presentation={presentation} actions={typeof cardActions == "function" ? cardActions({
      ignoreNextPreview: () => {
        gamma5.current = true;
      },
      openPreview: harbor5
    }) : cardActions} actionsPlacement="center" onClick={harbor5} />{<UseInfiniteListQuery {...{
      icon: cardIcon ?? null,
      ...
      isOpen: echo5,
      onOpenChange: falcon5,
      footer: typeof delta5 == "function" ? delta5({
        closePreview: () => {
          falcon5(false);
        }
      }) : delta5,
      children: typeof modalBody == "function" ? modalBody({
        isOpen: echo5
      }) : modalBody
    }} />}</>;
}
export var _pluginDetailPageY = esmInit(() => {
  ensureComposerEsm_Tft_Init();
