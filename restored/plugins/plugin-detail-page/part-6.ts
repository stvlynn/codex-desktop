...
export function pluginDetailPageIt(grove2: unknown) {
  let hill2 = new URLSearchParams(grove2),
    isle2 = violet.safeParse({
      limit: hill2.get("limit") ?? undefined,
      offset: hill2.get("offset") ?? undefined
    });
  return isle2.success ? isle2.data : umbra;
}
export function pluginDetailPageOt(juniper2: unknown, lagoon2: unknown) {
  let meadow2 = new URLSearchParams(juniper2);
  lagoon2 == null ? (meadow2.delete("limit"), meadow2.delete("offset")) : (meadow2.set("limit", lagoon2.limit.toString()), meadow2.set("offset", lagoon2.offset.toString()));
  let r = meadow2.toString();
  return r.length === 0 ? "" : `?${r}`;
}
