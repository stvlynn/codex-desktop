...
export function _pluginDetailPageS({
  pluginShareUrl,
  shareContext,
  savedShareUrl,
  configuredShareUrl
}: unknown) {
  return pluginShareUrl ?? shareContext?.shareUrl ?? savedShareUrl ?? configuredShareUrl;
}
export function _pluginDetailPageL({
  pluginPath,
  remotePluginId,
  savePluginShare
}: unknown) {
  return savePluginShare(remotePluginId == null ? {
    pluginPath,
