...
function UseWorkspaceUsersO(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
...
function Wheat1(seed8) {
  let {
    ...
    trail8 = Bloom1(isEditorRoleEnabled);
  let urn8 = <MemoizedFormattedMessage {...{
...
function Yarn1(vine8) {
  return <Moss {...{
    role: vine8
  }} />;
}
function Zephyr1(wind8) {
  let {
      isEditorRoleEnabled,
      role,
      onRoleChange
    } = wind8,
    yarrow8 = Bloom1(isEditorRoleEnabled);
  return <UseWorkspaceUsersO {...{
    options: yarrow8,
...
function Acorn1(azure8) {
  return <Moss {...{
    role: azure8
  }} />;
}
function Bloom1(birch8) {
  return ["reader", "editor"].map(item => item === "editor" && !birch8 ? {
    disabled: true,
    tooltipText: <MemoizedFormattedMessage {...{
      ...jewel1.editorComingSoon
    }} />,
    value: item
  } : {
    value: item
...
export var _pluginDetailPageD = esmInit(() => {
  eagle1 = reactCompilerRuntime();
  frost1 = commonJsInit(react(), 1);
...
var eagle1, frost1, glide1, honey1, iris1, jewel1;
export function _pluginDetailPageO({
  initialShare,
  saveShare
}: unknown) {
  let delta9 = initialShare,
    echo9 = null,
    falcon9 = async () => delta9?.shareUrl == null ? (echo9 ??= saveShare(delta9?.remotePluginId ?? null).then(value => (delta9 = {
      ...value,
      sharePrincipals: delta9?.sharePrincipals ?? null
    }, value)).finally(() => {
      echo9 = null;
    }), echo9) : {
      remotePluginId: delta9.remotePluginId,
      shareUrl: delta9.shareUrl
    };
  return {
    getCurrentShare: () => delta9,
    getOrCreateShare: falcon9,
    saveShareTargets: async (gamma9, harbor9) => {
      let indigo9 = await falcon9(),
        jade9 = await harbor9(indigo9.remotePluginId, gamma9);
      return delta9 = {
        ...indigo9,
        sharePrincipals: jade9
      }, jade9;
    }
  };
}
