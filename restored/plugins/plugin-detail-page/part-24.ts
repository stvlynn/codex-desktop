...
export function pluginDetailPageC({
  children,
  items,
  ...
  let [petal4, quiet4] = glide.useState(0),
    rain4 = visibleItemLimit + petal4 * batchSize,
    seed4 = items.slice(0, rain4),
    trail4 = items.slice(rain4),
    urn4 = pinnedItem != null && trail4.includes(pinnedItem),
    vine4 = urn4 ? trail4.filter(item => item !== pinnedItem) : trail4,
    wind4 = petal4 > 0 && vine4.length === 0;
  return <>{children(urn4 ? [...seed4, pinnedItem] : seed4)}{items.length > visibleItemLimit && (wind4 || vine4.length > 0) ? renderExpandRow({
      hiddenItems: vine4,
      isExpanded: wind4,
      onToggle: () => quiet4(yarrow4 => wind4 ? 0 : yarrow4 + 1)
    }) : null}</>;
}
export var _pluginDetailPageW = esmInit(() => {
  frost = reactCompilerRuntime();
  ensureComposerEsm_Tft_Init();
  glide = commonJsInit(react(), 1);
  ensureIntlFormattersInit();
});
var frost, glide, honey;
