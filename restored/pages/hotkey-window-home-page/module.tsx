...
function HotkeyWindowHomePageHelper1(xenon) {
  let {
    ...
    yellow = useIntl(),
    zinc = composerMode === "cloud" ? <AppInitialD className="icon-2xs" /> : composerMode === "worktree" && showWorktree ? <ProjectDropdownGlyphIcon {...{
      className: "icon-2xs"
    }} /> : <AppIconFB {...{
      className: "icon-2xs"
    }} />;
  let amber = showLabel ? <span className="max-w-40 truncate text-left whitespace-nowrap">
      {composerMode === "cloud" ? <MemoizedFormattedMessage {...{
      id: "composer.footer.v2.cloudTab",
      defaultMessage: "Cloud",
      description: "Cloud mode label"
    }} /> : composerMode === "worktree" && showWorktree ? <MemoizedFormattedMessage {...{
  ...
  let nova = canUseCloud ? undefined : yellow.formatMessage({
  ...
  let sage = !showWorktree,
    topaz = showWorktree ? undefined : yellow.formatMessage({
  ...
  let ultra = <MemoizedFormattedMessage {...{
    id: "composer.mode.worktreeSegment",
    defaultMessage: "Worktree",
    description: "Worktree mode label for the segmented toggle"
  }} />;
  let vapor = <DropdownMenu.Item {...{
...
function delta({
  ...
  if (projectId === undefined) return defaultToProjectless ? "~" : activeWorkspaceRoot;
...
function echo({
  ...
  return selectedRemoteProjectPath ?? (composerWorkspaceRoot === "~" ? null : composerWorkspaceRoot);
}
var falcon = esmInit(() => {
...
function HotkeyWindowHomePageHelper4(acorn) {
  ...
  return quest = {
    ...
    enabled: north,
    onSelect: pine
  }, SettingsDependenciesGuard(quest), null;
}
var gamma,
  harbor = esmInit(() => {
...
var jade,
  ...
  lemon = esmInit(() => {
    ...
    kite = ["[data-composer-overlay-floating-ui]", "[data-above-composer-portal] > *", "[data-radix-popper-content-wrapper] > *"];
  }),
  marble,
  nickel,
  onyx,
...
export function HotkeyWindowHomePage() {
  ...
  let dusk = useIntl(),
    ...
    petal = nest?.project,
    quiet = petal === undefined ? undefined : petal?.type === "local" ? petal.projectId : null,
    {
      data,
      isLoading: _isLoading
    } = CodexPluginActionResult(deferredHostsZ4),
    rain = useSettingValue(generalDesktopSettingsDefinitions.hotkeyWindowProjectlessDefaultEnabled),
    ...
    urn = petal === undefined ? selectedRemoteProject : petal?.type === "remote" ? remoteProjects.find(item => item.id === petal.projectId) ?? null : null,
    vine = urn == null && trail === "~",
    ...
    dew = urn == null && trail == null && (petal === undefined ? _isLoading : petal?.type === "remote" && isLoading),
    ever = AppInitialMJ({
      cwd: canyon,
      hostId: yarrow,
      isLoading: dew
    ...
    haven = canyon != null && grain != null && grove && !pathContainsWorktreesDir(canyon, birch),
    ink = urn == null && !_isLoading && !__isLoading && grain == null,
    jadeite = vine || ink,
    _e = {
      fallbackMode: "local",
      isAvailabilityLoading: !jadeite && (urn == null && _isLoading || __isLoading || hill),
    ...
    {
      gitRoot
    } = CHATGPT_COM_CODEX_PURCHASE_CODEX_TEAM_URL(canyon, {
  ...
  let upland = mire => {
    ...
    let pond = timber(window.getComputedStyle(nook).paddingBottom),
      ...
      spur = rapids.top;
    if (mire) for (let verge of willow) for (let weir of document.querySelectorAll(verge)) spur = Math.min(spur, weir.getBoundingClientRect().top);
    let tor = {
        minimumComposerTopInsetPx: Math.max(0, Math.ceil(rapids.top - spur)),
        restingComposerTopInsetPx: Math.ceil(Math.max(0, quarry.height - rapids.height - pond))
      },
      updraft = meadow.current;
    updraft?.minimumComposerTopInsetPx === tor.minimumComposerTopInsetPx && updraft.restingComposerTopInsetPx === tor.restingComposerTopInsetPx || (meadow.current = tor, appServices.hotkeyWindowHotkeys?.homeLayoutChanged(tor));
  };
  violet.useEffect(() => {
    AppIconMZ();
  }, [oak]);
  ...
  violet.useLayoutEffect(() => {
    let yard = null,
      ...
      cog = vista && typeof MutationObserver < "u" ? new MutationObserver(anchor) : null;
    return juniper.current != null && bolt?.observe(juniper.current), cog?.observe(document.body, {
      childList: true,
      subtree: true
    }), anchor(), () => {
  ...
  let wisp = violet.useCallback(disc => {
    ...
    zenith = gear => {
      ...
      ensureAppActionPayloadSchemasInit.dispatchMessage("show-settings", {
        section: gear == null ? `local-environments?${new URLSearchParams({
          workspaceRoot: canyon
        }).toString()}` : ProjectSelectorControl({
          configPath: gear,
          workspaceRoot: canyon
        }).slice(10),
        state: {
    ...
    crag = event => {
      lagoon.current === event.pointerId && (lagoon.current = null, event.currentTarget.hasPointerCapture?.(event.pointerId) && event.currentTarget.releasePointerCapture?.(event.pointerId), appServices.hotkeyWindowHotkeys?.homeDragEnd());
    },
    dome = event => {
      event.button === 0 && (event.preventDefault(), lagoon.current = event.pointerId, event.currentTarget.setPointerCapture?.(event.pointerId), appServices.hotkeyWindowHotkeys?.homeDragStart({
        pointerWindowX: event.clientX,
        pointerWindowY: event.clientY
      }));
    },
    eddy = event => {
      lagoon.current === event.pointerId && appServices.hotkeyWindowHotkeys?.homeDragMove();
    },
    fjord = AppInitialL(urn?.remotePath ?? canyon, urn?.label) ?? dusk.formatMessage({
    ...
    }) : nimbus === "cloud" ? dusk.formatMessage({
    ...
    }) : nimbus === "worktree" ? dusk.formatMessage({
  ...
  }) : vine ? hearth = dusk.formatMessage({
  ...
  let inlet = nimbus === "worktree" ? <WorktreeEnvironmentDropdown {...{
      startingState: leaf,
      setStartingState: yonder,
      hostConfig: azure,
      gitRootOverride: gitRoot ?? canyon,
      branchSource: "worktree"
    }} /> : grain == null ? null : <WorktreeEnvironmentDropdownR {...{
      gitRoot: grain,
      hostConfig: azure,
  ...
  return <div ref={isle} className={IntlProvider(quartz.home, "relative h-full w-full overflow-hidden bg-transparent pb-1")}>
      {<CLOUD_ENVIRONMENT_SELECTOR_FEATURE_GATE_ID {...{}} />}
      <div ref={juniper} className={IntlProvider(quartz.homeShell, "absolute inset-x-1 bottom-1 px-[15px] pb-[13px] pt-[17px]", thorn && "no-drag")} style={quillow == null ? undefined : {
      bottom: "auto",
      top: quillow
    }}>
        <div className={quartz.shellUnderlay} aria-hidden="true" onLostPointerCapture={crag} onPointerCancel={crag} onPointerDown={dome} onPointerMove={eddy} onPointerUp={crag} />
        ...
        <div className="no-drag relative">
          <AppInitialTT>
            <AppInitialZh showWorkspaceDropdownInUtilityBar={false} showUtilityBar={false} surfaceClassName={quartz.composerSurface} composerModeAvailability={_e} placeholderText={glen} defaultCwd={trail ?? (vine || petal !== undefined ? "~" : undefined)} selectedProject={vine ? null : petal ?? wind} hotkeyWindowHomeFooterControls={<UsePointerSurfaceInteractionGate {...{
            open: opal,
            onOpenChange: plume,
            children: [<AppInitialM asChild={true}>
                        {<CoerceLocalFilesystemPath {...{
                label: jetty,
                size: "composer"
              }} />}
                      </AppInitialM>, <ToggleSortedIdList {...{
              align: "center",
              side: "top",
              sideOffset: 10,
              className: "no-drag w-auto min-w-[320px] rounded-3xl p-0",
              children: <div className="flex flex-col gap-4 p-3">
                  ...
                  control: <ProjectSelectorControl activeProjectIdOverride={petal === undefined ? undefined : quiet} allowRemoteProjects={false} onProjectSelected={beacon} />
                }} />}
                              {<HotkeyWindowHomePageHelper7 {...{
                  label: <MemoizedFormattedMessage {...{
                  ...
                  control: <HotkeyWindowHomePageHelper1 {...{
                    canUseCloud: _e.isCloudAvailable && access === "enabled",
                    composerMode: nimbus,
                    setComposerMode: wisp,
                    showLabel: true,
                  ...
                  control: <EnsureWorktreeEnvironmentDropdownInit {...{
                    ...
                    onOpenSettings: zenith
                              ...
                              {inlet ? <HotkeyWindowHomePageHelper7 {...{
                  label: <MemoizedFormattedMessage {...{
                  ...
                  control: inlet
                              ...
                              {nimbus === "cloud" ? null : <CopyContinuousScale {...{
                  conversationId: null,
...
function HotkeyWindowHomePageHelper7(vale) {
  let {
    ...
    wave = <div className="shrink-0 text-sm font-medium text-token-foreground">
        {label}
      </div>;
...
esmInit(() => {
  ...
  lemon();
  river();
  willow = ["[data-composer-overlay-floating-ui]", "[data-above-composer-portal] > *", "[data-radix-popper-content-wrapper] > *"];
})();
