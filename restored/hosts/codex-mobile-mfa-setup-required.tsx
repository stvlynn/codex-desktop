...
export function setBindBindCodexMobileMfaSetupRequiredPeers(
  next: BindBindCodexMobileMfaSetupRequiredPeers,
): void {
  peers = next;
}
...
export function bindBindCodexMobileMfaSetupRequired() {
  if (peers == null) {
    throw new Error(
      "bindBindCodexMobileMfaSetupRequired peers are not configured",
    );
  }
  ...
  return peers.e(() => {
    (peers.Qt(),
      ...
      (Ijr = 3e4),
      (Ljr = 6e4),
      (Rjr = [`codex-mobile-mfa-setup-required`]),
      (zjr = `2055603567`),
      (Bjr = `3936985709`),
      (Vjr = [`remote-control-server-pairing`]),
      (Hjr = peers.Ia(peers.Q, () => ({
        queryKey: [`remote-control-mfa-requirement`],
        queryFn: peers.pjr,
        retry: !1,
        staleTime: peers.Ijr,
      }))),
      (Ujr = peers.Ia(peers.Q, ({ get: e }) => ({
      ...
      (Gjr = peers.ka(
        ...
        ({ hostId: e, openId: t }, { get: n, scope: r }) => ({
          enabled:
            n(peers.Eh, `2055603567`) &&
            !n(peers.Eh, `3936985709`) &&
            n(peers.R6n, peers.e),
          queryKey: [...peers.Vjr, peers.e, t],
          queryFn: async () => (
            await peers.A6n(r, peers.e),
            peers.Bf(`start-remote-control-pairing-for-host`, {
              hostId: peers.e,
              manualCode: !0,
            })
          ),
          ...
          staleTime: (e) => {
            ...
            return t == null
              ? 0
              : Math.max(0, peers.Fjr(t) - peers.e.state.dataUpdatedAt);
          },
          refetchInterval: (e) => {
            let t = peers.e.state.data?.expiresAt;
            return t == null ||
              peers.e.state.fetchStatus === `fetching` ||
              peers.e.state.error != null
              ? !1
              : Math.max(1, peers.Fjr(t) - Date.now() - peers.Ljr);
          },
        }),
        {
          key: ({ hostId: e, openId: t }) => `${peers.e}:${t}`,
        },
      )));
  });
}
