...
import { ensureComposerEsm_K9_Init, ensureComposerEsm_MT_Init, ensureComposerEsm_P5_Init, ensureComposerEsm_Utt_Init, ensureComposerEsm_Wlt_Init as EnsureComposerEsm_Wlt_Init } from "../../composer/composer-esm-inits";
...
import { ensurePluginDetailPageInit, ManagePluginDetailPage, pluginDetailF, pluginDetailG, pluginDetailH, pluginDetailI, pluginDetailL, pluginDetailM as PluginDetailM, pluginDetailP } from "../../plugins/plugin-detail-page";
import { ensurePluginsPageInit, PluginsPage, pluginsPageA, PluginsPageC, pluginsPageI, PluginsPageL, pluginsPageO, PluginsPageR, pluginsPageS, pluginsPageU } from "../../plugins/plugins-page";
import { commonJsInit, esmInit } from "../../runtime/rolldown-runtime";
import { ensureSettingsHostDropdownInit, SettingsHostDropdown } from "../../settings/settings-host-dropdown";
...
import { ensureUseElementInViewInit, useElementInView } from "../../ui/use-element-in-view";
...
import { ensureSkillsPageHelpersInit, usePluginsFeatureEnabled } from "../skills-page-helpers";
...
function SkillsPageHelper2(pearl) {
    ...
    sage;
  {
    ...
    sage = standaloneInstalledSkills.filter(item => {
      ...
      return skill.name.toLowerCase().includes(vale) || apex.includes(vale) || wave.includes(vale);
    });
  }
  let topaz = sage,
    _skillsPageN = async () => {
  ...
  let vapor = daisy ? null : <MemoizedFormattedMessage {...{
    id: "skills.page.heading",
    defaultMessage: "Skills",
    description: "Header title for the skills page"
  }} />;
  let wheat = timber != null && timber.length > 0 ? <SettingsHostDropdown {...{
  ...
  let acorn = <span className="hidden lg:inline">
  ...
  let bloom = <ReadLoginRouteQuerySnapshot {...{
    color: yarn,
    size: "toolbar",
  ...
  let eagle = <div className="hidden min-w-[160px] flex-1 lg:flex lg:w-[220px] lg:flex-none">
      {<DeferredWm {...{
      id: "skills-search",
      label: coral,
      placeholder: drift,
  ...
  let glide = !skillCreatorPath,
    honey,
    iris;
  honey = <EnsureComposerEsm_Wlt_Init {...{
    className: "icon-xs"
  }} />;
  iris = <MemoizedFormattedMessage {...{
  ...
  let knoll = <div className="flex flex-nowrap items-center gap-1.5">
      {wheat}
      {bloom}
      {eagle}
      {jewel}
    </div>;
  let lunar = <EnsureAppIconKhInit {...{
    start: vapor,
    trailing: knoll
  }} />;
  let moss = cliff => {
  ...
  let orbit = <div className="heading-xl font-normal text-token-foreground">
      {<MemoizedFormattedMessage {...{
      id: "skills.page.heading",
      defaultMessage: "Skills",
      description: "Header title for the skills page"
  ...
  let pine = <div className="flex items-end justify-between gap-4">
  ...
  let ridge = <SkillsPageHelper5 {...{
    title: quest,
    children: <SkillsPageHelper6 {...{
      hostId: xenon,
      isLoading,
  ...
  let $ = <div className="flex-1 overflow-y-auto p-panel" ref={moss}>
      {<PluginsPageC {...{
      className: "mx-auto flex min-h-full w-full max-w-[var(--thread-content-max-width)] flex-1 flex-col gap-8",
      children: [pine, <div className="flex min-h-0 w-full flex-1">
  ...
  return <div className="mr-4 flex h-full flex-col text-base">
      {lunar}
      {$}
    </div>;
}
function $e(elm) {
  return <a href={DEVELOPERS_OPENAI_COM_CODEX_SKILLS_URL} target="_blank" rel="noopener noreferrer" className="text-token-link">
      {elm}
    </a>;
}
function bravo() {
  return document.documentElement.dataset.hideHeaderDivider = "true", copper;
}
function copper() {
...
function SkillsPageHelper5(fern) {
  let {
    ...
    grove = <div className="pr-0.5 pl-2 text-base font-medium text-token-foreground opacity-75">
        {title}
      </div>;
  ...
  let isle;
  {
    ...
    nest = oak => {
      let {
        ...
        petal = skill.scope === "repo" ? [<SkillsPageHelper7 key={`${skill.path}-repo`} {...{
          label: getScopeBadgeLabel({
            scope: "repo",
            skillPath: skill.path
          })
        }} />] : [];
      return <PluginDetailM key={skill.path} {...{
        skill,
...
function SkillsPageHelper8(rain) {
  ...
  bb0: {
    let vine = canyon => !pluginDetailG({
      installedSkillMatchKeys,
      skill: canyon
    });
    let wind = skills.filter(vine),
      yarrow = searchQuery.trim().toLowerCase(),
      azure = wind.filter(item => yarrow.length === 0 ? true : [item.name, item.description, item.shortDescription ?? ""].join(" ").toLowerCase().includes(yarrow));
    if (isLoading) {
      let dew;
      dew = <div className="flex min-h-0 flex-1 items-center justify-center">
          <AppInitialNi>
            {<MemoizedFormattedMessage {...{
            id: "skills.page.loading",
            defaultMessage: "Loading skills…",
    ...
    if (errorMessage) {
      let ever = <MemoizedFormattedMessage {...{
      ...
      let field;
      field = <div className="flex min-h-0 flex-1 items-center justify-center">
          {<Dismiss {...{
          title: ever,
    ...
    if (wind.length === 0) {
      let grain;
      grain = <div className="flex min-h-0 flex-1 items-center justify-center">
          {<Dismiss {...{
          title: <MemoizedFormattedMessage {...{
            id: "skills.page.empty",
            defaultMessage: "No skills found",
    ...
    if (azure.length === 0) {
      let haven;
      haven = <div className="flex min-h-0 flex-1 items-center justify-center">
          {<Dismiss {...{
          title: <MemoizedFormattedMessage {...{
            id: "skills.page.filteredEmpty",
            defaultMessage: "No skills match your filters",
    ...
    birch = ink => {
      ...
      return <PluginsPageR key={ink.id} {...{
        skill: ink,
        canInstall,
        hostId,
        isInstalled: jadeite,
