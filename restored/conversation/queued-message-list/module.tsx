...
export function QueuedMessageListIcon(bravo: unknown) {
  ...
  let brook = isInterrupted ? (
    <CodexReferralInviteModalAction
      {...{
        title: (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Header shown above queued messages paused because the user interrupted the running turn",
            }}
          />
        ),
        actions: (
              ...
              children: (
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Button label to resume queued messages paused by an interruption",
                  }}
                />
              ),
            }}
  ...
  let dusk;
  {
    ...
    hill = (isle) => {
      let {
        ...
        juniper = filterItemsByMatchingPaths(
          isle.context.fileAttachments,
          isle.context.generatedPastedTextAttachmentPaths,
        );
      return (
        <QueuedMessageListHelper3
          key={isle.id}
          {...{
            messageId: isle.id,
            messageText: jade(tide, {
              ...
              imageCommentCount:
                isle.context.imageCommentDrafts?.reduce(gamma, 0) ??
                isle.context.imageCommentDraft?.comments.length ??
                0,
              pastedTextAttachmentPreview:
                isle.context.pastedTextAttachments?.[0]?.preview ??
                (juniper == null ? undefined : ""),
              pastedTextAttachmentCount:
                isle.context.pastedTextAttachments?.length ?? juniper?.length,
              selectedTextAttachmentCount:
                isle.context.selectedTextAttachments?.length ?? 0,
              responseAnnotationCount:
                isle.context.responseTextAnnotations?.length ?? 0,
            }),
...
function QueuedMessageListHelper3(slate) {
            ...
            <p>
              {
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Primary tooltip text for a queued message that failed to send",
                  }}
                />
              }
            </p>
            <p className="text-token-description-foreground">
              {
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Secondary tooltip text explaining how to resolve a queued message that failed to send",
                  }}
                />
              }
            </p>
  ...
  let lotus = isPaused ? (
      ...
      <p>
        {
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Primary tooltip text for retrying a queued follow-up that failed to send",
            }}
          />
        }
      </p>
      <p className="text-token-description-foreground">
        {
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Secondary tooltip text explaining alternatives when retrying a queued follow-up keeps failing",
            }}
          />
        }
      </p>
    ...
    <MemoizedFormattedMessage
      {...{
        ...
        description:
          "Primary tooltip text for steering with a queued follow-up without interrupting the current model run",
      }}
    />
  );
  ...
  let bloom =
    onOpenInSideChatMessage == null ? null : (
      <DropdownMenu.Item
        {...{
          ...
          children: (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Menu item to start a queued message as a side chat",
              }}
            />
          ),
        }}
  ...
  let coral = isQueueingEnabled ? (
    <MemoizedFormattedMessage
      {...{
        id: "composer.queuedMessage.turnOff",
        defaultMessage: "Turn off queueing",
        description:
          "Menu item to switch the default follow up behavior to steer",
      }}
    />
  ) : (
    <MemoizedFormattedMessage
      {...{
        ...
        description:
          "Menu item to switch the default follow up behavior to queue",
      }}
    />
  );
  let drift = (
...
function indigo(honey) {
  let { messageIds, onReorderMessages } = honey,
    iris = {
  ...
  let lunar = (moss) => {
    ...
    pine === -1 ||
      quest === -1 ||
      onReorderMessages(moveArrayItem(messageIds, pine, quest));
  };
  return {
    sensors: jewel,
...
function jade(
  ...
  return browserCommentCount > 0 ||
    responseAnnotationCount > 0 ||
    designTweakCount > 0 ||
    diffCommentCount > 0 ||
    imageCommentCount > 0
    ? CodexConversationalOnboardingExecutionAction(ridge, {
