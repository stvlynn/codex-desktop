...
export function applyHotkeyWindow() {
  ...
  return peers.e(() => {
    peers.Sl(), peers.V_e(), peers.Dge(), qwe = `/hotkey-window`, Jwe = peers.qwe, Ywe = `${peers.qwe}/new-thread`, Xwe = `/debug`, Zwe = `/global-dictation`, Qwe = peers.Zwe, $we = `/chatgpt/quick-chat`, eTe = `/chatgpt/quick-chat-prewarm`, tTe = `${peers.$we}/:conversationId`, nTe = `${peers.qwe}/thread`, rTe = `${peers.nTe}/:conversationId`, iTe = `${peers.qwe}/remote`, aTe = `${peers.iTe}/:taskId`, oTe = `/skills/plugins`, sTe = `${peers.oTe}/:pluginId`, cTe = `/skills/manage`, lTe = `${peers.cTe}/plugins`, uTe = `${peers.lTe}/:pluginId`, du = `openai-curated`, fu = `openai-curated-remote`, dTe = `openai-primary-runtime`, fTe = `chatgpt-workspace`, pTe = /^(?:plugins_[0-9a-f]{32}|(?:plugins~)?Plugin_[0-9a-f]{32}|plugin_[A-Za-z0-9][A-Za-z0-9_-]{0,247})$/, mTe = /^(?:plugins~)?Plugin_[0-9a-f]{32}$/, hTe = `/local`, gTe = `${peers.hTe}/:conversationId`, _Te = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i, vTe = peers.pl([`branch`, `last-turn`]), peers.sl({
    ...
    }), yTe = `/remote`, bTe = `${peers.yTe}/:taskId`, RegExp(`^${peers.nTe}/([^/?#]+)$`);
  });
}
