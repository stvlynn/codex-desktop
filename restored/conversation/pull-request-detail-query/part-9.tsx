...
export function pullRequestDetailQueryUnderscore(birch3: unknown) {
  ...
  for (let alpha4 of birch3 ?? []) {
    alpha4.reviewThreadId != null && !canyon3.has(alpha4.reviewThreadId) && canyon3.set(alpha4.reviewThreadId, alpha4);
...
function Vapor(copper4, delta4) {
  return delta4 == null ? copper4.line == null || copper4.path == null || copper4.side == null ? null : {
    line: copper4.line,
    path: copper4.path,
    side: copper4.side,
...
export function pullRequestDetailQueryX(echo4: unknown, falcon4: unknown, gamma4: unknown) {
  switch (falcon4) {
    case "closed":
      return "close";
    case "draft":
      return "mark-draft";
    case "open":
      return echo4 === "closed" ? gamma4 ? "reopen-ready" : "reopen" : "mark-ready";
  }
}
export function pullRequestDetailQueryB(harbor4: unknown) {
  return yarn.default([...harbor4.requested, ...harbor4.requestedTeams, ...harbor4.approved, ...harbor4.changesRequested, ...harbor4.commented]).map(item => ({
...
function Wheat(indigo4, jade4) {
  return indigo4.changesRequested.includes(jade4) ? "changes_requested" : indigo4.approved.includes(jade4) ? "approved" : "waiting";
}
export var yarn,
...
export function PullRequestDetailQueryH(kite4: unknown) {
  ...
  let slate4 = river4,
    timber4 = commentAttachment == null ? undefined : {
      ...commentAttachment,
      authorAvatarUrl: comment.authorAvatarUrl,
      authorLogin: comment.authorLogin,
      reviewThreadId: comment.reviewThreadId
    };
  let umbra4 = metadataAccessory ?? (account != null && pullRequest != null ? <Sage {...{
    account,
    comment,
  ...
  let violet4 = metadataAccessoryFlush ?? pullRequest != null,
    willow4 = account == null || pullRequest == null || slate4 == null ? undefined : () => {
      Gamma(lemon4, "code");
      Daisy(lemon4, {
        account,
        pullRequest
  ...
  let xenon4 = slate4 != null,
    ...
    zinc4 = (amber4, basalt4) => <_pullRequestMediaT {...{
      account,
      allowBasicHtml: true,
      className: IntlProvider("text-size-chat break-words text-token-foreground [&_details]:mt-3 [&_details]:rounded-[14px] [&_details]:border [&_details]:border-token-border/16 [&_details]:bg-token-bg-primary/40 [&_details]:px-3.5 [&_details]:py-3 [&_p]:leading-6 [&_summary]:cursor-interaction [&_summary]:font-medium [&_summary]:text-token-foreground", !basalt4 && "px-3 py-2.5"),
      cwd: null,
      children: amber4
    }} />;
  return <IsString {...{
