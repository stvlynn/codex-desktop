...
function Ember(umbra3, violet3, willow3) {
  ...
  let xenon3 = Flint(position.path),
    yellow3 = xenon3 == null ? null : findBrowserTabById({
      container: xenon3,
  ...
  if (yellow3 == null) {
    xenon3 != null && (xenon3.querySelector(AppActionSelector.reviewFileToggleCollapsed)?.click(), xenon3.scrollIntoView({
      behavior: "auto",
      block: "start"
    }));
  ...
  window.setTimeout(() => {
    ...
    let zinc3 = Flint(position.path),
      amber3 = zinc3 == null ? null : findBrowserTabById({
        container: zinc3,
        includeShadowRoots: true,
        lineNumber: position.line,
        side: position.side === "left" ? "deletions" : "additions"
...
function Flint(ember3) {
  ...
  for (let garnet3 of document.querySelectorAll(AppActionSelector.reviewFile)) if (garnet3.getAttribute("data-review-path") === flint3) return garnet3;
  return null;
}
var garnet,
  hazel,
...
function Lotus(jasper3) {
  ...
  let topaz3 = useResizeObserver(sage3),
    ultra3,
    vapor3;
  ultra3 = () => (onDirtyChange?.(mint3.length > 0), () => {
    onDirtyChange?.(false);
  });
  ...
  let zephyr3 = isReply ? kelp3.formatMessage({
  ...
  let acorn3 = zephyr3,
    bloom3 = isReply ? kelp3.formatMessage({
  ...
  let eagle3 = isReply ? kelp3.formatMessage({
    ...
    author: replyToAuthorLogin ?? kelp3.formatMessage({
  ...
  let glide3 = <textarea ref={drift3} aria-label={coral3} autoFocus={autoFocus} className="block min-h-7 w-full min-w-0 flex-1 resize-none overflow-y-auto bg-transparent px-0 pt-2.5 pb-0 text-base leading-7 text-token-foreground outline-none placeholder:text-token-input-placeholder-foreground" disabled={isPending} placeholder={eagle3} rows={1} value={mint3} onChange={frost3} onKeyDown={Mint} />;
  let honey3 = glide3,
    iris3 = onCancel == null ? null : <OptionalTooltip {...{
      delayDuration: 250,
  ...
  let jewel3 = reef3.length === 0,
    knoll3 = isPending ? null : <AppIconPlt {...{
      className: "icon-xs"
    }} />;
  let lunar3 = <ReadLoginRouteQuerySnapshot {...{
    "aria-label": acorn3,
    color: "primary",
    disabled: jewel3,
    loading: isPending,
  ...
  let moss3 = <OptionalTooltip {...{
    delayDuration: 250,
    tooltipContent: acorn3,
    children: lunar3
  }} />;
  let north3 = <>{iris3}{moss3}</>;
  let orbit3 = north3,
    pine3 = <CodexPluginActionResult.Input {...{
      layout: "multiline",
      children: honey3
    }} />;
  let quest3 = <CodexPluginActionResult.FooterInlineControls {...{
    gap: "normal",
    children: authorAvatar
  }} />;
  let ridge3 = <CodexPluginActionResult.FooterAction {...{
    children: null
  }} />;
  let storm3 = <CodexPluginActionResult.FooterControls {...{
    children: <CodexPluginActionResult.FooterActions {...{
      children: orbit3
    }} />
  }} />;
  let tide3 = <CodexPluginActionResult.Footer {...{
    children: [quest3, ridge3, storm3]
  }} />;
  let unity3 = <CodexPluginActionResult {...{
    layout: "multiline",
    radiusVariant: "single-line",
    surfaceVariant: "secondary",
    children: <CodexPluginActionResult.Body {...{
    ...
    wave3 = async function (event) {
      if (event.preventDefault(), !(reef3.length === 0 || isPending)) {
        ...
        try {
          ...
          let isle3 = hill3;
          prism3(isle3 instanceof Error ? isle3.message : kelp3.formatMessage({
  ...
  let dusk3 = olive3 == null ? null : <div className="px-1 text-sm text-token-charts-red" role="alert">{olive3}</div>;
  return <form className="flex w-full flex-col gap-2" onSubmit={brook3}>{cliff3}{dusk3}</form>;
}
function Mint(event) {
  (event.metaKey || event.ctrlKey) && event.key === "Enter" && (event.preventDefault(), event.currentTarget.form?.requestSubmit());
}
var nova,
  olive,
  prism,
...
export function PullRequestDetailQueryC(lagoon3: unknown) {
  ...
  let quiet3 = <AvatarCornerBadge {...{
    children: <span className="flex size-6 items-center justify-center rounded-full border border-token-border/20 bg-token-bg-primary/60 text-xs font-semibold text-token-foreground">{petal3}</span>
  }} />;
...
function Sage(trail3) {
  ...
  return urn3 ? <PullRequestDetailQueryC {...{
    account,
    pullRequest,
  ...
  }} /> : <div className="pe-3 pb-3">{<ReadLoginRouteQuerySnapshot {...{
      color: "secondary",
      size: "default",
      onClick: () => {
        vine3(true);
