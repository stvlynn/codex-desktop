...
export function _pullRequestDetailQueryO(cliff4: unknown) {
  ...
  }} /> : checks.length > 0 ? renderChecks(checks) : <Moss {...{
    children: empty ?? <MemoizedFormattedMessage {...{
...
export function _pullRequestDetailQueryS(fern4: unknown) {
  let {
    ...
    hill4 = <MemoizedFormattedMessage {...{
      id: "pullRequestDetail.comments.title",
      defaultMessage: "Comments",
      description: "Pull request comments section title"
    }} />;
  let isle4 = error == null ? loading || comments == null ? <Ridge {...{
    label: <MemoizedFormattedMessage {...{
      id: "pullRequestDetail.comments.loading",
      defaultMessage: "Loading comments",
      description: "Loading label for pull request comments"
    }} />
  }} /> : comments.length > 0 ? <LazyValueChildrenProvider {...{
    children: <div className="flex flex-col gap-3">{comments.map(item => <div key={item.id}>{renderComment(item)}</div>)}</div>
  }} /> : <Moss {...{
...
export function _pullRequestDetailQueryC(lagoon4: unknown) {
  ...
  return <div className="rounded-lg border border-token-border bg-token-bg-fog px-3 py-2 text-sm text-token-description-foreground" role="status">{type === "checks" ? <MemoizedFormattedMessage {...{
