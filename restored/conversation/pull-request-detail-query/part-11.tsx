...
export function _pullRequestDetailQueryP(cedar4: unknown) {
  let {
    ...
    daisy4 =
      headBranch == null && baseBranch == null ? null : (
            ...
            label: (
              <MemoizedFormattedMessage
                {...{
                  ...
                  description:
                    "Branch field label in the pull request overview",
                }}
              />
            ),
            children: (
              <span className="flex min-w-0 items-center gap-2">
                {headBranch == null ? null : (
  ...
  let kelp4 = (
    <span className="inline-flex shrink-0 text-token-text-tertiary [&_svg]:text-token-text-tertiary">
      {
        <Drift
          {...{
            data: checks,
  ...
  return (
    <dl className="flex flex-col px-[var(--detail-page-inline-inset,0px)] pb-2 select-none">
      {daisy4}
      {garnet4}
      {jasper4}
      {nova4}
...
function Drift(olive4) {
  let { data, hasError, loading } = olive4;
  if (hasError) {
    let prism4;
    return (
...
function Eagle(reef4) {
  let { data, hasError, loading } = reef4;
  if (hasError) {
    let sage4;
    return (
      <MemoizedFormattedMessage
...
function Frost(zephyr4) {
  let { data, hasError, loading } = zephyr4;
  if (hasError) {
    let bloom4;
    return (
      <MemoizedFormattedMessage
