...
const readLoginRouteQuerySnapshot: any = undefined;
function os({ currentNodeId, onClose, serverConversationId, title }) {
  let marble = useIntl(),
...
async function lemon(
  prism,
  { conversationId, isTemporaryChat = false, messageId, prompt },
) {
  if (
    prism.get(chatgptConversationStatusAtom, conversationId) === "streaming" ||
    prism.get(chatgptConversationServerIdAtom, conversationId)
  )
    throw Error("The chat is busy.");
  if (prism.get(chatgptConversationFlagsAtom, conversationId) === "tpp")
  ...
  if (quill == null)
    throw Error("The latest user message could not be edited.");
  let reef = fileAttachmentsFromMetadata(quill.message.metadata),
    sage = prompt.trim(),
    topaz =
      quill.message.content.content_type === "multimodal_text" &&
      quill.message.content.parts.some((item) => typeof item != "string");
  if (sage.length === 0 && reef.length === 0 && !topaz)
