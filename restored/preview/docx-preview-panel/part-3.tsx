...
function DocxPreviewPanelHelper41(header) {
  ...
  {
    let lagoonPrime;
    lagoonPrime = (meadowPrime) =>
      meadowPrime.localArtifactAnnotationContext?.path === injector &&
      willow(meadowPrime) === pageNumber;
  ...
  let marblePrime = lemonPrime,
    nickelPrime =
      harborPrime == null
        ? null
        : (marblePrime.find((item) => zinc(item) === harborPrime) ?? null);
  ...
  riverPrime =
    gammaPrime == null
      ? null
      : (marblePrime.find((item) => zinc(item) === gammaPrime) ?? null);
  quartzPrime = riverPrime == null ? null : xenon(riverPrime);
  pearlPrime = reducer ?? quartzPrime;
  slatePrime =
    reducer == null
      ? ((riverPrime == null ? null : ($e(riverPrime) ?? pageSize)) ?? pageSize)
      : (tee ?? pageSize);
  ...
  let kelpPrime = jasperPrime,
    lotusPrime = () =>
      violetPrime == null
        ? false
        : violetPrime.target.mode === "create" && copperPrime.trim().length > 0
          ? jadePrime
            ? (garnetPrime(), true)
            : (kitePrime(true), kelpPrime(), false)
          : (garnetPrime(), true);
  ...
  let olivePrime = vapor.useEffectEvent(novaPrime),
    prismPrime = () => {
      ...
      let urnPrime =
        typeof ResizeObserver > "u" ? null : new ResizeObserver(trailPrime);
      return (
        urnPrime?.observe(louver),
        trailPrime(),
        () => {
  ...
  let reefPrime = (event) => {
    ...
    (vinePrime instanceof Node && nip.current?.contains(vinePrime)) ||
      (event.preventDefault(),
      event.stopPropagation(),
      mintPrime() &&
        (vinePrime instanceof Node
          ? vinePrime.ownerDocument
          : louver?.ownerDocument
        )
          ?.getSelection()
          ?.removeAllRanges());
  };
  let sagePrime = vapor.useEffectEvent(reefPrime),
    topazPrime = () => {
      if (
        isCommentMode ||
        violetPrime?.target.mode !== "create" ||
        louver == null
      )
        return;
      ...
      return (
        windPrime.addEventListener("pointerdown", yarrowPrime, {
          capture: true,
        }),
        () => {
  ...
  let vaporPrime = () => {
    ...
    ((reducer != null || gammaPrime != null) && !mintPrime()) ||
      (wye(canyonPrime),
      strainer(null),
      union(null),
      deltaPrime(""),
  ...
  let wheatPrime = vapor.useEffectEvent(vaporPrime),
    yarnPrime = () => {
      ...
      return (
        dewPrime.addEventListener("mouseup", everPrime),
        dewPrime.addEventListener("keyup", everPrime),
        () => {
  ...
  let acornPrime = (event) => {
    ...
    havenPrime != null &&
      ...
      event.stopPropagation(),
      !((reducer != null || gammaPrime != null) && !mintPrime()) &&
        (grainPrime.ownerDocument.getSelection()?.removeAllRanges(),
        wye(havenPrime),
        strainer(null),
        union(null),
        deltaPrime(""),
  ...
  let bloomPrime = vapor.useEffectEvent(acornPrime),
    coralPrime = () => {
      ...
      return (
        inkPrime.addEventListener("click", jadeitePrime),
        () => {
  ...
  let glidePrime = frostPrime,
    honeyPrime = (event, rootPrime) => {
      ...
      silkPrime != null &&
        (event.preventDefault(),
        event.stopPropagation(),
        glidePrime(silkPrime, "ask_codex_shortcut"));
    };
  let irisPrime = vapor.useEffectEvent(honeyPrime),
    jewelPrime = () => {
      ...
      return (
        thornPrime.ownerDocument.addEventListener("keydown", uplandPrime, {
          capture: true,
        }),
        () => {
  ...
  let lunarPrime = (wispPrime) => {
    ((reducer != null || gammaPrime != null) && !mintPrime()) ||
      glidePrime(wispPrime, "annotation_mode_pointer");
  };
  ...
  let fernPrime =
    onyxPrime == null || gammaPrime != null ? null : (
      <DocxPreviewPanelHelper25
        {...{
          comment: onyxPrime,
          layer: louver,
        }}
  ...
  let $t =
    zener != null && reducer == null && gammaPrime == null ? (
  ...
  let grovePrime =
    vent != null && reducer == null ? (
      <>
        {vent.selectionKind === "text" ? null : (
          <DocxPreviewPanelHelper24
            {...{
              bordered: true,
              paddingPx: 4,
  ...
  let juniperPrime =
    pearlPrime == null || xenonPrime == null || violetPrime == null ? null : (
      <div
        ref={nip}
      ...
      >
        <DesignComposerSurface
          key={violetPrime.sessionId}
          allowImageAttachments={false}
          defaultCreateSubmitMode="direct"
          inputAriaLabel={amberPrime}
          keyboardEventTarget={typeof window > "u" ? undefined : window}
          placeholder={cedarPrime}
          session={violetPrime}
          windowHeight={Queued}
          onCancel={garnetPrime}
          onDelete={(quarryPrime) => {
            onCommentsChange((rapidsPrime) =>
              rapidsPrime.filter((item) => zinc(item) !== quarryPrime),
            );
            garnetPrime();
          }}
          onEscape={garnetPrime}
          onBodyChange={
            violetPrime.target.mode === "create" ? ivoryPrime : undefined
          }
          onLightDismissibilityChange={yarn}
          onMounted={yarn}
          onDirectSubmit={(spurPrime) => {
            $(spurPrime, {
              submitDirectly: true,
            });
          }}
  ...
  return (
    <div
      ...
      style={brookPrime}
      onPointerDown={orbitPrime}
      onPointerMove={questPrime}
      onPointerUp={stormPrime}
      onPointerCancel={cliffPrime}
      onPointerLeave={duskPrime}
      onClick={unityPrime}
