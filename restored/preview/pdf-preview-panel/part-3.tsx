...
function PdfPreviewPanelHelper32({
  ...
  let pinePrime = CodexPluginActionType(appScopeAtom),
    ...
    urnPrime = moss.useMemo(() => comments.filter(item => item.localPdfContext?.pageNumber === pageNumber), [comments, pageNumber]),
    vinePrime = lagoonPrime == null ? null : urnPrime.find(item => on(item) === lagoonPrime) ?? null,
    windPrime = islePrime == null ? null : urnPrime.find(item => on(item) === islePrime) ?? null,
    yarrowPrime = windPrime == null ? null : drift(windPrime),
    azurePrime = valePrime ?? yarrowPrime,
    birchPrime = moss.useMemo(() => valePrime == null ? yarrowPrime == null || windPrime == null || islePrime == null ? null : acorn({
      anchor: yarrowPrime,
      ...
      title
    }), [conversationId, valePrime, apexPrime, yarrowPrime, windPrime, islePrime, pageNumber, pageSize, path, title]),
    canyonPrime = azurePrime == null ? null : $t({
      anchor: azurePrime,
      layer: tidePrime,
      pageSize
    }),
    ...
    grainPrime = moss.useCallback(() => birchPrime == null ? false : birchPrime.target.mode === "create" && apexPrime.trim().length > 0 ? nestPrime ? (everPrime(), true) : (oakPrime(true), _e(), false) : (everPrime(), true), [birchPrime, everPrime, apexPrime, nestPrime, _e]),
  ...
  moss.useEffect(() => {
    ...
    let wispPrime = typeof ResizeObserver > "u" ? null : new ResizeObserver(vistaPrime);
    return wispPrime?.observe(tidePrime), vistaPrime(), () => {
  ...
  let be = moss.useEffectEvent(event => {
    ...
    yonderPrime instanceof Node && ridgePrime.current?.contains(yonderPrime) || (event.preventDefault(), event.stopPropagation(), grainPrime() && (yonderPrime instanceof Node ? yonderPrime.ownerDocument : tidePrime?.ownerDocument)?.getSelection()?.removeAllRanges());
  });
  moss.useEffect(() => {
    if (isCommentMode || birchPrime?.target.mode !== "create" || tidePrime == null) return;
    let zenithPrime = tidePrime.ownerDocument,
      anvilPrime = beaconPrime => {
        be(beaconPrime);
      };
    return zenithPrime.addEventListener("pointerdown", anvilPrime, {
      capture: true
    }), () => {
  ...
  let inkPrime = moss.useEffectEvent(() => {
    ...
    (valePrime != null || islePrime != null) && !grainPrime() || (fernPrime(domePrime), wavePrime(null), brookPrime(""), juniperPrime(null), meadowPrime(null), hillPrime(null), oakPrime(false));
  });
  moss.useEffect(() => {
    ...
    return eddyPrime.addEventListener("mouseup", fjordPrime), eddyPrime.addEventListener("keyup", fjordPrime), () => {
  ...
  let jadeitePrime = (glenPrime, hearthPrime, inletPrime) => {
      ...
      return jettyPrime == null ? null : amber({
        clientX: glenPrime,
        clientY: hearthPrime,
    ...
    kernelPrime = (knobPrime, ledgePrime) => {
      logArtifactAnnotationSubmitted(pinePrime, {
        artifactTabId: tabId,
        artifactType: "pdf",
        importKind: "pdf",
        threadId
    ...
    leafPrime = moss.useEffectEvent((event, mirePrime) => {
      if (valePrime != null || islePrime != null || !AppInitialXg(event)) return;
      let nookPrime = zinc({
        pageElement: mirePrime,
        pageSize
      }) ?? (elmPrime?.selectionKind === "text" ? null : elmPrime);
      nookPrime != null && (event.preventDefault(), event.stopPropagation(), kernelPrime(nookPrime, "ask_codex_shortcut"));
    });
  moss.useEffect(() => {
    if (tidePrime == null || isCommentMode) return;
    ...
    return oxbowPrime.ownerDocument.addEventListener("keydown", pondPrime, {
      capture: true
    }), () => {
  ...
  let maplePrime = rapidsPrime => {
      (valePrime != null || islePrime != null) && !grainPrime() || kernelPrime(rapidsPrime, "annotation_mode_pointer");
    },
    nimbusPrime = event => {
      if (!isCommentMode || !event.isPrimary || event.button !== 0) return;
      let spurPrime = yellow(event, pageSize);
      spurPrime != null && (event.preventDefault(), event.stopPropagation(), event.currentTarget.ownerDocument.getSelection()?.removeAllRanges(), event.currentTarget.setPointerCapture(event.pointerId), fernPrime(null), hillPrime(null), meadowPrime(null), duskPrime({
    ...
    opalPrime = event => {
      ...
      torPrime != null && (event.preventDefault(), event.stopPropagation(), duskPrime(_event => _event == null || _event.pointerId !== event.pointerId ? _event : {
        ..._event,
        current: torPrime,
        clientCurrent: {
          x: event.clientX,
    ...
    plumePrime = event => {
      if (cliffPrime == null || event.pointerId !== cliffPrime.pointerId) return;
    ...
    quillowPrime = async (boltPrime, {
      submitDirectly = false
    } = {}) => {
      let cogPrime = boltPrime.body.trim();
      if (islePrime != null) {
        if (cogPrime.length === 0 || petalPrime) return;
        onCommentsChange(discPrime => discPrime.map(item => on(item) === islePrime ? {
          ...item,
          content: [{
            content_type: "text",
            text: cogPrime
      ...
      if (!(valePrime == null || cogPrime.length === 0 || petalPrime)) try {
        quietPrime(true);
        let edgePrime = bloom({
            anchor: valePrime,
            pageSize
        ...
        logArtifactRefreshClicked(pinePrime, {
          artifactTabId: tabId,
          artifactType: "pdf",
          importKind: "pdf",
          threadId
  ...
  return <div ref={unityPrime} className={IntlProvider("absolute inset-0 z-[3]", !isCommentMode && "pointer-events-none")} data-testid="artifact-pdf-comment-layer" style={isCommentMode ? {
    cursor: pickSourcePosition
  } : undefined} onPointerDown={nimbusPrime} onPointerMove={opalPrime} onPointerUp={plumePrime} onPointerCancel={() => {
    duskPrime(null);
    hillPrime(null);
  }} onPointerLeave={() => hillPrime(null)}>
      {urnPrime.map(item => <PdfPreviewPanelHelper33 key={on(item)} {...{
      comment: item,
      ...
      {vinePrime == null || islePrime != null ? null : <PdfPreviewPanelHelper34 {...{
      comment: vinePrime,
      layer: tidePrime
    }} />}
      {grovePrime != null && valePrime == null && islePrime == null ? <QueueAutomationRun {...{
      paddingPx: 4,
      pageSize,
      rect: grovePrime.rect,
      testId: "artifact-pdf-element-hover-highlight"
    }} /> : null}
      {elmPrime != null && valePrime == null ? <IsWhitespaceOrEofCharCode {...{
      ...
      {cliffPrime == null ? null : <PdfPreviewPanelHelper37 {...{
      rect: cedar(cliffPrime.start, cliffPrime.current),
      pageSize
    }} />}
      {valePrime == null ? null : <PdfPreviewPanelHelper35 {...{
      anchor: valePrime,
      label: nextCommentNumber,
      pageSize
    }} />}
      {azurePrime == null || canyonPrime == null || birchPrime == null ? null : <div ref={ridgePrime} className={AppInitialOg} style={{
      left: canyonPrime.x,
      top: canyonPrime.y,
      height: Queued,
      width: 294
    }} onPointerDown={event => event.stopPropagation()}>
          <DesignComposerSurface key={birchPrime.sessionId} allowImageAttachments={false} defaultCreateSubmitMode="direct" inputAriaLabel={rainPrime} keyboardEventTarget={typeof window > "u" ? undefined : window} placeholder={seedPrime} session={birchPrime} windowHeight={Queued} onCancel={everPrime} onDelete={keystonePrime => {
        onCommentsChange(latchPrime => latchPrime.filter(item => on(item) !== keystonePrime));
        everPrime();
      }} onDirectSubmit={motorPrime => {
        quillowPrime(motorPrime, {
          submitDirectly: true
        });
      }} onEscape={everPrime} onBodyChange={birchPrime.target.mode === "create" ? fieldPrime : undefined} onLightDismissibilityChange={orbit} onMounted={orbit} onSubmit={nutPrime => {
...
function PdfPreviewPanelHelper33(pistonPrime) {
  ...
  if (screwPrime.kind === "region") {
    let handlePrime = screwPrime.selectionKind == null ? <PdfPreviewPanelHelper37 {...{
    ...
    let leverPrime = <PdfPreviewPanelHelper38 {...{
...
function PdfPreviewPanelHelper34(nozzlePrime) {
  ...
  return <AppInitialCg body={tappetPrime} layer={layer} pageSize={ratchetPrime.pageSize} point={ratchetPrime.point} testId="artifact-pdf-comment-preview" />;
}
