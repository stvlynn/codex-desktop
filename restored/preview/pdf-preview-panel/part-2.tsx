...
function ember({ includePreviewMetadata, pageElement, pageSize, target }) {
...
function flint(falconPrime, gammaPrime) {
  ...
  let indigoPrime = garnet(falconPrime, gammaPrime, (kitePrime) =>
    kitePrime.matches(".annotationLayer a, .annotationLayer .linkAnnotation"),
  );
  if (indigoPrime != null)
    return {
      element: indigoPrime,
  ...
  let jadePrime = garnet(falconPrime, gammaPrime, (lemonPrime) =>
    lemonPrime.matches(".annotationLayer section"),
  );
  return jadePrime == null
    ? null
    : {
        element: jadePrime,
...
function hazel(quartzPrime) {
  return (
    quartzPrime.matches(".textLayer span:not(.markedContent)") &&
    quartzPrime.getAttribute("role") !== "img"
  );
}
function ivory({ clientRects, pageElement, pageSize }) {
  ...
  let timberPrime = ultra(
      ((slatePrime.left - riverPrime.left) / riverPrime.width) * pageSize.width,
      0,
      pageSize.width,
    ),
    umbraPrime = ultra(
      ((slatePrime.top - riverPrime.top) / riverPrime.height) * pageSize.height,
      0,
      pageSize.height,
    ),
    violetPrime = ultra(
      ((slatePrime.right - riverPrime.left) / riverPrime.width) *
        pageSize.width,
      0,
      pageSize.width,
    ),
    willowPrime = ultra(
      ((slatePrime.bottom - riverPrime.top) / riverPrime.height) *
        pageSize.height,
      0,
      pageSize.height,
    );
  return violetPrime <= timberPrime || willowPrime <= umbraPrime
    ? null
    : {
        x: timberPrime,
        y: umbraPrime,
...
function jasper({ clientRects, pageElement, pageSize }) {
  let xenonPrime = [];
...
function kelp({ clientRects, pageElement, pageSize, selection }) {
  ...
  let emberPrime = daisyPrime.top + daisyPrime.height / 2,
    ...
    ivoryPrime =
      emberPrime > flintPrime && hazelPrime
        ? "below"
        : garnetPrime
          ? "above"
          : "below";
...
function lotus({ clientX, clientY, pageElement, pageSize }) {
  let jasperPrime = pageElement.getBoundingClientRect();
  return {
    x: ultra(
      ((clientX - jasperPrime.left) / jasperPrime.width) * pageSize.width,
      0,
      pageSize.width,
    ),
    y: ultra(
      ((clientY - jasperPrime.top) / jasperPrime.height) * pageSize.height,
      0,
      pageSize.height,
    ),
  };
...
function mint(kelpPrime) {
  let lotusPrime = [],
    mintPrime = kelpPrime
      ...
      .sort(
        (novaPrime, olivePrime) =>
          novaPrime.top - olivePrime.top || novaPrime.left - olivePrime.left,
      );
...
function nova(reefPrime, sagePrime) {
  return (
    Math.min(reefPrime.bottom, sagePrime.bottom) -
      Math.max(reefPrime.top, sagePrime.top) >=
    Math.min(reefPrime.height, sagePrime.height) * 0.5
  );
}
function olive(topazPrime) {
  let ultraPrime = 1 / 0,
    ...
    wheatPrime = -1 / 0,
    yarnPrime = -1 / 0;
  for (let zephyrPrime of topazPrime)
    zephyrPrime.width <= 0 ||
      zephyrPrime.height <= 0 ||
      ((ultraPrime = Math.min(ultraPrime, zephyrPrime.left)),
      (vaporPrime = Math.min(vaporPrime, zephyrPrime.top)),
      (wheatPrime = Math.max(wheatPrime, zephyrPrime.right)),
      (yarnPrime = Math.max(yarnPrime, zephyrPrime.bottom)));
  return !Number.isFinite(ultraPrime) ||
    !Number.isFinite(vaporPrime) ||
    !Number.isFinite(wheatPrime) ||
    !Number.isFinite(yarnPrime)
    ? null
    : {
        bottom: yarnPrime,
...
function prism(acornPrime) {
  if (acornPrime.anchorNode == null || acornPrime.focusNode == null)
    return true;
  if (acornPrime.anchorNode === acornPrime.focusNode)
    return acornPrime.focusOffset >= acornPrime.anchorOffset;
  let bloomPrime = acornPrime.anchorNode.compareDocumentPosition(
    acornPrime.focusNode,
  );
  return bloomPrime === Node.DOCUMENT_POSITION_FOLLOWING
    ? true
    : bloomPrime !== Node.DOCUMENT_POSITION_PRECEDING;
}
function quill(coralPrime, driftPrime) {
  return (
    coralPrime.anchorNode != null &&
    coralPrime.focusNode != null &&
    driftPrime.contains(coralPrime.anchorNode) &&
    driftPrime.contains(coralPrime.focusNode)
  );
}
function reef(eaglePrime) {
  ...
  return glidePrime.length > 0
    ? glidePrime
    : topaz(
        (eaglePrime instanceof HTMLAnchorElement
          ? eaglePrime
          : eaglePrime.querySelector("a")
        )?.href ?? "",
      );
}
