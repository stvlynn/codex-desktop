...
) => quillow.map((item) => thorn[item]);
function alpha(upland) {
  let { containerRef, pageSelector, totalPages } = upland,
  ...
  beacon = () => {
    ...
    if (jetty == null || totalPages < 1 || typeof IntersectionObserver > "u")
      return;
    let knob = bravo(jetty, pageSelector);
    if (knob.length === 0) return;
    let ledge = new Map(),
      mire = new IntersectionObserver(
        (oxbow) => {
          for (let quarry of oxbow)
            ledge.set(quarry.target, quarry.intersectionRatio);
          let pond = delta({
            container: jetty,
            pageElements: knob,
          ...
          pond != null &&
            ((yonder.current != null && pond !== yonder.current) ||
              (pond === yonder.current &&
                ((yonder.current = null),
                zenith.current?.(),
                (zenith.current = null)),
...
function delta({ container, pageElements, pageVisibilityByElement }) {
...
async function kite() {
  ...
  return (
    (!pearl || flange.GlobalWorkerOptions.workerSrc !== nickel) &&
      ((flange.GlobalWorkerOptions.workerSrc = nickel), (pearl = true)),
    flange
  );
}
function lemon(gib) {
  ...
  for (let keeper = 0; keeper < idler.length; keeper += 1)
    jig[keeper] = idler.charCodeAt(keeper);
  return jig;
}
var marble,
  $e,
...
function at(lug) {
  return (
    ...
    !!lug &&
    "num" in lug &&
    typeof lug.num == "number" &&
    "gen" in lug &&
    typeof lug.gen == "number"
  );
}
var slate,
  timber = esmInit(() => {
    slate = class {
      ...
      async goToDestination(pin) {
        let race =
            typeof pin == "string"
              ? await this.pdfDocument.getDestination(pin)
              : pin,
          sleeve =
            race == null ? null : await this.getDestinationPageNumber(race);
      ...
      getDestinationHash(eccentric) {
        return typeof eccentric == "string" && eccentric.length > 0
          ? this.getAnchorUrl(`#${window.encodeURIComponent(eccentric)}`)
          : this.getAnchorUrl("");
      }
      ...
      async getDestinationPageNumber(journal) {
        ...
        if (typeof kingpin == "number" && Number.isInteger(kingpin))
          return kingpin + 1;
        ...
        if (!(mesh < 1 || mesh > this.pdfDocument.numPages)) {
          ...
          (
            this.scrollRootRef.current?.querySelector(
              `${this.pageSelector}[data-page-number="${mesh}"]`,
            ) ?? null
          )?.scrollIntoView({
            block: "start",
            inline: "nearest",
...
function yellow(event, header) {
  ...
  return injector.width <= 0 || injector.height <= 0
    ? null
    : {
        x: ultra(
          ((event.clientX - injector.left) / injector.width) * header.width,
          0,
          header.width,
        ),
        y: ultra(
          ((event.clientY - injector.top) / injector.height) * header.height,
          0,
          header.height,
        ),
      };
}
function zinc({ pageElement, pageSize }) {
...
function amber({
  ...
  for (let reducer of pageElement.ownerDocument.elementsFromPoint?.(
    clientX,
    clientY,
  ) ?? []) {
    if (
      reducer === layerElement ||
      layerElement.contains(reducer) ||
      !pageElement.contains(reducer)
    )
      continue;
    let strainer = ember({
      includePreviewMetadata,
...
function _t(wye) {
  return (
    Math.abs(wye.clientCurrent.x - wye.clientStart.x) >= wheat ||
    Math.abs(wye.clientCurrent.y - wye.clientStart.y) >= wheat
  );
}
