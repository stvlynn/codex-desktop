...
function PdfPreviewPanelHelper35(arborPrime) {
  let { anchor, label, pageSize } = arborPrime;
...
function PdfPreviewPanelHelper36(gibPrime) {
  let { anchor, bordered = false, pageSize, testId } = gibPrime;
  if (anchor.selectionKind === "text") {
    ...
    {
      let lugPrime;
      lugPrime = (mandrelPrime, nipplePrime) => (
        <QueueAutomationRun
          key={`${mandrelPrime.x}:${mandrelPrime.y}:${mandrelPrime.width}:${mandrelPrime.height}:${nipplePrime}`}
          {...{
            paddingX: 4,
            paddingY: 0,
            pageSize,
...
function PdfPreviewPanelHelper37(orificePrime) {
  let { pageSize, rect } = orificePrime;
  return (
  ...
  return {
    ...
    anchorState: {
      anchor: {
        ...
        point: {
          xPercent:
            pageSize.width === 0 ? 0 : (sleevePrime.x / pageSize.width) * 100,
          y: sleevePrime.y,
        },
        rect: racePrime,
        isFixed: false,
...
function bloom({ anchor, pageSize }) {
...
function coral(trunnionPrime) {
  ...
  return bossPrime == null
    ? null
    : bossPrime.kind === "region"
      ? {
...
function drift(camPrime) {
  ...
  return detentPrime == null
    ? null
    : detentPrime.kind === "region"
      ? {
          kind: "region",
...
function $t({ anchor, layer, pageSize }) {
  return anchor.kind === "region" && anchor.selectionKind != null
    ...
    : OperationStatusOverlay({
        layer,
        markerPoint:
          anchor.kind === "region" ? daisy(anchor.rect) : anchor.point,
        pageSize,
      });
}
async function frost({
  ...
  if (
    ((helixPrime.width = Math.ceil(pageSize.width * guidePrime)),
    (helixPrime.height = Math.ceil(pageSize.height * guidePrime)),
    helixPrime.getContext("2d") == null)
  )
  ...
  return (
    journalPrime.drawImage(
      helixPrime,
      followerPrime.x * guidePrime,
      followerPrime.y * guidePrime,
      followerPrime.width * guidePrime,
      followerPrime.height * guidePrime,
      0,
      0,
      impellerPrime.width,
      impellerPrime.height,
    ),
    iris({
      anchor,
...
function glide(kingpinPrime, landPrime) {
  if (kingpinPrime.kind === "region") {
    let meshPrime = Math.max(
      24,
      Math.min(
        72,
        Math.min(kingpinPrime.rect.width, kingpinPrime.rect.height) * 0.25,
      ),
    );
    return honey(
      {
...
function iris({ anchor, context, crop, markerLabel, scale }) {
  if (
    (context.save(),
    (context.strokeStyle = moveThread),
    (context.fillStyle = moveThread),
    (context.lineWidth = 3),
...
function unity(zenerPrime) {
  let { onBeforeZoom, pageViewportSize, pageWidth } = zenerPrime,
    alphaSecond = {
      kind: "page-width",
    };
  ...
  let zincSecond = yellowSecond,
    amberSecond = (event) => {
      if (
        ...
        (jadeSecond.current != null && window.clearTimeout(jadeSecond.current),
        (jadeSecond.current = window.setTimeout(() => {
          indigoSecond.current = 0;
          jadeSecond.current = null;
        }, juniper)),
        (indigoSecond.current += event.deltaY),
        Math.abs(indigoSecond.current) < isle)
      )
        return;
...
function vale({ pageViewportSize, pageWidth, resize }) {
  if (resize.kind === "percentage") return brook(resize.value);
  let lotusSecond = pageViewportSize?.width ?? elm.width;
  return pageWidth == null || pageWidth <= 0 || lotusSecond <= 0
    ? 1
    : brook(pageWidth / lotusSecond);
}
function wave({ pageViewportSize, pageWidth, resize }) {
