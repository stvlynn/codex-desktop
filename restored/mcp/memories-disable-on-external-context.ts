...
export function setMemoriesDisableOnExternalContextPeers(next: MemoriesDisableOnExternalContextPeers): void {
  peers = next;
}
...
export function memoriesDisableOnExternalContext() {
  if (peers == null) {
    throw new Error("memoriesDisableOnExternalContext peers are not configured");
  }
  ...
  return peers.e(() => {
    peers.Sl(), Bx = {
    ...
    }, Wcn = peers.ul([peers.rl(), peers.Uue()]).transform(e => Number(peers.e)).pipe(peers.rl().int().nonnegative()), Gcn = peers.sl({
      generate_memories: peers.il().optional().catch(void 0),
      use_memories: peers.il().optional().catch(void 0),
      disable_on_external_context: peers.il().optional().catch(void 0),
      no_memories_if_mcp_or_web_search: peers.il().optional().catch(void 0),
      max_raw_memories_for_consolidation: peers.Wcn.optional().catch(void 0),
      ...
      extract_model: peers.X().nullable().optional().catch(void 0),
      consolidation_model: peers.X().nullable().optional().catch(void 0)
    }).partial(), Kcn = peers.fl(peers.X(), peers._l());
  });
}
