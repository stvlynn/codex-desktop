...
export function cachedFormatCellDisplay(
  afIn9582: unknown,
  afIn9583: unknown,
  afIn9584: unknown,
  afIn9585: unknown,
  ...
  let afBind20114 = formatCellValueWithNumFmt(
    afIn9582,
    afIn9583,
    afIn9584,
    afIn9585,
...
export function cellHasVisibleContent(afIn9422: unknown, afIn9423: unknown) {
  ...
  for (let afBind21995 of afIn9422.paragraphs ?? [])
    for (let afBind22787 of afBind21995.runs ?? [])
      if (afBind22787.text !== "") return true;
  return false;
}
export function tableHeaderCellKeys(afIn6548: unknown) {
  ...
  for (let afBind18260 of afIn6548) {
    ...
    for (
      let afBind21562 = afBind18260.startRow;
      afBind21562 <= afBind18768;
      afBind21562++
    )
      for (
        let afBind22668 = afBind18260.startCol;
        afBind22668 <= afBind18260.endCol;
        afBind22668++
      )
        afBind16792.add(`${afBind21562}:${afBind22668}`);
  }
  return afBind16792;
}
export function findTableResolverAt(
  afIn13287: unknown,
  afIn13288: unknown,
  afIn13289: unknown,
) {
...
export function filterButtonExtraWidth(afIn11074: unknown) {
  let { padLr } = cellPaddingForFontSize(
    afIn11074?.font?.size ?? 14.666666666666666,
  );
  return Math.max(0, 22 - padLr);
}
export function listValidationBounds(afIn4425: unknown) {
  let afBind13652 = [];
  for (let afBind14545 of afIn4425)
    for (let afBind15012 of afBind14545.ranges) {
...
export function isInListValidationRange(
  afIn10452: unknown,
  afIn10453: unknown,
  afIn10454: unknown,
) {
  return afIn10452.some(
    (item) =>
      afIn10453 >= item.startRow &&
      afIn10453 <= item.endRow &&
      afIn10454 >= item.startCol &&
      afIn10454 <= item.endCol,
  );
}
