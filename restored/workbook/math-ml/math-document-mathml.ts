...
export function normalizeMathDocumentViaMathMl(mathIn14350: unknown) {
  return mathIn14350?.root
    ? mathMlTreeToMathDocument(mathDocumentToMathMlTree(mathIn14350))
    : mathIn14350;
}
export function mathDocumentToMathMlTree(props: unknown) {
  let workbookBinding16419 = {
    display:
      props?.displayMode === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK
        ? "block"
        : "inline",
  };
  return (
    props?.paragraphProperties &&
      (workbookBinding16419[DATA_GRANOLA_PARAGRAPH_PROPERTIES] = JSON.stringify(
        props.paragraphProperties,
      )),
    {
      tag: "math",
...
export function mathMlTreeToMathDocument(
  mathIn3134: unknown,
  mathIn3135: unknown,
) {
  if (mathIn3134.tag !== "math")
    throw Error("MathML root must be a `<math>` element.");
  let workbookBinding11255 = normalizeMathAstNode(
      wrapMathAstSequence(
        mathIn3134.children
    ...
    workbookBinding11256 =
      mathIn3134.attributes.display === "block" || mathIn3135 === "block"
  ...
  return {
    ...
    paragraphProperties:
      parseJsonAttribute(
        mathIn3134.attributes[DATA_GRANOLA_PARAGRAPH_PROPERTIES],
      ) ??
      (workbookBinding11256 === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK
        ? {
