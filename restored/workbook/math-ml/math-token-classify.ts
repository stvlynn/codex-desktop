...
export function classifyCharTokenKind(
  mathIn5710: unknown,
  mathIn5711: unknown,
) {
  return isUnicodeNumberChar(mathIn5710)
    ? MathTokenKind.MATH_TOKEN_KIND_NUMBER
    : isMathOperatorChar(mathIn5710) || isMathPunctuationChar(mathIn5710)
      ? MathTokenKind.MATH_TOKEN_KIND_OPERATOR
      : mathIn5711 === MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER &&
          isUnicodeLetterChar(mathIn5710)
        ? MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER
        : isUnicodeLetterChar(mathIn5710)
          ? MathTokenKind.MATH_TOKEN_KIND_SYMBOL
          : (mathIn5711 ?? MathTokenKind.MATH_TOKEN_KIND_SYMBOL);
}
export function canMergeTokenKinds(
  mathIn8471: unknown,
  mathIn8472: unknown,
  mathIn8473: unknown,
) {
  return mathIn8471 === mathIn8472
    ? mathIn8472 === MathTokenKind.MATH_TOKEN_KIND_NUMBER
      ? true
      : mathIn8472 === MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER &&
        mathIn8473 === MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER
    : false;
}
export function splitTokenTextByKind(mathIn3769: unknown, mathIn3770: unknown) {
  let workbookBinding12498 = Array.from(mathIn3769);
  if (
    workbookBinding12498.length <= 1 ||
    mathIn3770 === MathTokenKind.MATH_TOKEN_KIND_TEXT ||
    (mathIn3770 === MathTokenKind.MATH_TOKEN_KIND_NUMBER &&
      workbookBinding12498.every((item) => isUnicodeNumberChar(item))) ||
    (mathIn3770 === MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER &&
      workbookBinding12498.every((item) => isUnicodeLetterChar(item)))
  )
    return [
      {
        text: mathIn3769,
  ...
  for (let workbookBinding19332 of workbookBinding12498) {
    let workbookBinding19836 = classifyCharTokenKind(
        workbookBinding19332,
        mathIn3770,
      ),
      workbookBinding19837 =
        workbookBinding12499[workbookBinding12499.length - 1];
    if (
      workbookBinding19837 &&
      canMergeTokenKinds(
        workbookBinding19837.kind,
        workbookBinding19836,
        mathIn3770,
      )
    ) {
...
export function splitMultiCharTokenNode(props: unknown) {
  ...
  let workbookBinding17582 = splitTokenTextByKind(
    workbookBinding17581.text,
    workbookBinding17581.kind,
  );
  return workbookBinding17582.length === 1
    ? props
    : {
        style: props.style,
        sequence: {
