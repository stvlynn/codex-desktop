...
export class MathLatexParser {
  ...
  parseMath() {
    ...
    return {
      ...
      paragraphProperties: this.#displayMode === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK ? {
  ...
  parseSequence(mathSlot2844) {
    ...
    for (; !this.isAtEnd();) {
      ...
      if (this.isAtEnd() || mathTmp12904 === "}" || mathSlot2844?.stopAt?.includes(mathTmp12904) || mathSlot2844?.stopAtRightCommand && this.startsWithCommand("right") || mathSlot2844?.stopBeforeRelation && this.isRelationOperator(mathTmp12904)) break;
      let mathTmp12905 = this.parseNode();
      if (!mathTmp12905) break;
      mathTmp10626.push(mathTmp12905);
    }
    if (mathTmp10626.length !== 0) return mathTmp10626.length === 1 ? mathTmp10626[0] : {
      sequence: {
  ...
  parseNode() {
    ...
    if (mathTmp13309 && (mathTmp13309.operator === "∫" || mathTmp13309.operator === "∑") && mathTmp13309.body === undefined) {
  ...
  parseBase() {
    ...
    if (mathTmp7408 === "{") {
      ...
      return this.peek() === "}" && this.consume(), mathTmp17680 ?? this.createToken("", MathTokenKind.MATH_TOKEN_KIND_TEXT);
    }
    return mathTmp7408 === "\\" ? this.parseCommand() : this.isDigit(mathTmp7408) ? this.createToken(this.readWhile(mathSlot16254 => this.isDigit(mathSlot16254)), MathTokenKind.MATH_TOKEN_KIND_NUMBER) : this.isIdentifierStart(mathTmp7408) ? this.createToken(this.readWhile(mathSlot15847 => this.isIdentifierPart(mathSlot15847)), MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER) : this.isOperator(mathTmp7408) ? (this.consume(), this.createToken(mathTmp7408, MathTokenKind.MATH_TOKEN_KIND_OPERATOR)) : (this.consume(), this.createToken(mathTmp7408, MathTokenKind.MATH_TOKEN_KIND_SYMBOL));
  }
  parseCommand() {
    this.consume();
    ...
    if (mathTmp3343 === "hat" || mathTmp3343 === "widehat") return {
      accent: {
        character: "ˆ",
    ...
    if (mathTmp3343 === "bar" || mathTmp3343 === "overline") return {
      bar: {
        base: this.parseRequiredArgument(),
        position: MathBarPosition.MATH_BAR_POSITION_TOP
      }
    };
    if (mathTmp3343 === "cancel") return {
      enclosure: {
        body: this.parseRequiredArgument(),
        strikeHorizontal: true
      }
    };
    if (mathTmp3343 === "phantom") return {
      phantom: {
        body: this.parseRequiredArgument(),
        show: false,
        zeroWidth: true
    ...
    if (mathTmp3343 === "Box" || mathTmp3343 === "square") return this.createToken("□", MathTokenKind.MATH_TOKEN_KIND_SYMBOL);
    if (mathTmp3343 === "text" || mathTmp3343 === "mathrm") return {
      ...this.createToken(this.readRequiredRawBraceText(), MathTokenKind.MATH_TOKEN_KIND_TEXT),
      style: {
    ...
    if (mathTmp3343 === "operatorname") {
      let mathTmp16580 = this.createToken(this.readRequiredRawBraceText(), MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER),
    ...
    if (mathTmp3343 === "begin") {
      ...
      return MATRIX_ENVIRONMENT_NAMES.has(mathTmp18613) ? this.parseMatrixEnvironment(mathTmp18613) : this.createToken(mathTmp18613, MathTokenKind.MATH_TOKEN_KIND_TEXT);
    }
    if (mathTmp3343 === "int") return {
      nary: {
        operator: "∫",
        limitPlacement: this.#displayMode === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK ? MathLimitPlacement.MATH_LIMIT_PLACEMENT_UNDER_OVER : MathLimitPlacement.MATH_LIMIT_PLACEMENT_SUB_SUP
      }
    };
    if (mathTmp3343 === "sum") return {
      nary: {
        operator: "∑",
        limitPlacement: this.#displayMode === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK ? MathLimitPlacement.MATH_LIMIT_PLACEMENT_UNDER_OVER : MathLimitPlacement.MATH_LIMIT_PLACEMENT_SUB_SUP
      }
    };
    if (mathTmp3343 === "lim") return {
      limit: {
        base: this.createToken("lim", MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER),
        limit: this.createToken("", MathTokenKind.MATH_TOKEN_KIND_TEXT),
        kind: MathLimitKind.MATH_LIMIT_KIND_LOWER
      }
    };
    if (mathTmp3343 === ",") return this.createToken(" ", MathTokenKind.MATH_TOKEN_KIND_TEXT);
    let mathTmp3344 = LATEX_GREEK_COMMANDS[mathTmp3343];
    if (mathTmp3344) return this.createToken(mathTmp3344, MathTokenKind.MATH_TOKEN_KIND_SYMBOL);
    let mathTmp3345 = LATEX_OPERATOR_COMMANDS[mathTmp3343];
    return mathTmp3345 ? this.createToken(mathTmp3345, MathTokenKind.MATH_TOKEN_KIND_OPERATOR) : this.createToken(mathTmp3343, MathTokenKind.MATH_TOKEN_KIND_IDENTIFIER);
  }
  parsePostfix(mathSlot2058) {
    ...
    for (;;) {
      ...
      mathTmp9636 === "_" ? mathTmp9638.subscript = mathTmp9637 : mathTmp9638.superscript = mathTmp9637;
      mathTmp8921 = {
        scripts: mathTmp9638
      };
    }
  ...
  parseRequiredArgument() {
    return this.parseScriptArgument() || this.createToken("", MathTokenKind.MATH_TOKEN_KIND_TEXT);
  }
  ...
  parseFunctionArgument() {
    if (this.skipWhitespace(), this.startsWithCommand("left")) return this.parseBase();
    if (this.peek() === "(" || this.peek() === "[") return this.parsePlainDelimited();
  }
  parsePlainDelimited() {
  ...
  parseDelimitedItems(mathSlot1019, mathSlot1020, mathSlot1021) {
    let mathTmp6194 = [];
    for (; !this.isAtEnd() && (this.skipWhitespace(), !(mathSlot1021.stopAt?.includes(this.peek()) || mathSlot1021.stopAtRightCommand && this.startsWithCommand("right")));) {
      let mathTmp12433 = this.parseSequence({
        stopAt: [",", ...(mathSlot1021.stopAt ?? [])],
        stopAtRightCommand: mathSlot1021.stopAtRightCommand
      });
      if (mathTmp12433 && mathTmp6194.push(mathTmp12433), this.skipWhitespace(), this.peek() === ",") {
        this.consume();
        continue;
      }
      if (mathSlot1021.stopAt?.includes(this.peek()) || mathSlot1021.stopAtRightCommand && this.startsWithCommand("right")) break;
    }
    let mathTmp6195 = mathSlot1020;
    return mathSlot1021.consumeEndDelimiter && this.peek() === mathSlot1020 && this.consume(), mathSlot1021.consumeRightCommand && this.startsWithCommand("right") && (this.consume(), this.readCommandName(), mathTmp6195 = this.readDelimiterToken()), {
      delimited: {
  ...
  parseMatrixEnvironment(mathSlot1601) {
    let mathTmp7773 = this.readUntil(`\\end{${mathSlot1601}}`).split(/\\\\/).map(item => item.trim()).filter(item => item.length > 0).map(item => item.split("&").map(_item => this.parseMathCell(_item.trim()))),
      mathTmp7774 = mathTmp7773.reduce((accumulator, current) => Math.max(accumulator, current.length), 0),
      mathTmp7775 = {
        matrix: {
          columns: Array.from({
            length: mathTmp7774
  ...
  parseMathCell(mathSlot8229) {
    ...
    if (!mathTmp18737.root) throw Error("LaTeX matrix cell did not produce a math root.");
  ...
  readRequiredRawBraceText() {
    ...
    for (; !this.isAtEnd() && mathTmp13720 > 0;) {
      ...
      mathTmp21739 === "{" ? mathTmp13720 += 1 : mathTmp21739 === "}" && --mathTmp13720;
    }
  ...
  readDelimiterToken() {
    ...
    return mathTmp16845 === "{" ? "{" : mathTmp16845 === "}" ? "}" : mathTmp16845 === "." ? "" : mathTmp16845;
  }
  readRightDelimiter() {
    return ")";
  }
  readCommandName() {
    ...
    return mathTmp18167 ? this.isLetter(mathTmp18167) ? this.readWhile(mathSlot16215 => this.isLetter(mathSlot16215)) : (this.consume(), mathTmp18167) : "";
  }
  ...
  skipWhitespace() {
    for (; !this.isAtEnd();) {
      ...
      if (mathTmp17408 !== " " && mathTmp17408 !== "\n" && mathTmp17408 !== "\t" && mathTmp17408 !== "\r") break;
      this.#cursor += 1;
    }
  }
  ...
  isOperator(mathSlot6405) {
    return mathSlot6405 === "=" || mathSlot6405 === "+" || mathSlot6405 === "-" || mathSlot6405 === "*" || mathSlot6405 === "/" || mathSlot6405 === "(" || mathSlot6405 === ")" || mathSlot6405 === "[" || mathSlot6405 === "]";
  }
