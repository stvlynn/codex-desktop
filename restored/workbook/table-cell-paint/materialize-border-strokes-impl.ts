...
export function mergeCollinearBorderStrokes(tcpIn2052: any) {
  let tcpBind8914 = [...tcpIn2052].sort(
      (tcpIn5973, tcpIn5974) =>
        tcpIn5973.orientation === tcpIn5974.orientation
          ? Math.abs(tcpIn5973.crossPx - tcpIn5974.crossPx) >
            0.001
            ? tcpIn5973.crossPx - tcpIn5974.crossPx
            : Math.abs(tcpIn5973.startPx - tcpIn5974.startPx) >
                0.001
              ? tcpIn5973.startPx - tcpIn5974.startPx
              : tcpIn5973.endPx - tcpIn5974.endPx
          : tcpIn5973.orientation.localeCompare(
              tcpIn5974.orientation,
            ),
    ),
    tcpBind8915 = [];
  for (let tcpBind12889 of tcpBind8914) {
    let tcpBind13149 =
      tcpBind8915[tcpBind8915.length - 1];
    if (
      tcpBind13149 &&
      tcpBind13149.orientation === tcpBind12889.orientation &&
      Math.abs(tcpBind13149.crossPx - tcpBind12889.crossPx) <=
        0.001 &&
      Math.abs(tcpBind13149.endPx - tcpBind12889.startPx) <=
        0.001 &&
      tcpBind13149.widthPx === tcpBind12889.widthPx &&
      tcpBind13149.line.style === tcpBind12889.line.style &&
      tcpBind13149.line.compound ===
        tcpBind12889.line.compound &&
