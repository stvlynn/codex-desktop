...
export function createRadialGradientFill(radIn924: unknown, radIn925: unknown, radIn926: unknown, radIn927: unknown, radIn928: unknown) {
  let {
      x: __x,
      y: __y,
      width,
    ...
    fillBind5913 = (radIn15788, radIn15789) => Math.hypot(radIn15788 - fillBind5903, radIn15789 - fillBind5904),
    fillBind5914 = Math.max(fillBind5913(__x, __y), fillBind5913(__x + width, __y), fillBind5913(__x, __y + height), fillBind5913(__x + width, __y + height)),
    fillBind5915 = Math.max(Math.abs(fillBind5909), Math.abs(fillBind5910), Math.abs(fillBind5911), Math.abs(fillBind5912)),
    fillBind5916 = Math.abs(fillBind5907 - fillBind5905) < 1e-6 || Math.abs(fillBind5908 - fillBind5906) < 1e-6,
    fillBind5917 = fillBind5903 < __x || fillBind5903 > __x + width || fillBind5904 < __y || fillBind5904 > __y + height,
    fillBind5918 = fillBind5916 || fillBind5917,
    fillBind5919;
  switch (radIn926.pathType) {
    case gradientPathTypeEnum.GRADIENT_PATH_TYPE_CIRCLE:
      fillBind5919 = fillBind5918 ? Math.max(1e-6, fillBind5914) : Math.max(1e-6, Math.min(fillBind5909, fillBind5910, fillBind5911, fillBind5912));
      break;
    case gradientPathTypeEnum.GRADIENT_PATH_TYPE_RECT:
      fillBind5919 = fillBind5918 ? Math.max(1e-6, fillBind5914) : Math.max(1e-6, fillBind5915);
      break;
  ...
  let fillBind5920 = radIn924.createRadialGradient(fillBind5903, fillBind5904, 0, fillBind5903, fillBind5904, fillBind5919),
  ...
  if (fillBind5921.length === 0) {
    ...
    return fillBind5920.addColorStop(0, fillBind19582), fillBind5920.addColorStop(1, fillBind19582), fillBind5920;
  }
  return applyGradientColorStops(fillBind5920, fillBind5921, radIn927, radIn928), fillBind5920;
}
