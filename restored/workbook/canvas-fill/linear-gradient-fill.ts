...
export function createLinearGradientFill(
  ...
  let fillBind9338 = (linIn2279.angleDeg ?? 0) * (Math.PI / 180),
    { x: __x, y: __y, width, height } = linIn2278,
    fillBind9339 = __x + width / 2,
    fillBind9340 = __y + height / 2,
    fillBind9341 =
      Math.cos(fillBind9338) * (linIn2279.isScaled === true ? width : 1),
    fillBind9342 =
      Math.sin(fillBind9338) * (linIn2279.isScaled === true ? height : 1),
    ...
    fillBind9346 =
      linIn2279.isScaled === undefined
        ? Math.sqrt(width * width + height * height) / 2
        : (Math.abs(width * fillBind9344) + Math.abs(height * fillBind9345)) /
          2,
    fillBind9347 = fillBind9344 * fillBind9346,
    fillBind9348 = fillBind9345 * fillBind9346,
    fillBind9349 = linIn2277.createLinearGradient(
      fillBind9339 - fillBind9347,
      fillBind9340 - fillBind9348,
      fillBind9339 + fillBind9347,
      fillBind9340 + fillBind9348,
    ),
  ...
  if (fillBind9350.length === 0) {
    ...
    return (
      fillBind9349.addColorStop(0, fillBind19698),
      fillBind9349.addColorStop(1, fillBind19698),
      fillBind9349
    );
  }
  return (
    applyGradientColorStops(fillBind9349, fillBind9350, linIn2280, linIn2281),
    fillBind9349
  );
}
