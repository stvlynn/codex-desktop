...
export function normalizeGradientStopOffset(
  gradIn12004: unknown,
  gradIn12005: unknown,
) {
...
export function colorsHaveTransparency(
  gradIn14688: unknown,
  gradIn14689: unknown,
) {
...
export function blendRgba(
  ...
  let fillBind18829 =
    gradIn8326 + (gradIn8324.a - gradIn8325.a) * gradIn8326 * (1 - gradIn8326);
  return {
...
export function applyGradientColorStops(
  gradIn2267: unknown,
  gradIn2268: unknown,
  gradIn2269: unknown,
  gradIn2270: unknown,
) {
  let fillBind9332 = [...gradIn2268].sort(
      (gradIn15094, gradIn15095) =>
        (gradIn15094.position ?? 0) - (gradIn15095.position ?? 0),
    ),
    fillBind9333 = fillBind9332.every(
  ...
  fillBind9334.forEach((item, index) => {
    ...
    if (
      item.offset > fillBind13829.offset &&
      colorsHaveTransparency(fillBind13829.color, item.color)
    )
      for (
        let fillBind19089 = 1;
        fillBind19089 < GRADIENT_BLEND_STEPS;
        fillBind19089 += 1
      ) {
        let fillBind20079 = fillBind19089 / GRADIENT_BLEND_STEPS;
        gradIn2267.addColorStop(
          fillBind13829.offset +
            (item.offset - fillBind13829.offset) * fillBind20079,
          rgbaCssFromChannels(
            blendRgba(fillBind13829.color, item.color, fillBind20079),
          ),
        );
      }
    gradIn2267.addColorStop(item.offset, rgbaCssFromChannels(item.color));
