...
import { wr, Zn as colorTypeEnum, tr as fillTypeEnum, nr as gradientKindEnum, dr as patternTypeEnum } from "../presentation-protobuf";
import { ensureCssNamedColorsInit } from "../presentation-theme";
import { resolveColorToCssRgba } from "../color-resolve";
import { createLinearGradientFill, ensureLinearGradientFillInit } from "./linear-gradient-fill";
import { createRadialGradientFill, ensureRadialGradientFillInit } from "./radial-gradient-fill";
import { resolvePatternFillStyle, ensurePatternFillResolveInit } from "./pattern-fill-resolve";
import { processPictureBitmapWithOpacity, ensurePictureEffectCachesInit } from "./picture-effects";
function isAutoSystemColor(fillIn8673) {
  return fillIn8673 ? fillIn8673.type === "system" ? fillIn8673.value === "auto" && !fillIn8673.lastColor : fillIn8673.type === colorTypeEnum.COLOR_TYPE_SYSTEM && fillIn8673.value === "auto" && !fillIn8673.lastColor : false;
}
function isUnspecifiedColor(fillIn11011) {
  return fillIn11011 ? fillIn11011.type === "unspecified" || fillIn11011.type === colorTypeEnum.COLOR_TYPE_UNSPECIFIED : true;
}
function isEmptyOrTransparentFill(fillIn4070) {
  if ("isSet" in fillIn4070 && !fillIn4070.isSet) return true;
  if (fillIn4070.type === fillTypeEnum.FILL_TYPE_PICTURE || (fillIn4070.gradientStops?.length ?? 0) > 0) return false;
  let fillBind13043 = !isUnspecifiedColor(fillIn4070.color) && !isAutoSystemColor(fillIn4070.color),
    fillBind13044 = fillIn4070.pattern?.color,
    fillBind13045 = !isUnspecifiedColor(fillBind13044) && !isAutoSystemColor(fillBind13044);
  if (fillBind13043 || fillBind13045) return false;
  let fillBind13046 = fillIn4070.pattern?.patternType ?? patternTypeEnum.PATTERN_TYPE_UNSPECIFIED;
  return fillBind13046 === patternTypeEnum.PATTERN_TYPE_UNSPECIFIED || fillBind13046 === patternTypeEnum.PATTERN_TYPE_NONE || fillBind13046 === patternTypeEnum.PATTERN_TYPE_SOLID;
}
function gradientKindOf(fillIn15246) {
  return fillIn15246.gradientKind;
}
function isLinearGradientKind(fillIn9737) {
  return fillIn9737 == null || fillIn9737 === "linear" || fillIn9737 === gradientKindEnum.GRADIENT_KIND_UNSPECIFIED || fillIn9737 === gradientKindEnum.GRADIENT_KIND_LINEAR;
}
...
function cropPictureBySrcRect(fillIn2697, fillIn2698, fillIn2699, fillIn2700) {
  ...
  return fillBind10280 ? (fillBind10280.drawImage(fillBind10267, fillBind10273, fillBind10274, fillBind10275, fillBind10276, 0, 0, fillBind10277, fillBind10278), {
...
function createPictureFillPattern(fillIn2068, fillIn2069, fillIn2070, fillIn2071, fillIn2072, fillIn2073) {
  ...
  let fillBind8959 = cropPictureBySrcRect(fillIn2070, fillBind8958.bitmap, fillIn2073, fillBind8958.contentType),
  ...
  if (fillBind8961) {
    if (fillIn2072 === "cover") {
      let fillBind15378 = Math.max(fillBind8960.width / fillBind8959.width, fillBind8960.height / fillBind8959.height),
        fillBind15379 = fillBind8959.width * fillBind15378,
        fillBind15380 = fillBind8959.height * fillBind15378;
      return fillBind8961.setTransform({
        a: fillBind15378,
        b: 0,
        c: 0,
        d: fillBind15378,
...
export function resolveFillStyle(fillIn3306: unknown, fillIn3307: unknown, fillIn3308: unknown, fillIn3309: unknown, fillIn3310: unknown = "transparent") {
  let fillBind11592 = typeof fillIn3310 == "string" ? {
  ...
  if (fillIn3308.type === fillTypeEnum.FILL_TYPE_PICTURE) return createPictureFillPattern(fillIn3306, fillIn3307, fillIn3308, fillBind11592.pictureFillBitmaps, fillBind11592.pictureFillFit, fillIn3309) ?? fillBind11593;
  if (fillIn3308.type === fillTypeEnum.FILL_TYPE_GRADIENT && fillIn3308.gradientStops?.length) {
    let fillBind20337 = gradientKindOf(fillIn3308);
    if (isLinearGradientKind(fillBind20337)) return createLinearGradientFill(fillIn3306, fillIn3307, fillIn3308, fillIn3309, fillBind11592);
    if (fillBind20337 === "path" || fillBind20337 === gradientKindEnum.GRADIENT_KIND_PATH) return createRadialGradientFill(fillIn3306, fillIn3307, fillIn3308, fillIn3309, fillBind11592);
  }
  return fillIn3308.pattern ? resolvePatternFillStyle(fillIn3306, fillIn3307, fillIn3308, fillIn3309, fillBind11592) : resolveColorToCssRgba(fillIn3308.color, fillIn3309, fillBind11592);
}
