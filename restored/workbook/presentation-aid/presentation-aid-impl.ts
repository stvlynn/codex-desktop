...
export function findElementByAid(paidIn3839: any, paidIn3840: any, paidIn3841: any) {
  for (let paidBind13837 of paidIn3839.slides.items)
    for (let paidBind14431 of paidBind13837.elements.items)
      if (
        ...
        !(
          paidIn3840 === "ea" &&
          paidBind14431.type !== "embeddedArtifact"
        ) &&
...
export function resolvePresentationAid(paidIn2755: any, paidIn2756: any) {
  ...
  if (paidBind10398 === "sl")
    return findSlideById(paidIn2755, suffix);
  if (paidBind10398 === "nt")
    return findSlideById(paidIn2755, suffix).speakerNotes;
  if (paidBind10398 === "th") {
    let paidBind21362 =
      paidIn2755.comments.threads.getById(suffix);
    if (!paidBind21362)
      throw Error(`Unknown thread aid: ${paidIn2756}`);
...
export function findSlideById(paidIn9682: any, paidIn9683: any) {
  ...
  if (!paidBind20211)
    throw Error(`Unknown slide aid: sl/${paidIn9683}`);
  return paidBind20211;
}
export function resolveTextRangeAid(paidIn2676: any, paidIn2677: any) {
  let paidBind10210 = /^([a-z]+)\/([^/]+)\/(\d+)\/(\d+)$/.exec(
    paidIn2677,
  );
  if (!paidBind10210)
    throw Error(`Invalid text range aid: tr/${paidIn2677}`);
  let [
      ,
      paidBind10211,
      paidBind10212,
  ...
  if (
    !Number.isFinite(paidBind10215) ||
    !Number.isFinite(paidBind10216)
  )
  ...
  if (
    typeof paidBind10217.text != "object" ||
    paidBind10217 === null
  )
    throw Error(
      `Text range target is not text-bearing: tr/${paidIn2677}`,
    );
  let paidBind10218 = paidBind10217.text;
  if (
    !paidBind10218 ||
    typeof paidBind10218.getRange != "function"
  )
    throw Error(
      `Text range target does not support ranges: tr/${paidIn2677}`,
    );
  return paidBind10218.getRange(
    paidBind10215,
    paidBind10216,
  );
}
export function splitAid(paidIn8539: any) {
  ...
  if (
    paidBind19049 <= 0 ||
    paidBind19049 === paidIn8539.length - 1
  )
