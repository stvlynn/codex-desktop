...
export function layoutCategoryChartPlot(cplIn251: any, cplIn252: any, cplIn253: any, cplIn254: any, cplIn255: any, ) {
  ...
  if (cplIn252.type === Cn.CHART_TYPE_COMBO) {
    ...
    cplBind3308 === _presentationYn.BAR_GROUPING_PERCENT_STACKED
  ) {
    cplBind3309 = 0;
    cplBind3310 = 1;
  } else {
  ...
  if (
    cplIn252.series.some((item) => (item?.trendlines?.length ?? 0) > 0)
  ) {
    let cplBind7760 = 1 / 0,
      cplBind7761 = -1 / 0;
          ...
          if (cplBind11738?.points?.length)
            for (let cplBind20960 of cplBind11738.points)
              Number.isFinite(cplBind20960.y) &&
                ((cplBind7760 = Math.min(
                  cplBind7760,
                  cplBind20960.y,
                )),
                (cplBind7761 = Math.max(
                  cplBind7761,
                  cplBind20960.y,
                )));
        }
    ...
    Number.isFinite(cplBind7760) &&
      ...
      ((cplBind3309 = Math.min(
        cplBind3309,
        cplBind7760,
      )),
      (cplBind3310 = Math.max(
        cplBind3310,
        cplBind7761,
      )));
  }
  let cplBind3311 = cplH.bh402({
      ...
      dataMax: cplBind3310,
      includeZeroBaseline:
        cplIn252.type !== Cn.CHART_TYPE_COMBO ||
        cplBind3310 >= 0,
    }),
    cplBind3312 = cplH.bh403(
      _AppInitialMat().range([
        cplIn253.y + cplIn253.height,
        cplIn253.y,
      ]),
      cplBind3311,
      cplBind3307,
    ...
    cplBind3315 = cplBind3305
      ? 0
      : (cplBind3313?.labelBandWidth ?? 0),
    cplBind3316 =
      cplBind3314 && !cplH.bh459(cplBind3304)
        ? AXIS_LABEL_PAD_PX + cplBind3314.width
        : 0,
    cplBind3317 = cplIn253.width * cplBind3301,
    cplBind3318 = Math.min(
      cplBind3317,
      cplBind3315 + cplBind3316,
    ),
    cplBind3319 = Math.max(
      0,
      cplIn255.availableTopPaddingPx ?? 0,
    ),
  ...
  let cplBind3322 = cplIn252.xAxis,
    ...
    cplBind3325 =
      cplBind3324 && !cplH.bh459(cplBind3322)
        ...
        : 0,
    cplBind3326 = {
      x: cplIn255.preservePlotArea
        ? cplIn253.x
        : cplIn253.x + cplBind3318,
      y: cplIn255.preservePlotArea
        ? cplIn253.y
        : cplIn253.y + cplBind3320,
    ...
    cplBind3327 =
      cplIn255.maxBottomPx ??
      cplIn253.height * cplBind3302,
    cplBind3328 =
      !cplBind3323 &&
      cplBind3326.width > 0 &&
      cplBind3326.height > 0
        ? cplH.paintCategoryAxisLabels({
            ...
            themeMap: cplIn254,
            maxLabelBandHeightPx: Math.max(
              0,
              cplBind3327 - cplBind3325,
            ),
          })
        : undefined,
    ...
    cplBind3330 = {
      ...
      height: cplIn255.preservePlotArea
        ...
        : Math.max(
            0,
            cplIn253.height - cplBind3320 - cplBind3329,
          ),
    };
  return {
