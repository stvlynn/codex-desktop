...
function coerceFiniteNumber(cbwIn12510: any) {
  if (
    typeof cbwIn12510 == "number" &&
    Number.isFinite(cbwIn12510)
  )
    return cbwIn12510;
}
...
function exclusiveQuartile(cbwIn6535: any, cbwIn6536: any) {
  ...
  if (cbwBind16772 >= cbwBind16770)
    return cbwIn6535[cbwBind16770 - 1];
  let cbwBind16774 = cbwIn6535[cbwBind16772 - 1];
  return (
    cbwBind16774 +
    cbwBind16773 *
      (cbwIn6535[cbwBind16772] - cbwBind16774)
  );
}
...
function pickCategoryValues(cbwIn10295: any, cbwIn10296: any, cbwIn10297: any, cbwIn10298: any, ) {
  let cbwBind20774 = cbwIn10295?.[cbwIn10297];
  if (cbwBind20774 && cbwBind20774.length > 0)
    return cbwBind20774;
  let cbwBind20775 = cbwIn10296[cbwIn10297];
  return cbwBind20775 && cbwBind20775.length > 0
    ? cbwBind20775
    : cbwIn10298;
}
export function prepareBoxWhiskerData(cbwIn476: any, cbwIn477: any = {}) {
      ...
      .map(({ series, seriesIndex }) => {
        ...
        for (
          ...
          cbwBind4087.has(cbwBind17328) ||
            (cbwBind4087.add(cbwBind17328),
            cbwBind4086.push(cbwBind17328));
          let cbwBind17329 =
            cbwBind11620.get(cbwBind17328);
          cbwBind17329
            ? cbwBind17329.push(cbwBind17327)
            : cbwBind11620.set(cbwBind17328, [
                cbwBind17327,
              ]);
        }
        return {
  ...
  cbwBind4086.length === 0 &&
    ...
    cbwBind4085.forEach((item) => {
      cbwBind4087.has(item) ||
        (cbwBind4087.add(item), cbwBind4086.push(item));
    });
  let cbwBind4089 = [],
    cbwBind4090 = 1 / 0,
    cbwBind4091 = -1 / 0;
  return (
    cbwBind4086.forEach((item) => {
      ...
      cbwBind4088.forEach(({ seriesIndex, valuesByCategory }) => {
        ...
        let cbwBind6950 = cbwBind6949
          .slice()
          .sort(
            (cbwIn16532, cbwIn16533) =>
              cbwIn16532 - cbwIn16533,
          );
        if (cbwBind6950.length === 0) return;
        let cbwBind6951 = computeQuartile(
            cbwBind6950,
            0.25,
            cbwBind4084,
          ),
          cbwBind6952 = computeQuartile(
            cbwBind6950,
            0.5,
            cbwBind4084,
          ),
          cbwBind6953 = computeQuartile(
            cbwBind6950,
            0.75,
            cbwBind4084,
          ),
    ...
    cbwBind4090 === cbwBind4091 &&
      (--cbwBind4090, (cbwBind4091 += 1)),
    {
...
export function buildBoxWhiskerScales(cbwIn4281: any, cbwIn4282: any) {
  ...
  return {
    ...
    y: _AppInitialMat()
      ...
      .nice()
      .range([
        cbwIn4282.y + cbwIn4282.height,
        cbwIn4282.y,
      ]),
  };
}

function fillStrokeCircle(cbwIn6846: any, cbwIn6847: any, cbwIn6848: any, cbwIn6849: any, cbwIn6850: any, cbwIn6851: any, cbwIn6852: any, ) {
...
function resolveBoxSeriesStyle(cbwIn4519: any, cbwIn4520: any, cbwIn4521: any, cbwIn4522: any, ) {
  let cbwBind13773 =
    cbwIn4519.series[cbwIn4520.seriesIndex];
  if (!cbwBind13773)
    return {
      strokeWidth: 1,
      noFill: false,
    };
  let cbwBind13774 = cbwH.bh474(
      cbwBind13773,
      cbwIn4521,
    ),
    cbwBind13775 =
...
export function paintBoxWhiskerChart(cbwIn140: any, cbwIn141: any, cbwIn142: any, cbwIn143: any, cbwIn144: any, cbwIn145: any = {}, ) {
  let { chartHoverTargets } = cbwIn145,
    cbwBind2873 =
      cbwIn141.boxWhiskerOptions?.showMeanMarker ?? false,
    cbwBind2874 =
      cbwIn141.boxWhiskerOptions?.showMeanLine ?? false,
    cbwBind2875 =
      cbwIn141.boxWhiskerOptions?.showOutliers ?? true,
    cbwBind2876 =
      cbwIn141.boxWhiskerOptions?.showNonOutliers ?? false,
    cbwBind2877 = new Map(),
    cbwBind2878 = new Map();
  cbwIn143.categories.forEach((item, index) => {
    cbwIn142.x(item) !== undefined &&
      cbwBind2878.set(item, index);
  });
  cbwIn143.byCategory.forEach((item) => {
    ...
    item.series.forEach((_item) => {
      ...
      if (
        ...
        return;
      let cbwBind3167 = Math.min(
          cbwBind3162,
          cbwBind3163,
        ),
        cbwBind3168 = Math.max(
          Math.abs(cbwBind3163 - cbwBind3162),
          1,
        ),
        cbwBind3169 =
          Math.min(
            cbwBind3157 * cbwBind1220,
            cbwBind3157,
          ) / 2,
      ...
      if (
        ...
        cbwBind3170.fillColor &&
          ((cbwIn140.fillStyle = cbwBind3170.fillColor),
          cbwIn140.fill()),
    ...
    cbwBind2877.forEach((item, index) => {
      ...
      let cbwBind11455 = cbwIn141.series[index],
        cbwBind11456 = cbwBind11455
          ...
          : undefined,
        cbwBind11457 =
          cbwBind11454[0]?.color ?? cbwBind11456 ?? "#444444";
      cbwIn140.beginPath();
