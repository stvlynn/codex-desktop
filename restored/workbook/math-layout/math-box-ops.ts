...
export function hboxMathBoxes(
  mathIn2935: unknown,
  mathIn2936: unknown,
  mathIn2937: unknown = 0,
) {
  let workbookBinding10855 =
      mathIn2935.reduce(
        (accumulator, current) => accumulator + current.widthPx,
        0,
      ) +
      Math.max(0, mathIn2935.length - 1) * mathIn2937,
    workbookBinding10856 = mathIn2935.reduce(
      (accumulator, current) => Math.max(accumulator, current.ascentPx),
      0,
    ),
    workbookBinding10857 = mathIn2935.reduce(
  ...
  return {
    ...
    draw: (mathIn11316, mathIn11317, mathIn11318) => {
      ...
      for (let workbookBinding22786 of mathIn2935) {
        workbookBinding22786.draw(
          mathIn11316,
          workbookBinding21784,
          mathIn11318,
        );
...
export function hboxMathBoxesWithGaps(
  mathIn2171: unknown,
  mathIn2172: unknown,
  mathIn2173: unknown,
) {
  if (mathIn2171.length === 0) return emptyMathBox(mathIn2173);
  let workbookBinding9161 =
      mathIn2171.reduce(
        (accumulator, current) => accumulator + current.widthPx,
        0,
      ) + mathIn2172.reduce((accumulator, current) => accumulator + current, 0),
    workbookBinding9162 = mathIn2171.reduce(
      (accumulator, current) => Math.max(accumulator, current.ascentPx),
      0,
    ),
    workbookBinding9163 = mathIn2171.reduce(
      (accumulator, current) => Math.max(accumulator, current.descentPx),
      0,
    ),
  ...
  return workbookBinding9164
    ? {
        ...
        draw: (mathIn8208, mathIn8209, mathIn8210) => {
          ...
          for (
            let workbookBinding20419 = 0;
            workbookBinding20419 < mathIn2171.length;
            workbookBinding20419 += 1
          ) {
            let workbookBinding21585 = mathIn2171[workbookBinding20419];
            workbookBinding21585 &&
              (workbookBinding21585.draw(
                mathIn8208,
                workbookBinding18722,
                mathIn8210,
              ),
              (workbookBinding18722 +=
                workbookBinding21585.widthPx +
                (mathIn2172[workbookBinding20419] ?? 0)));
          }
        },
      }
    : emptyMathBox(mathIn2173);
}
export function layoutMathSequence(
  mathIn5526: unknown,
  mathIn5527: unknown,
  mathIn5528: unknown,
) {
  let workbookBinding15321 = mathIn5526.map((item) =>
      layoutMathNode(item, mathIn5527, mathIn5528),
    ),
    workbookBinding15322 = normalizeBinAtomClasses(
      mathIn5526.map((item) => measureStyledGlyphBox(item)),
    );
  return hboxMathBoxesWithGaps(
    workbookBinding15321,
    mathIn5526
      .slice(0, -1)
      .map((item, index) =>
        texAtomSpacingPx(
          item,
          mathIn5526[index + 1] ?? item,
          workbookBinding15321[index] ?? emptyMathBox(mathIn5527),
          workbookBinding15322[index] ?? "ord",
          workbookBinding15322[index + 1] ?? "ord",
          mathIn5527,
        ),
      ),
    mathIn5527,
...
export function emptyMathBox(mathIn4987: unknown) {
  let workbookBinding14404 = mathRunFontDescriptor(mathIn4987),
    workbookBinding14405 = buildMeasureFontShorthand(
      workbookBinding14404,
      mathIn4987.fontPx,
    ),
...
export function layoutMathNodeChild(
  mathIn12709: unknown,
  mathIn12710: unknown,
) {
  return {
