...
import { extractSoleMathParagraph, firstMathTokenAsTextRun, mathAstHasExplicitFontSize, mergeInlineMathDocuments, resolveMathParagraphAlign, textStyleFontSizePx } from "./detect-paragraph-math";
import { layoutMathNode } from "./layout-math-node";
import { MATH_AUTOFIT_MAX_SCALE, MATH_LAYOUT_MAX_FONT_PX, MATH_LAYOUT_MIN_FONT_PX } from "./layout-tuning";
...
export function layoutParagraphMath(mathIn540: unknown) {
  let workbookBinding4435 = extractSoleMathParagraph(mathIn540.element.paragraphs?.[0]);
  if (!workbookBinding4435) return;
  let workbookBinding4436 = normalizeMathDocumentViaMathMl(mergeInlineMathDocuments(workbookBinding4435));
  if (!workbookBinding4436?.root) return;
  let workbookBinding4437 = mergeStyleLayers({}, workbookBinding4435.textStyle, mathIn540.elementStyle),
    workbookBinding4438 = textStyleFontSizePx(workbookBinding4437, mathIn540.textScale),
    workbookBinding4439 = resolveColorToCssRgba(workbookBinding4437.fill?.color, mathIn540.themeMap, mathIn540.defaultTextFill),
    workbookBinding4440 = {
      fontPx: workbookBinding4438 ?? Math.max(MATH_LAYOUT_MIN_FONT_PX, Math.min(MATH_LAYOUT_MAX_FONT_PX, mathIn540.boxHeightPx * 0.58)),
    ...
    workbookBinding4441 = {
      ...
      displayStyle: workbookBinding4436.displayMode === undefined || workbookBinding4436.displayMode === MathDisplayMode.MATH_DISPLAY_MODE_BLOCK ? "block" : "inline"
    },
    workbookBinding4442 = layoutMathNode(workbookBinding4436.root, workbookBinding4440, workbookBinding4441),
    workbookBinding4443 = mathAstHasExplicitFontSize(workbookBinding4436.root),
    workbookBinding4444 = Math.min(mathIn540.boxWidthPx / Math.max(workbookBinding4442.widthPx, 1), mathIn540.boxHeightPx / Math.max(workbookBinding4442.ascentPx + workbookBinding4442.descentPx, 1)),
    workbookBinding4445 = workbookBinding4438 === undefined && !workbookBinding4443,
    workbookBinding4446 = Number.isFinite(workbookBinding4444) ? workbookBinding4445 ? Math.min(MATH_AUTOFIT_MAX_SCALE, workbookBinding4444 * 0.94) : Math.min(1, workbookBinding4444 * 0.96) : 1;
  workbookBinding4446 > 0 && Math.abs(workbookBinding4446 - 1) > 0.01 && (workbookBinding4442 = scaleMathBox(workbookBinding4442, workbookBinding4446));
  let workbookBinding4447 = resolveMathParagraphAlign(workbookBinding4436, workbookBinding4437),
    workbookBinding4448 = workbookBinding4435.runs?.[0] ?? firstMathTokenAsTextRun(workbookBinding4436),
    ...
    workbookBinding4451 = workbookBinding4442.ascentPx + workbookBinding4442.descentPx;
  return [{
