...
export function paintCategoryAxisLabels(props: any) {
  ...
  (calBind3836.fontSize === undefined ||
    ...
    (calBind3836.fontSize = Math.round(
      calBind3835.fontSize * 75,
    ));
  let calBind3837 =
      labelHeightPx ?? calH.bh431(ctx, calBind3835.fontSize),
    ...
    calBind3846 =
      calBind3842 && calBind3843
        ? calBind3842.map(({ serial }) =>
            Hxe(serial, calBind3843, plotDims),
          )
        : undefined,
  ...
  let calBind3848 = calBind3845.map((item) => {
    ...
    calBind3850 =
      calBind3841 === undefined ? [..._L] : [calBind3841],
    calBind3851;
  for (let calBind11970 of calBind3849)
    for (let calBind12277 of calBind3850) {
      ...
      (!calBind3851 ||
        calBind12557.overflowPx < calBind3851.overflowPx) &&
...
export function layoutCategoryAxisLabelBand(props: any) {
  let {
    ...
    calBind5027 = Gxe(
      Math.min(calBind5025, calBind5026, plotDims.width),
    ),
    calBind5028 =
      rotationDeg === 0
    ...
    calBind5030 =
      rotationDeg === 0 && calBind5028 > 1 && !calBind5029,
    calBind5031 = calBind5029
      ? plotDims.width
      : calBind5027,
    calBind5032 = 0,
    calBind5033 = 0,
...
function splitAxisLabelLines(props: any) {
  ...
  return (
    calBind13812.length < calBind13810.length &&
      (calBind13812[calBind13812.length - 1] =
        calBind13810.slice(calBind13812.length - 1).join(" ")),
    calBind13812.map((item) =>
      calH.bh427(ctx, item.trim(), maxWidthPx),
    )
  );
}
function labelsFitWithoutOverlap(props: any) {
  ...
  for (let calBind14152 of visibleIndices) {
    ...
    let calBind14419 =
      tickPositionsPx?.[calBind14152] ??
      calH.bh425(scale, calBind14418);
    if (
      calBind14419 === undefined ||
      !Number.isFinite(calBind14419)
    )
      continue;
    ...
    if (calBind14423 < calBind10974 - calH.overlapEpsilon)
      return false;
    calBind10974 = calBind14424;
  }
  return true;
}
function wrapAxisLabelLines(calIn3161: any, calIn3162: any, calIn3163: any, calIn3164: any, ) {
  ...
  for (
    ;
    calBind11305 < calBind11303.length &&
    calBind11304.length < calBind11302;
  ) {
    ...
    for (
      ...
      if (
        calIn3161.measureText(calBind21548).width >
        calIn3163
      )
        break;
