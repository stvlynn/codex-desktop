...
export function tokenizeTextRuns(textIn1102: unknown) {
  function workbookHelper1182(textIn1750) {
    ...
    for (
      let workbookBinding10459 = 0;
      workbookBinding10459 < workbookBinding8200.length;
      ...
      let workbookBinding10843 = WHITESPACE_RE.test(workbookBinding10841)
        ? "whitespace"
        : "word";
      workbookBinding10843 !== workbookBinding8203 &&
        (workbookBinding8204(), (workbookBinding8203 = workbookBinding10843));
  ...
  for (; workbookBinding6478 < textIn1102.length; ) {
    let workbookBinding20386 = textIn1102.indexOf(
        DOCX_BREAK_RENDERED_MARKER,
        workbookBinding6478,
      ),
      workbookBinding20387 =
        workbookBinding20386 >= 0 ? workbookBinding20386 : textIn1102.length;
    if (
      (workbookBinding20387 > workbookBinding6478 &&
        workbookBinding6477.push(
          ...workbookHelper1182(
            textIn1102.slice(workbookBinding6478, workbookBinding20387),
          ),
        ),
      workbookBinding20386 < 0)
    )
      break;
    workbookBinding6478 = workbookBinding20386 + 22;
  }
  return (
    tokenizeCache.size >= TOKENIZE_CACHE_LIMIT && tokenizeCache.clear(),
    tokenizeCache.set(textIn1102, workbookBinding6477),
    workbookBinding6477
  );
}
