...
export function paintHistogramBars(chbIn746: any, chbIn747: any, chbIn748: any, chbIn749: any, chbIn750: any, chbIn751: any, ) {
  ...
  chbIn749.bins.forEach((item, index) => {
    ...
    if (
      !Number.isFinite(chbBind5555) ||
      !Number.isFinite(chbBind5556)
    )
      return;
    let chbBind5557 = Math.min(
        chbBind5555,
        chbBind5556,
      ),
      chbBind5558 = Math.max(
        0,
        Math.max(chbBind5555, chbBind5556) -
          chbBind5557,
      );
    if (chbBind5558 <= 0) return;
    let chbBind5559 = y(item.count);
    if (
      !Number.isFinite(chbBind5559) ||
      !Number.isFinite(chbBind5231)
    )
      return;
    let chbBind5560 = Math.max(
      0,
      chbBind5231 - chbBind5559,
    );
    if (chbBind5560 <= 0) return;
    let chbBind5561 = chbH.bh474(chbBind5230, index),
      chbBind5562 = chbH.bh469(
        chbBind5230,
        index,
        chbBind5229,
        chbIn750,
      ),
      chbBind5563 = chbH.bh473(
        chbBind5230,
        index,
        chbIn750,
      ),
    ...
    let chbBind5566 = chbBind5558 > 1,
      chbBind5567 = chbBind5566
        ? chbBind5557 + 0.5
        : chbBind5557,
      chbBind5568 = chbBind5566
        ? Math.max(0, chbBind5558 - 1)
        : chbBind5558;
    chbIn746.beginPath();
    chbIn746.rect(
      chbBind5567,
    ...
    !chbBind5561 &&
      chbBind5569 &&
      ((chbIn746.fillStyle = chbBind5569),
      chbIn746.fill());
    chbIn746.lineWidth > 0 &&
      chbBind5564 &&
      chbIn746.stroke();
