...
export function workbookHelper535(csscIn6424: any, csscIn6425: any) {
  if (
    ...
    (csscIn6424 === undefined ||
      (csscIn6424.min === undefined && csscIn6424.max === undefined) ||
...
export function workbookHelper537(csscIn7080: any, csscIn7081: any) {
  if (
    ...
    return;
  let csscBind17419 = Math.ceil(csscIn7081 / 0.1) * 0.1;
  if (Number(csscBind17419.toPrecision(15)) === 1 - 0.1) return 1;
}
export function workbookHelper538([csscIn7621, csscIn7622]: [any, any]) {
  if (!Number.isFinite(csscIn7621) || !Number.isFinite(csscIn7622))
    return [0, 1];
  if (csscIn7621 === csscIn7622) {
    if (csscIn7621 === 0) return [-1, 1];
    let csscBind21793 = Math.abs(csscIn7621) * 0.1 || 1;
    return [csscIn7621 - csscBind21793, csscIn7622 + csscBind21793];
  }
