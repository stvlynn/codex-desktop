...
export function autofitRangeColumns(host: any) {
  ...
  if (!(!rfBind8926 || !rfBind8927)) {
    if (rfBind8926.recalculate(), autofitColumnWidths({
      address: host.getAddress(),
      bounds: rfBind8925,
      workbook: rfBind8926,
      worksheet: rfBind8927
    }), rfBind8924) for (let rfBind22140 = 0; rfBind22140 < rfBind8924.cols; rfBind22140 += 1) rfBind8927.__syncColumnSizeRef(rfBind8924.startCol + rfBind22140);
...
export function autofitRangeRows(host: any) {
  ...
  if (!(!rfBind8964 || !rfBind8965)) {
    if (rfBind8964.recalculate(), autofitRowHeights({
      address: host.getAddress(),
      bounds: rfBind8963,
      workbook: rfBind8964,
      worksheet: rfBind8965
    }), rfBind8962) for (let rfBind22223 = 0; rfBind22223 < rfBind8962.rows; rfBind22223 += 1) rfBind8965.__syncRowSizeRef(rfBind8962.startRow + rfBind22223);
