...
import { columnWidthPx, pointsToPixels, pixelsToPoints, pxToCharCount } from "../../utils/spreadsheet-address-utils";
import { SpreadsheetFont, Borders } from "../stylesheet";
import { horizontalAlignment } from "./horizontal-alignment";
import { cloneFillFromValue, cloneFontOrUndefined, cloneBordersOrUndefined, borderSideConfigFromSide, fillFromConfigOrUndefined } from "./format-clone-helpers";
import { measure2dArrayShape, uniform2dArrayFirstValue } from "./array-shape";
import { cloneWorkbookColor, workbookColorToConfig, fillConfigFromFillOrValue, borderSideFromPartial, borderSideSnapshot, parseHorizontalAlignment, requireHorizontalAlignment, horizontalAlignmentToName, parseVerticalAlignment, requireVerticalAlignment, verticalAlignmentToCss } from "./border-alignment-helpers";
...
export class RangeFormat {
  ...
  set font(rfIn7562) {
    if (!(rfIn7562 instanceof RangeFormatFont)) {
      if (!rfIn7562 || typeof rfIn7562 != "object") throw Error("RangeFormat.font setter requires a config object.");
      this.setFont(rfIn7562);
    }
  }
  setFont(rfIn2064) {
    if (!rfIn2064 || typeof rfIn2064 != "object") throw Error("RangeFormat.setFont(patch) requires a config object.");
    let rfBind8934 = {},
    ...
    this.#m(rfIn5987 => {
      ...
      rfIn2064.color !== undefined && (rfIn5987.color = cloneWorkbookColor(rfIn2064.color));
    });
    ...
    Object.keys(rfBind8934).length > 0 && this.#w({
      font: rfBind8934
    });
  }
  get borders() {
  ...
  set borders(rfIn4513) {
    if (!rfIn4513) throw Error("RangeFormat.borders setter requires a config object.");
    if ("preset" in rfIn4513) {
      ...
      style !== undefined || color !== undefined ? this.#i.apply({
        preset,
        style,
        color
  ...
  get fill() {
    ...
    return rfBind21325 ? new RangeFormatFill(rfBind21325, rfIn16409 => this.#o(rfIn16409)) : undefined;
  }
  set fill(rfIn15221) {
    this.#o(rfIn15221);
  }
  ...
  #a(rfIn1727) {
    ...
    if (this.#S(() => {
      this.#e.editCells(true, rfIn3110 => {
        ...
        let rfBind11212 = rfIn3110.cell.styleIndex ?? this.#e.getLogicalStyleIndex(rfIn3110.row, rfIn3110.col) ?? 0,
        ...
        rfBind8093 || (rfBind8093 = this.#g(rfBind11214), rfBind8093.styleIndex = styleIndex);
      });
    }), rfBind8093) {
      this.#t = rfBind8093;
      return;
  ...
  set numberFormat(rfIn350) {
    if (rfIn350 === undefined || typeof rfIn350 == "string") {
      ...
      rfIn350 !== undefined && this.#w({
        numberFormat: rfIn350
      });
      return;
    }
    if (!Array.isArray(rfIn350)) throw Error("RangeFormat.numberFormat expects a format code string or a 2D array of strings.");
    let rfBind3673 = this.#e.getBounds();
    if (!rfBind3673) throw Error("RangeFormat.numberFormat 2D array assignment requires a range with bounds.");
    let rfBind3674 = measure2dArrayShape(rfIn350);
    if (rfBind3674.ragged) {
      let rfBind17881 = this.#e.getAddress() || "range";
      throw Error(`RangeFormat.numberFormat expects a rectangular 2D array for ${rfBind17881} (ragged rows: ${rfBind3674.colCounts.join(", ")}).`);
    }
    let rfBind3675 = rfIn350,
      rfBind3676 = rfBind3674.rows === rfBind3673.rows && rfBind3674.cols === rfBind3673.cols && !rfBind3674.ragged,
      rfBind3677 = rfBind3674.rows === rfBind3673.rows && rfBind3674.cols === 1 && rfBind3673.cols >= 1,
      rfBind3678 = rfBind3674.rows === 1 && rfBind3674.cols === rfBind3673.cols && rfBind3673.rows >= 1,
      rfBind3679 = rfBind3674.rows === 1 && rfBind3674.cols === 1;
    if (!rfBind3676) {
      if (rfBind3679) {
        let rfBind16359 = rfBind3675[0]?.[0];
        if (typeof rfBind16359 != "string") throw Error("RangeFormat.numberFormat 2D array expects string values for every cell.");
        this.numberFormat = rfBind16359;
        return;
      }
      if (rfBind3677) rfBind3675 = Array.from({
      ...
      });else if (rfBind3678) rfBind3675 = Array.from({
        length: rfBind3673.rows
      }, () => [...(rfBind3675[0] ?? [])]);else {
        if (!(rfBind3674.rows > 0 && rfBind3674.cols > 0 && rfBind3674.rows <= rfBind3673.rows && rfBind3674.cols <= rfBind3673.cols)) {
          let rfBind15826 = this.#e.getAddress() || "range";
          throw Error(`RangeFormat.numberFormat expects a ${rfBind3673.rows}x${rfBind3673.cols} matrix for ${rfBind15826}, got ${rfBind3674.rows}x${rfBind3674.cols}. You can also pass a 1xN or Nx1 matrix to broadcast across rows/columns.`);
        }
        ...
        console.warn(`RangeFormat.numberFormat expanded a ${rfBind3674.rows}x${rfBind3674.cols} matrix to ${rfBind3673.rows}x${rfBind3673.cols} for ${rfBind6623} (${rfBind6626}).`);
        rfBind3675 = Array.from({
          length: rfBind3673.rows
        }, (rfIn6020, rfIn6021) => {
          let rfBind16060 = rfBind6624 ? rfIn6021 % rfBind3674.rows : Math.min(rfIn6021, rfBind3674.rows - 1);
          return Array.from({
            length: rfBind3673.cols
          }, (rfIn9879, rfIn9880) => {
            let rfBind20427 = rfBind6625 ? rfIn9880 % rfBind3674.cols : Math.min(rfIn9880, rfBind3674.cols - 1);
  ...
  #s(rfIn1108) {
    ...
    if (this.#S(() => {
      this.#e.editCells(true, rfIn1921 => {
        ...
        let rfBind8633 = rfIn1108[rfIn1921.relativeRow]?.[rfIn1921.relativeCol];
        if (typeof rfBind8633 != "string") throw Error("RangeFormat.numberFormat 2D array expects string values for every cell.");
        let rfBind8634 = this.#e.getLogicalStyleIndex(rfIn1921.row, rfIn1921.col) ?? rfIn1921.cell.styleIndex ?? 0,
        ...
        rfBind6486 || (rfBind6486 = this.#g(rfBind8636), rfBind6486.styleIndex = styleIndex);
      });
    }), rfBind6486) {
      this.#t = rfBind6486;
      return;
  ...
  get horizontalAlignment() {
    return horizontalAlignmentToName(this.#f().horizontalAlignment ?? horizontalAlignment.general);
  }
  ...
  set rowHeight(rfIn11321) {
    this.#l(rfIn11321);
    Number.isFinite(rfIn11321) && this.#w({
      rowHeight: rfIn11321
    });
  }
  get rowHeightPx() {
  ...
  set rowHeightPx(rfIn7666) {
    if (!Number.isFinite(rfIn7666) || rfIn7666 < 0) throw Error("RangeFormat.rowHeightPx must be a non-negative number.");
  ...
  set columnWidth(rfIn11153) {
    this.#d(rfIn11153);
    Number.isFinite(rfIn11153) && this.#w({
      columnWidth: rfIn11153
    });
  }
  get columnWidthPx() {
    let rfBind20981 = this.columnWidth;
    return rfBind20981 === undefined ? undefined : columnWidthPx(rfBind20981, this.#E());
  }
  set columnWidthPx(rfIn7296) {
    if (!Number.isFinite(rfIn7296) || rfIn7296 < 0) throw Error("RangeFormat.columnWidthPx must be a non-negative number.");
    this.columnWidth = pxToCharCount(rfIn7296, this.#E());
  }
  get styleId() {
    let rfBind17584 = this.#e.getFirstCell(false);
    if (rfBind17584) {
      let rfBind21379 = this.#e.getLogicalStyleIndex(rfBind17584.row, rfBind17584.col);
  ...
  #c() {
    ...
    let rfBind11035,
      rfBind11036 = false;
    for (let rfBind13966 = 0; rfBind13966 < rfBind11033.rows; rfBind13966 += 1) {
      let rfBind14684 = rfBind11034.__getRow(rfBind11033.startRow + rfBind13966),
        rfBind14685 = rfBind14684 && (rfBind14684.customHeight || rfBind14684.height !== 0) ? rfBind14684.height : undefined;
  ...
  #l(rfIn3366) {
    if (!Number.isFinite(rfIn3366) || rfIn3366 < 0) throw Error("RangeFormat.rowHeight must be a non-negative number.");
    let rfBind11701 = this.#e.getBounds(),
      rfBind11702 = this.#e.getWorksheet();
    if (!(!rfBind11701 || !rfBind11702)) {
      for (let rfBind17823 = 0; rfBind17823 < rfBind11701.rows; rfBind17823 += 1) {
        let rfBind18855 = rfBind11702.__getOrCreateRow(rfBind11701.startRow + rfBind17823);
    ...
    for (let rfBind11703 = 0; rfBind11703 < rfBind9351.cols; rfBind11703 += 1) {
      ...
      for (let rfBind18451 of rfBind9353) {
        let rfBind18984 = Math.max(0, (rfBind18451.min ?? 1) - 1),
          rfBind18985 = Math.max(rfBind18984, (rfBind18451.max ?? rfBind18451.min ?? 1) - 1);
        rfBind12175 >= rfBind18984 && rfBind12175 <= rfBind18985 && (rfBind12176 = rfBind18451.hidden ? 0 : rfBind18451.width);
      }
  ...
  #d(rfIn1905) {
    if (!Number.isFinite(rfIn1905) || rfIn1905 < 0) throw Error("RangeFormat.columnWidth must be a non-negative number.");
    ...
    for (let rfBind13602 = 0; rfBind13602 < rfBind8583.cols; rfBind13602 += 1) {
      let rfBind14283 = rfBind8583.startCol + rfBind13602 + 1,
        rfBind14284 = rfBind8585.find(item => item.min === rfBind14283 && (item.max ?? item.min) === rfBind14283);
      if (rfBind14284) {
    ...
    for (let rfBind22256 = 0; rfBind22256 < rfBind8583.cols; rfBind22256 += 1) rfBind8584.__syncColumnSizeRef(rfBind8583.startCol + rfBind22256);
  ...
  applyBorderBlueprint(rfIn1513) {
    ...
    this.#S(() => {
      this.#e.editCells(true, rfIn2136 => {
        ...
        let rfBind9078 = this.#e.getLogicalStyleIndex(rfIn2136.row, rfIn2136.col) ?? rfIn2136.cell.styleIndex ?? 0,
          rfBind9079 = rfBind7557.describe(rfBind9078),
          rfBind9080 = this.#p(rfBind9079);
        rfBind9080.border = this.#y(rfIn1513, rfBind9080.border, rfIn2136.relativeRow, rfIn2136.relativeCol, rfBind7556.rows, rfBind7556.cols);
        let {
          styleId,
          styleIndex
        } = this.#x(rfBind9080);
        ...
        rfIn2136.relativeRow === 0 && rfIn2136.relativeCol === 0 && (this.#t = rfBind9080, this.#t.styleIndex = styleIndex);
      });
    });
  ...
  #p(rfIn2949) {
    return {
      ...
      horizontalAlignment: rfIn2949.horizontalAlignment ? parseHorizontalAlignment(rfIn2949.horizontalAlignment) : undefined,
      verticalAlignment: rfIn2949.verticalAlignment ? parseVerticalAlignment(rfIn2949.verticalAlignment) : undefined,
  ...
  #h(rfIn1874) {
    ...
    if (this.#S(() => {
      this.#e.editCells(true, rfIn3152 => {
        ...
        let rfBind11277 = this.#e.getLogicalStyleIndex(rfIn3152.row, rfIn3152.col) ?? rfIn3152.cell.styleIndex ?? 0,
        ...
        rfBind8498 || (rfBind8498 = this.#g(rfBind11279), rfBind8498.styleIndex = styleIndex);
      });
    }), rfBind8498) {
      this.#t = rfBind8498;
      return;
  ...
  #_() {
    ...
    return rfBind13174 && (rfBind13173.top = borderSideSnapshot(rfBind13174.top), rfBind13173.bottom = borderSideSnapshot(rfBind13174.bottom), rfBind13173.left = borderSideSnapshot(rfBind13174.left), rfBind13173.right = borderSideSnapshot(rfBind13174.right), rfBind13173.diagonal = borderSideSnapshot(rfBind13174.diagonal), rfBind13173.diagonalUp = rfBind13174.diagonalUp, rfBind13173.diagonalDown = rfBind13174.diagonalDown), rfBind13173;
  }
  #v(rfIn3319) {
    if (!rfIn3319) return;
    let rfBind11617 = {};
    return rfBind11617.top = borderSideConfigFromSide(rfIn3319.top), rfBind11617.bottom = borderSideConfigFromSide(rfIn3319.bottom), rfBind11617.left = borderSideConfigFromSide(rfIn3319.left), rfBind11617.right = borderSideConfigFromSide(rfIn3319.right), rfBind11617.insideHorizontal = borderSideConfigFromSide(rfIn3319.insideHorizontal), rfBind11617.insideVertical = borderSideConfigFromSide(rfIn3319.insideVertical), rfBind11617.diagonal = borderSideConfigFromSide(rfIn3319.diagonal), rfIn3319.diagonalUp !== undefined && (rfBind11617.diagonalUp = rfIn3319.diagonalUp), rfIn3319.diagonalDown !== undefined && (rfBind11617.diagonalDown = rfIn3319.diagonalDown), rfBind11617;
  }
  #y(rfIn1518, rfIn1519, rfIn1520, rfIn1521, rfIn1522, rfIn1523) {
    ...
    return rfIn1518.top && rfBind7568 && (rfBind7567.top = borderSideFromPartial(rfIn1518.top)), rfIn1518.bottom && rfBind7569 && (rfBind7567.bottom = borderSideFromPartial(rfIn1518.bottom)), rfIn1518.left && rfBind7570 && (rfBind7567.left = borderSideFromPartial(rfIn1518.left)), rfIn1518.right && rfBind7571 && (rfBind7567.right = borderSideFromPartial(rfIn1518.right)), rfIn1518.insideHorizontal && (rfIn1520 > 0 && (rfBind7567.top = borderSideFromPartial(rfIn1518.insideHorizontal)), rfIn1520 < rfIn1522 - 1 && (rfBind7567.bottom = borderSideFromPartial(rfIn1518.insideHorizontal))), rfIn1518.insideVertical && (rfIn1521 > 0 && (rfBind7567.left = borderSideFromPartial(rfIn1518.insideVertical)), rfIn1521 < rfIn1523 - 1 && (rfBind7567.right = borderSideFromPartial(rfIn1518.insideVertical))), rfIn1518.diagonal ? (rfBind7567.diagonal = borderSideFromPartial(rfIn1518.diagonal), rfBind7567.diagonalUp = rfIn1518.diagonalUp, rfBind7567.diagonalDown = rfIn1518.diagonalDown) : (rfBind7567.diagonalUp = rfIn1518.diagonalUp ?? rfBind7567.diagonalUp, rfBind7567.diagonalDown = rfIn1518.diagonalDown ?? rfBind7567.diagonalDown), rfBind7567.hasValues() ? rfBind7567 : undefined;
  }
  #b(rfIn2948) {
    return {
      ...
      horizontalAlignment: rfIn2948.horizontalAlignment ? horizontalAlignmentToName(rfIn2948.horizontalAlignment) : undefined,
      verticalAlignment: rfIn2948.verticalAlignment ? verticalAlignmentToCss(rfIn2948.verticalAlignment) : undefined,
  ...
  #x(rfIn8055) {
    ...
    return rfBind18568 ? rfBind18568.__registerCollaborativeStyle(rfBind18567) : {
      styleIndex: this.#T().register(rfBind18567)
    };
  }
  #S(rfIn10768) {
    let rfBind21291 = this.#e.getWorkbook();
    return rfBind21291 ? rfBind21291.runLocalCollabTransaction(rfIn10768) : rfIn10768();
  }
  ...
  #w(rfIn4233) {
    ...
    if (!rfBind13291 || !rfBind13292 || !rfBind13292.name || !rfBind13293) return;
  ...
  #E() {
    return this.#e.getWorkbook()?.getSpreadsheetRenderAssets().defaultFontMaxDigitWidthPx;
  }
}
