...
export function measure2dArrayShape(rfIn4780: unknown) {
  ...
  for (let rfBind19247 = 0; rfBind19247 < rfIn4780.length; rfBind19247 += 1) {
    ...
    if (!Array.isArray(rfBind20426))
      throw Error(
        `Expected a 2D array; row ${rfBind19247 + 1} is not an array.`,
      );
...
export function uniform2dArrayFirstValue(props: unknown) {
  ...
  for (let rfBind20166 = 0; rfBind20166 < props.length; rfBind20166 += 1) {
    ...
    if (rfBind21352) {
      for (
        let rfBind22614 = 0;
        rfBind22614 < rfBind21352.length;
        rfBind22614 += 1
      )
        if (rfBind21352[rfBind22614] !== rfBind17336) return null;
    }
  }
  return rfBind17336;
}
