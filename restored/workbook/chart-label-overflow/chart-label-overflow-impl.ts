...
export function categoryLabelOverflowPx(props: any) {
  ...
  for (let cloBind19685 of visibleIndices) {
    ...
    if (
      !cloH.isNonEmptyCategory(cloBind20177) ||
      /\s/.test(cloBind20177)
    )
      continue;
    let cloBind20178 = ctx.measureText(cloBind20177).width;
    Number.isFinite(cloBind20178) &&
      (cloBind13546 = Math.max(
        cloBind13546,
        cloBind20178 - labelMaxWidthPx,
      ));
  }
  return Math.max(0, cloBind13546 - LABEL_OVERFLOW_PAD);
}
export function minCategoryLabelGapPx(cloIn5912: any, cloIn5913: any, cloIn5914: any, cloIn5915: any, ) {
  ...
  for (let cloBind18916 of cloIn5914) {
    ...
    let cloBind19463 =
      ...
      cloH.measureCategoryAdvance(
        cloIn5912,
        cloIn5913[cloBind18916],
      );
    cloBind19463 === undefined ||
      !Number.isFinite(cloBind19463) ||
      (cloBind15910 !== undefined &&
        (cloBind15909 = Math.min(
          cloBind15909,
          cloBind19463 - cloBind15910,
        )),
