...
import { initAddressUtils, parseA1Range } from "../../utils/spreadsheet-address-utils";
import { ensureFormulaArrayInit as $u, attachRangeMetadata } from "../formula-array";
      ...
      rpt = class {
        ...
        resolveArea(tarIn6788, tarIn6789) {
          return qft(tarIn6788, {
            ...
            findTableByName: (tarIn15881) =>
              this.findTableByName(tarIn15881),
            findContainingTable: (tarIn16451) =>
              this.#t(tarIn16451),
          });
        }
        ...
        expand(tarIn10657, tarIn10658) {
          let tarBind21157 = this.resolveArea(
            tarIn10657,
            tarIn10658,
          );
          return tarBind21157.kind === "Error"
            ? []
            : Yft(tarBind21157.area);
        }
        #t(tarIn3044) {
          let tarBind11098 = tarIn3044.sheet?.sheetName?.trim();
          if (!tarBind11098) return null;
          let tarBind11099 =
            this.#e.getWorksheetByName(tarBind11098);
          if (!tarBind11099) return null;
          for (let tarBind14134 of tarBind11099.__getTables() ??
            []) {
            let tarBind14820 = parseA1Range(
              tarBind14134.ref ?? "",
            );
        ...
        #n(tarIn9784, tarIn9785) {
          if (!tarIn9785) return;
          let tarBind20326 =
            tarIn9785.sheet?.sheetName ?? tarIn9784;
          if (tarBind20326)
        ...
        #r(tarIn5313, tarIn5314) {
          ...
          for (let tarBind17576 of tarBind14957) {
            ...
            for (let tarBind20248 of tarBind17576) {
              let tarBind20756 =
                tarIn5314.cellAddressToKey(tarBind20248);
              tarBind18067.push(
                tarBind20756
          ...
          return (
            attachRangeMetadata(tarBind14958, tarBind14957),
            tarBind14958
          );
        }
      };
