...
import type { AutoFitOuterSize, TextElementAutofitProps } from "./types";

/** Legacy `Bue` — outer size needed to fit text at textScale=1. */
export function computeAutoFitOuterSize(
  ...
  let metrics = measureTextElementMetrics({
    ...
    contentPlusPadWidth =
      metrics.maxLineWidthPx + metrics.paddingPx.left + metrics.paddingPx.right,
    contentPlusPadHeight =
      metrics.textHeightPx + metrics.paddingPx.top + metrics.paddingPx.bottom;
  return {
    width:
      widthRatio > 0 ? contentPlusPadWidth / widthRatio : contentPlusPadWidth,
    height:
      heightRatio > 0
...
export function computeAutoFitOuterHeight(
  props: TextElementAutofitProps,
): number {
