...
// Careful split export-budget 3/6
...
export const useAvatarOverlaySelectionH = rolldownRuntimeN(() => {
  copper = 1600;
  delta = 160;
  echo = 8;
  falcon = 320;
});
export function useAvatarOverlaySelectionU(alpha: any, bravo: any) {
  return alpha
    .map((item) =>
      [
        item.id,
        ...
        item.waitingRequest == null
          ? ""
          : avatarOverlayPillDismissButtonF(item.waitingRequest, bravo),
      ].join(""),
    )
    .join("\0");
}
