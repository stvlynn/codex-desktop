...
function bravo(harbor) {
  let { selectedAvatar, selectedAvatarId } = harbor,
    ...
    { mascotWidthPx } = useAvatarOverlayMascotSize(),
    { data = [], refetch } = recentConversationsMetaQueryKey(),
    umbra = {
      taskFilter: "current",
      limit: 20,
    };
  let { data: _data = [], refetch: _refetch } = composerScope(umbra),
    violet = AppInitialEY(),
    willow = AppInitialAY(),
      ...
      if (ember.localConversationId != null && flint != null) {
        ...
        bb16: switch (flint.intent) {
          case "command-approval":
            if (
              flint.commandDecision != null &&
              (garnet?.kind === "exec" || garnet?.kind === "network")
            ) {
          ...
          case "permission-response":
            if (
              flint.permissionResponse != null &&
              garnet?.kind === "permission"
            ) {
              canonicalizeWorkspacePathKey(
          ...
          case "plan-start":
            if (
              garnet?.kind === "plan" &&
              flint.planStartCollaborationMode != null
            ) {
              canonicalizeWorkspacePathKey(
                ...
                .then(async () =>
                  canonicalizeWorkspacePathKey("send-follow-up-message", {
                    ...
                    serviceTier: await resolveStableWorktreeStatus(
                      indigo,
                      indigo.get(DeferredUi2, hazel) ?? "local",
                      flint.planStartCollaborationMode?.settings.model ?? null,
                    ),
                  }),
                )
                .then(() => {
  ...
  let yellow = _e,
    zinc = (ivory, jasper) => {
      ...
      ivory.localConversationId == null ||
        kelp?.kind !== "question" ||
          ...
          switch (nova.type) {
            case "app-server-conversation":
              await canonicalizeWorkspacePathKey("send-follow-up-message", {
                ...
                serviceTier: await resolveStableWorktreeStatus(
                  indigo,
                  indigo.get(DeferredUi2, nova.conversationId) ?? "local",
                  null,
                ),
              });
  ...
  let basalt = amber,
    cedar = (prism) =>
      prism == null ? undefined : indigo.get(DeferredUiN2, prism)?.items;
