...
import { ensureComposerEsm_Hlt_Init, ensureComposerEsm_K9_Init, ensureComposerEsm_MT_Init, ensureComposerEsm_Sut_Init, ensureComposerEsm_Tft_Init, ensureComposerEsm_Utt_Init } from "../../composer/composer-esm-inits";
...
function echo({
  ...
  let flint = useIntl(),
    garnet = avatarOverlaySelectionUpperS(notification) ? notification.localConversationId : null,
    hazel = CodexBrowserSurfaceActionType(DeferredUiN2, garnet),
    ivory = hazel == null ? null : AvatarOverlayPillDismissButtonLabel(hazel.items, flint),
    jasper = AvatarOverlayPillDismissButtonMark(notification),
    kelp = flint.formatMessage(jasper.labelMessage),
    lotus = ivory ?? notification.body ?? flint.formatMessage(jasper.fallbackBodyMessage),
    ...
    quill = notification.action == null ? `${notification.title}. ${prism}` : `${notification.title}. ${prism}. ${flint.formatMessage(violet.openNotification)}`,
    reef = notification.action != null,
    sage = notification.kind !== "activity" && onDismissNotification != null,
    topaz = notification.notificationPreferenceId != null && onOpenNotificationActions != null,
    ...
    north = timber.useCallback(vale => {
      if (lunar.current?.(), lunar.current = undefined, vale == null) return;
      let wave = () => {
  ...
  let orbit = frost > moss + basalt || honey,
    ...
    quest = notification.controlTarget?.type === "app-server-conversation" && onSubmitNotificationReply != null,
    ridge = zephyr.trim(),
    ...
    tide = async event => {
      if (event.preventDefault(), event.stopPropagation(), !(!quest || ridge.length === 0 || wheat)) {
        yarn(true);
        coral(null);
        try {
    ...
    unity = fern => {
      ...
      jewel.current !== grove && (jewel.current = grove, onReplyCaretPointChange(AvatarOverlayPillDismissButtonAction(fern)));
    };
  return <ensureIntlFormattersInit.div animate={{
    opacity: 1,
    y: 0
  }} initial={{
    opacity: 0,
    y: prefersReducedMotion ? 0 : 4
  }} role="listitem" className="group no-drag relative w-full min-w-0 snap-start scroll-mt-2 text-left" data-avatar-overlay-measure="notification-tray-row" transition={prefersReducedMotion ? {
    duration: 0
  } : {
    ...
    hill instanceof Node && event.currentTarget.contains(hill) || eagle(false);
  }} onFocusCapture={() => {
    eagle(true);
  }} onPointerEnter={() => {
    eagle(true);
    ...
    !topaz || isle instanceof HTMLElement && (isle.isContentEditable || isle.closest("input, textarea, select, [contenteditable='true']") != null) || (event.preventDefault(), event.stopPropagation(), onOpenNotificationActions(notification));
  }}><div className={IntlProvider("relative z-[1] overflow-hidden rounded-[18px] forced-colors:bg-[Canvas]", avatarOverlayPillDismissButtonValue.cssMaterial)}>{<ensureIntlFormattersInit.div role={reef || topaz ? "button" : undefined} className={IntlProvider("block w-full min-w-0 py-1.5 pr-3 text-left focus-visible:outline-token-focus focus-visible:outline focus-visible:outline-offset-[-2px]", sage ? "pl-5" : "pl-3", reef ? "cursor-interaction" : "cursor-default")} tabIndex={reef || topaz ? 0 : undefined} transition={prefersReducedMotion ? {
        duration: 0
      } : {
        duration: 0.12,
        ease: "easeOut"
      }} whileTap={reef && !prefersReducedMotion ? {
        scale: 0.995
      } : undefined} aria-label={reef ? quill : topaz ? flint.formatMessage(subAgentFromThreadSource.notificationActions, {
        title: notification.title
      }) : undefined} onClick={() => {
        reef && onRunNotificationAction?.(notification);
      }} onKeyDown={event => {
        if (!(!reef && !topaz || event.key !== "Enter" && event.key !== " ")) {
      ...
      }}><span className={IntlProvider("flex min-w-0 items-center", (!notification.isLoading || orbit) && "pr-7")}><span className={IntlProvider("text-size-chat min-w-0 truncate leading-[17px]", notification.kind === "activity" ? "text-token-text-secondary" : "font-semibold text-token-foreground")}>{notification.title}</span></span><ensureIntlFormattersInit.div ref={north} animate={{
          maxHeight: pine ? amber : moss
        }} className={IntlProvider("text-size-chat-sm mt-0.5 overflow-hidden leading-4 text-token-foreground", notification.kind === "activity" && "hidden", pine && "whitespace-pre-wrap", !pine && mint == null && notification.isLoading && "truncate", !pine && mint == null && !notification.isLoading && "line-clamp-2")} data-avatar-overlay-measure-body="true" initial={false} transition={prefersReducedMotion ? {
          duration: 0
        } : {
          duration: 0.18,
          ease: "easeOut"
        }}>{mint == null ? <span className={IntlProvider(notification.isLoading && "loading-shimmer-pure-text", notification.isLoading && !pine && "max-w-full truncate align-bottom")}>{lotus}</span> : <AvatarOverlayPageHelper13 isExpanded={pine} localConversationId={garnet} request={mint} onRunNotificationAction={juniper => {
            onRunNotificationAction?.(notification, juniper);
          }} onSubmitQuestionOption={lagoon => {
            onSubmitQuestionOption?.(notification, lagoon);
          }} />}</ensureIntlFormattersInit.div></ensureIntlFormattersInit.div>}<span role="img" aria-label={mint?.kind === "question" ? flint.formatMessage(violet.questionStatusIcon) : kelp} className={IntlProvider("pointer-events-none absolute top-1 right-1 z-0 flex size-6 items-center justify-center opacity-100", orbit && drift && "opacity-0 transition-opacity duration-basic motion-reduce:transition-none")}>{mint?.kind === "question" ? <AppInitialXct className={jasper.iconClassName} /> : river(jasper)}</span>{orbit ? <ensureIntlFormattersInit.div animate={{
        opacity: +!!drift,
        x: drift ? 0 : 6
      }} className={IntlProvider("absolute top-1 right-1 z-10", drift ? "pointer-events-auto" : "pointer-events-none")} data-avatar-overlay-control="expand" initial={false} transition={storm}><OptionalTooltip align="end" side="top" tooltipContent={flint.formatMessage(ultra ? violet.collapseNotificationTooltip : violet.expandNotificationTooltip)}>{<ReadLoginRouteQuerySnapshot className={IntlProvider("size-6", daisy, avatarOverlayPillDismissButtonValue.cssControl)} color="ghost" size="icon" aria-expanded={ultra} aria-label={flint.formatMessage(ultra ? violet.collapseNotification : violet.expandNotification, {
            title: notification.title
          })} onClick={() => {
            vapor(meadow => !meadow);
              ...
              duration: 0.12,
              ease: "easeOut"
            }}><AppIconYlt className="icon-xs" /></ensureIntlFormattersInit.span></ReadLoginRouteQuerySnapshot>}</OptionalTooltip></ensureIntlFormattersInit.div> : null}{quest && !isReplying ? <ensureIntlFormattersInit.div animate={{
        opacity: +!!drift,
        x: drift ? 0 : 6
      }} className={IntlProvider("no-drag absolute right-2 bottom-1 z-10", drift ? "pointer-events-auto" : "pointer-events-none")} data-avatar-overlay-control="reply" initial={false} transition={storm}><div className="flex justify-end pb-1">{<ReadLoginRouteQuerySnapshot className={IntlProvider("h-5 px-2 text-xs leading-none text-token-foreground shadow-[0px_5px_10px_-7px_rgba(0,0,0,0.22)]", daisy, avatarOverlayPillDismissButtonValue.cssControl)} color="outline" size="default" aria-label={flint.formatMessage(violet.replyNotification, {
            title: notification.title
          })} onClick={event => {
            ...
            event.stopPropagation();
          }}>{flint.formatMessage(violet.replyNotificationButton)}</ReadLoginRouteQuerySnapshot>}</div></ensureIntlFormattersInit.div> : null}{isReplying ? <ensureIntlFormattersInit.form className="no-drag mx-3 mb-2 border-t border-token-border/60 pt-2" animate={{
        opacity: 1,
        y: 0
      }} initial={{
        opacity: 0,
        ...
        tide(nest);
      }}><div className="flex min-w-0 items-center gap-1.5"><input ref={replyInputRef} className="text-size-chat-sm h-6 min-w-0 flex-1 rounded-md border border-token-border bg-token-main-surface-primary px-2 text-token-foreground outline-none placeholder:text-token-text-tertiary focus:border-token-focus-border" aria-label={flint.formatMessage(violet.replyNotification, {
            title: notification.title
          })} autoFocus={true} placeholder={flint.formatMessage(violet.notificationReplyPlaceholder)} value={zephyr} onChange={event => {
            ...
            event.key !== "Escape" || wheat || (event.stopPropagation(), onCloseReply(), coral(null));
          }} />{<ReadLoginRouteQuerySnapshot className="h-6 px-2 text-xs" color="primary" size="default" type="submit" aria-label={flint.formatMessage(violet.sendNotificationReply, {
            title: notification.title
          })} disabled={ridge.length === 0 || wheat} loading={wheat}>{flint.formatMessage(violet.replyNotificationButton)}</ReadLoginRouteQuerySnapshot>}</div>{bloom == null ? null : <div className="mt-1 text-[11px] leading-4 text-token-error-foreground" role="alert">{bloom}</div>}</ensureIntlFormattersInit.form> : null}</div>{sage && !isReplying ? <div className={IntlProvider("absolute -top-1 -left-1 z-20", drift ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0")} data-avatar-overlay-control="dismiss">{<OptionalTooltip align="start" side="top" tooltipContent={flint.formatMessage(violet.dismissNotificationTooltip)}><AvatarOverlayPillDismissButtonTrigger ariaLabel={flint.formatMessage(violet.dismissNotification, {
          title: notification.title
        })} onClick={() => {
          onDismissNotification?.(notification);
        }} /></OptionalTooltip>}</div> : null}</ensureIntlFormattersInit.div>;
}
    ...
    case "question":
      {
        ...
        let yarrow = <AvatarOverlayPageHelper19 actions={vine} onRunNotificationAction={wind} />;
        let azure;
        return <div data-avatar-overlay-compact-waiting-request="question">{urn}{yarrow}</div>;
      }
    case "patch":
      {
        let canyon = <AvatarOverlayPageHelper17 additions={request.additions} deletions={request.deletions} fileCount={request.fileCount} files={request.files} isExpanded={isExpanded} summary={request.summary} />;
        let dew = <AvatarOverlayPageHelper19 actions={request.actions} onRunNotificationAction={quiet} />;
        let ever;
        return <div data-avatar-overlay-compact-waiting-request="patch">{canyon}{dew}</div>;
      }
    case "plan":
      {
        let field;
        return <AvatarOverlayPageHelper15 isExpanded={isExpanded} localConversationId={localConversationId} onRunNotificationAction={quiet} request={request} />;
      }
    case "exec":
      {
        let grain = <AvatarOverlayPageHelper16 isExpanded={isExpanded} text={request.summary} />;
        let haven = <AvatarOverlayPageHelper19 actions={request.actions} onRunNotificationAction={quiet} />;
        let ink;
        return <div data-avatar-overlay-compact-waiting-request="exec">{grain}{haven}</div>;
      }
    case "network":
      {
        let jadeite = <AvatarOverlayPageHelper16 isExpanded={isExpanded} text={request.target} />;
        let kernel = <AvatarOverlayPageHelper19 actions={request.actions} onRunNotificationAction={quiet} />;
        let leaf;
        return <div data-avatar-overlay-compact-waiting-request="network">{jadeite}{kernel}</div>;
      }
    case "permission":
      {
        let maple = <AvatarOverlayPageHelper16 isExpanded={isExpanded} text={request.target} />;
        let nimbus = <AvatarOverlayPageHelper19 actions={request.actions} onRunNotificationAction={quiet} />;
        let opal;
        return <div data-avatar-overlay-compact-waiting-request="permission">{maple}{nimbus}</div>;
      }
    case "tool":
      {
        let plume = request.summary ?? request.target,
          quillow = <AvatarOverlayPageHelper16 isExpanded={isExpanded} text={plume} />;
        let root = <AvatarOverlayPageHelper19 actions={request.actions} onRunNotificationAction={quiet} />;
        let silk;
        return <div data-avatar-overlay-compact-waiting-request="tool">{quillow}{root}</div>;
      }
  }
}
...
function AvatarOverlayPageHelper15(vista) {
  let {
    ...
    {
      getModeForSelection
    } = AnnotationFocusMode(localConversationId),
    Wisp,
    yonder,
    zenith,
    anvil;
  {
    ...
    zenith = request.kind;
    anvil = <AvatarOverlayPageHelper16 isExpanded={isExpanded} text={request.summary} />;
    Wisp = AvatarOverlayPageHelper19;
    yonder = request.actions.map(item => item.intent === "plan-start" ? {
      ...item,
      planStartCollaborationMode: crag
    } : item);
  }
  let beacon = <Wisp actions={yonder} onRunNotificationAction={onRunNotificationAction} />;
  return <div data-avatar-overlay-compact-waiting-request={zenith}>{anvil}{beacon}</div>;
}
function AvatarOverlayPageHelper16(dome) {
  let {
    ...
    eddy = isExpanded ? "break-words whitespace-pre-wrap" : "truncate whitespace-nowrap",
    fjord = IntlProvider("min-w-0 text-token-text-secondary", eddy);
  return <div className={fjord} data-avatar-overlay-compact-waiting-summary-text="true">{text}</div>;
}
function AvatarOverlayPageHelper17(glen) {
  ...
  let jetty = inlet,
    knob = additions > 0 ? hearth.formatMessage(violet.compactPatchAdditions, {
      count: additions
    }) : null;
  let ledge = knob,
    mire = deletions > 0 ? hearth.formatMessage(violet.compactPatchDeletions, {
      count: deletions
    }) : null;
  let nook = mire;
  if (!isExpanded) {
    let updraft = <span>{jetty}</span>;
    let verge = ledge == null ? null : <span className="ml-1.5 text-token-charts-green">{ledge}</span>;
    let weir = nook == null ? null : <span className="ml-1.5 text-token-error-foreground">{nook}</span>;
    let yard = <BulletSeparator className="mx-1.5 text-token-text-tertiary" />;
    let anchor = <span>{summary}</span>;
    let bolt;
    return <div className="min-w-0 truncate whitespace-nowrap text-token-text-secondary" data-avatar-overlay-compact-waiting-summary-text="true">{updraft}{verge}{weir}{yard}{anchor}</div>;
  }
  let oxbow = <span className="text-[11px] leading-4 text-token-text-secondary">{jetty}</span>;
  let pond = ledge == null ? null : <span className="text-[11px] leading-4 text-token-charts-green">{ledge}</span>;
  let quarry = nook == null ? null : <span className="text-[11px] leading-4 text-token-error-foreground">{nook}</span>;
  let rapids = <div className="mt-0.5 flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">{oxbow}{pond}{quarry}</div>;
  let spur = files.map(gamma);
  let tor = <div className="mt-0.5 min-w-0 space-y-0.5 text-token-text-secondary">{spur}</div>;
  return <div data-avatar-overlay-compact-waiting-summary-text="true">{rapids}{tor}</div>;
}
function gamma(cog) {
  return <div key={cog} className="leading-4 break-words">{cog}</div>;
}
function AvatarOverlayPageHelper19(disc) {
  ...
  {
    let forge;
    forge = gear => <ReadLoginRouteQuerySnapshot key={gear.ariaLabel ?? gear.label} className="max-w-full min-w-0 enabled:active:hover:opacity-80" color={indigo(gear.tone)} size="toolbar" aria-label={gear.ariaLabel ?? gear.label} title={gear.ariaLabel ?? gear.label} onClick={event => {
      event.stopPropagation();
      onRunNotificationAction(gear);
    }} onPointerDown={harbor}><span className="truncate">{gear.label}</span></ReadLoginRouteQuerySnapshot>;
    edge = actions.map(forge);
  }
  return <div className="no-drag mt-1.5 flex min-w-0 flex-wrap items-center gap-1.5 overflow-visible pb-px">{edge}</div>;
}
...
function kite(motor, nut) {
  return motor.hasScrollableContent === nut.hasScrollableContent && motor.hasLatestNotificationsAbove === nut.hasLatestNotificationsAbove && motor.hiddenOlderNotificationCount === nut.hiddenOlderNotificationCount;
}
...
function nickel(valve) {
  return [valve, ...Array.from(valve.querySelectorAll("*"))].some(item => item.clientWidth > 0 && item.scrollWidth > item.clientWidth + basalt);
}
function onyx(axle, bracket, clamp) {
  let drill = clamp + axle.clientHeight - xenon;
  return bracket.filter(item => item.offsetTop + item.offsetHeight > drill).length;
}
function pearl(engine) {
  return Array.from(engine.children).filter(item => item instanceof HTMLElement);
}
...
function quartz(insert, jacket) {
  let knurl = 0;
  for (let lever = 0; lever < insert.length; lever += 1) insert[lever].offsetTop <= jacket && (knurl = lever);
  return knurl;
}
...
var slate,
  ...
  ember = esmInit(() => {
    ...
    amber = 512;
    basalt = 1;
    cedar = "group no-drag absolute left-1/2 z-10 flex h-5 cursor-interaction items-center justify-center gap-0.5 rounded-full border border-token-border bg-token-main-surface-primary px-2 text-[10px] leading-none font-medium text-token-text-secondary shadow-[0px_5px_10px_-7px_rgba(0,0,0,0.22)] backdrop-blur hover:text-token-foreground hover:shadow-[0px_7px_14px_-9px_rgba(0,0,0,0.26)] focus-visible:ring-1 focus-visible:ring-token-focus-border focus-visible:outline-none forced-colors:bg-[Canvas]";
...
export function AvatarOverlayPage() {
  let {
    ...
    ratchet;
  if (platen = () => {
    nozzle && ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-pointer-interaction-changed", {
      isInteractive: false
    });
  }, ratchet = [nozzle], $.useEffect(platen, ratchet), selectedAvatar == null) return null;
  let shim = alpha(selectedAvatar, selectedAvatarId);
  return <AvatarOverlayPageHelper31 key={shim} selectedAvatar={selectedAvatar} selectedAvatarId={selectedAvatarId} />;
}
