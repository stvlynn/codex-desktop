...
function gamma(reef) {
  ...
  let lunar = usePointerSurfaceInteractionGate(knoll),
    ...
    orbit = eagle !== "inactive" && (honey == null || honey === "global-overlay"),
    pine = jewel || orbit,
  ...
  let {
      data
    } = pluginMarketplaceRequestFields(storm),
    tide = orbit,
  ...
  let {
    ...
    trail = copper(seed),
    urn = quiet == null ? null : avatarOverlaySelectionR({
      intl: topaz,
    ...
    }) : null,
    {
      ...
      extraNotifications: orbit ? [...($e == null ? [] : [$e]), ...wind, ...vine] : pine || urn == null ? [] : [urn],
    ...
    kernel = (jig, keeper, lug, mandrel) => {
      ...
      yarn.logProductEvent(CODEX_ONBOARDING_WORKSPACE_CONTINUE_CLICKED_TYPE, buildAvatarOverlayAnalyticsPayload({
        action: jig,
  ...
  _t = () => {
    ...
    orifice.includes(quiet.avatarId) || setLocalStorageItem(flint, [...orifice, quiet.avatarId]);
  };
  maple = [quiet];
  $.useEffect(_t, maple);
  let nimbus, opal;
  nimbus = () => {
    jadeite.current || yarn !== NOOP_ANALYTICS_CLIENT && (jadeite.current = true, leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_OPENED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_UNSPECIFIED));
  };
  opal = [yarn, leaf];
  ...
  let quillow = () => {
    ...
    let race = {
      ...
      isTrayVisible: brook && notifications.length > 0 || coral.caption != null
    };
    zinc(ink.current, race) || (ink.current = race, ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-element-size-changed", {
      isGlobalRealtimeVoiceTransitioning: race.isGlobalRealtimeVoiceTransitioning,
  ...
  let root = quillow,
    silk = () => {
      grain.current != null && (window.clearTimeout(grain.current), grain.current = null);
    };
  let thorn = silk,
    upland = () => {
      dew.current != null && (window.cancelAnimationFrame(dew.current), dew.current = null);
      ever.current = null;
    };
  let vista = upland,
    wisp = sleeve => {
      ...
      dew.current ??= window.requestAnimationFrame(() => {
        ...
        ever.current = null;
        !(trunnion == null || canyon.current == null) && ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-mascot-resize-move", {
          width: trunnion
        });
      });
    };
  ...
  let anvil = zenith,
    beacon = () => willow(haven.current?.querySelector(hazel) ?? null)?.width ?? mascotWidthPx ?? ensureAvatarOverlayMascotSizeInit;
  let crag = beacon,
    dome = (cam, detent) => {
      let eccentric = canyon.current;
      eccentric == null || eccentric.pointerId !== cam || (canyon.current = null, grove(false), detent?.hasPointerCapture?.(cam) && detent.releasePointerCapture?.(cam));
    };
  let eddy = dome,
    fjord = (follower, guide) => {
      let helix = canyon.current;
      if (helix == null || helix.pointerId !== follower) return;
      let impeller = guide == null ? helix.currentWidthPx : quartz(helix, guide);
      helix.currentWidthPx = impeller;
      vista();
      juniper(impeller);
      setMascotWidthPx(impeller);
      anvil(impeller);
      ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-mascot-resize-end", {
        width: impeller
      });
    };
  let glen = fjord,
    hearth = (journal, kingpin) => {
      ...
      let {
        ...
        velocity
      } = avatarOverlaySelectionF(land, releaseSample, shouldPreserveOrbMomentum && land.usesOrbPhysics);
      if (haven.current?.hasPointerCapture?.(journal) && haven.current.releasePointerCapture?.(journal), hasMoved && !land.hasMoved && _releaseSample != null && ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-drag-move", {
        pointerScreenX: _releaseSample.screenX,
        pointerScreenY: _releaseSample.screenY
      }), shouldOpenMainWindow && land.startedOnMascot && !hasMoved && (leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_MASCOT_CLICKED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_MASCOT), orbit && iris != null ? north?.() : ensureAppActionPayloadSchemasInit.dispatchMessage("open-current-main-window", {
        focusComposer: true,
        stealFocus: true
      })), ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-drag-end", {
        pointerScreenX: _releaseSample?.screenX ?? land.screenX,
        pointerScreenY: _releaseSample?.screenY ?? land.screenY
      }), hasMoved && leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_DRAG_COMPLETED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_MASCOT), velocity != null) {
        let mesh = land.usesOrbPhysics ? 3 : 1;
        ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-drag-release", {
  ...
  let inlet = hearth,
    jetty = event => {
      event.button !== 0 || !(event.target instanceof Element) || event.target.closest(".no-drag") != null || (event.preventDefault(), event.currentTarget.setPointerCapture?.(event.pointerId), at.current = {
        ...
        usesOrbPhysics: tide
      }, ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-drag-start", {
  ...
  let knob = jetty,
    ledge = event => {
      ...
      Math.abs(quillshaft) < 4 && Math.abs(roller) < 4 || (neck.hasMoved = true, neck.screenX = pad.screenX, neck.screenY = pad.screenY, apex(spindle => timber({
        currentDragState: spindle,
        deltaX: quillshaft
      })), ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-drag-move", {
  ...
  let spur = rapids,
    tor = event => {
      ...
      juniper(thrust);
      grove(true);
      ensureAppActionPayloadSchemasInit.dispatchMessage("avatar-overlay-mascot-resize-start", {
        width: thrust
      });
    };
  let updraft = tor,
  ...
  let disc = cog,
    edge = (capstan, diaphragm) => {
      ...
      if (capstan.localConversationId != null && diaphragm != null) {
        ...
        bb396: switch (diaphragm.intent) {
          case "command-approval":
            if (diaphragm.commandDecision != null && (elbow?.kind === "exec" || elbow?.kind === "network")) {
          ...
          case "permission-response":
            if (diaphragm.permissionResponse != null && elbow?.kind === "permission") {
          ...
          case "mcp-elicitation":
            if (diaphragm.mcpElicitationAction != null && elbow?.kind === "tool") {
              canonicalizeWorkspacePathKey("reply-with-mcp-server-elicitation-response", {
                conversationId: ferrule,
                requestId: elbow.requestId,
                response: buildMcpToolApprovalResult(diaphragm.mcpElicitationAction)
              }).then(() => {
                refetch();
              });
              return;
            ...
            break bb396;
          case "plan-start":
            if (elbow?.kind === "plan" && diaphragm.planStartCollaborationMode != null) {
              canonicalizeWorkspacePathKey("update-thread-settings-for-next-turn", {
                ...
                serviceTier: await resolveStableWorktreeStatus(sage, sage.get(DeferredUi2, ferrule) ?? "local", diaphragm.planStartCollaborationMode?.settings.model ?? null)
              })).then(() => {
                refetch();
              });
              return;
      ...
      diaphragm != null && diaphragm.intent !== "open" || capstan.action != null && (leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_OPENED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_ROW, capstan), ensureAppActionPayloadSchemasInit.dispatchMessage("open-in-main-window", {
  ...
  let forge = edge,
    gear = (grommet, header) => {
      ...
      grommet.localConversationId == null || injector?.kind !== "question" || canonicalizeWorkspacePathKey("reply-with-user-input-response", {
  ...
  let hinge = gear,
    iron = jumper => {
      leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_DISMISSED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_ROW, jumper);
      meadow(kerf => {
  ...
  let latch = keystone,
    motor = nip => {
      ...
      outlet != null && avatarOverlaySelectionL({
        intl: topaz,
  ...
  let nut = motor,
    piston = async (packing, reducer) => {
      ...
      strainer.length !== 0 && (leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_REPLY_SUBMITTED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_REPLY, packing), await canonicalizeWorkspacePathKey("send-follow-up-message", {
        conversationId: packing.controlTarget.conversationId,
        prompt: strainer,
        serviceTier: await resolveStableWorktreeStatus(sage, sage.get(DeferredUi2, packing.controlTarget.conversationId) ?? "local", null)
      }));
    };
  let rivet = piston,
    screw = kite,
  ...
  torque = tee => {
    ...
    vale(layout);
    field.current === layout.mascot.width && (field.current = null, thorn(), juniper(null), window.requestAnimationFrame(root));
  };
  valve = [thorn, root];
  ...
  axle = () => {
    ...
    return document.documentElement.classList.toggle(olive, union), document.body.classList.toggle(olive, union), jade;
  };
  ...
  engine = () => {
    ...
    return window.addEventListener("pointerup", vent), window.addEventListener("pointercancel", wye), () => {
  ...
  let gasket = () => {
    ...
    if (copperPrime != null) {
      ...
      for (let deltaPrime of violet(copperPrime)) bravoPrime.observe(deltaPrime);
    }
    return window.addEventListener("resize", alphaPrime), alphaPrime(), () => {
  ...
  let jacket;
  jacket = [brook, root, selectedAvatar.id, yarrow, coral.caption, mascotWidthPx];
  ...
  let platen = [{
    ...
    message: $n,
    onSelect: () => {
      leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_CLOSE_REQUESTED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_CONTEXT_MENU);
  ...
  let bushing = fern && isle != null ? {
    ...unity.mascot,
    height: Math.ceil(isle / clampAvatarOverlayMascotWidthPx),
    width: isle
  } : unity.mascot;
  let collar = zephyr ? undefined : {
  ...
  let or = () => {
    leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_TRAY_CLOSED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_BADGE, undefined, false);
    cliff(false);
  };
  let gib = window.electronBridge?.showContextMenu == null ? undefined : nut,
    hub = harborPrime => {
      leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_REPLY_OPENED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_REPLY, harborPrime);
    };
  let idler = () => {
    leaf(CODEX_LOGIN_SUCCESS_TYPE.CODEX_AVATAR_OVERLAY_ACTION_NOTIFICATION_TRAY_OPENED, CodexPluginPageName.CODEX_AVATAR_OVERLAY_SOURCE_NOTIFICATION_BADGE, undefined, true);
    cliff(true);
  };
  return <AvatarOverlayPageHelper7 avatar={selectedAvatar} avatarMenuItems={platen} debugWindowBorderVisible={ultra} interactiveRegionRef={haven} realtimeVoiceSurface={arbor} isDragging={dusk} isNotificationTrayOpen={brook} layout={unity} mascotDragState={wave} mascotLayout={bushing} mascotResizeHandle={collar} mascotStyle={flange} notifications={notifications} onCloseNotificationTray={or} onLostPointerCapture={spur} onPointerCancel={quarry} onPointerDown={knob} onPointerMove={mire} onPointerUp={oxbow} onDismissNotification={joint} onOpenNotificationActions={gib} onNotificationReplyEditorActiveChange={screw} onOpenNotificationReply={hub} onRunNotificationAction={forge} onSubmitQuestionOption={hinge} onSubmitNotificationReply={rivet} onOpenNotificationTray={idler} />;
}
function harbor() {}
...
function quartz(onyxPrime, pearlPrime) {
  return avatarOverlayMascotWidthStyle(onyxPrime.startWidthPx + pearlPrime - onyxPrime.startScreenX);
}
function river(quartzPrime, riverPrime) {
  return SettingsSectionChrome(quartzPrime, riverPrime) || getPersistedAtomItem(flint, []).includes(quartzPrime.id) ? null : {
...
function slate(slatePrime, timberPrime) {
  return SettingsSectionChrome(slatePrime, timberPrime) ? "pending-custom-avatar" : "ready";
}
function timber({
  currentDragState,
  deltaX
...
function willow(yellowPrime) {
  ...
  return zincPrime.width <= 0 || zincPrime.height <= 0 ? null : {
...
function xenon(amberPrime) {
  ...
  let cedarPrime = Math.ceil(amberPrime.offsetWidth > 0 ? amberPrime.offsetWidth : basaltPrime.width),
  ...
  if (daisyPrime == null || emberPrime == null && flintPrime == null) return {
    width: cedarPrime,
    height: Math.ceil(basaltPrime.height)
  };
  let hazelPrime = emberPrime != null && (flintPrime == null ? 0 : 1) + (garnetPrime == null ? 0 : 1) > 1 ? yellow(emberPrime) : 0;
  return {
    width: cedarPrime,
    height: Math.ceil(daisyPrime.getBoundingClientRect().height + (flintPrime?.scrollHeight ?? 0) + (garnetPrime == null ? 0 : garnetPrime.getBoundingClientRect().height) + hazelPrime)
  };
}
function yellow(ivoryPrime) {
  let jasperPrime = Number.parseFloat(window.getComputedStyle(ivoryPrime).rowGap);
  return Number.isFinite(jasperPrime) ? jasperPrime : 0;
}
function zinc(kelpPrime, lotusPrime) {
  return kelpPrime != null && kelpPrime.isGlobalRealtimeVoiceTransitioning === lotusPrime.isGlobalRealtimeVoiceTransitioning && kelpPrime.isTrayVisible === lotusPrime.isTrayVisible && kelpPrime.realtimeCaptionBelowMascotPx === lotusPrime.realtimeCaptionBelowMascotPx && kelpPrime.mascot.width === lotusPrime.mascot.width && kelpPrime.mascot.height === lotusPrime.mascot.height && amber(kelpPrime.tray, lotusPrime.tray);
}
function amber(mintPrime, novaPrime) {
  return mintPrime === novaPrime || mintPrime != null && novaPrime != null && mintPrime.width === novaPrime.width && mintPrime.height === novaPrime.height;
}
function basalt(olivePrime) {
  return window.getComputedStyle(olivePrime).display === "none";
}
var cedar, $, daisy, ember, flint, garnet, hazel, ivory, jasper, kelp, lotus, mint, nova, olive, prism, quill;
esmInit(() => {
