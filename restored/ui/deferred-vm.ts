  ...
  return peers.e(() => {
    ...
    cVa = class {
      ...
      matchingDegree(copper) {
        ...
        if (delta != null)
          return peers.zBa(
            this.mainMatcher.matchingDegree(peers.e, false, delta),
            delta,
          );
        ...
        return echo == null
          ? peers.iVa
          : peers.zBa(
              this.fallbackMatcher.matchingDegree(peers.e, false, echo),
              echo,
            );
      }
    };
    lVa = class {
      ...
      matchingDegree(slate, timber = false, umbra = this.match(peers.e)) {
        ...
        let violet = umbra[0],
          ...
          yellow = 0,
          u = false;
        for (let cedar of umbra)
          for (
            let daisy = cedar.startOffset;
            daisy < cedar.endOffset;
            daisy += 1
          ) {
            ...
            willow += this.evaluateCaseMatching(
              a,
              s,
              u,
              daisy,
        ...
        let zinc = violet.startOffset,
          ...
          p =
            zinc === 0 ||
            (peers.UBa(peers.e, zinc) && !peers.UBa(peers.e, zinc - 1)),
      ...
      match(garnet) {
        ...
        for (
          let jasper = 0;
          jasper < peers.e.length && hazel < this.meaningfulCharacters.length;
          jasper += 1
        ) {
          let kelp = peers.e[jasper];
          (kelp === this.meaningfulCharacters[hazel] ||
            kelp === this.meaningfulCharacters[hazel + 1]) &&
            (hazel += 2);
        }
      ...
      evaluateCaseMatching(lotus, mint, nova, olive, i, a, prism) {
        return i && a && this.isLowerCase[mint]
          ? -10
          : prism === this.myPattern[mint]
            ? this.isUpperCase[mint]
              ? 50
              : olive === 0 && peers.e
                ? 150
                : +!!a
            : a || (this.isLowerCase[mint] && nova)
              ? -1
              : 0;
      }
      ...
      matchWildcards(ultra, vapor, wheat) {
        ...
        if (!this.isWildcard(yarn))
          return yarn === this.myPattern.length
            ? []
            : this.matchFragment(peers.e, yarn, wheat);
        do yarn += 1;
        while (this.isWildcard(yarn));
        if (yarn === this.myPattern.length) {
          if (
            this.isTrailingSpacePattern() &&
            wheat !== peers.e.length &&
            (yarn < 2 || !this.isUpperCaseOrDigit(yarn - 2))
          ) {
        ...
        return this.matchSkippingWords(
          peers.e,
          yarn,
          this.findNextPatternCharOccurrence(peers.e, wheat, yarn),
          true,
        );
      }
      ...
      matchSkippingWords(bloom, coral, drift, eagle) {
        let i = drift,
          a = 0;
        for (; i >= 0; ) {
          let frost = this.seemsLikeFragmentStart(peers.e, coral, i)
            ? this.maxMatchingFragment(peers.e, coral, i)
            : 0;
          if (
            frost > a ||
            (i + frost === peers.e.length && this.isTrailingSpacePattern())
          ) {
          ...
          i = eagle
            ? glide
            : this.checkForSpecialChars(peers.e, i + 1, glide, coral);
        }
        return null;
      }
      findNextPatternCharOccurrence(iris, jewel, knoll) {
        return !this.isPatternChar(knoll - 1, "*") &&
          !this.isWordSeparator[knoll]
          ? this.indexOfWordStart(peers.e, knoll, jewel)
          : this.indexOfIgnoreCase(peers.e, jewel, knoll);
      }
      checkForSpecialChars(lunar, moss, north, orbit) {
        return north < 0 ||
          (!this.hasSeparators &&
            !this.mixedCase &&
            peers.KBa(peers.e, this.hardSeparators, moss, north) !== -1) ||
          (this.hasDots &&
            !this.isPatternChar(orbit - 1, ".") &&
            peers.qBa(peers.e, ".", moss, north) !== -1)
          ? -1
          : north;
      }
      seemsLikeFragmentStart(pine, quest, ridge) {
        return !this.isUpperCase[quest] ||
          peers.$Ba(peers.e[ridge]) ||
          peers.UBa(peers.e, ridge)
          ? true
      ...
      charEquals(storm, tide, unity, vale) {
        return peers.e === unity
          ? true
          : vale
            ? this.toLowerCase[tide] === unity ||
              this.toUpperCase[tide] === unity
            : false;
      }
      matchFragment(wave, apex, brook) {
        ...
        return cliff === 0
          ? null
          : this.matchInsideFragment(peers.e, apex, brook, cliff);
      }
      maxMatchingFragment(dusk, elm, fern) {
        ...
        for (
          ;
          fern + grove < peers.e.length && elm + grove < this.myPattern.length;
        ) {
          let a = peers.e[fern + grove];
          if (
            !this.charEquals(this.myPattern[elm + grove], elm + grove, a, i)
          ) {
            if (this.isSkippingDigitBetweenPatternDigits(elm + grove, a))
              return 0;
            break;
          }
          grove += 1;
      ...
      matchInsideFragment(juniper, lagoon, meadow, nest) {
        ...
        return (
          this.improveCamelHumps(peers.e, lagoon, meadow, nest, i) ??
          this.findLongestMatchingPrefix(peers.e, lagoon, meadow, nest, i)
        );
      }
      isMiddleMatch(oak, petal, quiet) {
        return !this.isPatternChar(petal - 1, "*") ||
          this.isWildcard(petal + 1) ||
          !peers.nVa(peers.e[quiet])
          ? false
          : !peers.UBa(peers.e, quiet);
      }
      findLongestMatchingPrefix(rain, seed, trail, urn, i) {
        if (seed + urn >= this.myPattern.length)
          return [
            {
              startOffset: trail,
              endOffset: trail + urn,
        ...
        for (; a >= i || (a > 0 && this.isWildcard(seed + a)); ) {
          let vine = null;
          if (this.isWildcard(seed + a))
            vine = this.matchWildcards(peers.e, seed + a, trail + a);
          else {
            ...
            i >= 0 &&
              (vine = this.matchSkippingWords(peers.e, seed + a, i, false));
          }
      ...
      improveCamelHumps(wind, yarrow, azure, birch, i) {
        for (let a = i; a < birch; a += 1)
          if (
            ...
            let canyon = this.findUppercaseMatchFurther(
              peers.e,
              yarrow + a,
              azure + a,
            );
      ...
      isUppercasePatternVsLowercaseNameChar(dew, ever, field) {
        return (
          this.isUpperCase[ever] && this.myPattern[ever] !== peers.e[field]
        );
      }
      ...
      isFirstCharMatching(kernel, leaf, maple) {
        ...
        return this.charEquals(i, maple, peers.e[leaf], nimbus)
          ? this.matchingMode === "FIRST_LETTER" &&
            (maple === 0 || (maple === 1 && this.isWildcard(0))) &&
            this.hasCase(maple)
            ? this.isUpperCase[maple] === peers.$Ba(peers.e[0])
            : true
          : false;
      }
      hasCase(opal) {
      ...
      isPatternChar(quillow, root) {
        return peers.e < 0 || peers.e >= this.myPattern.length
          ? false
          : this.myPattern[peers.e] === root;
      }
      indexOfWordStart(silk, thorn, upland) {
        let vista = this.myPattern[thorn];
        if (
          upland >= peers.e.length ||
          (this.mixedCase &&
            this.isLowerCase[thorn] &&
            !(thorn > 0 && this.isWordSeparator[thorn - 1]))
        )
          return -1;
        let i = upland,
          a = !peers.nVa(vista);
        for (;;) {
          if (((i = this.indexOfIgnoreCase(peers.e, i, thorn)), i < 0))
            return -1;
