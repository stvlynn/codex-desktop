// Restored from ref/webview/assets/app-initial-C-fROkKo.js
// Materialized via extractFn(internal `gMs`) / export `dv`.

export type DeferredUidvPeers = {
  J: (...args: unknown[]) => unknown;
  e: (...args: unknown[]) => unknown;
  hMs: (...args: unknown[]) => unknown;
  mMs: (...args: unknown[]) => unknown;
  o: (...args: unknown[]) => unknown;
};
let peers: DeferredUidvPeers | null = null;

/** Wire deferredUidv peers once companions land. */
export function setDeferredUidvPeers(next: DeferredUidvPeers): void {
  peers = next;
}

/**
 * Bundle export `dv` / internal `gMs`.
 */
export function deferredUidv() {
  if (peers == null) {
    throw new Error("deferredUidv peers are not configured");
  }
  return peers.e(() => {
    peers.o();
    mMs = peers.J();
    hMs = (e) => {
      return (
        <svg
          width={20}
          height={20}
          viewBox="0 0 20 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          {...peers.e}
        >
          <path
            d="M10 5.14295V14.8441M10 5.14295C10 1.98209 14.8475 1.94468 14.8475 5.07797L14.8473 5.1067C17.1367 5.43539 18.0436 7.54498 16.4225 9.14006C17.8633 10.5577 17.3503 13.9054 15.4449 14.6301C14.8475 17.6527 10.8056 18.4821 10 14.8441M10 5.14295C10 1.98209 5.15249 1.94468 5.15249 5.07797L5.15265 5.1067C2.86325 5.43539 1.95642 7.54498 3.57746 9.14006C2.13674 10.5577 2.64974 13.9054 4.5551 14.6301C5.15249 17.6527 9.19444 18.4821 10 14.8441"
            stroke="currentColor"
            strokeWidth={1.33}
            strokeLinejoin="round"
          />
        </svg>
      );
    };
  });
}
