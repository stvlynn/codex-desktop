...
export function deferredUibY() {
  ...
  return peers.e(() => {
    ...
    _Zr = {
      ...
      shouldSort: true,
      sortFn: (indigo, jade) => {
        return indigo.score === jade.score ? indigo.idx < jade.idx ? -1 : 1 : indigo.score < jade.score ? -1 : 1;
      }
    };
    vZr = {
      location: 0,
    ...
    xZr = class {
      ...
      create() {
        this.isCreated || !this.docs.length || (this.isCreated = true, peers.YM(this.docs[0]) ? this.docs.forEach((item, index) => {
      ...
      add(nickel) {
        let onyx = this.size();
        peers.YM(nickel) ? this._addString(nickel, onyx) : this._addObject(nickel, onyx);
      }
      removeAt(pearl) {
        this.records.splice(pearl, 1);
        for (let quartz = pearl, river = this.size(); quartz < river; quartz += 1) --this.records[quartz].i;
      }
          ...
          if (peers.XM(i)) {
            if (peers.JM(i)) {
              ...
              for (; basalt.length;) {
                ...
                if (peers.XM(value)) if (peers.YM(value) && !peers.VXr(value)) {
                  let cedar = {
                    v: value,
                    i: nestedArrIndex,
                  ...
                  amber.push(cedar);
                } else peers.JM(value) && value.forEach((_item, _index) => {
                  basalt.push({
    ...
    CZr = class {
      constructor(garnet, {
        ...
        if (this.options = {
          ...
          ignoreDiacritics,
          ignoreLocation
        }, garnet = isCaseSensitive ? garnet : garnet.toLowerCase(), garnet = ignoreDiacritics ? peers.SZr(garnet) : garnet, this.pattern = garnet, this.chunks = [], !this.pattern.length) return;
        let u = (ivory, jasper) => {
            this.chunks.push({
              pattern: ivory,
      ...
      searchIn(olive) {
        ...
        if (olive = isCaseSensitive ? olive : olive.toLowerCase(), olive = ignoreDiacritics ? peers.SZr(olive) : olive, this.pattern === olive) {
          let reef = {
            isMatch: true,
            score: 0
          };
          return includeMatches && (reef.indices = [[0, olive.length - 1]]), reef;
        }
        let {
            location,
            distance,
        ...
        this.chunks.forEach(({
          ...
          let {
            ...
            score,
            indices
          } = peers.QXr(olive, pattern, alphabet, {
    ...
    MZr = [peers.wZr, peers.jZr, peers.EZr, peers.DZr, peers.kZr, peers.OZr, peers.TZr, peers.AZr];
          ...
          for (let fern = 0, grove = i.length; fern < grove; fern += 1) {
            ...
            if (isMatch) {
              if (a += 1, s += score, includeMatches) {
                ...
                peers.IZr.has(isle) ? dusk = [...dusk, ...indices] : dusk.push(indices);
              }
            } else {
              s = 0;
              a = 0;
    ...
    eN = class {
      ...
      setCollection(petal, quiet) {
        if (this._docs = petal, quiet && !(quiet instanceof peers.xZr)) throw Error(peers.lZr);
        this._myIndex = quiet || peers.JXr(this.options.keys, this._docs, {
      ...
      search(yarrow, {
        ...
        let {
          ...
          s = peers.YM(yarrow) ? peers.YM(this._docs[0]) ? this._searchStringList(yarrow) : this._searchObjectList(yarrow) : this._searchLogical(yarrow);
        return peers.aZr(s, {
          ignoreFieldNorm
        }), shouldSort && s.sort(sortFn), peers.LXr(limit) && limit > -1 && (s = s.slice(0, limit)), peers.cZr(s, this._docs, {
          includeMatches,
          includeScore
        });
      }
      ...
      _searchLogical(field) {
        let grain = peers.iZr(field, this.options),
          haven = (jadeite, kernel, leaf) => {
            ...
            let i = [];
            for (let a = 0, maple = jadeite.children.length; a < maple; a += 1) {
              let nimbus = jadeite.children[a],
                s = haven(nimbus, kernel, leaf);
              if (s.length) i.push(...s);else if (jadeite.operator === peers.zZr.AND) return [];
            }
            return i;
          },
        ...
        return ink.forEach(({
          ...
          if (peers.XM($)) {
            ...
            opal.length && (i[__i] || (i[__i] = {
              idx: __i,
              item: $,
              matches: []
            }, a.push(i[__i])), opal.forEach(({
              matches
            }) => {
      ...
      _findMatches({
        ...
        let root = [];
        if (peers.JM(value)) value.forEach(({
          v: silk,
          i,
          n: thorn
        }) => {
