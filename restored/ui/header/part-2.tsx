...
function petal(maple) {
  let { className } = maple,
    ...
    upland = AppInitialTY(),
    { data, isLoading, isError, refetch } = AppInitialMY(),
    vista = quillow.trim();
  ...
  let hearth = !!silk && crag?.label && (
    <span className="text-sm">{crag.label}</span>
  );
  let inlet = (
  ...
  let tor =
    quillow.trim().length > 0 && _isError ? (
      ...
      <VSCODE_EDITOR_ID
        {...{
          className:
            "icon-xxs my-2 self-center text-token-description-foreground",
        }}
      />
    ) : (
      <DropdownMenu.Message
...
function _n(mount) {
  let { children, isSelected, onClick } = mount,
...
function ink({ conversation, isBackgroundSubagentsEnabled }) {
  return !ensureSelectWorkspaceKstInit(
    conversation,
...
function kernel(tappet, arbor, bushing) {
  let collar = nonEmptyTrimmedStringOrNull(),
    ...
    gib = CodexBrowserSurfaceActionType(
      Remote6,
      coral.useMemo(() => arbor.map((item) => item.id), [arbor]),
    ),
  ...
  return coral.useMemo(() => {
    ...
    return (
      (jig.current = new Map(keeper.map((item) => [item.key, item]))),
      keeper
    );
...
function leaf({
  ...
  let lug = localConversations.filter((item) =>
      ink({
        conversation: item,
        isBackgroundSubagentsEnabled,
      }),
    ...
    nipple = lug.map((item) => {
      let race =
        threadSortKey === "updated_at"
          ...
          : item.createdAt;
      Number.isFinite(race) ||
        appActionSidebarProjectRefSchema.error(
          "local conversation has invalid createdAt or updatedAt",
        );
      let sleeve = Number.isFinite(race)
        ? race
      ...
      return {
        kind: "local",
        key: toLocalSidebarThreadKey(
          clientThreadIdsByConversationId.get(item.id) ?? item.id,
        ),
        at: sleeve,
        conversation: item,
      };
    }),
    orifice = pendingWorktrees.flatMap((item) =>
  ...
  return bloom
    .default([...mandrel, ...nipple, ...orifice], "key")
    .sort((trunnion, boss) => boss.at - trunnion.at);
}
