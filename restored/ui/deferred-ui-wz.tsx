// Restored from ref/webview/assets/app-initial-C-fROkKo.js
// Materialized via extractFn(internal `xFr`) / export `WZ`.

export type BindDeferredUiWZPeers = {
  J: (...args: unknown[]) => unknown;
  bFr: (...args: unknown[]) => unknown;
  e: (...args: unknown[]) => unknown;
  o: (...args: unknown[]) => unknown;
  yFr: (...args: unknown[]) => unknown;
};
let peers: BindDeferredUiWZPeers | null = null;

/** Wire bindDeferredUiWZ peers once companions land. */
export function setBindDeferredUiWZPeers(next: BindDeferredUiWZPeers): void {
  peers = next;
}

/**
 * Bundle export `WZ` / internal `xFr`.
 */
export function bindDeferredUiWZ() {
  if (peers == null) {
    throw new Error("bindDeferredUiWZ peers are not configured");
  }
  return peers.e(() => {
    peers.o();
    yFr = peers.J();
    bFr = (e) => {
      return (
        <svg
          width={20}
          height={20}
          viewBox="0 0 20 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          {...peers.e}
        >
          <path
            d="M4.33496 11C4.33496 10.6327 4.63273 10.335 5 10.335C5.36727 10.335 5.66504 10.6327 5.66504 11V14.335H9L9.13379 14.3486C9.43692 14.4106 9.66504 14.6786 9.66504 15C9.66504 15.3214 9.43692 15.5894 9.13379 15.6514L9 15.665H5C4.63273 15.665 4.33496 15.3673 4.33496 15V11ZM14.335 9V5.66504H11C10.6327 5.66504 10.335 5.36727 10.335 5C10.335 4.63273 10.6327 4.33496 11 4.33496H15L15.1338 4.34863C15.4369 4.41057 15.665 4.67857 15.665 5V9C15.665 9.36727 15.3673 9.66504 15 9.66504C14.6327 9.66504 14.335 9.36727 14.335 9Z"
            fill="currentColor"
          />
        </svg>
      );
    };
  });
}
