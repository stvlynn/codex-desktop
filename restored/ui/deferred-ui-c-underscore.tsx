...
// Materialized via extractFn(internal `YUs`) / export `c_`.

export type BindDeferredUicPeers = {
  J: (...args: unknown[]) => unknown;
  JUs: (...args: unknown[]) => unknown;
  e: (...args: unknown[]) => unknown;
  o: (...args: unknown[]) => unknown;
  v$: (...args: unknown[]) => unknown;
};
let peers: BindDeferredUicPeers | null = null;

/** Wire bindDeferredUic_ peers once companions land. */
export function setBindDeferredUicPeers(next: BindDeferredUicPeers): void {
  peers = next;
}
 ...
 */
export function bindDeferredUic_() {
  if (peers == null) {
