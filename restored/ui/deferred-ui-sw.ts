...
// Materialized via extractFn(internal `IPi`) / export `SW`.
...
export type BindDeferredUiSWPeers = {
  ...
  height: (...args: unknown[]) => unknown;
  heightDirty: (...args: unknown[]) => unknown;
  instanceChanged: (...args: unknown[]) => unknown;
  instancesChanged: (...args: unknown[]) => unknown;
  kPi: (...args: unknown[]) => unknown;
};

let peers: BindDeferredUiSWPeers | null = null;

/** Wire bindDeferredUiSW peers once companions land. */
 ...
 */
export function bindDeferredUiSW() {
  if (peers == null) {
  ...
  return class e {
    ...
    connect(e, t) {
      if (this.observers.has(peers.e))
        throw Error(`Virtualizer.connect: instance is already connected...`);
      return (
    ...
    handleIntersectionChange = (e) => {
      ...
      this.visibleInstancesDirty && peers.bF(this.computeRenderRangeAndEmit);
    };
    getScrollTop() {
