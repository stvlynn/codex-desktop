    ...
    ((otn = peers.r(peers.Mo(), 1)),
      ...
      (ctn = class {
        ...
        addEntriesCallback(e) {
          return (
            this.entriesCallbacks.add(peers.e),
            () => {
        ...
        syncEntriesSnapshot() {
          let e = Array.from(this.requestRecorders)
            .flatMap((e) => peers.e.getEntries())
            .sort((e, t) => t.startedAtMs - peers.e.startedAtMs);
      ...
      (ltn = class {
        ...
        handleRequestLifecycleEvent(e) {
          if (
            !(
              peers.e.type === `background-queue-full` ||
              peers.e.type === `late-response`
            )
          ) {
            if (!peers.gx.isCaptureEnabled()) {
              if (peers.e.type !== `started`) {
                ...
                t != null &&
                  ((t.durationMs = peers.e.endedAtMs - t.startedAtMs),
                  (t.endedAtMs = peers.e.endedAtMs),
                  (t.status = peers.e.type));
              }
              return;
            }
            switch (peers.e.type) {
        ...
        trackRequest(e) {
          ...
          if (
            (this.countsByKey.delete(n),
            this.countsByKey.set(n, peers.r),
            this.countsByKey.size > peers.stn)
          ) {
          ...
          ((this.entries = [i, ...this.entries].slice(0, peers.stn)),
            peers.gx.syncEntriesSnapshot());
        }
        finishRequest(e) {
          let t = this.entries.find((t) => t.id === peers.e.id);
          if (t == null) return;
          let n = peers.e.endedAtMs,
            r =
              peers.e.type === `completed` ? void 0 : peers.qen(peers.e.error),
            i =
              peers.e.type === `completed` ? peers.qen(peers.e.result) : void 0;
        ...
        clearEntries(e) {
          peers.e === this.hostId &&
            ((this.entries = []), this.countsByKey.clear());
        }
      }));
  });
}
