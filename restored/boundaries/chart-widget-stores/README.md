...
| Recharts surface aliases | `G`/`P`/`_`/`F`/`H`/`M` + Bar/Line/Area/… | ResponsiveContainer, Legend, Tooltip, Cell, PieChart, Pie, series | **Retired** — Chart+pie on npm; aliases re-export `recharts`; `setChartBundleSurface` no-op |
...
| Recharts (stock) | ~legend/pie/bar/line/area/scatter (~40k+) | `recharts-*` classNames; ComposedChart/Bar/Line/… | **Consumers on npm** — mega body still inlined pending joint delete |
| react-intl (stock) | post-Y ~4.6k (`L` / Formatted*) | formatjs.github.io strings; `MemoizedFormattedMessage` | **Consumers on npm** — mega `L` body still inlined pending joint delete |

## Decision

**Not a single npm-first shim.** The dump is a **composite** of stock Recharts 3.x + RTK/Immer + react-intl **plus** Codex WidgetContext / app-scope widget store / chart chrome / i18n / Chart / Box. A bare `export * from "recharts"` cannot replace `chartWidgetStores*` consumers until the inlined vendor bodies are jointly deleted.

## Wave-9 — Pie off surface bag (npm migrate complete for chart chrome)

### Migrated

1. **Pie** — `chatgpt-pie-chart-widget` uses bare `recharts` (`ResponsiveContainer`/`PieChart`/`Pie`/`Cell`/`Legend`/`Tooltip`) + `react` hooks. No `getRechartsSurfaceAliases` / mega `_chartWidgetStoresC` surface init.
2. **Surface bag** — `setChartBundleSurface` is a no-op; `ChartBundleSurface` emptied; `getRechartsSurfaceAliases` re-exports public `recharts` (deprecated).
3. **Mega call** — `vizSetChartBundleSurface()` no-arg stub call retained until joint vendor delete removes `_chartWidgetStoresC` wiring.

### Still blocked for joint delete (wave-10)

1. **Cluster ≠ Y alone** — Recharts+RTK+Immer spans pre-Y defs + `chartWidgetStoresY` (~50k).
2. **Post-Y react-intl (~4.6k)** — mega `chartWidgetStoresL` + Formatted* still inlined (unused by Chart/pie path).
3. **Codex Box wire still inside Y** — stubs at end of Y must survive vendor deletion (or already drained — verify before cut).
4. **No partial vendor cut** — delete pre-Y + Y + post-Y intl **together** via script/line-range.

See `_tmp-chart-widget-stores-drain/wave10-joint-delete-checklist.md`.

## Why it stays in `boundaries/`

Same as workbook-runtime: oversized flat dump + residual vendor body, parked as a documented vendor-runtime terminal so full-tree QG stays green. `openBoundary` remains until pre-Y+Y+post-Y intl deleted together and `setChartBundleSurface` call removed from mega.

## Exit criteria

1. ~~Migrate Chart off `Binding1022` / surface React / `Binding1317` onto public `recharts` + `react-intl` APIs.~~ **Done (wave-8).**
2. ~~Migrate pie off surface aliases onto public `recharts`.~~ **Done (wave-9).**
...
4. Drop `setChartBundleSurface` call from mega; promote remaining Codex glue; thin or remove this boundary; clear `openBoundary`.
...
- Chart + pie use public `recharts` + `react-intl` / `react`; surface bag is a no-op stub.
