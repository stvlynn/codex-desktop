...
var copper,
  ...
  environmentLabelsU = esmInit(() => {
    ...
    copper = useQuery(appScopeAtom, ({
      cwd,
      enabled,
      hostConfig,
      operationSource
    }) => AppInitialR3(cwd, getHostConfigId(hostConfig), hostConfig, operationSource, {
      enabled
    }), {
      key: alpha
    });
    environmentLabelsL = useQuery(appScopeAtom, ({
      cwd,
      enabled,
      hostConfig,
      operationSource,
      ...
      return getHostRpcClient("search-branches", xenon, xenon == null ? null : {
        root: xenon.root,
        operationSource,
        preserveRemoteRefs,
        query,
        limit: 20
      }, getHostConfigId(hostConfig), hostConfig, {
        enabled,
...
function harbor(vapor, wheat) {
  ...
  let {
      data
    } = AppInitialF3(vapor, wheat, acorn),
  ...
  let frost = iris => {
    vapor != null && data != null && iris?.status === "success" && AppInitialJ3(yarn, {
...
function kite(jewel, knoll) {
  ...
  let {
      data
    } = AppInitialF3(jewel, knoll, north),
...
function nickel(cliff) {
  return cliff?.type === "success" ? Math.max(cliff.stagedCount, cliff.unstagedCount) + (cliff.untrackedCount ?? 0) : 0;
}
var onyx = esmInit(() => {});
export function environmentLabelsR(dusk: unknown) {
  ...
  let [wind, yarrow] = umbra.useState(vine),
    ...
    canyon = gitRoot == null ? null : clearPendingWorktreeConversationStart({
      hostConfig,
      root: gitRoot
    });
  let dew = canyon,
  ...
  let grain = CodexBrowserSurfaceActionType(AppInitialRW, field),
    ...
    jadeite = CodexBrowserSurfaceActionType(ensureComposerEsm_E4_Init, localConversationId),
  ...
  let vista = upland,
    wisp = tor => {
      ...
      return errorType === "blocked-by-working-tree-changes" ? (yarrow(conflictedPaths ?? []), urn(nextAction), quillow(), oak(false), meadow(true), true) : false;
    };
  let yonder = wisp,
    zenith = async updraft => {
      if (!(nimbus || dew == null)) {
        ...
        try {
          ...
          localConversationId != null && ensureEnvironmentLabelsO3Init(localConversationId, updraft);
          quillow();
        } catch (weir) {
          let yard = weir;
    ...
    anvil = async anchor => {
      if (!(nimbus || dew == null)) try {
        ...
        localConversationId != null && ensureEnvironmentLabelsO3Init(localConversationId, anchor);
        oak(false);
      } catch (disc) {
        let edge = disc,
  ...
  let eddy = dome,
    fjord = () => {
      dew != null && (elm.set(GitActionIncludeUnstagedChanges, EditorDiffPage(dew), ""), birch("idle"), seed(at), meadow(false), quiet(true));
    };
  let glen = fjord,
    hearth = hinge => {
  ...
  let inlet = hearth;
  if (!ink) return shouldShow && grain.isSuccess ? renderControl({
    currentBranch: null,
    disabled: false,
    isPending: false,
    switchTooltipText: opal
  }) : null;
  if (gitRoot == null || isReadOnly) return renderStaticBranch?.({
    currentBranch: haven
  }) ?? null;
  ...
  let ledge = renderControl({
    ...
    mire = isle ? <EnvironmentLabelsHelper9 {...{
      currentBranch: haven,
      gitRoot,
      hostConfig,
      isOpen: grove,
  ...
  let oxbow = nest ? <EnvironmentLabelsHelper10 {...{
    open: nest,
    onOpenChange: oak,
  ...
  let pond = lagoon && gitRoot != null ? <EnvironmentLabelsHelper11 {...{
    open: lagoon,
    onOpenChange: eddy,
    conflictFiles: wind,
    gitRoot,
  ...
  let quarry = kernel && dew != null ? <EnsurePullRequestMediaBDInit key={rain} {...{
    open: petal,
    onOpenChange: quiet,
...
function EnvironmentLabelsHelper9(latch) {
  ...
  let {
      data
    } = environmentLabelsC(gitRoot, hostConfig, $, screw),
    torque = {
      enabled: isOpen
    };
  let {
  ...
  let {
      data: __data
    } = semanticAccentColors(gitRoot, hostConfig, $, valve),
    axle = nut.trim();
  let bracket = axle,
    clamp = rivet.trim();
  let drill = clamp,
  ...
  let {
    ...
    lever = jacket ? motor.formatMessage({
  ...
  let tappet = gib => gib === currentBranch && uncommittedFileCount > 0 ? <span className="inline-flex items-center gap-1 text-xs text-token-input-placeholder-foreground">
  ...
  let dowel = <MemoizedFormattedMessage {...{
...
function EnvironmentLabelsHelper10(hub) {
  ...
  let {
      ...
      isFetching,
      isError
    } = environmentLabelsO(gitRoot, hostConfig, $, orifice),
    pin,
    race,
    sleeve,
    trunnion;
  ...
  let boss = sleeve,
    ...
    detent = !isPending && !isLoading && !isFetching && !cam && trunnion.length > 0 && !race && !boss && gitRoot != null,
    eccentric = <DeferredUiU {...{
  ...
  let kingpin = <input autoFocus={true} className="h-10 w-full rounded-xl border border-token-border bg-token-dropdown-background px-3 text-sm text-token-foreground outline-none placeholder:text-token-description-foreground" value={lug} onChange={guide} onKeyDown={helix} placeholder={impeller} aria-label={journal} />;
  let land = race ? <p className="text-xs text-token-error-foreground">
      {<MemoizedFormattedMessage {...{
    ...
    </p> : boss && !isPending ? <p className="text-xs text-token-error-foreground">
      {<MemoizedFormattedMessage {...{
  ...
  let pad = <MemoizedFormattedMessage {...{
  ...
  let quillshaft = <ReadLoginRouteQuerySnapshot {...{
    color: "secondary",
    onClick: neck,
    children: pad
  }} />;
  ...
  let thrust = <MemoizedFormattedMessage {...{
  ...
  let yoke = <ReadLoginRouteQuerySnapshot {...{
    color: "primary",
    disabled: roller,
    loading: isPending,
    onClick: spindle,
