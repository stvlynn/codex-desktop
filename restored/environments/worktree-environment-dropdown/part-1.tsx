...
import { ensureComposerEsm_Hlt_Init, ensureComposerEsm_K1_Init, ensureComposerEsm_MT_Init, ensureComposerEsm_Sut_Init, ensureComposerEsm_Tft_Init, ensureComposerEsm_Wdt_Init } from "../../composer/composer-esm-inits";
...
import { ensureStarOutlineIconInit as EnsureStarOutlineIconInit, StarOutlineIcon } from "../../icons/star-outline-icon";
import { commonJsInit, esmInit } from "../../runtime/rolldown-runtime";
import { ensureLocalEnvironmentWorkflowMessagesInit, localEnvironmentWorkflowMessages } from "../../settings/local-environment-workflow-messages";
...
import { environmentLabelsC, environmentLabelsI, environmentLabelsL, environmentLabelsN, environmentLabelsR as EnvironmentLabelsR, environmentLabelsS, environmentLabelsT, environmentLabelsU } from "../environment-labels";
...
export function worktreeEnvironmentDropdownA(kelp: unknown) {
  ...
  let {
      ...
      isLoading,
      refetch: _refetch
    } = AppInitialEz(vapor, hostConfig, "async_task_starting_state_dropdown", iris),
  ...
  let {
      data: __data
    } = environmentLabelsC(vapor, hostConfig, "async_task_starting_state_dropdown", knoll),
    {
      branches,
  ...
  let {
    ...
    pine = startingState.type === "branch" ? startingState.remoteRef ?? startingState.branchName : undefined,
    quest = bloom ? _worktreeEnvironmentDropdownA : yarn,
    $ = bloom ? lunar ? orbit : branches : lunar ? data?.filter(canyon => canyon.toLowerCase().includes(_worktreeEnvironmentDropdownR)) : undefined,
    _worktreeEnvironmentDropdownN = !lunar || quest.toLowerCase().includes(_worktreeEnvironmentDropdownR),
    ridge = $?.filter(dew => dew !== quest),
    storm = acorn && lunar ? ___data?.remoteBranchRefs ?? [] : [];
  let tide = storm,
    unity = acorn && tide.length > 0,
    vale = bloom ? lunar ? _worktreeEnvironmentDropdownI : fetching : isFetching,
    wave = bloom ? lunar ? _worktreeEnvironmentDropdownT : _error : error,
    apex = bloom ? lunar ? __refetch : _worktreeEnvironmentDropdownO : refetch,
    brook = !bloom && hasNextPage,
    cliff = _data ?? quest,
    dusk = startingState.type === "branch" ? startingState.branchName : cliff,
    $e = __data?.type === "success" ? __data.stagedCount + __data.unstagedCount + (__data.untrackedCount ?? 0) > 0 : false,
    elm = bloom && $e,
    fern = startingState.type === "working-tree" && $e,
    grove = startingState.type === "working-tree" ? <MemoizedFormattedMessage {...{
  ...
  let at = <>
        ...
        {<ElectronOnly {...{
        extension: true,
        children: fern ? <MemoizedFormattedMessage {...{
  ...
  let meadow = <div className="flex flex-col gap-2">
      {<MemoizedFormattedMessage {...{
  ...
  let oak = startingState.type === "working-tree" && $e,
    petal = <CoerceLocalFilesystemPath {...{
      className: "icon-xs"
    }} />;
  let quiet = <ProjectPinnedActionsMenu borderColor="border-token-side-bar-background" badgeEnabled={oak}>
      {petal}
  ...
  let urn = <DropdownMenu.SearchInput {...{
    autoFocus: false,
    placeholder: rain,
    value: quill,
    onChange: seed,
  ...
  let _t = elm ? <div className="flex flex-col">
        ...
        {<DropdownMenu.Item {...{
        LeftIcon: alpha,
        RightIcon: startingState.type === "working-tree" ? AppIconZlt : undefined,
        SubText: $e ? <span className="text-token-description-foreground">
                  {<MemoizedFormattedMessage {...{
    ...
    vine = <WorktreeEnvironmentDropdownHelper4 {...{
      children: acorn ? <MemoizedFormattedMessage {...{
  ...
  let wind = drift && vale ? <div className="flex h-full items-center justify-center">
          {<VSCODE_EDITOR_ID {...{
        className: "icon-xxs"
      }} />}
        </div> : drift && wave ? <div className="flex h-full flex-col items-center justify-center gap-1 text-sm text-token-error-foreground">
          {<MemoizedFormattedMessage {...{
        ...
        children: <AppIconAlt {...{
          ...
          {_worktreeEnvironmentDropdownN && <DropdownMenu.Item {...{
        ...
        children: quest
          ...
          {ridge?.map(ever => <DropdownMenu.Item key={ever} {...{
        LeftIcon: CoerceLocalFilesystemPath,
        ...
        children: ever
          ...
          children: <MemoizedFormattedMessage {...{
            ...
            description: "Section heading for fetched remote branch search results"
              ...
              {tide.map(item => <DropdownMenu.Item key={item} {...{
          LeftIcon: CoerceLocalFilesystemPath,
    ...
    yarrow = <div className="vertical-scroll-fade-mask flex h-[200px] flex-col gap-1.5 overflow-y-auto">
        {_t}
        {vine}
        {wind}
      </div>;
  let azure;
  return azure = <div className="flex w-72 flex-col gap-1.5 overflow-hidden">
        {urn}
        {yarrow}
    ...
    triggerButton: <OptionalTooltip {...{
      ...
      children: <ComposerCategoryValueChip data-composer-navigation-target="starting-state" categoryLabel={null} className={nest} collapse="secondary" icon={quiet} indicator="collapsible-chevron" value={at} valueClassName="max-w-40" />
    }} />,
    children: azure
  }} />;
}
