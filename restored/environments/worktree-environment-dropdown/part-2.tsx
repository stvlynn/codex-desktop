...
export function worktreeEnvironmentDropdownT(grain: unknown) {
    ...
    vista;
  {
    ...
    let hearth = glen,
      inlet = environments.find(item => toPosixPath(item.configPath) === hearth) ?? null,
      jetty = !isLoading && !hasError,
      knob = dome && showDefaultOption ? crag.filter(item => toPosixPath(item.configPath) !== toPosixPath(dome.configPath)) : crag,
      ledge = nickel({
        isLoading,
    ...
    opal = side;
    plume = align;
    let mire = <MemoizedFormattedMessage {...{
    ...
    let oxbow = labelClassName == null ? "secondary" : "sm",
      pond = showIcon ? <DeferredConversationWH3 {...{
        className: "icon-xs"
      }} /> : null;
    let quarry = isLoading ? "pending" : "collapsible-chevron",
      rapids;
    rapids = IntlProvider("max-w-40", labelClassName);
    quillow = <OptionalTooltip {...{
      tooltipContent: mire,
      children: <ComposerCategoryValueChip data-composer-navigation-target="environment" categoryLabel={null} className={nook} collapse={oxbow} icon={pond} indicator={quarry} value={ledge} valueClassName={rapids} />
    }} />;
    vista = "flex w-64 flex-col overflow-hidden";
    leaf = <DropdownMenu.Title {...{
    ...
    root = "vertical-scroll-fade-mask flex max-h-[220px] flex-col overflow-y-auto";
    silk = jetty ? <DropdownMenu.Item {...{
      RightIcon: selectedConfigPath == null && repairConfigPath == null ? AppIconZlt : undefined,
      onClick: () => {
        haven(null);
jadeite(false);
      },
    ...
    thorn = showDefaultOption && dome ? <DropdownMenu.Item {...{
      RightIcon: fjord != null && toPosixPath(dome.configPath) === fjord ? AppIconZlt : undefined,
      onClick: () => {
        haven(dome.configPath);
        jadeite(false);
      },
      children: <div className="flex min-w-0 items-center gap-2">
                {<OptionalTooltip {...{
          ...
          children: <EnsureStarOutlineIconInit {...{
            className: "icon-xxs shrink-0 text-token-description-foreground"
          }} />
        }} />}
                <span className="truncate">
                  {environmentLabelsT(dome)}
                </span>
              </div>
    }} /> : null;
    upland = isLoading ? <div className="flex items-center justify-center py-4">
        {<VSCODE_EDITOR_ID {...{
        className: "icon-xxs"
      }} />}
      </div> : hasError ? <DropdownMenu.Message {...{
      compact: true,
      tone: "error",
      children: <MemoizedFormattedMessage {...{
    ...
    }} /> : knob.length > 0 ? <div className="flex flex-col">
        {knob.map(item => {
        let spur = selectedConfigPath != null && toPosixPath(item.configPath) === fjord;
        return <DropdownMenu.Item key={item.configPath} {...{
          RightIcon: spur ? AppIconZlt : undefined,
          onClick: () => {
            haven(item.configPath);
            jadeite(false);
          },
          children: <span className="min-w-0 truncate">
                    {environmentLabelsT(item)}
                  </span>
        }} />;
      })}
      </div> : environments.length > 0 && crag.length === 0 ? <DropdownMenu.Message {...{
      compact: true,
      tone: "error",
      children: <MemoizedFormattedMessage {...{
        ...localEnvironmentWorkflowMessages.needsAttention
      }} />
    }} /> : environments.length === 0 ? <DropdownMenu.Message {...{
      compact: true,
      children: <MemoizedFormattedMessage {...{
  ...
  let yonder = <MemoizedFormattedMessage {...{
  ...
  let zenith = <DropdownMenu.Item {...{
    LeftIcon: AppIconKtt,
    onClick: wisp,
    children: yonder
  }} />;
  let anvil = <div className={root}>
      {silk}
      {thorn}
      {upland}
      {zenith}
    </div>;
  let beacon = <div className={vista}>
      {leaf}
      {anvil}
    </div>;
...
function nickel({
  ...
  return isLoading ? <MemoizedFormattedMessage {...{
  ...
  }} /> : environmentResult?.type === "error" ? <MemoizedFormattedMessage {...{
    ...localEnvironmentWorkflowMessages.needsAttention
  }} /> : environmentResult?.type === "success" ? <>{environmentResult.environment.name}</> : <MemoizedFormattedMessage {...{
