...
export function workspaceScopedRequestKey(
  input: WorkspaceScopedRequestKey,
): string {
  return JSON.stringify([input.hostId, input.cwd]);
}

/** Create (and register) an AbortSignal for a workspace-scoped request. */
export function createWorkspaceScopedAbortSignal(
  input: WorkspaceScopedRequestKey,
): AbortSignal {
...
export function abortWorkspaceScopedRequest(
  input: WorkspaceScopedRequestKey,
): void {
...
export function clearWorkspaceScopedAbortSignal(
  input: WorkspaceScopedRequestKey,
  signal: AbortSignal,
): void {
