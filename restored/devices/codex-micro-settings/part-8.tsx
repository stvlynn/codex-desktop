...
function lagoon(nest) {
  let { keycap, skills, slot, voiceButtonMode } = nest,
    oak = useIntl(),
    ...
    yarrow;
  {
    ...
    let dew = trail;
    dew == null &&
      keycap.action.type === "command" &&
      (dew = getCodexMicroWebviewCommands(keycap.action.command));
        ...
        field = (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Availability description for Codex Micro composer text keycaps",
            }}
          />
        );
        rain = field;
        ...
        grain = (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Description for an unassigned Codex Micro custom keycap",
            }}
          />
        );
        rain = grain;
    ...
    let ever;
    ever = gamma(keycap);
    yarrow = <span className="font-mono text-xs font-semibold">{ever}</span>;
    urn = "text-xs";
    vine =
      petal == null ? (
        trail == null ? (
          ...
          collaborationModeFromTurnOrParams(trail, oak)
        )
      ) : (
        ensureCodexMicroAnalogActionTitleInit(petal, skills, oak)
      );
  }
  let azure = <span className={urn}>{vine}</span>;
  let birch = petal == null ? (seed ?? (trail == null ? rain : null)) : null,
    canyon = (
      <span className="text-xs text-token-description-foreground">{birch}</span>
    );
  return (
    <div className={wind}>
      {yarrow}
...
function meadow(jadeite) {
    ...
    nook;
  {
    let cog = codexMicroLayoutR(maple),
      disc =
        plume?.type === "command"
          ? getCodexMicroWebviewCommands(plume.commandId)
          : null;
    beacon =
      cog.action.type === "custom-shortcut"
        ? kernel.formatMessage({
            id: "settings.codexMicro.editor.customShortcut.none",
            defaultMessage: "Unassigned",
            description:
              "Dropdown option that clears a Codex Micro keycap command",
          })
        : kernel.formatMessage({
            id: "settings.codexMicro.editor.customShortcut.useDefault",
            defaultMessage: "Use keycap default",
            description:
              "Dropdown option that restores a Codex Micro keycap's predefined action",
          });
                      ...
                      children: (
                        <MemoizedFormattedMessage
                          {...{
                            ...
                            description:
                              "Subtitle for the Codex Micro keycap slot editor",
                            values: {
                              slotId,
                            },
                          }}
    ...
    let joint = kernel.formatMessage({
      ...
      description:
        "Accessible label for searching Codex Micro keycaps in the keycap editor",
    });
    let keystone = kernel.formatMessage({
      id: "settings.codexMicro.editor.searchPlaceholder",
      defaultMessage: "Search keycaps",
      description:
        "Placeholder for searching Codex Micro keycaps in the keycap editor",
    });
    let latch;
    latch = (event) => {
    ...
    jetty = (
      <div className="flex h-10 shrink-0 items-center gap-2 rounded-lg border border-token-input-border bg-token-input-background/75 px-3 text-sm text-token-input-foreground">
        {iron}
        <input
          autoFocus={true}
    ...
    let motor =
      gear.length === 0 ? (
        <div className="bg-token-surface-secondary flex min-h-28 items-center justify-center rounded-lg border border-token-border px-4 text-sm text-token-description-foreground">
          {
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Empty state shown when keycap search returns no results",
              }}
            />
          }
        </div>
      ) : (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(4.75rem,4.75rem))] justify-between gap-2">
          {gear.map((item) => {
            let piston = item.id === cog.id,
              rivet = cliff.find(
                (_item) => layout.slots[_item.slotId].keycapId === item.id,
              );
            return (
              <OptionalTooltip
                key={item.id}
                  ...
                  children: (
                    <button
                      ...
                      onClick={() => {
                        ...
                        nimbus(item.id);
                        let screw =
                          rivet == null ? null : layout.slots[rivet.slotId];
                        quillow(
                          screw?.action ??
                            (screw?.commandId == null
                              ? undefined
                              : {
                                  type: "command",
    ...
    knob = (
      <div className="vertical-scroll-fade-mask min-h-0 flex-1 overflow-y-auto px-1 pt-1 pb-1 [--edge-fade-distance:1rem]">
        {motor}
      </div>
    );
    ...
    glen = (
      <MemoizedFormattedMessage
        {...{
          ...
          description:
            "Label for choosing the shortcut or skill assigned to a Codex Micro keycap",
        }}
      />
    );
    hearth =
      plume?.type === "skill" ? (
        ensureCodexMicroAnalogActionTitleInit(plume, skills, kernel)
      ) : disc == null ? (
        cog.action.type === "custom-shortcut" ? (
          <MemoizedFormattedMessage
            {...{
              ...
              defaultMessage: "Unassigned",
              description:
                "Label shown when a Codex Micro keycap has no command assigned",
            }}
          />
        ) : (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Description shown when a Codex Micro keycap uses its predefined action",
              values: {
                action: (
                  <CodexMicroSettingsHelper25
                    key={"action"}
    ...
    <DropdownTriggerButton>
      {
        <MemoizedFormattedMessage
          {...{
            ...
            description:
              "Button label for choosing the command assigned to a Codex Micro keycap",
          }}
        />
      }
    </DropdownTriggerButton>
  ...
  let verge = (
    <MemoizedFormattedMessage
      {...{
        ...
        description:
          "Cancel button label for the Codex Micro keycap slot editor",
      }}
    />
  );
  let weir = (
        ...
        children: (
          <MemoizedFormattedMessage
            {...{
              ...
              defaultMessage: "Save",
              description:
                "Save button label for the Codex Micro keycap slot editor",
            }}
          />
        ),
      }}
