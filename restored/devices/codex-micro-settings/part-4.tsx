...
function wave(dusk) {
  let { agentKeyId, commands, skills, onOpenChange } = dusk,
    ...
    [isle, juniper] = slate.useState(
      hill == null || "threadKey" in hill
        ? "chat"
        : hill.type === "command"
          ? "shortcut"
          : hill.type,
    ...
    nest = lagoon.trim();
  let oak = nest,
    { loading: petal, options } = gamma(isle === "chat" ? oak : ""),
    quiet,
    rain,
    seed,
    trail,
    ...
    jadeite;
  {
    ...
    bb0: switch (isle) {
      ...
      case "keycap": {
        ...
        glen = (hearth) => {
          ...
          return oak.length > 0 &&
            ![hearth.id, inlet, ...codexMicroLayoutU(hearth)].some(
              (item) => ensureKeyboardShortcutsHMInit(item, oak) > 0,
            )
            ? []
            : [
                {
      ...
      zenith = fern.formatMessage(
        {
          ...
          defaultMessage:
            "Search {type, select, chat {chats} shortcut {shortcuts} keycap {keycaps} skill {skills} other {assignments}} to assign",
          description:
            "Accessible label for searching Codex Micro agent-key assignments by assignment type",
        },
        {
          type: isle,
        },
          ...
          children: (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Title of the Codex Micro custom agent-key assignment dialog",
                values: {
                  position: beacon,
                },
              }}
          ...
          children: (
            <MemoizedFormattedMessage
              {...{
                ...
                defaultMessage:
                  "Choose a chat, shortcut, keycap, or skill for this key",
                description:
                  "Description of the Codex Micro custom agent-key assignment dialog",
              }}
            />
          ),
        }}
    ...
    fjord = fern.formatMessage(
      {
        ...
        defaultMessage:
          "Search {type, select, chat {chats} shortcut {shortcuts} keycap {keycaps} skill {skills} other {assignments}}",
        description:
          "Placeholder for searching Codex Micro agent-key assignments by assignment type",
      },
      {
        type: isle,
      },
    ...
    dew =
      isle === "chat" && petal ? (
        <div className="flex h-24 items-center justify-center text-token-text-secondary">
          {
            <VSCODE_EDITOR_ID
              {...{
                className: "icon-xs",
            ...
            children: (
              <MemoizedFormattedMessage
                {...{
                  ...
                  description:
                    "Empty state for the Codex Micro custom agent-key task search",
                }}
              />
            ),
          }}
                ...
                {oak.length === 0 ? (
                  <MemoizedFormattedMessage
                    {...{
                      ...
                      description:
                        "Heading for recent tasks in the Codex Micro custom assignment dialog",
                    }}
                  />
                ) : (
                  <MemoizedFormattedMessage
                    {...{
                      ...
                      description:
                        "Heading for task search results in the Codex Micro custom assignment dialog",
                    }}
                  />
                )}
              </span>
              ...
              let { assignment, result } = item,
                ...
                pond = ARTIFACT_GENERATION_STAGE_IDS.find((_item) => {
                  ...
                  return (
                    tor != null &&
                    "threadKey" in tor &&
                    tor.threadKey === threadKey
                  );
                }),
                quarry =
                  pond == null
                    ? null
                    : ARTIFACT_GENERATION_STAGE_IDS.indexOf(pond) + 1,
                rapids =
                  result.title ??
                  fern.formatMessage({
                    ...
                    description:
                      "Fallback title for an untitled task in the Codex Micro custom assignment dialog",
                  }),
                spur =
                  result.kind === "local"
                    ? (result.projectLabel ?? result.cwd)
                    : (result.projectLabel ?? result.environmentLabel);
              return (
                        ...
                        <span className="text-xs text-token-text-secondary">
                          {
                            <MemoizedFormattedMessage
                              {...{
                                ...
                                description:
                                  "Label for the Codex Micro agent key currently assigned to a task",
                                values: {
                                  position: quarry,
                                },
                              }}
            ...
            children: (
              <MemoizedFormattedMessage
                {...{
                  ...
                  defaultMessage:
                    "No matching {type, select, shortcut {shortcuts} keycap {keycaps} skill {skills} other {actions}}",
                  description:
                    "Empty state when searching actions for a Codex Micro agent key",
                  values: {
                    type: isle,
                  },
                }}
              ...
              children: item.options.map((_item) => (
                <SettingsCommandMenuSectionItem
                  ...
                  {...{
                    ...
                    LeftIcon:
                      _item.assignment.type === "skill"
                        ? LoadingPreviewIcon
                        : _item.assignment.type === "command"
                          ? AppIconOi
                          : undefined,
                    leftAccessory:
                      _item.keycap == null ? undefined : (
                        <CodexMicroKeyboardSurfaceLabel
                          {...{
                            keycap: _item.keycap,
                    ...
                    rightAccessory:
                      hill != null &&
                      "type" in hill &&
                      hill.type === _item.assignment.type &&
                      yonder === _item.value ? (
                        <AppIconZlt
                          {...{
                            className: "icon-xs text-token-text-primary",
  ...
  let nimbus = umbra[isle],
    opal = (
      <DropdownTriggerButton className="w-36">
        {
          <MemoizedFormattedMessage
            {...{
              ...nimbus,
          ...
          children: (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Button that removes a custom Codex Micro agent-key assignment",
              }}
            />
          ),
        }}
  ...
  let thorn = (
    <MemoizedFormattedMessage
      {...{
        ...
        description:
          "Button that closes the Codex Micro custom agent-key assignment dialog",
      }}
    />
  );
  let upland = (
...
function cliff(weir) {
  let { command, description, title } = weir;
  return {
    assignment: {
      type: "command",
