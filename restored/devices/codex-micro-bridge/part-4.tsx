...
function nickel(reef, sage = window) {
  ...
  (topaz != null && (!(sage instanceof Element) || sage.contains(topaz))
    ? topaz
    : sage
  ).dispatchEvent(
...
function pearl(wheat, yarn) {
  ...
  let acorn = document.activeElement,
    bloom =
      acorn instanceof HTMLButtonElement && yarn.contains(acorn)
        ? zephyr.indexOf(acorn)
        : zephyr.length - 1,
  ...
  zephyr[
    (coral + (wheat === "ArrowDown" ? 1 : -1) + zephyr.length) % zephyr.length
  ]?.focus();
}
function on(drift) {
  if (yellow() != null) return;
  let eagle =
    ensureConversationPageEsm_GZ_Init()?.root.closest(
      AppActionSelector.timelineScroll,
    ) ?? document.querySelector(AppActionSelector.timelineScroll);
...
function river() {
  return (
    ...
    ).find((item) => item.querySelector(jasper) != null) ?? null
  );
}
function slate(glide) {
  return (
    glide.querySelector(kelp) != null &&
    glide.querySelector(jasper)?.getAttribute("aria-expanded") === "false"
  );
}
...
function xenon(lunar) {
  return Array.from(lunar.querySelectorAll(prism)).filter(
