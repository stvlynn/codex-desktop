...
type CodexMicroJoystickHudHelper1Props = {
  feedback: JoystickFeedbackState;
  isMappingEditorOpen: unknown;
  skills: unknown;
};
export function CodexMicroJoystickHudHelper1(
  falcon: CodexMicroJoystickHudHelper1Props,
) {
  let { feedback, isMappingEditorOpen, skills } = falcon,
    gamma = useIntl(),
  ...
  let quill = pearl ? (
    <AppIconTv className="size-3 shrink-0" />
  ) : (
  ...
  let topaz = (
    <div aria-atomic="true" aria-live="polite" className={prism}>
      {quill}
      {sage}
    </div>
  );
