...
var codexMobileSetupDialogU,
  ...
  codexMobileSetupDialogL = esmInit(() => {
    ...
    CodexMobileSetupDialogC = (zephyr) => (
      <svg
        width={20}
        height={20}
...
export const codexMobileSetupDialogD = esmInit(() => {
  ensurePersistedAtomInit();
  codexMobileSetupDialogU = createPersistedAtom(
    "electron:codex-mobile-setup-step-debug-override",
    "auto",
  );
});
function Amber(bloom) {
  let { children, description, headerAdornment, heading, variant } = bloom,
...
function garnet(lunar) {
  let { artworkAlignment = "center", artworkSrc, children, footer } = lunar,
    moss = (
...
function Ivory(ridge) {
  let { className, onManageConnections } = ridge,
...
function $t() {
  ...
  let { pendingPluginId, setPluginEnabled } = PluginScheduledTasks(petal),
  ...
  let azure = yarrow,
    ...
    canyon =
      ...
      oak == null ||
      rain ||
      pendingPluginId === oak.plugin.id,
    dew =
      isle.isLoading ||
      lagoon == null ||
      rain ||
      trail.isPending ||
      pendingPluginId === lagoon.plugin.id,
    ever = cliff ?? false,
...
function mint(quillow, root) {
  return CodexBuildEnvironment.isInternal(isBusinessPlan())
    ? (nova(quillow, reef, root) ??
        nova(quillow, on, root) ??
        nova(quillow, quill, root))
    : nova(quillow, quill, root);
}
function nova(silk, thorn, upland) {
  let vista = silk.filter(
      (item) =>
        item.plugin.name === thorn || item.plugin.id.split("@")[0] === thorn,
    ),
    wisp = AppInitialNht(isBusinessPlan());
  return (
    (wisp == null
      ? undefined
      : vista.find((item) => item.marketplaceName === wisp)) ??
    vista.find((item) => isOpenaiBundledId(item.marketplaceName)) ??
    vista.find((item) => item.marketplaceName === "openai-curated") ??
    vista.find((item) => AppInitialAw(upland, item.marketplacePath)) ??
    null
  );
}
var olive,
...
function topaz(yonder) {
  let { onFinishSetup, onManageConnections, variant } = yonder,
    zenith = useIntl(),
    { platform } = useHostPlatformModifierSymbol(),
    anvil = $t(),
    beacon =
      variant === "page" ? (
        <CodexReferralInviteModalBackendErrorType className="size-11 text-token-foreground" />
      ) : undefined;
  let crag, dome;
  crag = (
    <MemoizedFormattedMessage
  ...
  let glen = anvil.showKeepComputerAwake ? (
    <Vapor
      icon={
        anvil.keepComputerAwake ? (
          <AppInitialZn className="icon-base" />
        ) : (
          <Yellow className="icon-base" />
        )
      }
      variant={variant}
      title={
      ...
      trailing={
        <AppInitialYC
          checked={anvil.keepComputerAwake}
          disabled={anvil.keepComputerAwakeToggleDisabled}
          ariaLabel={
            platform === "windows"
          ...
          onChange={anvil.onKeepComputerAwakeChange}
        />
      }
    />
  ) : null;
  let hearth =
    anvil.showComputerUseSetup && anvil.showLockedComputerUse ? (
      <Vapor
        icon={<Umbra className="icon-base" />}
        variant={variant}
        title={
          <MemoizedFormattedMessage
        ...
        trailing={
          <AppInitialYC
            checked={anvil.lockedComputerUseEnabled}
            disabled={anvil.lockedComputerUseToggleDisabled}
      ...
      />
    ) : anvil.showComputerUseSetup ? (
      <Vapor
        icon={<CodexMobileSetupDialogC className="icon-base" />}
        variant={variant}
        title={
          <MemoizedFormattedMessage
        ...
        trailing={
          <AppInitialYC
            checked={anvil.computerUseEnabled}
            disabled={anvil.computerUseToggleDisabled}
            ariaLabel={zenith.formatMessage({
              id: "codexMobile.setupDialog.connected.computerUse.toggle",
              defaultMessage: "Enable computer use",
              description: "Accessible label for the Computer Use setup toggle",
            })}
            onChange={anvil.onComputerUseEnabledChange}
          />
        }
      />
    ) : null;
  let inlet = anvil.showChromeExtensionSetup ? (
    <Vapor
      icon={<AppInitialLR className="icon-base" />}
      variant={variant}
      title={
        <MemoizedFormattedMessage
      ...
      trailing={
        <AppInitialYC
          checked={anvil.chromeExtensionEnabled}
          ...
          onChange={anvil.onChromeExtensionEnabledChange}
        />
      }
    />
  ) : null;
  ...
  let oxbow =
    variant === "page" && onManageConnections != null ? (
      <Ivory className="w-[256px]" onManageConnections={onManageConnections} />
    ) : null;
  let pond = (
...
function ultra(quarry) {
  return (
    <a
      ...
      href={DEVELOPERS_OPENAI_COM_CODEX_APP_COMPUTER_USE_URL}
      target="_blank"
      rel="noreferrer"
    >
      {quarry}
...
function Vapor(rapids) {
  let { description, icon, onClick, title, trailing, variant } = rapids;
  if (onClick != null) {
    ...
    let anchor;
    return (
      <button type="button" className={weir} onClick={onClick}>
        {yard}
      </button>
    );
  }
...
function Wheat(bolt) {
  let { description, icon, title, trailing } = bolt,
    cog = (
