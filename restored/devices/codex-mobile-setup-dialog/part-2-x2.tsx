...
var rainThird = esmInit((wispSecond) => {
  ...
  canyonThird = esmInit((novaSecond) => {
    ...
    novaSecond.getCharCountIndicator = function (screwPrime, torquePrime) {
      ...
      if (!basaltThird.isValid(torquePrime))
        throw Error("Invalid version: " + torquePrime);
      return torquePrime >= 1 && torquePrime < 10
        ? screwPrime.ccBits[0]
        : torquePrime < 27
          ? screwPrime.ccBits[1]
          : screwPrime.ccBits[2];
    ...
    novaSecond.getBestModeForData = function (leverPrime) {
      return daisyThird.testNumeric(leverPrime)
        ? novaSecond.NUMERIC
        : daisyThird.testAlphanumeric(leverPrime)
          ? novaSecond.ALPHANUMERIC
          : daisyThird.testKanji(leverPrime)
            ? novaSecond.KANJI
            : novaSecond.BYTE;
    };
  ...
  nimbusThird = esmInit((alphaSecond) => {
    ...
    function birchPrime(acornSecond, bloomSecond, coralSecond) {
      for (let anchorFourth = 1; anchorFourth <= 40; anchorFourth++)
        if (
          bloomSecond <=
          alphaSecond.getCapacity(anchorFourth, coralSecond, acornSecond)
        )
          return anchorFourth;
    }
    ...
    function dewPrime(deltaSecond, echoSecond) {
      let jadeiteFourth = 0;
      return (
        deltaSecond.forEach(function (item) {
    ...
    function everPrime(quillSecond, reefSecond) {
      for (let quarryFourth = 1; quarryFourth <= 40; quarryFourth++)
        if (
          dewPrime(quillSecond, quarryFourth) <=
          alphaSecond.getCapacity(quarryFourth, reefSecond, violetThird.MIXED)
        )
          return quarryFourth;
    }
    alphaSecond.from = function (duskSecond, elmSecond) {
      return willowThird.isValid(duskSecond)
        ? parseInt(duskSecond, 10)
        : elmSecond;
    };
    alphaSecond.getCapacity = function (nimbusPrime, opalPrime, plumePrime) {
      if (!willowThird.isValid(nimbusPrime))
        throw Error("Invalid QR Code version");
      plumePrime === undefined && (plumePrime = violetThird.BYTE);
      let cogThird =
        (slateThird.getSymbolTotalCodewords(nimbusPrime) -
          timberThird.getTotalCodewordsCount(nimbusPrime, opalPrime)) *
        8;
    ...
    alphaSecond.getEncodedBits = function (gasketPrime) {
      if (!willowThird.isValid(gasketPrime) || gasketPrime < 7)
        throw Error("Invalid QR Code version");
      let quietFourth = gasketPrime << 12;
      for (; slateThird.getBCHDigit(quietFourth) - xenonThird >= 0; )
        quietFourth ^=
          7973 << (slateThird.getBCHDigit(quietFourth) - xenonThird);
  ...
  zenithThird = esmInit((ironPrime) => {
    ...
    ironPrime.getEncodedBits = function (gammaSecond, indigoSecond) {
      ...
      for (; mossFourth.getBCHDigit(nimbusFourth) - northFourth >= 0; )
        nimbusFourth ^=
          1335 << (mossFourth.getBCHDigit(nimbusFourth) - northFourth);
  ...
  anvilThird = esmInit((hearthSecond, inletSecond) => {
    ...
    ridgePrime.getBitsLength = function (northSecond) {
      return (
        10 * Math.floor(northSecond / 3) +
        (northSecond % 3 ? (northSecond % 3) * 3 + 1 : 0)
      );
    };
    ...
    ridgePrime.prototype.write = function (weirPrime) {
      ...
      for (
        wheatFourth = 0;
        wheatFourth + 3 <= this.data.length;
        wheatFourth += 3
      ) {
      ...
      acornFourth > 0 &&
        ((yarnFourth = this.data.substr(wheatFourth)),
        (zephyrFourth = parseInt(yarnFourth, 10)),
        weirPrime.put(zephyrFourth, acornFourth * 3 + 1));
    };
  ...
  jettyThird = esmInit((jettySecond, knobSecond) => {
    ...
    stormPrime.prototype.write = function (rivetPrime) {
      ...
      this.data.length % 2 &&
        rivetPrime.put(yarrowThird.indexOf(this.data[apexFourth]), 6);
    };
  ...
  updraftThird = esmInit((havenPrime, inkPrime) => {
    ...
    function wavePrime(lemonSecond) {
      ...
      typeof lemonSecond == "string"
        ? (this.data = new TextEncoder().encode(lemonSecond))
        : (this.data = new Uint8Array(lemonSecond));
    }
    ...
    wavePrime.prototype.write = function (ultraSecond) {
      for (
        let updraftFourth = 0, vergeFourth = this.data.length;
        updraftFourth < vergeFourth;
        updraftFourth++
      )
        ultraSecond.put(this.data[updraftFourth], 8);
    };
  ...
  gearThird = esmInit((yonderSecond, zenithSecond) => {
    ...
    pinePrime.prototype.write = function (fjordPrime) {
      ...
      for (copperFourth = 0; copperFourth < this.data.length; copperFourth++) {
        ...
        if (zincFourth >= 33088 && zincFourth <= 40956) zincFourth -= 33088;
        else if (zincFourth >= 57408 && zincFourth <= 60351)
          zincFourth -= 49472;
        else
          throw Error(
            "Invalid SJIS character: " +
              this.data[copperFourth] +
    ...
    var yellowThird = {
      single_source_shortest_paths: function (
        ...
        for (
          ...
          for (quillowThird in ((opalThird = mapleThird.pop()),
          (plumeThird = opalThird.value),
          (rootThird = opalThird.cost),
          weirSecond[plumeThird] || {}))
            silkThird.hasOwnProperty(quillowThird) &&
              ((thornThird = silkThird[quillowThird]),
              (uplandThird = rootThird + thornThird),
              (vistaThird = leafThird[quillowThird]),
              (wispThird = leafThird[quillowThird] === undefined),
              (wispThird || vistaThird > uplandThird) &&
                ((leafThird[quillowThird] = uplandThird),
                mapleThird.push(quillowThird, uplandThird),
                (kernelThird[quillowThird] = plumeThird)));
        if (
          anchorSecond !== undefined &&
          leafThird[anchorSecond] === undefined
        ) {
          var yonderThird = [
      ...
      find_path: function (timberSecond, umbraSecond, violetSecond) {
        var wispFourth = yellowThird.single_source_shortest_paths(
          timberSecond,
          umbraSecond,
          violetSecond,
        );
        return yellowThird.extract_shortest_path_from_predecessor_list(
          wispFourth,
          violetSecond,
        );
      },
      PriorityQueue: {
        make: function (enginePrime) {
          var juniperFourth = yellowThird.PriorityQueue,
            lagoonFourth = {},
            meadowFourth;
          for (meadowFourth in ((enginePrime ||= {}), juniperFourth))
            juniperFourth.hasOwnProperty(meadowFourth) &&
              (lagoonFourth[meadowFourth] = juniperFourth[meadowFourth]);
          return (
            (lagoonFourth.queue = []),
            (lagoonFourth.sorter =
              enginePrime.sorter || juniperFourth.default_sorter),
            lagoonFourth
          );
        },
