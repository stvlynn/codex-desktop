...
export function codexMicroKeyboardSurfaceO(amber: unknown) {
  let {
    ...
    daisy =
      assignmentTitle ??
      (slot?.threadKey == null
        ? basalt.formatMessage(
            {
              id: "settings.codexMicro.agentKeyPreview.unassigned",
              defaultMessage:
                "No chat or action assigned to agent key {position}",
              description:
                "Fallback title for an unassigned Codex Micro agent key",
            },
            {
              position,
            },
        ...
        : (slot.title ??
          basalt.formatMessage({
            ...
            description:
              "Fallback title for a Codex Micro agent key whose task has no title",
          })));
  let ember = daisy,
    flint = CodexAutomationCapabilityOrigin(cedar)
      .toString(16)
      .padStart(6, "0");
  let garnet = `#${flint}`,
    hazel = cedar === "off",
  ...
  let prism = (
    <div aria-label={kelp} className={mint} role={nova}>
      {olive}
    </div>
  );
  let quill = prism,
    reef =
      onClick == null ? (
        ...
        <button
          type="button"
          aria-label={basalt.formatMessage(
            {
              id: "settings.codexMicro.agentKeyPreview.assign",
              defaultMessage:
                "Assign a chat, shortcut, keycap, or skill to agent key {position}",
              description:
                "Accessible label for assigning a task to a Codex Micro agent key",
            },
            {
              position,
            },
  ...
  let vapor = ultra,
    wheat = (
      <span className="font-medium text-token-text-primary">{ember}</span>
    );
  ...
  let coral = (
    <span className="flex items-center gap-1.5 text-xs text-token-text-secondary">
      {bloom}
      {vapor}
    </span>
  );
  let drift = (
    <div className="flex max-w-64 flex-col gap-1.5 p-1">
      {wheat}
      {coral}
    </div>
  );
