...
const CodexMicroKeyboardSurfaceL: any = undefined;
const CodexMicroKeyboardSurfaceC: any = undefined;
...
/** Wave FZ unresolved companion (missing-export:conversation/local-conversation-git-actions3.tsx) */
const LocalConversationGitActions3: any = undefined;
/** Wave FZ unresolved companion (missing-export:ui/deferred-ui-e.ts) */
const deferredUiE: any = undefined;
/** Wave FZ unresolved companion (missing-export:review/line-info.tsx) */
const AppInitialEL: any = undefined;
/** Wave FZ unresolved companion (jsx-collision:copyContinuousScale@d3/copy-continuous-scale.ts) */
...
const AppInitialGD: any = undefined;
...
/** Wave FZ unresolved companion (missing-export:ui/deferred-ui-lc.ts) */
...
/** Wave FZ unresolved companion (missing-export:ui/deferred-ui-uc.ts) */
...
var codexMicroKeyboardSurfaceD,
  ...
  lemon = esmInit(() => {
    react();
    kite = xenon => <svg width={20} height={20} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" {...xenon}>
        <path fillRule="evenodd" clipRule="evenodd" d="M5.27916 5.50684C5.70791 4.58738 6.94757 4.34064 7.6942 5.08691L14.9129 12.3057L15.0487 12.4609C15.6235 13.2137 15.3513 14.3204 14.493 14.7207L14.3006 14.7949L3.79967 18.0771C2.64613 18.4371 1.56223 17.3528 1.92272 16.1992L5.20397 5.69922L5.27916 5.50684ZM3.19225 16.5967C3.15211 16.726 3.27383 16.8477 3.40318 16.8076L9.08873 15.0293L4.96861 10.9092L3.19225 16.5967ZM6.75377 6.02734C6.6755 5.9494 6.54869 5.96802 6.49303 6.05469L6.4735 6.0957L5.41393 9.4834C5.43241 9.49826 5.4524 9.5122 5.46959 9.5293L10.4696 14.5293L10.5145 14.584L13.9032 13.5254L13.9452 13.5068C14.0195 13.459 14.0437 13.359 13.9999 13.2822L13.9725 13.2461L6.75377 6.02734Z" fill="currentColor" />
        <path d="M17.0829 8.75C17.7732 8.75 18.3329 9.30964 18.3329 10C18.3329 10.6904 17.7732 11.25 17.0829 11.25C16.3927 11.2498 15.8329 10.6902 15.8329 10C15.8329 9.30976 16.3927 8.75019 17.0829 8.75Z" fill="currentColor" />
        <path d="M17.4999 2.66797C17.8669 2.66814 18.1647 2.96599 18.1649 3.33301C18.1649 6.00146 16.0013 8.16504 13.3329 8.16504C12.9659 8.16485 12.668 7.867 12.6678 7.5C12.668 7.13299 12.9659 6.83515 13.3329 6.83496C15.2668 6.83496 16.8348 5.26692 16.8348 3.33301C16.835 2.966 17.1329 2.66816 17.4999 2.66797Z" fill="currentColor" />
        <path fillRule="evenodd" clipRule="evenodd" d="M11.6669 1.00195C12.9545 1.00213 13.9979 2.04635 13.9979 3.33398C13.9976 4.62133 12.9542 5.66486 11.6669 5.66504C10.3794 5.66498 9.33517 4.6214 9.33482 3.33398C9.33482 2.04628 10.3792 1.00201 11.6669 1.00195ZM11.6669 2.33203C11.1137 2.33209 10.6649 2.78082 10.6649 3.33398C10.6652 3.88686 11.1139 4.3349 11.6669 4.33496C12.2197 4.33478 12.6675 3.88679 12.6678 3.33398C12.6678 2.78089 12.2199 2.33221 11.6669 2.33203Z" fill="currentColor" />
      </svg>;
  }),
  marble,
  nickel,
  ...
  river = esmInit(() => {
    react();
    quartz = zinc => <svg width={16} height={16} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...zinc}>
        <path d="M11.912 21.413c-.383.45-.883.683-1.5.7-.616.016-1.116-.192-1.5-.625-.375-.434-.454-1.034-.237-1.8L9.687 16H4.575c-.567 0-1.008-.162-1.325-.488a1.68 1.68 0 0 1-.475-1.2c0-.474.154-.9.462-1.274l8.9-10.563c.384-.45.884-.683 1.5-.7.617-.017 1.113.192 1.488.625.383.433.467 1.033.25 1.8L14.312 8h5.113c.567 0 1.008.167 1.325.5.325.333.488.737.488 1.212 0 .467-.159.884-.476 1.25l-8.85 10.45Z" stroke="currentColor" strokeWidth={1.875} strokeLinejoin="round" />
      </svg>;
  });
export const codexMicroKeyboardSurfaceP = esmInit(() => {
  codexMicroKeyboardSurfaceD = ["codex-micro-input-monitoring-permission"];
  codexMicroKeyboardSurfaceF = 1e3;
});
export function codexMicroKeyboardSurfaceO(amber: unknown) {
  let {
      assignmentTitle,
      isPressed,
      keycap,
    ...
    basalt = useIntl(),
    cedar = slot?.status ?? "off",
    daisy = assignmentTitle ?? (slot?.threadKey == null ? basalt.formatMessage({
      ...
      position
    }) : slot.title ?? basalt.formatMessage({
  ...
  let ember = daisy,
    flint = CodexAutomationCapabilityOrigin(cedar).toString(16).padStart(6, "0");
  let garnet = `#${flint}`,
    hazel = cedar === "off",
    ivory = slot?.selected ? "opacity-40" : "opacity-25",
    jasper = slot?.selected ? "opacity-55" : "opacity-40";
  hazel && (ivory = "opacity-0", jasper = "opacity-0");
  isPressed && (ivory = "opacity-45", jasper = "opacity-65");
  let kelp = onClick == null ? ember : undefined,
    lotus = isPressed && "translate-y-px scale-[0.985]",
    mint = IntlProvider("relative z-10 aspect-square w-full rounded-xl transition-transform duration-basic ease-out", lotus);
  let nova = onClick == null ? "img" : undefined,
    olive = keycap == null ? <>
          <div className={IntlProvider("pointer-events-none absolute -inset-[5%] rounded-xl blur-sm transition-[background-color,opacity] duration-basic ease-out", ivory)} style={{
        backgroundColor: garnet
      }} />
          <div className="absolute inset-0 overflow-hidden rounded-xl border border-white/[0.82] bg-white/[0.46] shadow-[inset_0_0_0_1px_rgba(228,228,228,0.4),inset_0_1px_2px_rgba(255,255,255,0.5),0_1px_2px_rgba(79,72,63,0.16)] backdrop-blur-sm dark:border-white/[0.14] dark:bg-white/[0.12] dark:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.09),inset_0_1px_2px_rgba(255,255,255,0.08),0_1px_2px_rgba(0,0,0,0.3)]">
            <div className={IntlProvider("absolute inset-0 transition-[background-color,opacity] duration-basic ease-out", jasper)} style={{
          backgroundColor: garnet
        }} />
            <div className="absolute inset-0 bg-gradient-to-br from-white/[0.38] via-white/[0.1] to-transparent dark:from-white/[0.16] dark:via-white/[0.04]" />
            <div className={IntlProvider("absolute top-1/2 left-1/2 size-3 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#4f477f]/20 bg-[#685fae]/85 shadow-[0_0_5px_rgba(87,76,151,0.42)] transition-transform duration-basic ease-out dark:border-[#b5adf0]/20 dark:bg-[#8177c8]/90 dark:shadow-[0_0_6px_rgba(118,104,197,0.5)]", isPressed && "scale-110")} />
          </div>
        </> : <CodexMicroKeyboardSurfaceC {...{
      className: "size-full",
      children: <CodexMicroKeyboardSurfaceL {...{
        keycap,
        className: "size-5"
      }} />
    }} />;
  let prism = <div aria-label={kelp} className={mint} role={nova}>
      {olive}
    </div>;
  let quill = prism,
    reef = onClick == null ? quill : <button type="button" aria-label={basalt.formatMessage({
      ...
      position
    })} className="focus-visible:ring-token-border-focus relative z-10 aspect-square w-full cursor-interaction rounded-xl transition-transform outline-none focus-visible:ring-2" onClick={onClick}>
          {quill}
        </button>;
  let sage = reef;
  if (!showTooltip) return sage;
  let topaz = timber[cedar],
    ultra = <MemoizedFormattedMessage {...{
      ...topaz
    }} />;
  let vapor = ultra,
    wheat = <span className="font-medium text-token-text-primary">
        {ember}
      </span>;
  let yarn = hazel && "bg-token-border",
    zephyr = IntlProvider("size-1.5 rounded-full border border-black/10 dark:border-white/15", yarn);
  let acorn = hazel ? undefined : {
    backgroundColor: garnet
  };
  let bloom = <span aria-hidden={true} className={zephyr} style={acorn} />;
  let coral = <span className="flex items-center gap-1.5 text-xs text-token-text-secondary">
      {bloom}
      {vapor}
    </span>;
  let drift = <div className="flex max-w-64 flex-col gap-1.5 p-1">
      {wheat}
      {coral}
    </div>;
