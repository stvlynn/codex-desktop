...
function bravo({ reviewInstructions, requestMessage }) {
  return [
    AppInitialFst,
    be.trim(),
...
async function copper({ context, hostId, intl }) {
  ...
  return {
    diffFilter: "branch",
    prompt: bravo({
      reviewInstructions: falcon
        .replaceAll("{baseBranch}", context.baseBranch)
        .replaceAll("{mergeBaseSha}", river.mergeBaseSha.trim()),
...
function delta(umbra) {
  let { hostId, onError, onSuccess } = umbra,
    violet = CodexPluginActionType(appScopeAtom),
    willow = useIntl(),
    xenon = AppInitialQX(hostId),
    yellow = async (cedar) => {
      let { context, conversationId, delivery } = cedar,
        { gitRoot, cwd } = context,
        daisy = await copper({
          context,
          hostId,
...
function marble({ currentBranch, defaultTargetBranch, recentBranches }) {
  let quill = [],
    reef = [defaultTargetBranch ?? nickel],
    sage = new Set();
  return (
    currentBranch && sage.add(currentBranch),
    recentBranches != null && reef.push(...recentBranches),
    reef.forEach((item) => {
...
export function ReviewSlashCommandSubmenuRegistration(ultra: unknown) {
  let { conversationId, cwd, gitRoot, hostConfig, reviewSubmissionLock } =
      ultra,
  ...
  let glide = (unity) => {
    if (
      (reviewSubmissionLock.release(),
      AppInitialOW(vapor, unity.diffFilter),
      unity.delivery === "detached")
    ) {
  ...
  let { mutate, isPending, variables } = delta(honey),
    ...
    quest;
  {
    ...
    let dusk = cliff,
      elm = (seed) => {
        reviewSubmissionLock.acquire() &&
          (logProductEvent(vapor, CODEX_REVIEW_TARGET_SELECTED_TYPE, {
            target: "unstaged",
          }),
          mutate(
            {
