...
let chatgptConversationUpdatesSubscriptionAtom,
  ...
  ensureChatgptConversationUpdatesSubscriptionInit = rolldownRuntimeN(() => {
    ...
    void _AppInitialJI;
    appInitialXP();
    chatgptConversationUpdatesSubscriptionAtom = _appInitialIvt(
      appInitialFft,
      null,
      {
        onMount: (request1217, request1218) => {
          appInitialDdt.info(
            "chatgpt_conversation_updates_subscription_mounted",
          );
          let slot4729 = () => {
              return request1218
                .get(appInitialCI)
            ...
            slot4730 = appInitialAI("conversations", slot4729),
            slot4731 = appInitialAI("alder-conversations", slot4729),
            slot4732 = (request1877) => {
              let slot5950 = useChatgptComposerControllerHelper11(request1877);
              ...
              for (let slot6490 of slot5950) {
                if (
                  request1218.get(appInitialBP, slot6490.conversationId) == null
                ) {
                  appInitialDdt.info("chatgpt_conversation_update_ignored", {
                    ...
                    sensitive: {
                      conversationId: slot6490.conversationId,
                    },
                  });
                  continue;
                }
                if (slot6490.type === "async-status") {
                  AppInitialP(
                    request1218,
                  ...
                  appInitialDdt.info(
                    ...
                    {
                      safe: {
                        asyncStatus: slot6490.asyncStatus,
                      },
                      sensitive: {
                        conversationId: slot6490.conversationId,
                      },
            ...
            slot4733 = slot4730.onMessage(slot4732),
            slot4734 = slot4731.onMessage(slot4732);
          return (
            slot4730.subscribe(),
            slot4731.subscribe(),
            () => {
    ...
    chatgptConversationGenUiRefreshAtom = appInitialAvt(
      ...
      {
        onMount: (request3189, request3190) => {
          let { key } = request3190;
          if (key == null) return;
          let slot8486 = useChatgptComposerControllerHelper10({
              refreshWidget: (request8727) => {
                return request3190.get(appInitialCP, key)
                  ? Promise.resolve({
                      next_poll_after_ms: null,
                    })
                  : request3190.get(appInitialCI).refreshGenUiWidget({
                      conversationId: key,
                      ...request8727,
                    });
              },
            ...
            slot8487 = null,
            slot8488 = appInitialAI("conversations", () => {
              return request3190
                .get(appInitialCI)
                .getConversationWebSocketUrl();
            }),
            slot8489 = slot8488.onSubscribed(() => {
              slot8487 ??= request3190.watch(({ get }) => {
                let slot13995 = get(appInitialBP, key);
                slot8486.setTargets(
                  slot13995 == null || get(appInitialCP, key)
                    ? []
            ...
            slot8490 = slot8488.onPotentialMissedMessages(() => {
              useChatgptComposerControllerHelper16(
                request3190,
                key,
                "missed_messages",
