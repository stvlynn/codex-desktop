...
export function useChatgptComposerControllerHelper10({
  ...
  refreshWidget: (target: any) => Promise<{ next_poll_after_ms?: number | null }>;
}): {
  ...
  let slot6223 = new Map(),
    ...
    slot6226 = async (event) => {
      let slot10124 =
        useChatgptComposerControllerHelper12(event.target);
      if (
        !(
          slot6224 ||
          slot6223.get(
        ...
        event.inFlight = true;
        try {
          let slot13928 = await refreshWidget(
            event.target,
          );
          if (
            slot6224 ||
          ...
          if (
            ((event.failures = 0),
            slot13928.next_poll_after_ms == null)
          ) {
            slot6223.delete(
              slot10124,
            );
  ...
  return {
    setTargets: (request6550) => {
      ...
      for (let [
        ...
        slot12884.has(
          ...
          (slot15682.timeout != null &&
            clearTimeout(slot15682.timeout),
          slot6223.delete(
            slot15681,
          ));
      for (let [
    ...
    stop: () => {
      ...
      for (let slot15880 of slot6223.values())
        slot15880.timeout != null &&
          clearTimeout(slot15880.timeout);
      slot6223.clear();
    },
  };
}
