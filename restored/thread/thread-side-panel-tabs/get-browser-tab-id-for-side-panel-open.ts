...
export function setGetBrowserTabIdForSidePanelOpenPeers(
  next: GetBrowserTabIdForSidePanelOpenPeers,
): void {
  peers = next;
}
 ...
 */
export function getBrowserTabIdForSidePanelOpen(
  e: unknown,
  t: unknown,
  n: unknown,
) {
  ...
  return e.get(peers.lT)
    ? (n ?? peers.Dl(crypto.randomUUID()))
    : n != null &&
        (peers.xYn(e, t, n) || (peers.Rj.isBrowserUseTab(t, n) && r === n))
      ? n
      : (peers.Q$o(e, t, n) ?? peers.xT(t));
}
