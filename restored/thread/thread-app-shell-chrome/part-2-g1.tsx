...
export function threadAppShellChromeT(bravo: unknown) {
  let echo = CodexPluginActionType(ensureComposerEsm_S8_Init),
    ...
    quillPrime = CodexPluginActionResult(requestRealtimeVoicePresentationSurface),
    reefPrime = CodexPluginActionResult(ensureConversationPageEsm_A0_Init.activeTab$),
    sagePrime = useIntl(),
    ...
    yarnPrime = ultraPrime && bravo.threadType === "remote" ? bravo.showReviewTab : ultraPrime && olivePrime.kind === "git",
  ...
  acornPrime = () => {
    ...
    zephyrPrime.current !== irisPrime && (zephyrPrime.current = irisPrime, closeSidePanelUnlessWindowResize(echo, reefPrime));
  };
  ...
  glidePrime = () => {
    !ultraPrime || bravo.threadType === "remote" || prismPrime == null || openOrFocusFilePanelTab(echo, null, {
  ...
  return <>
      {bravo.threadType === "remote" ? <>
        ...
        children: <InitThreadPanelToggleButton {...{
          ...
          children: topazPrime ? <AppIconAZ className="icon-xs rotate-180" /> : <AppIconMZ {...{
            className: "icon-xs rotate-180"
          }} />
        }} />
      }} />}
        </> : <>
          {ultraPrime && bravo.threadType === "local" && bravo.conversationId != null ? <ThreadAppShellChromeHelper14 {...{
