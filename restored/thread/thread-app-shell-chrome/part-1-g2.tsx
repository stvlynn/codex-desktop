...
const lotus = {
  closed: {
    opacity: 0,
    y: -6,
    transition: { duration: 0.045, ease: DEFAULT_MOTION_TRANSITION.ease },
  },
  open: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.075, ease: DEFAULT_MOTION_TRANSITION.ease },
  },
};
...
export function ThreadAppShellChromeHelper17(
  gasket: ThreadAppShellChromeHelper17Props,
) {
  let { actions, onOpenArtifact, outputArtifacts } = gasket,
    handle =
  ...
  let dowel = (
    <div ref={nozzle} className={arbor}>
      {bushing}
      {collar}
    </div>
  );
...
function ThreadAppShellChromeHelper18(gib) {
  let { headerHeight, root, onCoveredChange } = gib,
    hub =
      headerHeight == null
        ? "0px"
    ...
    idler = (jig) => {
      ...
      let keeper = new IntersectionObserver(
        (lug) => {
          let mandrel = lug[0];
