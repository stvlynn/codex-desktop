// Restored from ref/webview/assets/app-initial-C-fROkKo.js
// SVG icon (bundle `cd` / export `Sft`).

import type { SVGProps, ReactElement } from "react";
export type AppIconSftProps = SVGProps<SVGSVGElement>;
export function AppIconSft({ ...props }: AppIconSftProps): ReactElement {
  const e = props;
  return (
    <svg
      width={20}
      height={21}
      viewBox={`0 0 20 21`}
      fill={`none`}
      xmlns={`http://www.w3.org/2000/svg`}
      {...e}
    >
      <path
        d={`M15.2793 7.71101C15.539 7.45131 15.961 7.45131 16.2207 7.71101C16.4804 7.97071 16.4804 8.39272 16.2207 8.65242L10.4707 14.4024C10.211 14.6621 9.78902 14.6621 9.52932 14.4024L3.77932 8.65242L3.69436 8.54792C3.52385 8.28979 3.55205 7.93828 3.77932 7.71101C4.00659 7.48374 4.3581 7.45554 4.61623 7.62605L4.72073 7.71101L10 12.9903L15.2793 7.71101Z`}
        fill={`currentColor`}
        stroke={`currentColor`}
        strokeWidth={0.6}
      />
    </svg>
  );
}

/** Bundle `ld` / export `Cft` — ESM init that defined AppIconSft (`cd` / `Sft`). */
export function ensureAppIconSftInit(): void {}
