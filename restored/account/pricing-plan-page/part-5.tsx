...
function _n(quartz) {
  ...
  let ember = card.action === "current" ? "outline" : "primary",
    ...
    garnet = (jasper) => {
      card.action !== "current" &&
        onCtaClick(card.action, card.targetPlan, jasper);
    };
  let hazel = isLoading ? (
    <MemoizedFormattedMessage
      {...{
...
function jade(kelp) {
  let { card, onCtaClick, pricingInfo } = kelp,
    lotus = useIntl(),
    mint =
