...
import { ensureComposerEsm_Ilt_Init, ensureComposerEsm_K9_Init, ensureComposerEsm_MT_Init, ensureComposerEsm_P5_Init, ensureComposerEsm_Qtt_Init, ensureComposerEsm_Tft_Init, ensureComposerEsm_Utt_Init } from "../../composer/composer-esm-inits";
...
function PricingPlanPagePricingPlanPageContent(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
...
export function pricingPlanPagePricingPlanPage() {
  let marble = CodexPluginActionType(appScopeAtom),
    ...
    timber = slate?.source ?? "direct_navigation",
    {
      data,
      isLoading
    } = reuseArrayIfShallowEqual("always"),
    umbra = parseFormattedCreditsQuantityString(data?.plan_type)?.pricePlan ?? null,
  ...
  if (delta.useEffect(() => {
    umbra != null && logProductEvent(marble, CODEX_PRICING_PLAN_PAGE_SHOWN_TYPE, {
      defaultTab: violet,
      source: timber
    });
  }, [umbra, violet, marble, timber]), data == null || umbra == null) return isLoading ? <main className="h-full bg-token-main-surface-primary" /> : <Navigate {...{
    to: "/",
    replace: true
  }} />;
  let cedar = (garnet, hazel) => {
    ...
    daisy = async (ivory, jasper, kelp) => {
      ...
      if (jasper === ChatGptPlanId.SELF_SERVE_BUSINESS_USAGE_BASED && CREDIT_PURCHASE_MIN_SPEND_MINOR_UNITS(marble, {
        audience: "workspace",
      ...
      if (umbra === ChatGptPlanId.FREE || jasper !== ChatGptPlanId.PLUS && jasper !== ChatGptPlanId.PROLITE && jasper !== ChatGptPlanId.PRO) {
        cedar(mint, kelp);
        return;
      }
      let nova = chatGptPlanIdToBillingSlug(jasper),
        olive = umbra === ChatGptPlanId.GO,
        prism = umbra === ChatGptPlanId.PLUS && (jasper === ChatGptPlanId.PROLITE || jasper === ChatGptPlanId.PRO),
      ...
      zinc(jasper);
      try {
        let sage = await quartz.fetchQuery(AppInitialXb({
          accountId: data.id,
          updatedPlan: nova
        }));
        if (quill) {
        ...
        if (!topaz?.card_last4?.trim() || __data?.minorUnitExponent == null || __data.currencyCode.toUpperCase() !== sage.currency.toUpperCase()) {
          cedar(mint, kelp);
          return;
        }
        xenon({
    ...
    ember = ultra => {
      willow != null && logProductEvent(marble, CODEX_PRICING_PLAN_PAGE_CTA_CLICKED_TYPE, {
        ctaAction: ultra,
      ...
      if (willow != null) {
        ...
        try {
          ...
          if (vapor.status == null || vapor.status === "pending") {
            xenon(null);
            vapor.status == null && willow.kind === "saved_card_upgrade" && (marble.get(toastAtom).success(nickel.formatMessage({
            ...
            await Promise.all([quartz.invalidateQueries({
  ...
  return <>
      {<PricingPlanPagePricingPlanPageContent key={umbra} {...{
      currentPlan: umbra,
      defaultTab: violet,
      ...
      {willow == null ? null : <SubscriptionUpdatePlanDialog {...{
      isUpdating: amber,
      pricingInfo: __data ?? null,
      ...
      onConfirm: () => {
        ember(willow.kind === "saved_card_upgrade" && willow.preview.amount_due.amount > 0 ? "pay_now" : "confirm");
        flint();
      },
      onGoToWeb: acorn => {
        ember("go_to_web");
...
function pricingPlanPagePricingPlanPageContent(coral) {
  ...
  let orbit = <ReadLoginRouteQuerySnapshot {...{
    color: "ghostMuted",
    size: "default",
    onClick: onBack,
    children: [moss, north]
  }} />;
  let pine = <header className="draggable flex h-toolbar w-full shrink-0 items-center pe-2" style={lunar}>
      {orbit}
    </header>;
  let quest = <h1 className="text-[19px] font-medium [@container_(min-width:44rem)]:text-[24px]">
  ...
  let unity = <SubscriptionPlanSegment {...{
    ariaLabel: ridge,
    options: tide,
    selectedId: eagle,
    onSelect: frost
  }} />;
  let vale = eagle === "personal" ? <div className={IntlProvider("grid w-full grid-cols-[minmax(0,24rem)] justify-center gap-4 [@container_(min-width:44rem)]:grid-cols-[repeat(2,minmax(0,28rem))] [@container_(min-width:64rem)]:gap-6", currentPlan === ChatGptPlanId.FREE ? "[@container_(min-width:76rem)]:grid-cols-[repeat(4,minmax(0,24rem))]" : "[@container_(min-width:76rem)]:grid-cols-[repeat(3,minmax(0,24rem))]")}>
        {alpha(currentPlan, honey).map(item => <_n key={item.id} {...{
      card: item,
      ...
      </div> : <div className="grid w-full grid-cols-[minmax(0,29.25rem)] justify-center gap-4 [@container_(min-width:44rem)]:grid-cols-[repeat(2,minmax(0,29.25rem))] [@container_(min-width:64rem)]:gap-6 [@container_(min-width:76rem)]:grid-cols-[repeat(3,minmax(0,29.25rem))]">
        {<_n {...{
      ...
      onSelectProPlan: iris
        ...
        {gamma.map(item => <Alpha key={item.id} {...{
      card: item,
      pricingInfo,
      onCtaClick
    }} />)}
      </div>;
  let wave = <section className="@container flex min-h-0 flex-1 flex-col overflow-y-auto px-4 pb-8 sm:px-8">
  ...
  return <main className="flex h-full min-h-0 flex-col bg-token-main-surface-primary text-token-text-primary [--cursor-interaction:pointer] [--pricing-plan-highlight:#635ef4]">
      {pine}
      {wave}
    </main>;
}
