...
function bravo(garnet) {
  let { detailsLabel, getPlansUrl, onOpenUrl } = garnet,
    hazel = (kelp) => {
...
function copper({ intl, plan, pricingInfo }) {
...
function delta({ intl, pricingInfo }) {
  ...
  return lotus == null ? null : (
    <MemoizedFormattedMessage
      {...{
        id: "settings.usage.upgradePlan.business.team.price",
        defaultMessage: "{price} / user / month",
        description:
          "Localized monthly per-user price for the ChatGPT and Codex Business plan",
        values: {
          price: lotus,
        },
      }}
