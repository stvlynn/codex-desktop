...
function delta(jade) {
  let kite = gamma.useId(),
    ...
    marble =
      !lemon &&
      jade.variant === "setup-auto-reload" &&
      !jade.serverState.isEnabled &&
      jade.isDiscountOfferPending &&
      jade.discountOffer == null,
    nickel =
      !lemon &&
      jade.variant !== "one-time-purchase" &&
      (jade.enableAutoTopUpMutation.isPending ||
        jade.updateAutoTopUpMutation.isPending ||
        jade.disableAutoTopUpMutation.isPending),
    onyx;
  if (lemon) {
    ...
    let yellow =
        jade.variant === "setup-auto-reload"
          ? jade.discountOffer?.auto_reload == null
...
function $t(basalt) {
  let { variant, state, dialogDescriptionId, onRetry, onClose } = basalt,
    cedar =
  ...
  let kelp = (
    <DeferredUiU
      {...{
        children: <div role={hazel}>{jasper}</div>,
      }}
    />
  );
  let lotus =
...
export function CreditReloadDialogHostIcon(prism: any) {
  let { onClose, source, variant } = prism,
    { accountId, authMethod, userId } = useAuth(),
    quill = AppIconAlt(),
    { data } = CodexPluginActionResult(titleCaseColonSegments),
    reef = variant !== "one-time-purchase",
    sage = {
      enabled: reef,
    };
  let { data: _data, isError, refetch } = QueuedMessageTraySurface(sage),
    topaz = variant !== "one-time-purchase",
    ultra = {
      enabled: topaz,
    };
  let { data: __data, isPending } = quickChatSurfaceCollapsedAtom(ultra),
    vapor =
      variant === "setup-auto-reload" &&
      _data?.isEnabled === false &&
      (__data?.payment_methods.length ?? 0) > 0,
    wheat = AppInitialJb({
      accountId,
      authMethod,
      enabled: vapor,
  ...
  let {
    ...
    zephyr = _isPending
      ? undefined
      : (____data ?? quill.getContext().user?.country ?? null);
  let acorn = zephyr,
    bloom = {
