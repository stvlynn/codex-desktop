...
const HooksSettingsHelper13: any = undefined;
...
export function HooksSettingsIcon() {
  ...
  let iris = mint.useEffectEvent(honey),
    jewel = () => {
      if (acorn != null && acorn !== selectedHostId) {
        iris(acorn);
        return;
      }
  ...
  let knoll;
  knoll = [quill.key, topaz, selectedHostId, acorn];
  ...
  let pine = orbit,
    quest = glide.data?.data,
    ridge = ultra.kind !== "local",
    storm = glide.isFetching && !glide.isPending,
    tide = data?.labels,
    unity,
    vale;
  unity = (event, fern) => {
