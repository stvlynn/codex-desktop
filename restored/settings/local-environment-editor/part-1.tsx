...
async function localEnvironmentEditorY(jasper, kelp, lotus, mint) {
  let nova = [...Array.from(new Set(lotus)).flatMap(item => [buildVscodeQueryKey("local-environment-config", {
    configPath: item,
    hostId: kelp
  }), buildVscodeQueryKey("local-environment", {
    configPath: item,
    hostId: kelp
  })]), ...(mint == null ? [] : [buildVscodeQueryKey("local-environments", {
    hostId: kelp,
    workspaceRoot: mint
  })])];
  await Promise.all(nova.map(item => jasper.invalidateQueries({
    queryKey: item
  })));
}
...
function echo(topaz, ultra) {
  return {
    incompleteActionIds: ultra.flatMap(item => item.name.trim().length > 0 == item.command.trim().length > 0 ? [] : [item.id]),
    missingName: topaz.trim().length === 0
  };
}
function falcon(vapor, wheat = "default") {
  return vapor === "action" ? nickel : wheat === "win32" ? vapor === "setup" ? lemon : marble : vapor === "setup" ? jade : kite;
}
function gamma({
  version,
  name,
...
function harbor(drift, eagle, frost, glide) {
  ...
  if ((eagle === "setup" || frost.length > 0 || honey.length > 0) && (drift.push(""), drift.push(`[${eagle}]`), drift.push(`script = ${indigo(frost)}`)), honey.length !== 0) {
    drift.push("");
...
async function onyx({
  ...
  if (north == null) throw Error("Local environments are not supported by this host.");
  let orbit = await north.saveConfig({
    ...
    quest = localEnvironmentEditorY(queryClient, hostId, [configPath, pine], workspaceRoot);
  return orbit.type === "conflict" ? {
    type: "conflict"
  } : (await quest, {
    configPath: pine,
    type: (await selectEnvironment?.(pine)) === false ? "selection-failed" : "success"
  });
}
...
async function pearl({
  ...
  return codexWorktree ? gitRoot == null ? false : selectForWorktree(gitRoot, configPath) : (selectForWorkspace(configPath), true);
}
...
function LocalEnvironmentEditorA(storm) {
  ...
  return <AppInitialPl className="flex-wrap" selectedId={selectedId} onSelect={onSelect} size="default" ariaLabel={ariaLabel} options={wave} />;
}
var river,
  slate,
...
function LocalEnvironmentEditorHelper9(brook) {
    ...
    isle;
  {
    let lagoon = seed => ({
      ...
      value: seed.value,
      icon: <LocalEnvironmentEditorG {...{
        icon: seed.value
      }} />
    });
    let meadow = localEnvironmentEditorM.map(lagoon),
      nest = (trail, urn) => {
        onChange(actions.map(item => item.id === trail ? {
          ...item,
          ...urn
        } : item));
      };
    ...
    fern = "gap-1";
    grove = <div className="text-sm text-token-text-secondary">
        {<MemoizedFormattedMessage {...{
        ...localEnvironmentEditorU.actionsDescription
      }} />}
      </div>;
    hill = actions.length === 0 ? <ParseUrlOrFallback {...{
      children: <div className="p-3 text-sm text-token-text-secondary">
          ...
          {actions.map(item => {
        let vine = meadow.find(_item => _item.value === (item.icon ?? "tool")) ?? meadow[0],
        ...
        return <div key={item.id} className="flex flex-col gap-3 rounded-lg border border-token-border bg-token-input-background p-3">
                ...
                children: meadow.map(_item => <DropdownMenu.Item key={_item.value} {...{
                  onSelect: () => {
                    oak(item.id, {
                      icon: _item.value
                    });
                  ...
                  children: <span className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <input id={`local-env-action-name-${item.id}`} className="focus-visible:ring-token-focus w-full rounded-md border border-token-border bg-token-input-background px-2.5 py-1.5 text-sm text-token-text-primary outline-none focus-visible:ring-2" value={item.name} aria-describedby={yarrow ? birch : undefined} aria-invalid={yarrow} onChange={event => {
                  oak(item.id, {
                  ...
                  {yarrow ? <div id={birch} className="text-xs text-token-error-foreground">
                      {<MemoizedFormattedMessage {...{
                ...
                ...localEnvironmentEditorU.actionScript
                  ...
                  <textarea id={`local-env-action-command-${item.id}`} className="focus-visible:ring-token-focus w-full rounded-md border border-token-border bg-token-input-background px-2.5 py-2 font-mono text-sm text-token-text-primary outline-none focus-visible:ring-2" value={item.command} placeholder={falcon("action")} rows={4} aria-describedby={azure ? canyon : undefined} aria-invalid={azure} onChange={event => {
              oak(item.id, {
                command: event.target.value
              });
            }} />
                  {azure ? <div id={canyon} className="text-xs text-token-error-foreground">
                      {<MemoizedFormattedMessage {...{
                ...
                children: <ReadLoginRouteQuerySnapshot {...{
                  ...
                  onClick: () => {
                    onChange(actions.filter(_item => _item.id !== item.id));
                  },
                  children: <AppIconBO {...{
                    className: "icon-sm"
                  }} />
  ...
  let juniper = <UseChromeAndCodeThemeSync.Content {...{
    className: fern,
    children: [grove, hill]
  }} />;
...
function LocalEnvironmentEditorHelper10(ever) {
  let {
    ...
    jadeite = grain === "default" ? ink ? draft.setupScript : draft.cleanupScript : ink ? draft.setupPlatformScripts[grain] ?? "" : draft.cleanupPlatformScripts[grain] ?? "";
  let kernel, leaf;
  kernel = ink ? <MemoizedFormattedMessage {...{
    ...localEnvironmentEditorU.setupScript
  }} /> : <MemoizedFormattedMessage {...{
    ...localEnvironmentEditorU.cleanupScript
  }} />;
  leaf = ink ? <MemoizedFormattedMessage {...{
  ...
  let maple = <UseChromeAndCodeThemeSync.Header {...{
    title: kernel,
    subtitle: leaf,
    actions
  }} />;
  let nimbus = ink ? field.formatMessage(localEnvironmentEditorU.setupPlatformSelector) : field.formatMessage(localEnvironmentEditorU.cleanupPlatformSelector);
  let opal = <LocalEnvironmentEditorA {...{
  ...
  let plume = `local-environment-${scriptType}-script-${grain}`,
    quillow = jadeite,
    silk = ink ? field.formatMessage(localEnvironmentEditorU.setupScript) : field.formatMessage({
  ...
  let vista = <textarea id={plume} className="focus-visible:ring-token-focus w-full rounded-md border border-token-border bg-token-input-background px-2.5 py-2 font-mono text-sm text-token-text-primary outline-none focus-visible:ring-2" value={quillow} aria-label={silk} placeholder={thorn} rows={6} onChange={upland} />;
  let wisp = <UseChromeAndCodeThemeSync.Content {...{
    className: "gap-2",
    children: [opal, vista]
  }} />;
  return <UseChromeAndCodeThemeSync {...{
...
function LocalEnvironmentEditorR() {
  let yonder = <AppInitialM asChild={true}>
  ...
  let zenith = <AppInitialJ className="px-2 py-1 text-sm font-medium text-token-text-primary">
      {<MemoizedFormattedMessage {...{
  ...
  let anvil = <LocalEnvironmentEditorHelper11 {...{
  ...
  return <UsePointerSurfaceInteractionGate {...{
    children: [yonder, <ToggleSortedIdList {...{
      align: "end",
      className: "w-80 max-w-[min(20rem,var(--radix-popover-content-available-width))] gap-1",
      children: [zenith, <div className="flex flex-col gap-1">
                  {anvil}
...
function LocalEnvironmentEditorHelper11(beacon) {
  let {
    ...
    crag = <div className="text-sm text-token-text-secondary">{description}</div>;
  let dome = <div className="overflow-x-auto rounded-md border border-token-input-background bg-token-text-code-block-background px-2 py-1.5">
