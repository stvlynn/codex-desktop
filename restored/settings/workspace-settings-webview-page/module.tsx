...
export function workspaceSettingsWebviewPageWorkspaceSettingsWebviewHost() {
  let jewel = CodexPluginActionType(appScopeAtom),
    { authMethod, isLoading, userId } = useAuth(),
  ...
  let tide = storm,
    unity =
      knoll.pathname === "/workspace-settings/web"
        ? (copper(new URLSearchParams(knoll.search))?.accountId ?? null)
        : null;
  let vale = unity,
    wave =
      !isCurrentSpecialWindowRoute() &&
      authMethod === "chatgpt" &&
      !isLoading &&
      userId != null &&
      !north &&
      ...
      quest === data.id &&
      moss.get("enable_workspace_settings_webview", false)
        ? data.id
        : null;
  let apex = wave,
  ...
  let fern = elm,
    ...
    hill =
      dusk != null && grove.accountId === dusk && grove.userId === userId
        ? grove.generation
        : grove.generation + 1,
    _workspaceSettingsWebviewPageWorkspaceSettingsWebviewPage =
      indigo.useRef(null),
  ...
  lagoon = () => {
    ...
    if (dusk == null) {
      azure.accountId != null &&
        !isLoading &&
        !_isLoading &&
        !north &&
        jewel.set(violet, tide, {
          accountId: null,
    ...
    (azure.accountId === dusk && azure.userId === userId) ||
      jewel.set(violet, tide, {
        accountId: dusk,
        failedAttemptCount: 0,
        generation: hill,
  ...
  meadow = () => {
    let birch =
      cliff != null &&
      _workspaceSettingsWebviewPageWorkspaceSettingsWebviewPage.current !==
        cliff;
    if (
      ((_workspaceSettingsWebviewPageWorkspaceSettingsWebviewPage.current =
        cliff),
    ...
    if (
      canyon.accountId !== cliff ||
      canyon.userId !== userId ||
      canyon.loadState !== "failed" ||
      canyon.failedAttemptCount < timber
    )
      return;
    let dew = {
      ...canyon,
  ...
  let oak = (ever) => {
    ...
    field.accountId !== dusk ||
      field.generation !== hill ||
      field.userId !== userId ||
      field.loadState === ever ||
      jewel.set(violet, tide, {
        ...field,
        failedAttemptCount:
          ever === "failed"
            ? field.failedAttemptCount + 1
            : ever === "ready"
              ? 0
  ...
  if (
    ((quiet = () => {
      if (
        grove.accountId == null ||
        (cliff != null && grove.loadState !== "failed") ||
        (grove.loadState === "failed" && grove.failedAttemptCount >= timber)
      )
        return;
      let grain = window.setTimeout(
        () => {
          ...
          if (
            haven.accountId !== grove.accountId ||
            haven.generation !== grove.generation ||
            haven.userId !== grove.userId
          )
            return;
          let ink = {
            ...haven,
        ...
        grove.loadState === "failed"
          ? Math.min(
              river * 2 ** Math.max(0, grove.failedAttemptCount - 1),
              slate,
            )
          : pearl,
      );
    ...
    indigo.useEffect(quiet, rain),
    vale != null && !brook && cliff == null)
  ) {
    let jadeite;
    return (
      <Navigate
  ...
  return (
    <div aria-hidden={trail} inert={urn} className={wind}>
      {yarrow}
    </div>
  );
}
function alpha(willow) {
  let { accountId, onLoadStateChange, partition, routingOverride } = willow,
  ...
  daisy = () => {
    ...
    let mint = false,
      ...
      prism = () => {
        ...
        nova = window.setTimeout(() => {
          ...
          (topaz != null && echo(topaz) && !zinc.isLoading()) ||
            (onLoadStateChange("failed"), mint || yellow(true));
        }, quartz);
      },
      quill = () => {
        ...
        if (!(ultra == null || ultra.protocol === "about:")) {
          ...
          ultra.origin === lemon &&
            !ultra.pathname.startsWith(marble) &&
            (onLoadStateChange("failed"), mint || yellow(true));
        }
      },
      reef = (vapor) => {
        vapor.isMainFrame !== false &&
          (prism(), onLoadStateChange("loading"), mint || yellow(true));
      },
      sage = (wheat) => {
        let yarn = wheat;
    ...
    return (
      zinc.addEventListener("dom-ready", quill),
      zinc.addEventListener("did-start-navigation", reef),
      zinc.addEventListener("did-fail-load", sage),
      zinc.addEventListener("did-finish-load", quill),
      zinc.addEventListener("did-stop-loading", quill),
      zinc.addEventListener("did-navigate-in-page", quill),
      prism(),
      quill(),
      () => {
        mint = true;
    ...
    kelp;
  {
    ...
    routingOverride != null &&
      zephyr.searchParams.set("routingOverride", routingOverride);
...
function bravo({ accountId, userId }) {
  let acorn = JSON.stringify([userId, accountId]),
    bloom = umbra.get(acorn);
  return (
    bloom ??
      ((bloom = `${StackedCardHeader}${crypto.randomUUID()}`),
      umbra.set(acorn, bloom)),
    bloom
  );
}
...
function echo(frost) {
  return (
    frost.origin === lemon &&
    (frost.pathname === nickel || frost.pathname.startsWith(`${nickel}/`))
  );
}
function falcon(glide) {
  return (
    glide != null &&
    "getURL" in glide &&
    typeof glide.getURL == "function" &&
    "isLoading" in glide &&
    typeof glide.isLoading == "function"
  );
}
var gamma,
  harbor,
