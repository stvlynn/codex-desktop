...
export function setBindSettingsAutomationsScheduleModeHourlyPeers(next: BindSettingsAutomationsScheduleModeHourlyPeers): void {
  peers = next;
}
...
export function bindSettingsAutomationsScheduleModeHourly() {
  if (peers == null) {
    throw new Error("bindSettingsAutomationsScheduleModeHourly peers are not configured");
  }

  return peers.e(() => {
    peers.$u(), peers.u3(), L$c = 1440, R$c = 15, z$c = Array.from({
    ...
    }, ...peers.B$c.filter(e => peers.e.id !== `hourly`)], g3 = [{
      id: `MO`,
      labelMessage: peers.Zu({
