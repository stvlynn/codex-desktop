...
function frost(lunar) {
  let {
    ...
    north = useIntl(),
    { data } = CodexPluginActionResult(amber),
    orbit = data == null ? null : alpha(data);
  let pine = orbit,
    quest =
      (pine?.applicationBundleIdentifiers.length ?? 0) +
      (pine?.urlDomains.length ?? 0),
    ridge = CodexPluginActionResult(reef),
    storm =
      currentState?.activationState === "waiting_for_permissions" ||
      (currentState?.recorderState === "stopped" &&
        currentState.activationState === "idle"),
  ...
  let apex = tide ? (
    <MemoizedFormattedMessage
      {...{
        ...
        defaultMessage:
          "Computer history is temporarily unavailable. <LearnMore>Learn more</LearnMore>",
        description:
          "Status shown when Codex cannot connect to Computer history, followed by a learn-more link",
        values: {
          learnMore: glide,
        },
      }}
  ...
  let brook = unity ? (
    <ReadLoginRouteQuerySnapshot
      {...{
        ...
        children: activationInProgress ? (
          <MemoizedFormattedMessage
            {...{
              id: "settings.chronicle.activationInProgress",
              defaultMessage: "Resume",
              description:
                "Computer history action shown while activation is already in progress",
            }}
          />
        ) : storm ? (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Retry Computer history startup while waiting for permissions",
            }}
          />
        ) : (
          <MemoizedFormattedMessage
    ...
    <ReadLoginRouteQuerySnapshot
      {...{
        ...
        onClick: () =>
          ridge.mutate(currentState?.recorderState, {
            onError: () => {
              moss.get(toastAtom).danger(
                <MemoizedFormattedMessage
                  {...{
                    id: "settings.chronicle.pauseResumeErrorToast",
                    defaultMessage:
                      "Computer history could not be paused or resumed",
                    description:
                      "Toast shown when pausing or resuming Computer history fails",
                  }}
                />,
              );
            },
...
function glide(nest) {
  return (
    <ChronicleSettingsPageHelper1
      key={"computer-history-unavailable-learn-more"}
      {...{
          ...
          <h2 className="sr-only">
            {
              <MemoizedFormattedMessage
                {...{
                  ...
                  description:
                    "Accessible title for clear Computer history dialog",
                }}
              />
            }
          </h2>
  ...
  let wind = (
    <MemoizedFormattedMessage
      {...{
        ...
        defaultMessage:
          "Your actions and any memories created from them will be deleted. This cannot be undone.",
