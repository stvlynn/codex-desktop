...
function quillow(juniperSecond) {
  let {
    ...
    { appServerVersion, error, installedCodexVersion, state } = iconXs(
      connection.hostId,
    ),
    ...
    plumeSecond;
  {
    ...
    let { statusError, isRestartAvailableNotice, statusState } = root({
      error,
    ...
    opalSecond =
      statusState == null
        ? null
        : logHostLoginSurfaceEvent(meadowSecond, {
            canLogin: true,
            error: nimbusSecond,
    ...
    let rapidsSecond =
        (petalSecond || !fieldSecond) &&
        (nimbusSecond?.code === "remote-codex-not-found" ||
          nimbusSecond?.code === "update-required"),
      spurSecond;
    spurSecond = function () {
      ensureAppActionPayloadSchemasInit.dispatchMessage(
        "codex-app-server-restart",
        {
          ...
          remoteControlEnabled: lagoonSecond.get(
            AppInitialX3,
            connection.hostId,
          ),
        },
  ...
  let quillowSecond = plumeSecond,
    ...
    thornSecond = (torSecond) =>
      onToggleConnection(connection.hostId, torSecond);
  ...
  let vistaSecond = (
    <AppInitialYC
      checked={silkSecond}
      size="sm"
      onChange={thornSecond}
      ariaLabel={uplandSecond}
    />
  );
  ...
  let knobSecond =
    (quietSecond || oneToOnePairingInAppEnabled) && state === "connected" ? (
      <OptionalTooltip
        {...{
...
function root({ error, restartAvailableNotice, state }) {
  return error != null || state === "error"
    ? {
        statusError:
          ...
          (restartAvailableNotice == null
            ? null
            : classifyInstalledVsCurrentAppServerVersions(
    ...
    : restartAvailableNotice == null
      ...
      : {
          statusError: classifyInstalledVsCurrentAppServerVersions(
            restartAvailableNotice,
          ),
...
function silk({ appServerVersion, installedCodexVersion, state }) {
  return state == null ||
    appServerVersion == null ||
    installedCodexVersion == null ||
    !isInstalledCodexNewerThanAppServer({
      appServerVersion,
      installedCodexVersion,
    })
    ? null
...
function upland(updraftSecond) {
  let { connection, error, state } = updraftSecond,
    vergeSecond = useIntl();
  if (state === "error") {
...
function zenith({
  ...
  let gearSecond = CodexPluginActionType(appScopeAtom),
    ...
    { data } = CodexBrowserSurfaceActionType(codexMobileSetupFlowD, {
    ...
    axleSecond =
      valveSecond == null
        ? null
        : (connections.find((item) => item.hostId === valveSecond) ?? null),
    ...
    drillSecond = async (insertSecond) => {
      let jacketSecond = connections.find(
        (item) => item.hostId === insertSecond,
      );
      jacketSecond != null &&
        isWslHostSource(jacketSecond) &&
        (await jetty({
          connection: jacketSecond,
          environmentId:
            gearSecond.get(deferredHostsS3, insertSecond)?.environmentId ??
            null,
        }));
      let knurlSecond = {
      ...
      let leverSecond = gearSecond.query.snapshot(
          codexMobileSetupFlowC,
          insertSecond,
        ),
        mountSecond = leverSecond.getOptions().enabled
          ? await leverSecond
              .invalidate({
                exact: true,
                refetchType: "none",
              })
              .then(() => leverSecond.fetch())
              .catch(() => null)
          : (leverSecond.getData() ?? []);
      if (mountSecond == null) {
        screwSecond((nozzleSecond) =>
          nozzleSecond === knurlSecond ? null : nozzleSecond,
        );
        return;
      }
      screwSecond((platenSecond) =>
        platenSecond === knurlSecond
          ? {
              existingClientIds: new Set(
                mountSecond.map((item) => item.clientId),
              ),
              hostId: insertSecond,
            }
          : platenSecond,
    ...
    engineSecond = (ratchetSecond) => {
      if (
        remoteControlAllowedHostIds.includes(ratchetSecond) &&
        gearSecond.get(AppInitialX3, ratchetSecond)
      ) {
  ...
  return (
    <>
        ...
        <QuietFifth
          {...{
            ...
            onAllowed: (dowelSecond) => {
              pistonSecond(null);
              nutSecond.startPairingAfterAllow
                ? drillSecond(dowelSecond)
                : motorSecond(dowelSecond);
            },
      ...
      {rivetSecond?.existingClientIds != null && !data ? (
        <CodexMobileSetupDialog
          {...{
