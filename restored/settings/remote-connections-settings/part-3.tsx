...
function olive({
  ...
  let urnPrime = CodexPluginActionType(appScopeAtom),
    vinePrime = useIntl(),
    [windPrime, yarrowPrime] = prism.useState(null),
    [azurePrime, birchPrime] = prism.useState(""),
    [canyonPrime, dewPrime] = prism.useState(false),
    [everPrime, fieldPrime] = prism.useState(false),
    grainPrime = windPrime != null,
    havenPrime = () => {
      yarrowPrime(null);
      birchPrime("");
      dewPrime(false);
    ...
    inkPrime = ({
      ...
      abortPendingLogin && windPrime?.abort();
      havenPrime();
      onOpenChange(false);
    },
    jadeitePrime = kernelPrime => {
      urnPrime.get(toastAtom).warning(vinePrime.formatMessage({
        ...
        message: AVATAR_OVERLAY_SURFACE_STACK_IDS(kernelPrime)
      }));
    };
  return <UsePointerSurfaceInteractionGate {...{
    open,
    onOpenChange: leafPrime => {
      if (!leafPrime) {
        inkPrime({
    ...
    children: <DeferredUiB {...{
      children: [<RealtimeVoiceHostId {...{
        className: "sr-only",
        children: vinePrime.formatMessage({
        ...
        children: <div className="flex items-start justify-between gap-3">
                        {<DeferredUiH {...{
            ...
            "aria-label": vinePrime.formatMessage({
        ...
        children: <div className="flex justify-center pt-2">
                        {<OnboardingLoginContent {...{
            apiKeyValue: azurePrime,
            isApiKeyEntryVisible: canyonPrime,
            isApiKeySignInPending: everPrime,
            isChatGptSignInPending: grainPrime,
            ...
            onApiKeySubmit: async () => {
              if (hostId == null) {
                jadeitePrime("Remote connection manager is unavailable.");
                return;
              }
              let maplePrime = azurePrime.trim();
              if (!(!maplePrime || everPrime)) {
                fieldPrime(true);
                try {
                  await canonicalizeWorkspacePathKey("login-with-api-key-for-host", {
                    hostId,
                    apiKey: maplePrime
                  });
                  inkPrime({
                  ...
                  appActionSidebarProjectRefSchema.warning(`${reef} api_key_sign_in_failed`, {
                    safe: {},
                    sensitive: {
                      error: nimbusPrime,
                      hostId
                  ...
                  jadeitePrime(nimbusPrime);
                } finally {
                  fieldPrime(false);
                }
              }
            },
            onApiKeyValueChange: birchPrime,
            onChatGptSignIn: async () => {
              ...
              let opalPrime = new AbortController();
              yarrowPrime(opalPrime);
              try {
                let {
                  authUrl,
                  completion
                  ...
                  hostId,
                  signal: opalPrime.signal
                });
                PdfPermissionFlag({
                  href: ensureBuildChatgptDesktopAuthUrlInit({
                    authUrl,
                    codexAppVersion: ensureChatgptDesktopAuthUrlInit(urnPrime),
                ...
                let plumePrime = await completion;
                if (!plumePrime.success) {
                  appActionSidebarProjectRefSchema.warning(`${reef} chatgpt_sign_in_failed`, {
                    safe: {},
                    sensitive: {
                      hostId,
                      error: plumePrime.error ?? "Unknown error"
                    }
                  });
                  jadeitePrime(plumePrime.error ?? "Unknown error");
                  return;
                }
                inkPrime({
                  abortPendingLogin: false
                ...
                if (quillowPrime?.name === "AbortError") return;
                appActionSidebarProjectRefSchema.warning(`${reef} chatgpt_sign_in_failed`, {
                  safe: {},
                  sensitive: {
                    error: quillowPrime,
                ...
                jadeitePrime(quillowPrime);
              } finally {
                yarrowPrime(null);
              }
            },
            onShowApiKeyEntry: () => dewPrime(true),
...
var prism,
  ...
  sage = esmInit(() => {
    ...
    posixPathBasename();
    reef = "[remote-connections/auth]";
  });
function topaz(rootPrime) {
  let {
      emptyState,
      footer,
      list,
    ...
    silkPrime = <DeferredUiU {...{
      children: <DeferredUiH {...{
        title
      }} />
    }} />;
  let thornPrime = list == null ? "h-full" : "max-h-full overflow-y-auto",
    uplandPrime = list ?? <div className="flex h-full items-center justify-center px-4 text-sm text-token-text-tertiary">
        {emptyState}
      </div>;
  let vistaPrime = <DeferredUiU {...{
    className: "pt-6",
    children: <div className="h-[300px]">
            {<ParseUrlOrFallback {...{
        className: thornPrime,
        children: uplandPrime
      }} />}
          </div>
  }} />;
  let wispPrime = <DeferredUiU {...{
    className: "pt-6",
    children: footer
  }} />;
  let yonderPrime = <DeferredUiB {...{
    as: "form",
    onSubmit,
    children: [silkPrime, vistaPrime, wispPrime]
  }} />;
  return <UsePointerSurfaceInteractionGate {...{
    open,
    onOpenChange,
    children: yonderPrime
  }} />;
}
var ultra,
  vapor,
...
function yarn(zenithPrime) {
  ...
  let cragPrime = beaconPrime,
    domePrime = <span className="select-none">
  ...
  let eddyPrime = anvilPrime.formatMessage({
  ...
  let fjordPrime = refreshing || isSaving,
    glenPrime = <AppIconAlt {...{
      className: "icon-sm"
    }} />;
  let hearthPrime = <ReadLoginRouteQuerySnapshot {...{
    "aria-label": eddyPrime,
    color: "ghost",
    size: "icon",
    disabled: fjordPrime,
    loading: refreshing,
  ...
  let inletPrime = <MemoizedFormattedMessage {...{
  ...
  let jettyPrime = <button type="button" className="min-w-0 flex-1 cursor-interaction text-left text-sm text-token-text-primary" onClick={onAddManually}>
      {inletPrime}
    </button>;
  let knobPrime = isSaving || !connections.some(item => selectedHostIds.includes(item.hostId));
  let ledgePrime = <MemoizedFormattedMessage {...{
  ...
  let mirePrime = <ReadLoginRouteQuerySnapshot {...{
    type: "submit",
    size: "large",
    disabled: knobPrime,
    children: ledgePrime
  }} />;
  let nookPrime = <div className="flex items-center gap-3">
      {hearthPrime}
      {jettyPrime}
      {mirePrime}
    </div>;
  let oxbowPrime = connections.length === 0 ? null : connections.map(item => {
    let quarryPrime = item.sshAlias?.trim() || item.displayName,
      rapidsPrime = selectedHostIds.includes(item.hostId);
    return <div key={item.hostId} aria-disabled={isSaving} className={isSaving ? "cursor-not-allowed" : "cursor-interaction"} onClick={isSaving ? undefined : () => {
      onToggleConnection(item.hostId);
    }} onKeyDown={isSaving ? undefined : event => {
      event.key !== "Enter" && event.key !== " " || (event.preventDefault(), onToggleConnection(item.hostId));
    }} role="button" tabIndex={isSaving ? -1 : 0}>
              {<EnsurePersonalizationCInit {...{
        icon: <AppIconKS className="icon-sm" />,
        label: quarryPrime,
        description: <span className="[text-wrap:wrap] break-all">
                        {item.sshHost}
                      </span>,
        control: <div className="relative flex items-center" onClick={acorn} onKeyDown={zephyr}>
                        {<Queued {...{
            "aria-label": anvilPrime.formatMessage({
  ...
  let pondPrime = <MemoizedFormattedMessage {...{
...
function zephyr(event) {
  event.stopPropagation();
}
function acorn(event) {
  event.stopPropagation();
}
var bloom,
  coral,
  drift = esmInit(() => {
    bloom = reactCompilerRuntime();
    ensureIntlFormattersInit();
    ensureSettingsQueryAtomsInit();
...
function frost(spurPrime) {
  return {
    displayName: spurPrime.displayName,
    targetKind: spurPrime.sshAlias?.trim() ? "alias" : "hostname",
    sshHost: spurPrime.sshAlias?.trim() || spurPrime.sshHost,
    sshPort: spurPrime.sshPort == null ? "" : String(spurPrime.sshPort),
    authMode: spurPrime.identity == null ? "none" : "identity",
    identity: spurPrime.identity ?? ""
  };
}
function glide(torPrime, {
  connectionAnalyticsId
} = {}) {
  let updraftPrime = torPrime.displayName.trim(),
    vergePrime = torPrime.sshHost.trim(),
    weirPrime = torPrime.targetKind === "alias" ? vergePrime : null;
  return weirPrime == null ? {
    hostId: remoteSshCodexManagedHostKey(updraftPrime),
    connectionAnalyticsId,
    displayName: updraftPrime,
    source: "codex-managed",
    alias: null,
    hostname: vergePrime,
    sshPort: iris(torPrime.sshPort),
    identity: torPrime.authMode === "identity" ? torPrime.identity.trim() : null
  } : {
    hostId: remoteSshDiscoveredHostKey(weirPrime),
    connectionAnalyticsId,
    displayName: updraftPrime,
    source: "discovered",
    alias: weirPrime,
...
function honey({
  ...
  let yardPrime = [],
    anchorPrime = draft.displayName.trim();
  anchorPrime.length === 0 && yardPrime.push("displayNameRequired");
  draft.sshHost.trim().length === 0 && yardPrime.push("sshHostRequired");
  let boltPrime = draft.sshPort.trim();
  if (boltPrime.length > 0) if (!/^\d+$/.test(boltPrime)) yardPrime.push("sshPortInteger");else {
    let cogPrime = Number(boltPrime);
    (!Number.isSafeInteger(cogPrime) || cogPrime < 1 || cogPrime > 65535) && yardPrime.push("sshPortRange");
  }
  return draft.authMode === "identity" && draft.identity.trim().length === 0 && yardPrime.push("identityRequired"), existingConnections.some(item => editingHostId != null && item.hostId === editingHostId ? false : item.displayName.trim() === anchorPrime) && yardPrime.push("duplicateDisplayName"), yardPrime;
}
function iris(discPrime) {
  let edgePrime = discPrime.trim();
  if (edgePrime.length === 0 || !/^\d+$/.test(edgePrime)) return null;
  let forgePrime = Number(edgePrime);
  return Number.isSafeInteger(forgePrime) ? forgePrime : null;
}
var jewel = esmInit(() => {
  strongMarkerFromOptions();
});
