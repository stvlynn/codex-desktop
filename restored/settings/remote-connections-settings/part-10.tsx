...
export function RemoteConnectionsSettingsIcon()  {
  let torThird = CodexPluginActionType(appScopeAtom),
    ...
    {
      data: gearThird
    } = ensurePersonalizationK0Init(DesktopPersistenceKeys.HOST_ID_REMOTE_CONTROL_ALLOWED),
    [hingeThird] = PLEASE_IMPLEMENT_THIS_PLAN_HEADER("remote_control_connections"),
    [ironThird] = PLEASE_IMPLEMENT_THIS_PLAN_HEADER("local_remote_control_client_id"),
    [jointThird] = PLEASE_IMPLEMENT_THIS_PLAN_HEADER("remote_control_connections_state"),
    ...
    {
      data
    } = ensurePersonalizationK0Init(DesktopPersistenceKeys.ADDED_REMOTE_CONTROL_ENV_IDS),
    ...
    collarThird = cogThird ? _s : AppInitialNQ({
      addedRemoteControlEnvIds: data,
      remoteControlConnections: tappetThird
    }),
    ...
    keeperThird = shimThird.map(item => glide(frost(item), {
    ...
    camThird = connectionsTab != null && raceThird == null || connectionsAction != null && (connectionsTab !== "ssh" || connectionsAction !== "add"),
    ...
    helixThird = () => {
      torThird.get(toastAtom).success(weirThird.formatMessage({
    ...
    $e = jadeFourth => {
      ...
      torThird.get(toastAtom).danger(jadeFourth instanceof Error ? jadeFourth.message : weirThird.formatMessage({
    ...
    impellerThird = kiteFourth => {
      appActionSidebarProjectRefSchema.warning(`${idler} ssh_config_entry_not_found`, {
        safe: {},
        sensitive: {
          sshConfigName: kiteFourth
        }
      });
      torThird.get(toastAtom).danger(weirThird.formatMessage({
    ...
    landThird = applyU0000U001FU007F2("authorize-remote-control-connections", {
      ...
      onError: lemonFourth => {
        ...
        appActionSidebarProjectRefSchema.warning(`${idler} remote_control_authorize_failed`, {
          safe: {},
          sensitive: {
            error: lemonFourth
          }
        });
        torThird.get(toastAtom).danger(lemonFourth instanceof ensureReviewFileAppActionAttrsInit && lemonFourth.errorCode === "remote_control_enrollment_account_mismatch" ? weirThird.formatMessage({
    ...
    meshThird = applyU0000U001FU007F2("save-codex-managed-remote-ssh-connections", {
      onSuccess: () => {
        torThird.get(toastAtom).success(weirThird.formatMessage({
    ...
    neckThird = noop({
      mutationFn: nickelFourth => readLoginRouteQuerySnapshotStub(torThird, DesktopPersistenceKeys.ADDED_REMOTE_CONTROL_ENV_IDS, nozzle.default([...data, ...nickelFourth]))
    }),
    padThird = noop({
      ...
      onError: onyxFourth => {
        appActionSidebarProjectRefSchema.warning(`${idler} remote_control_pairing_failed`, {
          safe: {},
          sensitive: {
            error: onyxFourth
          }
    ...
    quillshaftThird = applyU0000U001FU007F2("set-remote-connection-auto-connect", {
      onError: (pearlFourth, {
        autoConnect,
        hostId
      }) => {
        let quartzFourth = shimThird.find(item => item.hostId === hostId);
        autoConnect && quartzFourth != null && AppInitialU9(updraftThird, {
          action: CodexRemoteSshConnectionAction.CODEX_REMOTE_SSH_CONNECTION_ACTION_CONNECT,
          result: CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_FAILED,
          source: CodexRemoteSshConnectionEventSource.CODEX_REMOTE_SSH_CONNECTION_EVENT_SOURCE_SETTINGS,
          connection: quartzFourth,
          errorCategory: wtt.CODEX_REMOTE_SSH_CONNECTION_ERROR_CATEGORY_CONNECTION_FAILED
        });
        appActionSidebarProjectRefSchema.warning(`${idler} connect_toggle_failed`, {
          safe: {
            autoConnect
          },
          sensitive: {
    ...
    rollerThird = applyU0000U001FU007F2("install-remote-codex", {
      onError: (riverFourth, {
        ...
        appActionSidebarProjectRefSchema.warning(`${idler} install_remote_codex_failed`, {
          safe: {},
          sensitive: {
            error: riverFourth,
            hostId
        ...
        torThird.get(toastAtom).danger(riverFourth instanceof Error ? riverFourth.message : weirThird.formatMessage({
    ...
    spindleThird = applyU0000U001FU007F2("rename-remote-control-environment", {
      ...
      onError: (slateFourth, {
        ...
        appActionSidebarProjectRefSchema.warning(`${idler} remote_control_rename_failed`, {
          safe: {},
          sensitive: {
            envId,
            error: slateFourth
    ...
    thrustThird = applyU0000U001FU007F2("delete-remote-control-environment", {
      ...
      onError: (timberFourth, {
        envId
      }) => {
        appActionSidebarProjectRefSchema.warning(`${idler} remote_control_delete_failed`, {
          safe: {},
          sensitive: {
            envId,
            error: timberFourth
    ...
    baffleThird = journalThird.isPending || at.isPending || kingpinThird.isPending || landThird.isPending || meshThird.isPending || neckThird.isPending || padThird.isPending || quillshaftThird.isPending || rollerThird.isPending || spindleThird.isPending || thrustThird.isPending,
    capstanThird = baffleThird || flangeThird || cogThird && ironThird == null,
    diaphragmThird = capstanThird ? <BirchFifth {...{
      authRequired: flangeThird,
      refreshRequired: cogThird && ironThird == null,
      updating: baffleThird
    }} /> : null,
    elbowThird = umbraFourth => {
      vergeThird.getForHostId(umbraFourth) ?? vergeThird.addManager(AppInitialF4(torThird, umbraFourth, torThird.get(DeferredUiR22, umbraFourth)));
    },
    ferruleThird = async () => {
      try {
        let violetFourth = [];
        AppInitialU9(updraftThird, {
          action: CodexRemoteSshConnectionAction.CODEX_REMOTE_SSH_CONNECTION_ACTION_REFRESH,
          result: CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_STARTED,
          source: CodexRemoteSshConnectionEventSource.CODEX_REMOTE_SSH_CONNECTION_EVENT_SOURCE_SETTINGS
        });
        violetFourth.push(journalThird.mutateAsync(undefined).then(({
          remoteConnections
        }) => {
          updraftThird.logProductEvent(CODEX_REMOTE_SSH_CONNECTION_EVENT_TYPE, buildRemoteSshConnectionsRefreshedPayload(remoteConnections.filter(firstNonNullFromParsers)));
        }).catch(error => {
          throw AppInitialU9(updraftThird, {
            action: CodexRemoteSshConnectionAction.CODEX_REMOTE_SSH_CONNECTION_ACTION_REFRESH,
            result: CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_FAILED,
            source: CodexRemoteSshConnectionEventSource.CODEX_REMOTE_SSH_CONNECTION_EVENT_SOURCE_SETTINGS,
            errorCategory: wtt.CODEX_REMOTE_SSH_CONNECTION_ERROR_CATEGORY_UNKNOWN
          }), error;
        }));
    ...
    grommetThird = async () => {
      ...
      } = await at.mutateAsync(undefined);
    ...
    injectorThird = yellowFourth => {
      torqueThird(zincFourth => zincFourth.includes(yellowFourth) ? zincFourth.filter(item => item !== yellowFourth) : [...zincFourth, yellowFourth]);
    },
    jumperThird = () => {
      if (neckThird.isPending) return;
      let amberFourth = collarThird.filter(item => screwThird.includes(item.envId));
      ...
      neckThird.mutate(amberFourth.map(item => item.envId), {
        onSuccess: () => {
    ...
    kerfThird = basaltFourth => {
      ironThird == null || padThird.isPending || padThird.mutate({
        clientId: ironThird,
    ...
    manifoldThird = async () => {
      ...
      return updraftThird.logProductEvent(installModalBrowserExtensionMessages, tide(remoteControlConnections)), remoteControlConnections;
    },
    nipThird = platen.useEffectEvent(async cedarFourth => {
      if (!baffleThird) try {
        let daisyFourth = [];
        daisyFourth.push(invokeDesktopRpc("refresh-remote-connections", {
          signal: cedarFourth
        }));
        leverThird && daisyFourth.push(invokeDesktopRpc("refresh-remote-control-connections", {
          signal: cedarFourth
        }));
        await Promise.all(daisyFourth);
      } catch (emberFourth) {
        if (emberFourth instanceof DOMException && emberFourth.name === "AbortError") return;
        appActionSidebarProjectRefSchema.debug(`${idler} auto_refresh_failed`, {
          safe: {},
          sensitive: {
            error: emberFourth
          }
  ...
  let outletThird = async () => {
    ...
    strainerThird = novaFourth => {
      ...
      let oliveFourth = keystoneThird.mode === "edit" ? shimThird.find(item => item.hostId === keystoneThird.hostId) ?? null : null,
        prismFourth = keystoneThird.mode === "add" ? true : oliveFourth?.autoConnect ?? false,
        quillFourth = glide(novaFourth, {
          connectionAnalyticsId: oliveFourth?.connectionAnalyticsId
        }),
        reefFourth = keystoneThird.mode === "add" ? [...keeperThird, quillFourth] : keeperThird.filter(item => item.hostId !== keystoneThird.hostId).concat(quillFourth).sort(gasket);
      meshThird.mutate({
        ...
        onSuccess: ({
          ...
          let sageFourth = remoteConnections.find(item => item.hostId === quillFourth.hostId) ?? quillFourth;
          AppInitialU9(updraftThird, {
            action: CodexRemoteSshConnectionAction.CODEX_REMOTE_SSH_CONNECTION_ACTION_SAVE_CONNECTION,
            result: CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_SUCCEEDED,
            source: CodexRemoteSshConnectionEventSource.CODEX_REMOTE_SSH_CONNECTION_EVENT_SOURCE_SETTINGS,
    ...
    teeThird = topazFourth => {
      yokeThird || meshThird.mutate({
        remoteConnections: keeperThird.filter(item => item.hostId !== topazFourth)
      });
    },
    unionThird = ultraFourth => {
      rivetThird(vaporFourth => vaporFourth.includes(ultraFourth) ? vaporFourth.filter(item => item !== ultraFourth) : [...vaporFourth, ultraFourth]);
    },
    ventThird = () => {
      if (yokeThird) return;
      let wheatFourth = mandrelThird.filter(item => pistonThird.includes(item.hostId));
      if (wheatFourth.length === 0) {
        axleThird(false);
        return;
      }
      let yarnFourth = wheatFourth.map(item => glide({
        ...frost(item),
    ...
    wyeThird = (zephyrFourth, acornFourth) => {
      yokeThird || (elbowThird(zephyrFourth), quillshaftThird.mutate({
        hostId: zephyrFourth,
        ...
        onSuccess: ({
          ...
          let bloomFourth = remoteConnections.find(item => item.hostId === zephyrFourth && firstNonNullFromParsers(item));
          if (acornFourth && bloomFourth != null) {
            let coralFourth = state === "connected" && error == null;
            AppInitialU9(updraftThird, {
              action: CodexRemoteSshConnectionAction.CODEX_REMOTE_SSH_CONNECTION_ACTION_CONNECT,
              result: coralFourth ? CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_SUCCEEDED : CodexRemoteSshConnectionResult.CODEX_REMOTE_SSH_CONNECTION_RESULT_FAILED,
              source: CodexRemoteSshConnectionEventSource.CODEX_REMOTE_SSH_CONNECTION_EVENT_SOURCE_SETTINGS,
              connection: bloomFourth,
              connectionState: state,
              ...(coralFourth ? {} : error == null ? {
                errorCategory: wtt.CODEX_REMOTE_SSH_CONNECTION_ERROR_CATEGORY_UNKNOWN
              } : {
