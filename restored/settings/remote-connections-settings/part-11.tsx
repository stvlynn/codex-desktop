...
// Wave5d careful split (part-11 resplit 1/3)
...
const packingThird: any = undefined;
...
const mandrel: any = undefined;
const platen: any = undefined;

export function RemoteConnectionsSettingsIcon__splitRest1(__splitParams: any) {
  platen.useEffect(() => {
    trunnionThird == null ||
      nozzleThird ||
      yokeThird ||
      mandrel.has(trunnionThird) ||
      (mandrel.add(trunnionThird),
      zenerThird(trunnionThird).finally(() => {
        mandrel.delete(trunnionThird);
      }));
  }, [nozzleThird, yokeThird, trunnionThird]);
  let alphaFourth = async (frostFourth) => {
      bushingThird.includes(frostFourth) ||
        (await readLoginRouteQuerySnapshotStub(
          torThird,
          DesktopPersistenceKeys.HOST_ID_REMOTE_CONTROL_ALLOWED,
          [...bushingThird, frostFourth],
        ));
    },
    bravoFourth = () => {
      headerThird();
    },
    copperFourth = async (glideFourth) => {
      try {
        await canonicalizeWorkspacePathKey("logout", {
          hostId: glideFourth,
        });
        packingThird(glideFourth, "error", {
          code: "login-required",
        });
      } catch (honeyFourth) {
        appActionSidebarProjectRefSchema.warning(`${idler} logout_failed`, {
          safe: {},
          sensitive: {
            error: honeyFourth,
            hostId: glideFourth,
    ...
    harborFourth = [
      ...(nippleThird ? echoFourth : []),
      ...(orificeThird ? deltaFourth : []),
      ...falconFourth,
      ...(be ? gammaFourth : []),
    ];
