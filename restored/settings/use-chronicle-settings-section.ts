...
export function setUseChronicleSettingsSectionPeers(next: UseChronicleSettingsSectionPeers): void {
  peers = next;
}
...
export function useChronicleSettingsSection(e: unknown, t: unknown) {
  ...
  return t instanceof peers.P6n ? (e.get(peers.rh).danger(e.get(peers.LE).formatMessage({
  ...
  }), !0) : peers.pxu(t) ? (e.get(peers.rh).danger(e.get(peers.LE).formatMessage({
