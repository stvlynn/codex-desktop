...
// Lane b of 2 — split of part-3.tsx at top-level declaration boundaries.
import {
  ChatGptPlanId,
...
const UsageSettingsHelper59: any = undefined;
...
function UsageSettingsHelper62(pinPrime) {
  let {
    ...
    racePrime =
      isResetFillActive && resetFillRateLimitStatus != null
        ? resetFillRateLimitStatus
        : rateLimitStatus,
    sleevePrime,
    trunnionPrime,
  ...
  bb0: {
    let camPrime = AppInitialHb(racePrime),
      ...
      eccentricPrime =
        isResetFillActive && resetFillRateLimitStatus != null
          ? oak(petal(AppInitialHb(resetFillRateLimitStatus), null))
          : null,
      followerPrime = showEnterpriseMonthlyUsageLimit
        ? seed(workspaceMonthlyUsage, racePrime)
        : rain({
            rateLimitStatus: racePrime,
            coreUsageLimitRows: detentPrime,
          }),
      guidePrime =
        followerPrime == null ? detentPrime : detentPrime.filter(nest),
            ...
            children: [
                  ...
                  title: (
                    <MemoizedFormattedMessage
                      {...{
                        ...
                        description:
                          "Title for the usage limits section in usage settings",
                      }}
                    />
                  ),
                }}
              ...
              <UseChromeAndCodeThemeSync.Content
                {...{
                  children: [
                    kingpinPrime ? (
                      <UsageSettingsHelper65
                        {...{
                          currentMonthUsage:
                            workspaceMonthlyUsage?.current_month_usage ?? null,
                        }}
                      />
                    ) : null,
                    followerPrime != null || guidePrime.length > 0 ? (
                      <ParseUrlOrFallback
                        {...{
                          children: [
                            followerPrime == null ? null : (
                              <$r
                                key={followerPrime.key}
                                {...{
                                  isResetFillActive: isResetFillActive,
                                  resetFillAnimation,
                                  resetFillRemainingPercent:
                                    eccentricPrime?.get(followerPrime.key),
                                  usageLimit: followerPrime,
                                }}
                              />
                            ),
                            guidePrime.map((item) => (
                              <UsageSettingsHelper67
                                key={item.key}
                                {...{
                                  isResetFillActive: isResetFillActive,
                                  rateLimitRow: item,
                                          ...
                                          <MemoizedFormattedMessage
                                            {...{
                                              ...
                                              defaultMessage:
                                                "Request Increase",
                                              description:
                                                "Member-facing button in Codex usage settings that opens the workspace's custom HTTPS monthly usage-limit-increase request page in the browser.",
                                            }}
                                          />
                                        ),
                                      }}
                  ...
                  title: (
                    <MemoizedFormattedMessage
                      {...{
                        ...
                        description:
                          "Title for the GPT-5.3-Codex-Spark usage limits section",
                      }}
                    />
                  ),
                }}
  ...
  if (sleevePrime !== Symbol.for("react.early_return_sentinel"))
    return sleevePrime;
  return (
    <>
      {trunnionPrime}
...
function UsageSettingsHelper65(neckPrime) {
  let { currentMonthUsage } = neckPrime,
    padPrime = (
      <EnsurePersonalizationCInit
        {...{
          icon: (
          ...
          label: (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Informational message shown when the administrator has not set a monthly usage limit for the current user.",
              }}
            />
          ),
          control: null,
          ...
          label: (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Label for the current user's monthly credit usage when no monthly usage limit is set.",
              }}
            />
          ),
          control: (
            <span className="text-sm text-token-text-secondary tabular-nums">
              {
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Credit amount used by the current user this month when no monthly usage limit is set.",
                    values: {
  ...
  let { pendingRequest, isLoading, isSaving, saveWorkspaceAdminRequest } =
    ...
    elbowPrime = () => {
      ...
      saveWorkspaceAdminRequest?.({
        ...
        .then(() => {
          spindlePrime.get(toastAtom).success(
            thrustPrime.formatMessage(
              pendingRequest == null
                ? {
                    ...
                    description:
                      "Toast shown when a workspace usage limit request is submitted",
                  }
                : {
                    id: "settings.usage.limits.requestUpdated",
                    defaultMessage: "Request updated",
                    description:
                      "Toast shown when a workspace usage limit request is updated",
                  },
            ),
          );
          bafflePrime(false);
        ...
        <div className="text-sm text-token-text-secondary">
          {
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Status text for a pending workspace usage limit request",
              }}
            />
          }
        </div>
    ...
    pendingRequest == null ? (
      ...
      <MemoizedFormattedMessage
        {...{
          ...
          description:
            "Button to update an existing pending workspace usage limit request",
        }}
      />
    );
  let kerfPrime = (
            ...
            <label className="flex flex-col gap-1.5 text-sm font-medium text-token-text-primary">
              {
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Label for the workspace usage limit request justification",
                  }}
                />
              }
              <textarea
                    ...
                    children: (
                      <MemoizedFormattedMessage
                        {...{
                          ...
                          description:
                            "Cancel button for the workspace usage limit request form",
                        }}
                      />
                    ),
                  }}
                      ...
                      pendingRequest == null ? (
                        <MemoizedFormattedMessage
                          {...{
                            ...
                            description:
                              "Submit button for a workspace usage limit request",
                          }}
                        />
                      ) : (
                        <MemoizedFormattedMessage
                          {...{
                            ...
                            description:
                              "Submit button for updating a pending workspace usage limit request",
                          }}
                        />
                      ),
                  }}
