...
function $r(nipPrime) {
    ...
    strainerPrime;
  {
    ...
    strainerPrime = echoSecond == null ? null : mapQuickChatSurfaceMode(echoSecond);
  }
  let teePrime = strainerPrime,
    unionPrime = usageLimit.usedCredits == null || usageLimit.limitCredits == null ? null : outletPrime.formatMessage({
  ...
  let ventPrime = unionPrime,
    wyePrime = teePrime == null ? null : <MemoizedFormattedMessage {...{
  ...
  let zenerPrime = wyePrime,
    alphaSecond = <MemoizedFormattedMessage {...{
      id: "settings.usage.limits.monthly.label",
      defaultMessage: "Monthly usage limit",
      description: "Label for a monthly usage limit row"
    }} />;
  let bravoSecond = ventPrime != null || zenerPrime != null ? <span className="flex flex-wrap gap-x-2">
  ...
  let deltaSecond = <UsageSettingsHelper78 {...{
...
function UsageSettingsHelper67(falconSecond) {
    ...
    gammaSecond;
  {
    ...
    gammaSecond = pearlSecond == null ? null : mapQuickChatSurfaceMode(pearlSecond);
  }
  let harborSecond = gammaSecond,
    indigoSecond = harborSecond == null ? null : <MemoizedFormattedMessage {...{
      id: "settings.usage.limits.window.resetAt",
      defaultMessage: "Resets {time}",
      description: "Description showing when a usage limit window resets",
      values: {
  ...
  let onyxSecond = <UsageSettingsHelper78 {...{
    isResetFillActive,
...
function oak(quartzSecond) {
  return new Map(quartzSecond.map(item => [item.key, setQuickChatDockPosition(item.bucket.usedPercent ?? 0)]));
}
function petal(riverSecond, slateSecond) {
  let timberSecond = slateSecond == null ? riverSecond.find(item => item.limitName == null) ?? null : AppInitialVb(riverSecond, slateSecond);
  return timberSecond == null ? [] : AppInitialT({
    entry: timberSecond,
...
function seed(zincSecond, amberSecond) {
  ...
  return basaltSecond == null ? null : {
    key: "workspace-monthly",
    remainingPercent: basaltSecond.remainingPercent,
    resetAt: Number(amberSecond?.spend_control?.individual_limit?.limit) === basaltSecond.limit ? amberSecond?.spend_control?.individual_limit?.reset_at ?? null : null,
    usedCredits: String(basaltSecond.used),
    limitCredits: String(basaltSecond.limit)
  };
}
function trail(cedarSecond) {
  return cedarSecond?.items.find(daisySecond => daisySecond.status === "pending" && urn(daisySecond) != null) ?? null;
}
function urn(emberSecond) {
  return emberSecond.payloads.find(vine) ?? null;
}
function vine(event) {
  return event.kind === "spend_limit" && event.target.source === haven && event.target.source_id === ink;
}
function wind(flintSecond) {
  return ui(flintSecond.bucket.windowDurationMins ?? 0, grain);
}
function ui(garnetSecond, hazelSecond) {
  return !Number.isFinite(garnetSecond) || garnetSecond <= 0 ? false : Math.abs(garnetSecond - hazelSecond) <= hazelSecond * 0.05;
}
function yarrow(ivorySecond, jasperSecond) {
  let kelpSecond = Number(jasperSecond);
  return Number.isFinite(kelpSecond) ? ivorySecond.formatNumber(Math.max(kelpSecond, 0), {
...
function UsageSettingsHelper78(lotusSecond) {
  let {
    ...
    quillSecond = isResetFillActive && resetFillRemainingPercent == null ? undefined : resetFillAnimation,
    reefSecond = quillSecond?.progress ?? prismSecond,
    sageSecond = isResetFillActive && quillSecond != null && resetFillRemainingPercent != null,
    topazSecond = driftSecond => oliveSecond + (100 - oliveSecond) * driftSecond;
  let ultraSecond = FormattedDate(reefSecond, topazSecond),
    ...
    yarnSecond = accessibleLabel ?? mintSecond.formatMessage({
  ...
  let zephyrSecond = yarnSecond,
    acornSecond = sageSecond ? <ensureIntlFormattersInit.span key={"remaining-percent"} {...{
      children: vaporSecond
    }} /> : mintSecond.formatNumber(novaSecond, {
      maximumFractionDigits: 0
    }),
    bloomSecond = <div className="w-24">
        {quillSecond == null ? <progress max={100} value={novaSecond} aria-label={zephyrSecond} className="h-1.5 w-24 overflow-hidden rounded-full [&::-moz-progress-bar]:bg-token-foreground [&::-ms-fill]:bg-token-foreground [&::-webkit-progress-bar]:bg-token-foreground/10 [&::-webkit-progress-value]:bg-token-foreground" /> : <AnimatedPercentFillBar accessibleLabel={zephyrSecond} animatedPercent={ultraSecond} isResetFillActive={sageSecond} remainingPercent={wheatSecond} resetFillAnimation={quillSecond} />}
      </div>;
  let coralSecond = <span className="min-w-[72px] text-right text-sm whitespace-nowrap text-token-text-secondary tabular-nums">
      {remainingLabel === "remaining" ? <MemoizedFormattedMessage {...{
...
function birch(frostSecond) {
  return Number.isFinite(frostSecond) ? Math.max(0, Math.min(100, frostSecond)) : 0;
}
var canyon,
  dew,
  ever,
...
export function UsageSettings() {
  let {
    ...
    glideSecond = usePrefersReducedMotion(),
    honeySecond = NativeContextMenuSurface("85924660"),
    irisSecond = AppInitialBv(glideSecond),
    [jewelSecond, knollSecond] = root.useState(null),
    [lunarSecond, mossSecond] = root.useState(null),
    northSecond = isBusinessPlanExcludingEducation(plan);
  let orbitSecond = northSecond,
    pineSecond = orbitSecond && isUsageSettingsVisible && !isUsageSettingsAccessLoading,
    {
      data = null,
      isLoading,
      isError,
    ...
    questSecond = (plan === ChatGptPlanId.FREE || plan === ChatGptPlanId.GO) && data?.credits?.balance != null,
    ridgeSecond = canManageCreditSettings || questSecond,
    stormSecond = canManageCreditSettings,
    tideSecond = {
      enabled: stormSecond
    };
  let {
      data: _data
    } = AppInitialNx(tideSecond),
    unitySecond = _data?.one_time.tiers.reduce(kernel, 0) ?? 0;
  let valeSecond = unitySecond,
    waveSecond = {
      enabled: canManageCreditSettings
    };
  let {
      data: __data
    } = quickChatSurfaceCollapsedAtom(waveSecond),
    apexSecond = (__data?.payment_methods.length ?? 0) > 0,
    brookSecond = canManageCreditSettings && true,
    cliffSecond = {
      enabled: brookSecond,
  ...
  let {
      ...
      isError: _isError,
      refetch: _refetch
    } = QueuedMessageTraySurface(cliffSecond),
    duskSecond = canManageCreditSettings && (apexSecond || ___data?.isEnabled === true && CHATGPT_COM_ADMIN_URL(___data.autoReloadCreditDiscountPolicy));
  ...
  let {
      ...
      isError: __isError,
      refetch: __refetch
    } = AppInitialCx(fernSecond),
    groveSecond = ____data?.effective_monthly_limit?.limit != null && ____data.effective_monthly_limit.limit >= 0 && ____data.effective_monthly_limit.enforcement_mode === "HARD_CAP",
    hillSecond = pineSecond && groveSecond,
    isleSecond = {
      enabled: hillSecond
    };
  let {
      data: _____data,
      refetch: ___refetch
    } = AppInitialLx(isleSecond),
    juniperSecond = _____data?.kind === "openai_native",
    lagoonSecond = pineSecond && groveSecond && juniperSecond,
    meadowSecond = {
  ...
  let {
      ...
      isError: ___isError,
      refetch: ____refetch
    } = useEffectEventWhenReady(meadowSecond),
    nestSecond = AppInitialOx(),
    {
      enableAutoTopUpMutation,
      updateAutoTopUpMutation,
      disableAutoTopUpMutation
    } = AppInitialEx(),
    oakSecond = <SettingsSectionTitle slug="usage" />;
  let petalSecond = oakSecond;
  if (!isUsageSettingsVisible && !ridgeSecond && !isUsageSettingsAccessLoading) return null;
  let quietSecond = !isUsageSettingsVisible && !ridgeSecond && isUsageSettingsAccessLoading,
    rainSecond = orbitSecond ? __isLoading && ____data == null || groveSecond && juniperSecond && ___isLoading && ______data == null : elmSecond && _isLoading && ___data == null || isLoading && data == null,
    seedSecond = orbitSecond ? __isError && ____data == null || groveSecond && juniperSecond && ___isError && ______data == null : elmSecond && _isError && ___data == null || isError && data == null,
    trailSecond = () => {
      if (!orbitSecond) {
        if (elmSecond) {
          Promise.all([_refetch(), refetch()]);
          return;
        }
        refetch();
        return;
      ...
      __refetch();
      groveSecond && (___refetch(), juniperSecond && ____refetch());
    };
  let urnSecond = trailSecond,
    vineSecond = () => {
      knollSecond(plumeSecond => plumeSecond ?? data ?? null);
    };
  ...
  let azureSecond = yarrowSecond,
    birchSecond = () => {
      let quillowSecond = {
        ...
        rateLimitStatus: jewelSecond ?? data ?? null,
        refreshComplete: false
      };
      knollSecond(null);
      mossSecond(quillowSecond);
      irisSecond.start(() => {
        mossSecond(rootSecond => rootSecond === quillowSecond ? {
          ...rootSecond,
  ...
  let canyonSecond = birchSecond,
    dewSecond = lunarSecond != null && nimbus(lunarSecond.rateLimitStatus, data ?? null);
  let everSecond = dewSecond;
  lunarSecond != null && lunarSecond.animationComplete && !lunarSecond.refreshComplete && everSecond && mossSecond({
    ...lunarSecond,
    refreshComplete: true
  });
  let fieldSecond = honeySecond && lunarSecond != null && !lunarSecond.refreshComplete && (!lunarSecond.animationComplete || !everSecond);
  if (quietSecond || seedSecond || rainSecond) {
    let silkSecond,
      thornSecond = null;
    if (quietSecond) {
      let vistaSecond;
      vistaSecond = <MemoizedFormattedMessage {...{
      ...
      silkSecond = vistaSecond;
    } else if (seedSecond) {
      let wispSecond;
      wispSecond = <MemoizedFormattedMessage {...{
      ...
      silkSecond = wispSecond;
      let yonderSecond = <MemoizedFormattedMessage {...{
        id: "settings.usage.load.retry",
        defaultMessage: "Retry",
        description: "Retry button for usage settings fetch errors"
      }} />;
      let zenithSecond;
      zenithSecond = <ReadLoginRouteQuerySnapshot {...{
        color: "secondary",
        size: "toolbar",
        onClick: urnSecond,
        children: yonderSecond
      }} />;
      thornSecond = zenithSecond;
    } else {
      let anvilSecond;
      anvilSecond = <MemoizedFormattedMessage {...{
      ...
      silkSecond = anvilSecond;
    }
    let uplandSecond;
    return <UsageSettingsHelper84 {...{
      title: petalSecond,
      rowLabel: silkSecond,
      rowControl: thornSecond
    }} />;
  }
  if (elmSecond && ___data == null) return null;
  if (orbitSecond) {
    let beaconSecond, cragSecond;
    beaconSecond = <MemoizedFormattedMessage {...{
      id: "settings.usage.enterprise.title",
      defaultMessage: "Usage limits",
      description: "Title for enterprise usage limits settings"
    }} />;
    cragSecond = <MemoizedFormattedMessage {...{
    ...
    let domeSecond = data ?? null,
      eddySecond = ____data ?? null,
      fjordSecond = ______data ?? null,
      glenSecond;
    return <OpenInBrowser {...{
      title: beaconSecond,
      subtitle: cragSecond,
      subtitleClassName: "whitespace-normal",
      children: <UsageSettingsHelper62 {...{
        rateLimitStatus: domeSecond,
        showEnterpriseMonthlyUsageLimit: true,
        canRequestEnterpriseMonthlyUsageLimit: groveSecond,
        workspaceRequestPolicy: _____data,
        workspaceMonthlyUsage: eddySecond,
        workspaceAdminRequests: fjordSecond,
        isWorkspaceAdminRequestsLoading: ___isLoading,
        isSavingWorkspaceAdminRequest: nestSecond.isPending,
        saveWorkspaceAdminRequest: nestSecond.mutateAsync
      }} />
    }} />;
  }
  let grainSecond = <UsageSettingsHelper82 {...{}} />;
  let havenSecond = <$n {...{
    plan
  }} />;
  let inkSecond = elmSecond && ___data != null ? <UsageSettingsHelper11 {...{
    serverState: ___data,
    hasSavedPaymentMethod: apexSecond,
    maximumCreditPurchaseDiscountPercent: valeSecond,
    creditDetails: data?.credits ?? null,
    enableAutoTopUpMutation,
    updateAutoTopUpMutation,
    disableAutoTopUpMutation
  }} /> : ridgeSecond ? <UsageSettingsHelper10 {...{
    canPurchaseCredits: canManageCreditSettings,
    maximumCreditPurchaseDiscountPercent: valeSecond,
    creditDetails: data?.credits ?? null,
    hasSavedPaymentMethod: __data == null ? undefined : apexSecond
  }} /> : null;
  let jadeiteSecond = honeySecond ? irisSecond : undefined,
    kernelSecond = lunarSecond?.rateLimitStatus,
    leafSecond = <UsageSettingsHelper62 {...{
      isResetFillActive: fieldSecond,
      rateLimitStatus: data,
      resetFillAnimation: jadeiteSecond,
      resetFillRateLimitStatus: kernelSecond
    }} />;
  let mapleSecond = <UsageSettingsHelper61 {...{
    isResetFillActive: fieldSecond,
    onResetAttemptCancel: azureSecond,
    onResetAttemptStart: windSecond,
    onResetComplete: canyonSecond
  }} />;
  let nimbusSecond = <ElectronOnly {...{
    browser: true,
    children: <UsageSettingsHelper52 {...{
      showCreditHistory: ridgeSecond
    }} />
  }} />;
  let opalSecond = <UsageSettingsHelper45 {...{
    plan
  }} />;
  return <OpenInBrowser {...{
    title: petalSecond,
    subtitle: grainSecond,
    subtitleClassName: "whitespace-normal",
    children: [havenSecond, inkSecond, leafSecond, mapleSecond, nimbusSecond, opalSecond]
  }} />;
}
function kernel(hearthSecond, inletSecond) {
  return Math.max(hearthSecond, inletSecond.percent_off);
}
function UsageSettingsHelper82() {
  let jettySecond = leaf;
  return <MemoizedFormattedMessage {...{
    ...
    values: {
      settings: knobSecond => <a className="inline-flex cursor-interaction text-token-text-link-foreground" href={CHATGPT_COM_URL} target="_blank" rel="noreferrer" onClick={jettySecond}>
              {knobSecond}
            </a>
    }
  }} />;
}
function leaf(ledgeSecond) {
  deferredUiEnt({
    event: ledgeSecond,
    href: CHATGPT_COM_URL,
    initiator: "open_in_browser_bridge"
  });
}
function UsageSettingsHelper84(mireSecond) {
  let {
      title,
      rowLabel,
      rowControl
    } = mireSecond,
    nookSecond = rowControl == null ? <AppInitialNi>{rowLabel}</AppInitialNi> : <UseChromeAndCodeThemeSync {...{
...
function maple(oxbowSecond, pondSecond) {
  return plume(oxbowSecond?.rate_limit?.primary_window, pondSecond?.rate_limit?.primary_window) || plume(oxbowSecond?.rate_limit?.secondary_window, pondSecond?.rate_limit?.secondary_window);
}
function nimbus(quarrySecond, rapidsSecond) {
  return maple(quarrySecond, rapidsSecond) || !opal(quarrySecond) && !opal(rapidsSecond);
}
function opal(spurSecond) {
  return spurSecond?.rate_limit?.primary_window != null || spurSecond?.rate_limit?.secondary_window != null;
}
function plume(torSecond, updraftSecond) {
  return torSecond == null || updraftSecond == null ? torSecond != null || updraftSecond != null : torSecond.used_percent !== updraftSecond.used_percent || torSecond.reset_at !== updraftSecond.reset_at || torSecond.limit_window_seconds !== updraftSecond.limit_window_seconds;
}
var quillow, root, $;
esmInit(() => {
