...
// Lane a of 2 — split of part-2.tsx at top-level declaration boundaries.
import {
  ChatGptPlanId,
...
function UsageSettingsHelper21(deltaPrime) {
  let { saveIntent, amount, creditCount } = deltaPrime;
  return (
          ...
          saveIntent === "enable" ? (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Informational banner shown before enabling auto top up when a one-time immediate top up will occur",
                values: {
                  amount,
                  creditCount,
                  strong: slate,
            ...
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Informational banner shown before updating auto top up when a one-time immediate top up will occur",
                values: {
                  amount,
                  creditCount,
                  strong: river,
...
function river(echoPrime) {
  return (
    <span className="font-medium text-token-text-primary">{echoPrime}</span>
  );
}
function slate(falconPrime) {
  return (
    <span className="font-medium text-token-text-primary">{falconPrime}</span>
  );
}
          ...
          amount == null ? (
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Inline error shown in the auto top up settings dialog when the initial top up attempt fails without a price estimate",
                values: {
                  actionLine: umbra,
                  managePayment: (harborPrime) => (
                    <a
                      href="#"
            ...
            <MemoizedFormattedMessage
              {...{
                ...
                description:
                  "Inline error shown in the auto top up settings dialog when the initial top up attempt fails and a price estimate is available",
                values: {
                  amount,
                  actionLine: timber,
                  managePayment: (kitePrime) => (
                    <a
                      href="#"
                      className={IntlProvider(
                        "cursor-interaction font-medium underline underline-offset-2",
...
function UsageSettingsHelper29(xenonPrime) {
  ...
  return (
    <input
      ...
      className={yellowPrime}
      onChange={zincPrime}
      onBlur={onBlur}
    />
  );
}
function violet({ intl, creditDetails, pricingInfo }) {
  ...
  return creditDetails == null
    ...
    : creditDetails.unlimited
      ...
      : creditDetails.balance == null || pricingInfo == null
        ? amberPrime
        : (formatCreditReloadCurrency({
            intl,
...
function xenon({ intl, rechargeThreshold, rechargeTarget, pricingInfo }) {
  if (pricingInfo == null) return null;
  let cedarPrime = echo({
    rechargeThreshold,
    rechargeTarget,
  });
  return cedarPrime == null
    ? null
    : {
...
function zinc({ rechargeThreshold, rechargeTarget }) {
  ...
  if (
    !(
      hazelPrime.rechargeThresholdError == null &&
      hazelPrime.rechargeTargetError == null
    )
  )
    return {
      fields: {
...
function amber(ivoryPrime) {
  ...
  for (let jasperPrime of ivoryPrime)
    if (cedar(jasperPrime)) return jasperPrime;
  return null;
}
...
function cedar(mintPrime) {
  return (
    mintPrime === "missing" ||
    mintPrime === "not-whole-number" ||
    mintPrime === "below-threshold-minimum"
  );
}
function daisy(novaPrime) {
  return (
    novaPrime === "missing" ||
    novaPrime === "not-whole-number" ||
    novaPrime === "target-difference-too-small"
  );
}
