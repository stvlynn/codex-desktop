...
function north(pine) {
  let { hostId, primaryRuntime } = pine,
  ...
  let vine = urn,
    wind =
      elm.data?.bundleVersion == null || elm.data.bundleVersion.length === 0
        ? null
        : elm.data.bundleVersion,
    yarrow = () => {
      let glen = Date.now();
      grove
        .mutateAsync()
        .then((value) => {
          if (
            (logProductEvent(
              quest,
              CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_TYPE,
              FloatingSurfaceCssClass({
                diagnostics: value,
            ...
            seed.success(
              ridge.formatMessage({
                ...
                description:
                  "Toast shown when dependency diagnostics find no problems",
              }),
            );
            return;
          }
          seed.warning(
            ridge.formatMessage({
              ...
              description:
                "Toast shown when dependency diagnostics find problems",
            }),
          );
        })
        .catch(() => {
    ...
    birch = () => {
      ...
      lagoon
        ...
        .then((value) => {
          logProductEvent(
            quest,
            CODEX_PROFILE_INLINE_PHOTO_CHANGE_EVENT_TYPE,
            AppInitialNr({
          ...
          seed.success(
            ridge.formatMessage({
              ...
              description:
                "Toast shown when dependency reset and reinstall succeeds",
            }),
          );
        })
        .catch((error) => {
          if (isAbortError(error)) {
            ...
            seed.info(
              ridge.formatMessage({
                ...
                description:
                  "Toast shown when dependency reset and reinstall is canceled",
              }),
              {
        ...
        .then((value) => {
          ...
          if ((quest.set(deferredUiYn, null), elm.refetch(), !canceled)) {
            seed.info(
              ridge.formatMessage({
                ...
                description:
                  "Toast shown when canceling a Codex dependency download but no download is running",
              }),
            );
            return;
          }
          seed.info(
            ridge.formatMessage({
              ...
              description:
                "Toast shown after requesting cancellation of a Codex dependency download",
            }),
            {
              id: "install-primary-runtime",
            },
        ...
        .catch(() => {
          seed.danger(
            ridge.formatMessage({
              ...
              description:
                "Toast shown when canceling a Codex dependency download fails",
            }),
          );
        });
    };
  ...
  haven = (
    <MemoizedFormattedMessage
      {...{
        ...
        defaultMessage:
          "Allow ChatGPT to install and expose bundled Node.js and Python tools",
  ...
  root = (
    <MemoizedFormattedMessage
      {...{
        ...
        defaultMessage:
          "Downloads a fresh bundle, installs it, and reloads tools",
        description:
          "Description for resetting and reinstalling dependencies in settings",
      }}
    />
  );
        ...
        }
        {
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Button label for resetting and reinstalling dependencies",
            }}
          />
        }
      </>
  ...
  let anvil = vine ? (
    ...
    <MemoizedFormattedMessage
      {...{
        ...
        description:
          "Status while loading the current dependency bundle version",
      }}
    />
  ) : (
    (wind ?? (
  ...
  let crag = elm.data?.problems.length ? (
    <span>
      {
        <MemoizedFormattedMessage
          {...{
            ...
            description:
              "Description shown when dependency diagnostics have problems",
          }}
        />
      }
    </span>
