...
import { ensureComposerEsm_K9_Init, ensureComposerEsm_MT_Init, ensureComposerEsm_P5_Init, ensureComposerEsm_Sut_Init, ensureComposerEsm_Tft_Init, ensureComposerEsm_Utt_Init } from "../../composer/composer-esm-inits";
...
function ImportSettingsHelper1(timber) {
  ...
  let {
    ...
    garnet = flint && xenon != null ? CodeThemePreviewGlyph({
  ...
  let jasper = <ValueOrEmptyString {...{
      type: "conjunction",
      value: ember.map(ivory)
    }} />,
    kelp = () => {
      daisy != null && (logProductEvent(umbra, CodexProfileEditAction, {
        source: "settings",
        action: "shown",
  ...
  let mint = lotus,
    ...
    prism = <UseChromeAndCodeThemeSync.Header {...{
  ...
  let Quill = UseChromeAndCodeThemeSync,
    ...
    sage = flint ? <EnsurePersonalizationCInit {...{
      ...
      description: garnet == null ? null : <ValueOrEmptyString {...{
        type: "unit",
    ...
    }} /> : hazel ? <EnsurePersonalizationCInit {...{
      label: <MemoizedFormattedMessage {...{
    ...
    }} /> : isDetectionError ? <EnsurePersonalizationCInit {...{
      label: <MemoizedFormattedMessage {...{
    ...
    }} /> : daisy == null && unsupportedProjects.length === 0 && latestSuccessfulImportAtMs == null ? <EnsurePersonalizationCInit {...{
      label: <MemoizedFormattedMessage {...{
    ...
    }} /> : daisy == null && unsupportedProjects.length === 0 ? <EnsurePersonalizationCInit {...{
      label: <MemoizedFormattedMessage {...{
      ...
      description: latestSuccessfulImportAtMs == null ? null : <MemoizedFormattedMessage {...{
        ...
        values: {
          relativeTime: <INLINE_MENTION_GROUP_CLASS key={"last-imported-at"} {...{
            dateString: new Date(latestSuccessfulImportAtMs).toISOString()
          }} />
        }
      }} />,
      control: <ReadLoginRouteQuerySnapshot {...{
    ...
    }} /> : daisy == null ? null : <EnsurePersonalizationCInit {...{
      label: latestSuccessfulImportAtMs == null ? <MemoizedFormattedMessage {...{
      ...
      control: <ReadLoginRouteQuerySnapshot {...{
        ...
        size: "toolbar",
        onClick: kelp,
        children: latestSuccessfulImportAtMs == null ? <MemoizedFormattedMessage {...{
    ...
    topaz = !hazel && !isDetectionError && unsupportedProjects.length > 0 ? <EnsurePersonalizationCInit {...{
      icon: <ApplyActive {...{
      ...
      description: <MemoizedFormattedMessage {...{
        ...
        values: {
          count: unsupportedProjects.length,
          projects: <ValueOrEmptyString key={"unsupported-projects"} {...{
            type: "conjunction",
  ...
  let ultra = <ParseUrlOrFallback {...{
    children: [sage, topaz]
  }} />;
  let vapor = <Quill.Content {...{
    children: ultra
  }} />;
...
function $e(bloom) {
    ...
    eagle;
  {
    ...
    let jewel = history.failures.filter(iris),
      knoll;
    knoll = (event, orbit) => <ImportSettingsHelper5 key={`success-${orbit}`} {...{
    ...
    let lunar;
    lunar = (pine, quest) => <ImportSettingsHelper5 key={`failure-${quest}`} {...{
      itemType: category.itemType,
  ...
  let frost = category.itemType === "HOOKS" ? <EnsurePersonalizationCInit {...{
    variant: "nested",
    label: <MemoizedFormattedMessage {...{
...
function ImportSettingsHelper5(event) {
  let ridge = useIntl(),
    storm = useNavigate(),
    tide = null;
  if (event.status === "error") tide = event.failureMessage;else if (event.target == null) {
    let dusk;
  ...
  let unity = event.status === "success" && event.label != null ? harbor(event.itemType, event.label, event.target) : null;
  let vale = unity,
    wave = event.label == null ? <MemoizedFormattedMessage {...{
  ...
  let apex = wave,
    brook = event.status === "error" ? <ApplyActive {...{
      className: "icon-2xs shrink-0 text-token-editor-error-foreground"
    }} /> : null;
  let cliff = vale == null ? null : <ReadLoginRouteQuerySnapshot {...{
    ...
    onClick: () => {
      storm(vale, {
        state: event.itemType === "SKILLS" || event.itemType === "COMMANDS" ? {
          initialHostId: LOCAL_HOST_ID,
          initialTab: "skills"
        } : undefined
      });
    },
    children: event.itemType === "SKILLS" || event.itemType === "COMMANDS" ? <MemoizedFormattedMessage {...{
    ...
    }} /> : event.itemType === "MCP_SERVER_CONFIG" ? <MemoizedFormattedMessage {...{
    ...
    }} /> : event.itemType === "SESSIONS" ? <MemoizedFormattedMessage {...{
...
function harbor(elm, fern, grove) {
  return elm === "SKILLS" || elm === "COMMANDS" || elm === "MCP_SERVER_CONFIG" ? `${elm === "MCP_SERVER_CONFIG" ? "/settings/mcp-settings" : "/skills"}?search=${encodeURIComponent(grove ?? fern)}` : elm === "SESSIONS" && grove != null ? AriaIconBadge(resolveScienceModelLabel(grove)) : elm === "PLUGINS" ? LOCAL_ENVIRONMENT_CONFIG_PATH_KEY({
    hostId: LOCAL_HOST_ID,
    pluginId: grove ?? fern
  }) : null;
}
...
function At(juniper) {
    ...
    meadow;
  {
    let nest = ensureImportSettingsDsInit(runningImport.items),
      oak = <EnsurePersonalizationCInit {...{
        label: <MemoizedFormattedMessage {...{
    ...
    let petal;
    petal = quiet => <EnsurePersonalizationCInit key={quiet} {...{
      variant: "nested",
      label: deferredUiJs(lagoon, quiet),
      control: <span className="text-xs text-token-text-secondary">
              {<MemoizedFormattedMessage {...{
...
function ImportSettingsHelper8(rain) {
  ...
  let azure = wind + yarrow.length,
    ...
    ink = seed.length === 0 ? <EnsurePersonalizationCInit {...{
      variant: "nested",
      label: <MemoizedFormattedMessage {...{
      ...
      return <SettingsRowDisclosure key={item.itemType} {...{
        content: <$e {...{
          category: item,
          history
        }} />,
        contentId: silk,
        expanded: thorn,
        children: <ImportSettingsHelper13 {...{
          category: item,
          contentId: silk,
          expanded: thorn,
          history,
    ...
    kernel = <FormattedDate {...{
      value: jadeite,
      dateStyle: "medium",
      timeStyle: "short"
    }} />;
  let leaf = urn > 0 && azure > 0 ? <MemoizedFormattedMessage {...{
  ...
  }} /> : urn > 0 ? <MemoizedFormattedMessage {...{
  ...
  }} /> : azure > 0 ? <MemoizedFormattedMessage {...{
  ...
  let maple = birch && "rotate-180",
    nimbus = IntlProvider("icon-2xs text-token-input-placeholder-foreground transition-transform", maple);
  let opal = <AppIconSft {...{
    "aria-hidden": true,
    className: nimbus
  }} />;
  ...
  let quillow = <ImportSettingsHelper14 {...{
    contentId: field,
    expanded: birch,
    label: kernel,
    description: leaf,
  ...
  let root = <SettingsRowDisclosure {...{
    contentId: field,
    expanded: birch,
    content: ink,
    children: quillow
...
function ImportSettingsHelper13(beacon) {
  let {
    ...
    crag = useIntl(),
    dome = category.itemType === "MCP_SERVER_CONFIG" ? AppInitialOs({
      histories: [history],
      mcpServerStatuses
    }).length : 0;
  let eddy = dome,
    fjord = deferredUiJs(crag, category.itemType);
  let glen = category.itemType === "COMMANDS" ? <MemoizedFormattedMessage {...{
  ...
  }} /> : category.itemType === "HOOKS" && category.importedCount > 0 ? <MemoizedFormattedMessage {...{
  ...
  }} /> : eddy > 0 ? <MemoizedFormattedMessage {...{
  ...
  let hearth = category.failedCount > 0 || eddy > 0 ? <ApplyActive {...{
    className: IntlProvider("icon-2xs shrink-0", category.failedCount > 0 ? "text-token-editor-error-foreground" : "text-token-editor-warning-foreground")
  }} /> : null;
  let inlet = <span>
      {category.failedCount > 0 ? <MemoizedFormattedMessage {...{
  ...
  let jetty = expanded && "rotate-90",
    knob = IntlProvider("icon-2xs shrink-0 text-token-input-placeholder-foreground transition-transform", jetty);
  let ledge = <AppIconYlt {...{
    className: knob
  }} />;
  let mire = <div className="flex shrink-0 items-center gap-3 text-xs whitespace-nowrap text-token-text-secondary">
      {hearth}
      {inlet}
      {ledge}
    </div>;
  return <ImportSettingsHelper14 {...{
    contentId,
    expanded,
    variant: "nested",
    label: fjord,
...
function ImportSettingsHelper14(nook) {
  let {
    ...
    oxbow = <EnsurePersonalizationCInit {...{
      variant,
      label,
      description,
      control
    }} />;
  return <button aria-controls={contentId} aria-expanded={expanded} className="focus-visible:outline-token-focus w-full cursor-interaction text-left hover:bg-token-list-hover-background" onClick={onClick} type="button">
      {oxbow}
    </button>;
}
var pearl,
