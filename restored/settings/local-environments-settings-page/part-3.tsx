...
function LocalEnvironmentsSettingsPageHelper13(onyx) {
  let { settingsHostId } = onyx,
    ...
    { error, isFetching, isLoading, refetch } = CodexBrowserSurfaceActionType(
      AppInitialQut,
    ...
    ),
    {
      ...
      AppInitialQut,
      DesktopPersistenceKeys.REMOTE_PROJECTS,
    ),
    xenon = AppInitialCO(
      CodexPluginActionResult(DeferredEnvironmentsTB2),
      willow,
    ),
    ...
    [cedar, daisy] = kite.useState(
      violet?.workspaceRoot ?? river.get("workspaceRoot"),
    ),
    [ember, flint] = kite.useState(
      violet?.configPath ?? river.get("configPath"),
    ),
    [garnet, hazel] = kite.useState(
      violet != null || river.get("mode") === "edit",
    ),
    [ivory, jasper] = kite.useState(0),
    kelp = cedar ?? null,
    lotus =
      kelp != null && (violet != null || yellow.includes(kelp) || ember != null)
        ? kelp
        : null,
    mint =
      lotus == null
        ? null
        : (xenon.find((item) => item.path === lotus) ?? null),
    nova = lotus == null,
    olive = () => {
      if (willow !== "local") {
  ...
  let quill = prism,
    reef = () => {
      if (violet != null) {
        ...
        violet.reopenStableWorktreeId != null &&
          ensurePetNotificationMenuInit(pearl, {
  ...
  let zephyr = yarn,
    acorn =
      violet != null && violet.configPath == null && wheat && lotus != null
        ? AppInitialYA(data, lotus)
        : null,
  ...
  let coral = ember ?? bloom,
    ...
    eagle =
      _data?.environment?.type === "success"
  ...
  let lunar = knoll,
    moss =
      __isLoading || (__isFetching && coral == null) || (drift && ___isLoading);
                ...
                label: (
                  <MemoizedFormattedMessage
                    {...{
                      ...
                      description:
                        "Breadcrumb label for the local environments page",
                    }}
                  />
                ),
                onClick: quill,
            ...
            current: garnet ? (
              <MemoizedFormattedMessage
                {...{
                  ...
                  defaultMessage: "edit",
                  description:
                    "Breadcrumb label for local environment edit mode",
                }}
              />
            ) : (
              north
            ...
            children: [
              ...
              <MemoizedFormattedMessage
                {...{
                  ...
                  defaultMessage: "Back",
                  description:
                    "Button label to return from local environment creation",
                }}
              />,
            ],
          }}
              ...
              {...{
                children: (
                  <ParseUrlOrFallback
                    {...{
                      children: (
                        <div className="flex items-center justify-between gap-3 p-3 text-sm text-token-text-secondary">
                          {nest}
                          {oak}
                        </div>
                      ),
...
export function LocalEnvironmentsSettingsPage() {
  ...
  return (
    <LocalEnvironmentsSettingsPageHelper13
      key={canyon}
      {...{
