...
function LocalEnvironmentsSettingsPageHelper2(quartz) {
  let { actions, lifecycle, scriptType } = quartz,
    river = useIntl(),
    ...
    yellow =
      umbra.length > 0 ||
      SUPPORTED_NODE_PLATFORMS.some((item) => !!lifecycle?.[item]?.script);
  let zinc = yellow,
  ...
  let ember = zinc ? (
    <LocalEnvironmentEditorA
      {...{
        ...
        onSelect: timber,
        ariaLabel: amber
          ? river.formatMessage(localEnvironmentEditorU.setupPlatformSelector)
          : river.formatMessage(
              localEnvironmentEditorU.cleanupPlatformSelector,
            ),
      }}
    />
  ) : null;
  let flint =
    slate !== "default" && willow ? (
      <p className="text-sm text-token-text-secondary">
        {
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Message shown when a platform uses the default local environment script",
            }}
          />
        }
      </p>
                  ...
                  slate === "default" || !zinc ? (
                    <MemoizedFormattedMessage
                      {...{
                        ...
                        description:
                          "Empty state for a local environment script",
                      }}
                    />
                  ) : (
                    <MemoizedFormattedMessage
                      {...{
                        ...
                        description:
                          "Empty state for a local environment platform script",
                      }}
                    />
                  ),
                control: null,
...
function gamma(ivory) {
  let { configExists, initialEnvironment, hasParseError, hasReadError } = ivory,
    jasper = initialEnvironment != null,
    kelp = initialEnvironment?.actions,
    lotus =
      !jasper || hasParseError || hasReadError ? (
        <UseChromeAndCodeThemeSync
          {...{
            className: "gap-2",
            children: (
...
function LocalEnvironmentsSettingsPageHelper5(prism) {
  ...
  let bloom = (
    <code className="font-mono text-wrap break-all" dir="ltr">
      {ultra[0]}
    </code>
  );
  let coral = vapor ? (
            ...
            {quill ? (
              <MemoizedFormattedMessage
                {...{
                  ...
                  description:
                    "Accessible label for collapsing a local environment action command",
                  values: {
              ...
              <MemoizedFormattedMessage
                {...{
                  ...
                  description:
                    "Accessible label for expanding a local environment action command",
                  values: {
                    actionName: action.name,
                  },
                }}
  ...
  _t = esmInit(() => {
    ...
    marble = identity({
      ...
      workspaceSelectDescription: {
        ...
        defaultMessage:
          "Local environments tell ChatGPT how to set up worktrees for a project. {learnMore}",
...
function LocalEnvironmentsSettingsPageHelper7(knoll) {
      ...
      <span className="text-token-error-foreground" role="alert">
        {
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Error shown when local environment projects cannot be loaded",
            }}
          />
        }
      </span>
    ...
    wave = (apex) => {
      let brook =
        groups.find(
          (item) =>
            item.path != null &&
            pathsReferToSameWorkspaceResource(item.path, apex),
        ) ?? null;
      return (
        <LocalEnvironmentsSettingsPageHelper8
          key={apex}
          {...{
            hostId,
  ...
  return (
    <div className="flex flex-col gap-3" role="list" aria-label={moss}>
      {north}
    </div>
  );
}
