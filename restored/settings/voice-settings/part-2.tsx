...
function harbor() {
  let onyx = CodexPluginActionType(appScopeAtom),
    pearl = useIntl(),
    quartz = useSettingValue(generalDesktopSettingsDefinitions.microphoneInputDeviceId),
    river = typeof navigator < "u" && navigator.mediaDevices?.enumerateDevices != null,
    slate = {
      enabled: river,
      queryFn: _t,
      queryKey: gamma,
    ...
    willow;
  {
    ...
    let nova = umbra.findIndex(mint),
      olive = umbra[nova];
    if (quartz == null) violet = willow;else if (umbra.length === 0) {
      let quill;
  ...
  amber = river ? <MemoizedFormattedMessage {...{
  ...
  let cedar = <DropdownTriggerButton disabled={!river}>
      <span className="truncate">{violet}</span>
    </DropdownTriggerButton>;
  let daisy = quartz == null ? AppIconZlt : undefined,
    ember = () => {
      setSettingValue(onyx, generalDesktopSettingsDefinitions.microphoneInputDeviceId, null);
    };
  let flint = <DropdownMenu.Item {...{
    RightIcon: daisy,
    onSelect: ember,
    children: willow
  }} />;
  ...
  let hazel;
  {
    ...
    topaz = (ultra, vapor) => {
      ...
      return <DropdownMenu.Item key={ultra.deviceId} {...{
        RightIcon: ultra.deviceId === quartz ? AppIconZlt : undefined,
        onSelect: () => {
          setSettingValue(onyx, generalDesktopSettingsDefinitions.microphoneInputDeviceId, ultra.deviceId);
        },
        children: wheat
      }} />;
    };
  ...
  let ivory = timber.isFetching ? <DropdownMenu.Message {...{
    compact: true,
    children: <span className="flex items-center justify-center gap-2">
            {<VSCODE_EDITOR_ID {...{
        className: "size-3"
      }} />}
            {<MemoizedFormattedMessage {...{
  ...
  let jasper = !timber.isFetching && timber.isError ? <DropdownMenu.Message {...{
    tone: "error",
  ...
  let kelp = !timber.isFetching && !timber.isError && umbra.length === 0 ? <DropdownMenu.Message {...{
    children: <MemoizedFormattedMessage {...{
...
function indigo(yarn, zephyr, acorn) {
  return zephyr.label || yarn.formatMessage({
...
async function _t() {
  ...
  return bloom?.enumerateDevices == null ? [] : (await bloom.enumerateDevices()).filter(item => item.kind === "audioinput" && item.deviceId.length > 0 && item.deviceId !== "default");
}
function jade() {
  let {
      data
    } = CodexPluginActionResult(bravo),
    coral = alpha(data);
  let drift = coral,
    eagle = <UseChromeAndCodeThemeSync.Header {...{
  ...
  return <UseChromeAndCodeThemeSync {...{
    children: [eagle, <UseChromeAndCodeThemeSync.Content {...{
      children: drift ? <>
                  {<ParseUrlOrFallback {...{
          children: [<VoiceSettingsHelper14 {...{
            hotkeyState: data,
            mode: "hold"
        ...
        children: <AppInitialLj layout="settings-row">
                        {<MemoizedFormattedMessage {...{
...
function VoiceSettingsHelper8(frost) {
  ...
  let north = hotkeyState?.keepVisible === true,
    orbit = hotkeyState == null || hotkeyState.configuredHotkey == null && hotkeyState.configuredToggleHotkey == null || knoll.isPending,
    pine = tide => {
      knoll.mutate({
        keepVisible: tide
      });
  ...
  return <EnsurePersonalizationCInit {...{
    label: lunar,
    description: moss,
    control: <AppInitialYC checked={north} disabled={orbit} onChange={pine} ariaLabel={quest} />
  }} />;
}
function VoiceSettingsHelper9() {
  let unity = CodexPluginActionType(appScopeAtom),
    vale = useIntl(),
    wave = useSettingValue(generalDesktopSettingsDefinitions.dictationDictionary),
    ...
    fern = async function (petal) {
      ...
      await setSettingValue(unity, generalDesktopSettingsDefinitions.dictationDictionary, quiet);
      brook(null);
    };
  let grove = fern,
    hill,
  ...
  let juniper = () => {
    ...
    requestAnimationFrame(() => {
      document.querySelector(`[data-dictation-dictionary-entry-index="${elm.length}"]`)?.focus();
    });
  };
  let lagoon, meadow;
  lagoon = <EnsureComposerEsm_Wlt_Init {...{
    className: "icon-2xs"
  }} />;
  meadow = <MemoizedFormattedMessage {...{
    id: "settings.general.dictationDictionary.addEntry",
    defaultMessage: "Add entry",
    description: "Button label for adding one dictation dictionary entry"
  }} />;
  let nest = <EnsurePersonalizationCInit {...{
    label: hill,
    description: isle,
  ...
  let oak = elm.map((item, index) => <EnsurePersonalizationCInit key={index} {...{
    variant: "nested",
    label: null,
    control: <div className="flex w-full items-center gap-2">
            <input data-dictation-dictionary-entry-index={index} aria-label={vale.formatMessage({
        ...
        index: index + 1
      })} className="w-full rounded-md border border-token-input-border bg-token-input-background px-2.5 py-1.5 text-sm text-token-input-foreground outline-none placeholder:text-token-input-placeholder-foreground focus:border-token-focus-border" placeholder={falcon[index] ?? falcon[0] ?? ""} value={item} onChange={event => {
        let rain = [...elm];
        ...
        let seed = [...elm.slice(0, index + 1), delta, ...elm.slice(index + 1)];
        cliff.current = true;
        brook(seed);
        requestAnimationFrame(() => {
          document.querySelector(`[data-dictation-dictionary-entry-index="${index + 1}"]`)?.focus();
        });
      }} />
...
function VoiceSettingsHelper14(vine) {
  let {
    ...
    grain = {
      onSuccess: dome => {
        ...
        Promise.all([birch(eddy), birch(buildVscodeQueryKey("codex-command-keymap-state"))]);
      }
    };
  let haven = applyU0000U001FU007F2(mode === "hold" ? "global-dictation-set-hotkey" : "global-dictation-set-toggle-hotkey", grain),
  ...
  let jadeite = ink,
    kernel = wind ? hotkeyState?.configuredToggleHotkey ?? null : hotkeyState?.configuredHotkey ?? null,
    leaf = kernel == null ? null : moveArrayItem(kernel);
  let maple = leaf,
    nimbus = wind ? yarrow.formatMessage($.toggleDictationHotkey) : yarrow.formatMessage($.holdToDictateHotkey);
  let opal = nimbus,
    plume = wind ? yarrow.formatMessage({
  ...
  let quillow = plume,
    root = wind ? <MemoizedFormattedMessage {...{
      ...$.toggleDictationHotkey
    }} /> : <MemoizedFormattedMessage {...{
      ...$.holdToDictateHotkey
    }} />;
  let silk = wind ? <MemoizedFormattedMessage {...{
  ...
  let thorn = ever ? <span className="text-token-error-foreground">{ever}</span> : null;
  let upland = <div className="flex flex-col gap-1">
      {silk}
      {thorn}
    </div>;
