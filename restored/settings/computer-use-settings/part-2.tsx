...
function petal(urn) {
  let { computerUseAvailability, platform } = urn,
    vine = useIntl(),
    wind = useNavigate(),
  ...
  let { isUpdating, setAppEnabled: dew, updatingAppId } = AppInitialD(canyon),
    ever = [];
  ...
  let maple = leaf,
    ...
    opal = ink
      ? (field.availablePlugins.find(deferredConversationR) ?? null)
      : null,
    plume = [
      {
  ...
  let vista = seed(upland),
    wisp = [],
    yonder =
      maple == null && yarrow && !birch.isLoading && !birch.allowed
        ? [
            {
              description: (
  ...
  if (haven && (platform === "macOS" || platform === "windows"))
    for (let jetty of plume) {
    ...
    (anvil = (
      <MemoizedFormattedMessage
        {...{
          ...
          description:
            "Empty state shown when computer use plugins cannot be found",
        }}
      />
    )),
    (beacon = (
...
function seed(nook) {
  let { hostId, plugin, shouldReadBrowserExtension } = nook,
    oxbow = {
...
function trail() {
            ...
            <AppInitialNi>
              {
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Loading state for Google Chrome computer use settings",
                  }}
                />
              }
            </AppInitialNi>
  ...
  let screw = joint == null || latch || !keystone,
    torque = () => {
      joint != null &&
        invokeDesktopRpc("chrome-extension-settings-open", {
          params: {
            extensionId: joint,
          },
        }).catch(() => {
          weir.get(toastAtom).danger(
            <MemoizedFormattedMessage
              {...{
                id: "settings.computerUse.chrome.openExtensionSettingsError",
                defaultMessage: "Unable to open Chrome extension settings",
                description:
                  "Toast shown when the app fails to open Chrome extension settings",
              }}
            />,
          );
        });
        ...
        title: (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Title for Google Chrome browser permissions settings section",
            }}
          />
        ),
      }}
