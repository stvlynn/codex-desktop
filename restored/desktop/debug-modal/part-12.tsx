...
function Coral1(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
}
function Daisy1(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
}
function Drift1(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
}
function Eagle1(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
...
function Falcon2(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
}
function Kite2(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
}
function Lemon2(props: {
  children?: unknown;
  [key: string]: unknown;
} = {}): unknown {
  return props.children ?? null;
...
function Marble3(onyx22) {
  let pearl22 = [],
    ...
    slate22 = 0,
    timber22 = 0;
  for (let umbra22 of onyx22) for (let violet22 of umbra22.items) Nickel3(violet22) && (timber22 += 1, violet22.status === "completed" ? quartz22 += 1 : violet22.status === "failed" ? river22 += 1 : violet22.status === "inProgress" && (slate22 += 1), pearl22.push({
    conversationId: umbra22.conversationId ?? null,
    item: violet22,
    turnId: umbra22.turnId
  }), pearl22.length > cedar3 && pearl22.shift());
  return pearl22.reverse(), {
...
function Onyx3({
  ...
  let yellow22 = [{
    label: `call[${xenon22}]`,
    value: item.durationMs == null ? `${item.tool}: ${item.status}` : `${item.tool}: ${item.status}, ${item.durationMs}ms`
  }, {
    label: `call[${xenon22}].id`,
    value: item.id
  }, ...Yellow3(item.arguments, xenon22)];
  return conversationId != null && yellow22.push({
    label: `call[${xenon22}].threadId`,
    value: conversationId
  }), yellow22.push({
    label: `call[${xenon22}].turnId`,
    value: turnId ?? "none"
  }), item.error != null && yellow22.push({
    label: `call[${xenon22}].error`,
    value: item.error.message
  }), item.result != null && (yellow22.push(...River3(item.result, xenon22)), yellow22.push({
    kind: "json",
...
function Quartz3(basalt22, cedar22) {
  return basalt22 === "data" && typeof cedar22 == "string" && Violet3(this) ? `<Base64 image data: ${cedar22.length} chars>` : cedar22;
}
function River3(daisy22, ember22) {
  let flint22 = Slate3(daisy22);
  return flint22 == null ? [] : flint22.flatMap((item, index) => {
    let garnet22 = Timber3(item);
    if (garnet22 != null) return [{
      kind: "multiline",
...
function Umbra3(mint22) {
  ...
  if (olive22.startsWith("data:image/")) {
    ...
    return quill22 == null ? null : {
      data: olive22,
      mimeType: quill22,
      src: olive22
    };
  }
  let prism22 = nova22.data.mimeType ?? nova22.data.mime_type ?? Xenon3(olive22);
...
function Xenon3(topaz22) {
  return topaz22.startsWith("iVBOR") ? "image/png" : topaz22.startsWith("/9j/") ? "image/jpeg" : topaz22.startsWith("R0lGOD") ? "image/gif" : topaz22.startsWith("UklGR") ? "image/webp" : topaz22.startsWith("PHN2Zy") ? "image/svg+xml" : "image/png";
}
function Yellow3(ultra22, vapor22) {
  let wheat22 = Zinc3(ultra22);
  if (wheat22 == null) return [{
    label: `call[${vapor22}].arguments`,
    value: Pearl3(ultra22)
  }];
  let yarn22 = [];
  return wheat22.title != null && yarn22.push({
    label: `call[${vapor22}].title`,
    value: wheat22.title
  }), wheat22.timeoutMs != null && yarn22.push({
  ...
  }), Object.keys(wheat22.extraArguments).length > 0 && yarn22.push({
    label: `call[${vapor22}].arguments`,
...
function Amber3(bloom22) {
  return Basalt3(bloom22.replaceAll("\r\n", "\n").replaceAll("\r", "\n").trim(), flint3);
}
function Basalt3(coral22, drift22 = daisy3) {
  return coral22.length <= drift22 ? coral22 : `${coral22.slice(0, drift22)}… (${coral22.length} chars)`;
}
var cedar3,
  daisy3,
  ember3,
...
function Mint3(honey22) {
  let {
    ...
    iris22 = CodexBrowserSurfaceActionType(MCP_SERVERS_STATUS_PATH, conversationId) ?? eagle3,
    jewel22;
  {
    let lunar22;
    lunar22 = moss22 => ({
...
function Olive3({
  ...
  let orbit22 = coral3.useMemo(() => lines.map((item, index) => ({
  ...
  let brook22 = useEventCallback(() => {
    ...
    cliff22 = useEventCallback(seed22 => {
      ...
      for (let [vine22, wind22] of seed22) {
        ...
        trail22[vine22] !== yarrow22 && (urn22 === trail22 && (urn22 = {
          ...trail22
    ...
    elm22 = useEventCallback(bravo23 => {
      ...
      let copper23 = dusk22();
      return copper23.observe(bravo23), () => {
  ...
  coral3.useLayoutEffect(() => {
    ...
    return echo23.observe(delta23), echo23.observe(ridge22), brook22(), () => {
  ...
  return <div className="py-1.5"><div ref={storm22} className="relative" style={{
      height: `${apex22.totalHeightPx}px`
    }}><div style={{
        transform: `translateY(${grove22}px)`
      }}>{fern22.map(item => <Falcon2 key={item.turnKey} {...{
          entry: item,
...
function Prism3(lemon23) {
  ...
  let onyx23 = <span className="shrink-0 text-left break-words text-token-description-foreground" style={nickel23}>{label}</span>;
  let pearl23 = <div className="min-w-0 flex-1 pr-3 text-left">{children}</div>;
  return <div className="relative flex items-start justify-between border-t-[0.5px] border-token-border py-1.5 tabular-nums first:border-t-0" style={marble23}>{onyx23}{pearl23}</div>;
}
...
function Reef3({
  ...
  if (viewportBottomPx <= 0) return {
    startIndex: 0,
    endIndex: Math.min(river23, glide3)
  };
  if (viewportTopPx >= layout.totalHeightPx) return {
...
function Sage3(umbra23, violet23) {
  return umbra23.startIndex === violet23.startIndex && umbra23.endIndex === violet23.endIndex;
}
function Topaz3(willow23) {
  return willow23.closest("[data-debug-panel-scroll-container]") ?? Ultra3(willow23);
}
function Ultra3(xenon23) {
  let yellow23 = xenon23.parentElement;
  for (; yellow23 != null;) {
    let zinc23 = window.getComputedStyle(yellow23).overflowY;
    if (zinc23 === "auto" || zinc23 === "scroll" || zinc23 === "overlay") return yellow23;
