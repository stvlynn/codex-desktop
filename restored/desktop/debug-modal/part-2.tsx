...
function Willow() {
  ...
  let knoll5 = bloom5.formatMessage(coral5 ? {
  ...
  let north5 = bloom5.formatMessage(glide5 ? {
  ...
  let quest5 = bloom5.formatMessage(drift5 ? {
  ...
  let storm5 = <div className="flex flex-col py-1.5">{lunar5}{orbit5}{ridge5}</div>;
  let tide5 = <div className="text-xs text-token-description-foreground">{<MemoizedFormattedMessage {...{
  ...
  let wave5 = <div className="flex items-center justify-between gap-3 py-1.5">{tide5}<AppInitialYC ariaLabel={unity5} checked={glide5} onChange={vale5} /></div>;
  let apex5 = eagle5.canUseLocalOverride ? <div className="flex items-center justify-between gap-3 border-t-[0.5px] border-token-border py-2"><div className="text-xs text-token-description-foreground">{<MemoizedFormattedMessage {...{
      ...
      }} />}</div>{<ReadLoginRouteQuerySnapshot {...{
      color: "outline",
      disabled: honey5.phase !== "active",
      onClick: () => {
        honey5.phase === "active" && appServices.realtimeVoice?.control(honey5.locator, {
  ...
  let brook5 = eagle5.canUseLocalOverride ? <div className="flex flex-col gap-2 border-t-[0.5px] border-token-border py-2">{<Falcon {...{
      ...
      value: eagle5.source === "local-override" ? bloom5.formatMessage({
    ...
    }} />}<div className="flex items-center justify-between gap-3"><div className="text-xs text-token-description-foreground">{<MemoizedFormattedMessage {...{
        ...
        }} />}</div><AppInitialYC ariaLabel={bloom5.formatMessage({
      ...
      })} checked={frost5.enabled} onChange={dusk5 => {
        acorn5.set(deferredVoiceC4, {
          enabled: dusk5,
          config: dusk5 && frost5.config.trim().length === 0 ? JSON.stringify(eagle5.statsigValue, null, 2) : frost5.config
        });
      }} /></div><textarea aria-label={bloom5.formatMessage({
    ...
    })} className="min-h-36 w-full resize-y rounded border border-token-border bg-token-input-background px-2 py-1.5 font-mono text-xs text-token-foreground focus:border-token-focus-border focus:outline-none" value={frost5.config} onChange={event => {
      acorn5.set(deferredVoiceC4, {
        ...frost5,
        config: event.currentTarget.value
      });
    }} />{eagle5.error ? <div className="text-xs text-token-error-foreground">{eagle5.error}</div> : null}<div className="flex items-start justify-between gap-3"><div className="text-xs text-token-description-foreground">{<MemoizedFormattedMessage {...{
        ...
        }} />}</div><button type="button" className="cursor-interaction text-xs text-token-text-link-foreground hover:underline" onClick={() => {
...
function Amber({
  ...
  let elm5 = new Map();
  if (workspaceRoots != null) for (let fern5 of workspaceRoots) elm5.set(fern5, {
    projectRoot: fern5,
    title: labels?.[fern5] ?? fern5
  });
  if (statuses != null) for (let grove5 of statuses) elm5.has(grove5.projectRoot) || elm5.set(grove5.projectRoot, {
    projectRoot: grove5.projectRoot,
    title: labels?.[grove5.projectRoot] ?? grove5.projectRoot
  });
  return projectlessThreadCwd != null && elm5.set(projectlessThreadCwd, {
...
function Cedar({
    ...
    oak5 = async () => {
      if (!meadow5) {
        ...
        try {
          ...
          lagoon5(`Error: ${seed5}\n\nInput JSON:\n${hill5}`);
          appActionSidebarProjectRefSchema.error("Failed to run debug app action", {
            safe: {},
            sensitive: {
              actionJson: hill5,
              error: rain5
  ...
  return <Copper {...{
    ...
    variant: "global",
    actions: <button type="button" className="inline-flex items-center rounded border border-token-border px-3 py-1 text-xs text-token-foreground hover:bg-token-foreground/5 disabled:cursor-not-allowed disabled:opacity-50" disabled={meadow5} onClick={() => {
      oak5();
    }}>{meadow5 ? "Running…" : "Run action"}</button>,
    children: [<p className="text-token-foreground-secondary pt-2 text-xs">{"Run a raw app-control action payload against the primary app window. Agent tool availability is controlled by the app-control Statsig gate."}</p>, <div className="flex flex-col gap-2 py-2"><textarea className="bg-token-background-primary min-h-48 resize-y rounded border border-token-border p-2 font-mono text-xs text-token-foreground outline-none" spellCheck={false} value={hill5} onChange={event => {
        isle5(event.target.value);
      }} /><div className="rounded border border-token-border bg-token-foreground/5 px-3 py-2 text-xs whitespace-pre-wrap text-token-foreground">{juniper5}</div></div>]
  }} />;
}
var daisy,
  $i,
...
function Ivory(bravo6) {
  ...
  let indigo6 = harbor6.length,
    jade6 = CodexBrowserSurfaceActionType(brook, hostId) ?? hostDetails.stateType === "local",
  ...
  let slate6 = <span className="truncate font-medium text-token-foreground">{hostDetails.displayName}</span>;
  let timber6 = <Pearl {...{
    state: kite6
  }} />;
  let umbra6 = <div className="flex items-center gap-2">{slate6}{timber6}</div>;
  let violet6 = hostDetails.stateType === "connection" ? ` · ${hostId}` : null,
    willow6 = ` · v${echo6 ?? "Unavailable"}`,
    xenon6 = <div className="mt-1 truncate text-xs text-token-description-foreground">{hostDetails.connectionType}{violet6}{willow6}</div>;
  let yellow6 = <div className="min-w-0">{umbra6}{xenon6}</div>;
  let zinc6 = <span>{requests.length}{" requests"}</span>;
  let amber6 = <span>{notifications.length}{" notifications"}</span>;
  let basalt6 = marble6 > 0 ? <span className="rounded-full bg-token-charts-yellow/15 px-2 py-0.5 text-token-charts-yellow">{marble6}{" live"}</span> : null;
  let cedar6 = indigo6 > 0 ? <span className="rounded-full bg-token-charts-red/15 px-2 py-0.5 text-token-charts-red">{indigo6}{" failed"}</span> : null;
  let daisy6 = onyx6 > 0 ? <span className="rounded-full bg-token-charts-red/15 px-2 py-0.5 text-token-charts-red">{onyx6}{" notification errors"}</span> : null;
  let ember6 = quartz6 > 0 ? <span className="rounded-full bg-token-foreground/10 px-2 py-0.5">{quartz6}{" deltas"}</span> : null;
  let flint6 = <div className="flex shrink-0 items-center gap-2 text-xs text-token-description-foreground tabular-nums">{zinc6}{amber6}{basalt6}{cedar6}{daisy6}{ember6}</div>;
  let garnet6 = <summary className="flex cursor-interaction list-none items-center justify-between gap-3 px-3 py-2.5 marker:content-none">{yellow6}{flint6}</summary>;
  let hazel6 = gamma6 ? <Quartz {...{
    hostDetails
  }} /> : null;
  let ivory6 = <Timber {...{
    hostId,
  ...
  let lotus6 = <div className="border-t border-token-border px-3 pb-3"><div className="flex flex-col gap-3 py-3">{hazel6}{ivory6}{jasper6}{kelp6}</div></div>;
  return <details className="group rounded-xl border border-token-border bg-token-foreground/[0.03] shadow-sm" onToggle={river6} open={jade6}>{garnet6}{lotus6}</details>;
}
...
function Mint(prism6) {
  ...
  return <div className="rounded-lg border border-token-border bg-token-foreground/[0.025] px-3">{quill6}</div>;
}
function Nova(reef6) {
  return <Falcon key={reef6.label} {...{
    label: reef6.label,
    value: reef6.value
  }} />;
}
function Olive(sage6) {
  ...
  let bloom6 = `debug-app-server-requests-${hostId}`,
    coral6 = wheat6.length > 0 || topaz6 ? <div className="mb-2 flex justify-end"><label className="flex cursor-interaction items-center gap-2 text-xs text-token-description-foreground"><input checked={topaz6} className="cursor-interaction" onChange={event => {
          ultra6(event.currentTarget.checked);
        }} type="checkbox" />{"Failed"}<span className="tabular-nums">{"("}{wheat6.length}{")"}</span></label></div> : null;
  let drift6 = yarn6.length > 0 ? <div className="flex max-h-[360px] flex-col gap-2 overflow-y-auto pr-1">{yarn6.map(Prism)}</div> : <Xenon {...{
    message: topaz6 ? "No failed requests recorded for this manager yet" : "No requests recorded for this manager yet"
  }} />;
  return <Yellow {...{
...
function Quill({
  ...
  let [frost6, glide6] = tide.useState(null),
    ...
    lunar6 = knoll6 == null ? jewel6 : jewel6.filter(item => item.status === knoll6),
    moss6 = knoll6 == null ? `All states (${threadStatusEntries.length})` : Jewel({
      count: jewel6.find(item => item.status === knoll6)?.entries.length ?? 0,
      status: knoll6
    });
  return <div className="overflow-hidden rounded-lg border border-token-border bg-token-foreground/[0.025]">{<Copper {...{
      ...
      title: "Thread status",
      variant: "global",
      children: <div className="py-3">{jewel6.length === 0 ? <Xenon {...{
          message: "No in-memory threads for this manager yet"
        }} /> : <div className="flex flex-col gap-3"><div className="flex flex-wrap items-center justify-between gap-2">{<DropdownMenuPopover {...{
              contentWidth: "xs",
              triggerButton: <button type="button" className="bg-token-background inline-flex max-w-80 min-w-40 cursor-interaction items-center justify-between gap-2 rounded-md border border-token-border px-2 py-1 text-xs text-token-foreground hover:bg-token-foreground/5"><span className="truncate">{moss6}</span>{<AppIconSft {...{
                  className: "icon-2xs shrink-0 opacity-70"
                }} />}</button>,
              children: [<DropdownMenu.Item {...{
                "aria-checked": knoll6 == null,
                role: "menuitemradio",
                RightIcon: knoll6 == null ? AppIconZlt : undefined,
                onSelect: () => {
                  iris6(null);
                },
                children: ["All states (", threadStatusEntries.length, ")"]
              }} />, apex.map(item => <DropdownMenu.Item key={item} {...{
                "aria-checked": knoll6 === item,
                role: "menuitemradio",
                RightIcon: knoll6 === item ? AppIconZlt : undefined,
                onSelect: () => {
                  iris6(item);
                },
                children: Jewel({
                  count: jewel6.find(_item => _item.status === item)?.entries.length ?? 0,
                  status: item
                })
              }} />)]
            }} />}</div><div className="flex max-h-[300px] flex-col gap-3 overflow-y-auto pr-1">{lunar6.length === 0 ? <Xenon {...{
              message: "No in-memory threads match this state"
            }} /> : lunar6.map(item => <div key={item.status} className="flex flex-col gap-2"><div className="text-xs font-medium text-token-description-foreground">{Iris(item)}</div><div className="flex flex-col gap-2">{item.entries.map(_item => <Basalt key={_item.conversationId} {...{
                  entry: _item,
                  isPending: frost6 === _item.conversationId,
                  onUnsubscribe: async north6 => {
                    glide6(north6);
                    try {
                      await canonicalizeWorkspacePathKey("unsubscribe-thread-for-host", {
                        hostId,
                        threadId: north6
                      });
                    } finally {
                      glide6(orbit6 => orbit6 === north6 ? null : orbit6);
                    }
                  }
                }} />)}</div></div>)}</div></div>}</div>
    }} />}</div>;
...
function Reef(pine6) {
  let {
    ...
    quest6 = CodexBrowserSurfaceActionType(DeferredUiX232, entry.conversationId),
    ridge6 = entry.resumeState === "resumed" && quest6?.role === "owner" && !isPending,
    storm6 = useColdNavigate(),
    tide6 = entry.title?.trim() || "Untitled thread";
  let unity6 = <div className="truncate text-sm font-medium text-token-foreground">{tide6}</div>;
  let vale6 = <div className="truncate font-mono text-[11px] text-token-description-foreground">{entry.conversationId}</div>;
  let wave6 = () => {
  ...
  let brook6 = <button type="button" aria-label="Open thread" className="cursor-interaction rounded p-0.5 text-token-charts-blue hover:bg-token-foreground/10" onClick={wave6}>{apex6}</button>;
  let cliff6 = <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">{vale6}{brook6}</div>;
  let dusk6 = <div className="min-w-0">{unity6}{cliff6}</div>;
  let elm6 = !ridge6,
    fern6 = () => {
      onUnsubscribe(entry.conversationId);
    };
  let grove6 = isPending ? "Unsubscribing..." : "Unsubscribe",
    hill6 = <button type="button" className="cursor-interaction rounded border border-token-border px-2 py-1 text-xs text-token-foreground hover:bg-token-foreground/10 disabled:cursor-default disabled:opacity-50" disabled={elm6} onClick={fern6}>{grove6}</button>;
  let isle6 = <div className="flex items-start justify-between gap-3">{dusk6}{hill6}</div>;
  ...
  let seed6 = <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-token-description-foreground">{lagoon6}{nest6}{petal6}{rain6}</div>;
  return <div className="bg-token-background rounded-lg border border-token-border px-3 py-2">{isle6}{seed6}</div>;
}
