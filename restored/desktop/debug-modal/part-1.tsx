...
/** Wave GA unresolved companion (jsx-collision:ensureComposerEsm_Qtt_Init@composer/composer-esm-inits.ts) */
...
function Alpha(oak4) {
  ...
  let canyon4 = actions ? (
    <span className="flex items-center gap-1" onClick={Bravo}>
      {actions}
    </span>
  ) : null;
  let dew4 =
  ...
  let falcon5 = unmountChildrenWhenClosed && !quiet4 ? null : children,
    gamma5 = (
      <div className="px-3" style={echo5} data-open={quiet4}>
        {falcon5}
      </div>
    );
  return (
...
function Harbor() {
  let indigo5 = CodexPluginActionType(appScopeAtom).get(
    ...
    violet5 = async (xenon5, yellow5) => {
      if (!timber5) {
        ...
        try {
          ...
          let basalt5 =
            amber5 instanceof Error ? amber5.message : String(amber5);
    ...
    willow5 = async () => {
      if (!(river5 != null || timber5)) {
        ...
        try {
          ...
          let kelp5 =
            jasper5 instanceof Error ? jasper5.message : String(jasper5);
              ...
              }
              {
                <Delta
                  {...{
                    ...
                    onClick: () => {
                      river5 != null &&
                        (indigo5.cancelStream(river5),
                        slate5(null),
                        quartz5("Stream cancel requested"));
                    },
                  }}
                />
              }
...
function Indigo(lotus5) {
  let { disabled = false, label, onClick } = lotus5;
  return (
    <button
      type="button"
...
function Jade(mint5) {
  let { label, onChange, value } = mint5,
    nova5 = (event) => {
...
function Kite(prism5) {
  ...
  if (typeof quill5 != "object" || !quill5)
      ...
      {...{
        ...
        onCopy: () => {
          navigator?.clipboard?.writeText &&
            navigator.clipboard.writeText(value).catch(Slate);
        },
      }}
    />
  );
