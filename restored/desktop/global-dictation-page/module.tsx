...
import { ensureComposerEsm_Hlt_Init, ensureComposerEsm_Sut_Init, ensureComposerEsm_Tft_Init } from "../../composer/composer-esm-inits";
...
import { ensureGlobalDictationOrbAtomsInit, ensureGlobalDictationOrbHelpersInit, ensureGlobalDictationOrbInit, GlobalDictationOrb, GlobalDictationOrbListening, useGlobalDictationOrbState as UseGlobalDictationOrbState } from "../global-dictation-orb";
...
function falcon() {
  ...
  return queueMicrotask(() => {
    pearl && ensureAppActionPayloadSchemasInit.dispatchMessage("global-dictation-renderer-ready", {});
  }), () => {
    pearl = false;
  };
}
function Be(quartz) {
  let {
    ...
    [umbra, violet] = kite.useState(initialStatus !== "initializing" && initialStatus !== "idle"),
    willow = kite.useRef(null),
    xenon = () => {
      slate != null && ensureAppActionPayloadSchemasInit.dispatchMessage("global-dictation-close", {
        sessionId: slate
      });
    };
  let yellow = xenon,
  ...
  let amber = IntlProvider(delta.darkTheme, "flex h-screen w-screen items-end justify-center overflow-hidden bg-transparent text-token-text-primary");
  ...
  let daisy = initialStatus === "initializing" ? "idle" : initialStatus,
    ember = <UseGlobalDictationOrbState {...{
  ...
  let flint = <div ref={willow} data-testid="global-dictation-hitbox" className={cedar}>
      {ember}
    </div>;
  return <main className={amber}>
...
function gamma(garnet) {
  ensureAppActionPayloadSchemasInit.dispatchMessage("global-dictation-pointer-interaction-changed", {
...
function GlobalDictationPageHelper3(hazel) {
  ...
  let eagle = drift,
    frost = () => {
      kelp != null && (vapor("transcribing"), yarn(null), acorn(false), ensureGlobalDictationOrbHelpersInit(kelp, false).catch(error => {
        eagle("transcription", error);
      }));
    };
  let glide = frost,
    honey = () => {
      kelp != null && (ensureAppActionPayloadSchemasInit.dispatchMessage("global-dictation-dismiss", {
        sessionId: kelp
      }), lotus(null), yarn(null), acorn(false));
    };
  let _GlobalDictationPage = honey,
    iris = () => {
      kelp == null && ultra !== "idle" || ensureAppActionPayloadSchemasInit.dispatchMessage("global-dictation-close", {
        sessionId: kelp
      });
    };
  let be = iris,
  ...
  moss = grain => {
    ...
    acorn(false);
    topaz(harbor);
    vapor(grain.configuredHotkey != null || grain.configuredToggleHotkey != null ? "idle" : "initializing");
  };
  north = [];
  ...
  let orbit = haven => {
    ...
    acorn(false);
    vapor("listening");
    GlobalDictationOrb(haven.sessionId, {
      startWaveformCapture,
      stopWaveformCapture,
      resetWaveformDisplay,
  ...
  let storm = ultra === "error" && "p-1",
    tide = IntlProvider(delta.darkTheme, "flex h-screen w-screen items-end justify-center overflow-hidden bg-transparent text-token-text-primary", storm);
  let unity = ultra !== "idle",
    vale = ultra === "idle" && quill,
    wave = IntlProvider(delta.darkTheme, "!rounded-full px-4 py-2");
  let apex = <GlobalDictationPageHelper7 {...{
  ...
  let dusk = ultra === "initializing" ? undefined : ultra === "idle" ? ivory.formatMessage({
  ...
  let elm = ultra === "error" ? "draggable" : "no-drag",
    ...
    grove = (ultra === "initializing" || ultra === "idle") && "h-2 w-10 justify-center rounded-[4px] border-token-text-secondary/70 px-0",
    hill = ultra === "idle" && "group-hover:h-[30px] group-hover:w-[72px] group-hover:rounded-full group-hover:border-token-border-default/80 group-data-[state=delayed-open]:h-[30px] group-data-[state=delayed-open]:w-[72px] group-data-[state=delayed-open]:rounded-full group-data-[state=delayed-open]:border-token-border-default/80",
    isle = ultra !== "initializing" && ultra !== "idle" && delta.expandedSurface,
    juniper = ultra !== "initializing" && ultra !== "idle" && "border-token-border-default/80",
    lagoon = (ultra === "listening" || ultra === "transcribing") && "h-[30px] w-[72px] justify-center rounded-full px-2",
    meadow = ultra === "error" && "h-8 w-fit max-w-[304px] gap-2 rounded-2xl px-2",
    nest = IntlProvider("flex items-center overflow-hidden border shadow-lg shadow-black/20 transition-[width,height,border-radius,background-color] duration-basic [transition-timing-function:cubic-bezier(0.77,0,0.175,1)] forced-colors:bg-[Canvas] forced-colors:backdrop-blur-none motion-reduce:transition-none", elm, fern, grove, hill, isle, juniper, lagoon, meadow);
  let oak = ultra === "idle" ? <span className="relative flex h-full w-full items-center justify-center text-token-text-secondary">
  ...
  let petal = ultra === "transcribing" ? <VSCODE_EDITOR_ID {...{
    className: "icon-xs text-token-text-secondary"
  }} /> : null;
  let quiet = ultra === "error" ? <>
        <span className="max-w-[252px] min-w-0 truncate text-xs font-medium text-token-error-foreground">
          {wheat}
        </span>
        {zephyr ? <button type="button" className="no-drag flex size-5 shrink-0 cursor-interaction items-center justify-center rounded-full text-token-text-secondary hover:bg-token-list-hover-background hover:text-token-text-primary focus:outline-none" aria-label={ivory.formatMessage({
      ...
      description: "Accessible label for the button that retries global dictation transcription"
        ...
        <button type="button" className="no-drag flex size-5 shrink-0 cursor-interaction items-center justify-center rounded-full text-token-text-secondary hover:bg-token-list-hover-background hover:text-token-text-primary focus:outline-none" aria-label={ivory.formatMessage({
  ...
  let rain = ultra === "listening" ? <canvas ref={waveformCanvasRef} className="h-4 min-w-0 flex-1 text-token-text-primary" aria-hidden="true" /> : null;
  let seed = ultra === "idle" ? <MemoizedFormattedMessage {...{
  ...
  let vine = ultra === "error" ? wheat : null,
    wind = <span className="sr-only">
        {seed}
        {trail}
        {urn}
        {vine}
      </span>;
  let yarrow = <section aria-live="polite" aria-label={dusk} className={nest}>
      {oak}
      {petal}
      {quiet}
      {rain}
  ...
  let azure = <div ref={bloom} data-testid="global-dictation-hitbox" className={cliff}>
      {yarrow}
    </div>;
  let birch = <OptionalTooltip {...{
    delayDuration: 250,
  ...
  let $ = <GlobalDictationPageHelper6 {...{
    onClose: be,
    children: birch
  }} />;
...
function GlobalDictationPageHelper6(maple) {
  ...
  return <NativeContextMenuSurfaceStub {...{
    items: opal,
    children
  }} />;
}
function GlobalDictationPageHelper7(plume) {
  ...
  if (configuredHotkey != null && configuredToggleHotkey != null) {
    ...
    let root = <HotkeyKeysLabel key={"hold"} {...{
      keysLabel: quillow
    }} />;
    let silk = moveArrayItem(configuredToggleHotkey);
    let thorn = <HotkeyKeysLabel key={"toggle"} {...{
      keysLabel: silk
    }} />;
    let upland;
  ...
  if (configuredHotkey != null) {
    ...
    return <MemoizedFormattedMessage {...{
      ...
      values: {
        shortcut: <HotkeyKeysLabel key={"hold"} {...{
          keysLabel: vista
        }} />
      }
    }} />;
  ...
  if (configuredToggleHotkey != null) {
    ...
    return <MemoizedFormattedMessage {...{
      ...
      values: {
        shortcut: <HotkeyKeysLabel key={"toggle"} {...{
          keysLabel: yonder
        }} />
      }
    }} />;
