...
} from "../../appgen/appgen-share-dialog";
...
export function AppgenPage() {
  let olive = CodexPluginActionType(appScopeAtom),
    ...
    { setSelectedMode: bloom } = AnnotationFocusMode(),
  ...
  let lagoon = (
    <div className="mx-auto flex min-h-full w-full max-w-[760px] flex-col">
      {isLoading ? (
        ...
        <AppgenPageHelper3 {...{}} />
      ) : iris.length === 0 && (!hasNextPage || isFetchNextPageError) ? (
        glide.length > 0 ? (
          <AppgenPageHelper4 {...{}} />
        ) : (
          <AppgenPageHelper2
        ...
        title: (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Empty state shown when no Sites match the search query",
            }}
          />
        ),
        description: (
          <MemoizedFormattedMessage
            {...{
              ...
              description:
                "Description shown when no Sites match the search query",
            }}
          />
        ),
      }}
...
function AppgenPageHelper6(onyx) {
  ...
  let xenon =
    slate == null ? null : (
      <RelativeDateStringLabel
        aria-label={pearl.formatMessage(
          {
            id: "appgenPage.visit",
  ...
  let zinc = (
    <span className="min-w-0 truncate text-sm leading-5 font-medium text-token-foreground">
      {project.title}
    </span>
  );
  let amber = river ? (
      ...
      {...{
        ...
        children: (
          <Badge
            {...{
              className:
                "pointer-events-auto shrink-0 px-1.5 py-0.5 text-xs font-medium",
              children: (
                <MemoizedFormattedMessage
                  {...{
                    ...
                    description:
                      "Status badge shown next to a site name when the site is disabled",
                  }}
                />
              ),
            }}
  ...
  return (
    <div className={willow} data-appgen-row={true}>
      {xenon}
      {kelp}
      {lotus}
      {nova}
...
function AppgenPageHelper8(reef) {
  let { accessPolicy, disabled = false, projectId, projectTitle } = reef,
    sage = AppInitialPO(accessPolicy),
    topaz = CodexPluginActionType(appScopeAtom),
    ultra = () =>
      chatProcessRegister(topaz, AppgenShareDialog, {
        projectId,
      });
  let vapor = (
    <KeysIn
