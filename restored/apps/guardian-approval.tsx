...
export function setBindGuardianApprovalPeers(
  next: BindGuardianApprovalPeers,
): void {
  peers = next;
}
...
export function bindGuardianApproval() {
  ...
  return peers.e(() => {
    ((Jpe = peers.r(peers.Mo())),
      (Ype = `guardian_approval`),
      (Xpe = `read-only`),
      (Zpe = [
        `read-only`,
        `auto`,
      ...
      ($pe = [`custom`, `auto`, `granular`, `guardian-approvals`, `read-only`]),
      (eme = {
        type: `readOnly`,
        networkAccess: !1,
      }),
