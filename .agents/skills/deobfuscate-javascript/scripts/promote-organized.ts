...
function addRenamedImportAliases(
  ...
  for (const spec of imp.specifiers ?? []) {
    ...
    const renamed = firstAutoRenameForLocal(local, consumerRenames, imp.target);
    if (renamed && !exports[renamed]) exports[renamed] = replacement;
  }
}

/**
 * Resolve a polished local name from auto-renames.json.
 * Keys are either `local@<numericOffset>` (classic) or `local@<producerBasename>`
 * (basename-qualified, used by newer polish runs). Prefer the producer match
 * when provided; otherwise lowest numeric offset; else first basename hit.
 */
function firstAutoRenameForLocal(
  local: string,
  consumerRenames?: Record<string, string>,
  producerBasename?: string,
): string | null {
  ...
  for (const [key, renamed] of Object.entries(consumerRenames ?? {})) {
    ...
    const qualifier = key.slice(at + 1);
    if (name !== local) continue;
    const offset = Number(qualifier);
    if (Number.isFinite(offset)) {
      if (!best || offset < best.offset) best = { offset, renamed };
      continue;
    }
    if (producerBasename && qualifier === producerBasename) return renamed;
    if (!basenameHit) basenameHit = renamed;
  }
  return best?.renamed ?? basenameHit;
}
...
export function inferManualExportMap(
    ...
    traverse(ast, {
      ExportNamedDeclaration(exportPath) {
        ...
        for (const spec of exportPath.node.specifiers) {
          ...
          else if (t.isExportNamespaceSpecifier(spec) && t.isIdentifier(spec.exported))
            publicExports.push(spec.exported.name);
        }
      },
  ...
  // Prefer identity / local-name matches over index pairing — re-export
  // declaration order rarely matches the original chunk export order.
  let matchedByName = 0;
  ...
  if (matchedByName === sourceExports.length && sourceExports.length > 0) {
    return out;
  }
  if (
    matchedByName === 0 &&
    sourceExports.length === uniqueExports.length &&
