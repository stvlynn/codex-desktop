...
const SHORT_ALLOWLIST = new Set([
  ...
  "bg",
  "by",
  "cx", // ubiquitous className joiner (clsx-style)
  "do",
  "el",
  "eq", // lodash canonical comparison helpers
  "fg", // editor theme foreground (paired with bg)
  "fs",
  "gt",
  "id",
  "if",
...
const SEGMENT_MIDDLEWARE_PLUGINS_SPECIFIER =
  "@segment/analytics-next/dist/cjs/plugins/middleware";
...
const PUBLIC_NPM_VENDOR_SHIMS: Record<string, PublicNpmVendorSpecifiers> = {
  ...
  "segment-analytics-video-plugins":
    "@segment/analytics.js-video-plugins/dist/index.umd.js",
  "segment-remote-middleware":
    "@segment/analytics-next/dist/cjs/plugins/remote-middleware",
  "segment-schema-filter":
    "@segment/analytics-next/dist/cjs/plugins/schema-filter",
  "segment-legacy-video-plugins":
    "@segment/analytics-next/dist/cjs/plugins/legacy-video-plugins",
  "segment-middleware": [
    "@segment/analytics-next",
    "@segment/analytics-core",
    "@segment/facade",
  ],
  "segment-middleware-plugins": SEGMENT_MIDDLEWARE_PLUGINS_SPECIFIER,
...
> = {
  ...
  "middleware-Br1NemKK": SEGMENT_MIDDLEWARE_PLUGINS_SPECIFIER,
  "metric-helpers-CshXbF1V": [
    "@segment/analytics-core",
    "@segment/analytics-generic-utils",
  ],
  "arc-D3MQZVTw": "d3-shape",
  "ajs-destination-BeM_T5LQ": SEGMENT_AJS_DESTINATION_SPECIFIER,
  "ajs-destination-BCUeq1l3": SEGMENT_AJS_DESTINATION_SPECIFIER,
  "pkg-H9k3jTTq": ["@segment/analytics-next", "@segment/analytics-core"],
  "remote-middleware-CaiwI1u_":
    "@segment/analytics-next/dist/cjs/plugins/remote-middleware",
  "schema-filter-Dbw8php-":
    "@segment/analytics-next/dist/cjs/plugins/schema-filter",
  "legacy-video-plugins-BgbR1n5U":
    "@segment/analytics-next/dist/cjs/plugins/legacy-video-plugins",
  ...
  try {
    ...
    traverse(ast, {
      ...
      JSXIdentifier(identifierPath) {
        const name = identifierPath.node.name;
        if (name.length > 2 || SHORT_ALLOWLIST.has(name)) return;
        // Intrinsic SVG/HTML tags (`g`, `a`, …) and short JSX attribute names
        // (`d`, `x`, `y`, `r`, …) are markup, not cryptic bindings.
        if (INTRINSIC_JSX_TAGS.has(name)) return;
        if (identifierPath.parentPath?.isJSXAttribute()) return;
        counts.set(name, (counts.get(name) ?? 0) + 1);
      },
    });
  } catch {
