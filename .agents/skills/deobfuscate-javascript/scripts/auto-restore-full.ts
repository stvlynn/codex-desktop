...
import {
  extractSymbols,
  HUGE_FILE_LEDGER_LINE_THRESHOLD,
  type SymbolEntry,
} from "./extract.ts";
...
async function main(): Promise<void> {
  ...
  for (const [basename, file] of localEntries) {
    ...
    if (
      ...
      // Still record cached rows so --resume yields a complete report.
      const renamesPath = path.join(workspace, "auto-renames.json");
      let renameCount = 0;
      if (fs.existsSync(renamesPath)) {
        try {
          renameCount = Object.keys(
            JSON.parse(fs.readFileSync(renamesPath, "utf-8")) as Record<
              string,
              string
            >,
          ).length;
    ...
    const huge = lineCount >= HUGE_FILE_LEDGER_LINE_THRESHOLD;
    const sourceRel = repoRelativeSourcePath(
      file.path,
      basename,
      manifest.rootDir,
    );
    ...
    let ignoredRenames = 0;
    if (huge) {
      // Babel apply+polish on 100k+ line aggregators stalls/OOMs on 16GB hosts.
      // Keep the rename plan on disk for Stage 3 agents; ship the original body
      // as the mechanical checkpoint (always needsAgentRewrite).
      rewritten =
        `// Restored from ${sourceRel}\n` +
        `// Mechanical checkpoint (huge aggregator — rename plan in auto-renames.json; ` +
        `Babel apply/polish skipped).\n` +
        `// ${descriptionFor(basename)}\n` +
        original;
      fs.writeFileSync(polishedPath, rewritten);
      ignoredRenames = Object.keys(built.renames).length;
      prog.tick(
        1,
        `${basename} (huge — skip apply; ${ignoredRenames} renames planned)`,
      );
    } else {
      ...
      rewritten = fixLowercaseJsxComponents(
  ...
  if (parsed.values.format) {
    // Format mechanical checkpoints, but skip huge aggregators — prettier on
    // 100k+ line files is not worth the wall-clock on typical hosts.
    const formatTargets = report.files
      .filter((f) => {
        const mf = manifest.files[f.basename];
        const lines = mf?.lineCount ?? 0;
        return lines < HUGE_FILE_LEDGER_LINE_THRESHOLD;
      })
      .map((f) => f.checkpointOutput)
