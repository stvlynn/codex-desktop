...
describe("buildImportGraph (BFS)", () => {
  ...
  test("Lucide icons, locale catalogs, and Shiki stubs become npm-leaf terminals", () => {
    const rootDir = makeTmpRoot();
    const targetDir = path.join(rootDir, "decode");
    fs.mkdirSync(targetDir, { recursive: true });
    const assets = path.join(rootDir, "assets");
    fs.mkdirSync(assets, { recursive: true });
    const entry = path.join(assets, "shell-Aa11Bb22.js");
    fs.writeFileSync(
      entry,
      [
        `import { ZoomOut } from "./zoom-out-Cc33Dd44.js";`,
        `import messages from "./fr-FR-Ee55Ff66.js";`,
        `import rust from "./rust-Gg77Hh88.js";`,
        `export const x = [ZoomOut, messages, rust];`,
      ].join("\n"),
    );
    fs.writeFileSync(
      path.join(assets, "createLucideIcon-BrIFp4GW.js"),
      `export function n(e,t){return {className: a(\`lucide\`, e), children: t};}\n`,
    );
    fs.writeFileSync(
      path.join(assets, "zoom-out-Cc33Dd44.js"),
      `import { n as t, t as n } from "./createLucideIcon-BrIFp4GW.js";\nexport const ZoomOut = n("ZoomOut", []);\n`,
    );
    fs.writeFileSync(
      path.join(assets, "fr-FR-Ee55Ff66.js"),
      `export default {"CopyButton.copyTooltip":"Copier","app.sidebar.hide":"a","app.sidebar.show":"b","app.sidebar.tooltip":"c","appHeader.installUpdate":"d","composer.send":"e"};\n`,
    );
    fs.writeFileSync(
      path.join(assets, "rust-Ii99Jj00.js"),
      `export default {name:"rust",scopeName:"source.rust",patterns:[{include:"#x"}],repository:{}};\n`,
    );
    fs.writeFileSync(
      path.join(assets, "rust-Gg77Hh88.js"),
      `import { n as e, t } from "./rust-Ii99Jj00.js";\ne();\nexport { t as default };\n`,
    );

    const manifest = buildImportGraph(entry, { rootDir: assets, targetDir });
    expect(manifest.files["zoom-out-Cc33Dd44"]?.kind).toBe("npm-leaf");
    expect(manifest.files["zoom-out-Cc33Dd44"]?.vendorSpecifier).toBe(
      "lucide-react",
    );
    expect(manifest.files["fr-FR-Ee55Ff66"]?.kind).toBe("npm-leaf");
    expect(manifest.files["fr-FR-Ee55Ff66"]?.vendorSpecifier).toBe(
      "locales/fr-fr",
    );
    expect(manifest.files["rust-Gg77Hh88"]?.kind).toBe("npm-leaf");
    expect(manifest.files["rust-Gg77Hh88"]?.vendorSpecifier).toBe(
      "@shikijs/langs/rust",
    );
    // The underlying grammar body is not BFS-entered once the stub is terminal —
    // consumers already rewrite to `@shikijs/langs/rust`.
    expect(manifest.files["rust-Ii99Jj00"]).toBeUndefined();
    const local = Object.values(manifest.files).filter((f) => f.kind === "local");
    expect(local.map((f) => f.basename)).toEqual(["shell-Aa11Bb22"]);
  });
