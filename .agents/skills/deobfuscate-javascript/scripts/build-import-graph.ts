    ...
    const vendor =
      basename === entryBasename
        ...
        : classifyVendorDataChunk(basename, source, {
            readSiblingSource: (siblingBasename) => {
              const candidate = path.join(rootDir, `${siblingBasename}.js`);
...
const USAGE =
  ...
  "[--out <manifest.json>] [--treat-as-npm name,name,...|@file] " +
  ...
  "  --treat-as-npm accepts comma-separated stems and/or @file (one stem per line).";
...
async function main(): Promise<void> {
  ...
  let treatAsNpm: Set<string>;
  let forceInclude: Set<string>;
  try {
    treatAsNpm = new Set(parseNameListArg(values["treat-as-npm"]));
    forceInclude = new Set(parseNameListArg(values.include));
  } catch (err) {
    console.error((err as Error).message);
    process.exit(64);
  }
