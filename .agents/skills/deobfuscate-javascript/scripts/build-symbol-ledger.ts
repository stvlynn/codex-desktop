...
import { extractSymbols, HUGE_FILE_LEDGER_LINE_THRESHOLD, type SymbolEntry } from "./extract.ts";
import {
  writeJsonAtomic,
  type ImportEdge,
  type ImportSpec,
 ...
 *
 * Pass `raw` when the caller already extracted symbols (avoids a second Babel
 * parse on huge aggregators).
 */
export function buildLedgerFile(
  source: string,
  manifestFile: ManifestFile,
  prior?: LedgerFile,
  rawSymbols?: SymbolEntry[],
): LedgerFile {
  ...
  const lineCount = manifestFile.lineCount ?? 0;
  const huge = lineCount >= HUGE_FILE_LEDGER_LINE_THRESHOLD;
  const raw: SymbolEntry[] =
    rawSymbols ??
    extractSymbols(source, huge ? 0 : undefined, {
      programScopeOnly: huge,
      omitSameScope: huge,
    });
...
export function buildLedger(opts: BuildLedgerOptions): Ledger {
  ...
  for (const [basename, file] of Object.entries(manifest.files)) {
    ...
    const huge = lineCount >= HUGE_FILE_LEDGER_LINE_THRESHOLD;
    // Extract once — reused for ledger symbols + per-file symbols.json.
    const rawSymbols = extractSymbols(source, huge ? 0 : undefined, {
      programScopeOnly: huge,
      omitSameScope: huge,
    });
    const ledgerFile = buildLedgerFile(source, file, previous, rawSymbols);
    ledgerFiles[basename] = ledgerFile;

    // Persist per-file symbols.json (the same shape extract.ts emits).
    // Huge aggregators stay program-scope-only so the JSON stays writable.
...
const USAGE =
  ...
  "[--out <ledger.json>] [--rebuild] [--only basename1,basename2,...|@file]\n" +
  "  --only accepts comma-separated basenames and/or @file (one basename per line).";
...
async function main(): Promise<void> {
  ...
  let onlyBasenames: Set<string> | undefined;
  try {
    const onlyList = parseNameListArg(values.only);
    onlyBasenames = onlyList.length > 0 ? new Set(onlyList) : undefined;
  } catch (err) {
    console.error((err as Error).message);
    process.exit(64);
  }
