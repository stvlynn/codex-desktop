...
export function extractSymbols(
  ...
  opts: {
    /** When true, only Program-scope bindings are indexed (huge aggregators). */
    programScopeOnly?: boolean;
    /** When true, leave `sameScopeBindings` empty (avoids N² JSON blow-ups). */
    omitSameScope?: boolean;
  } = {},
): SymbolEntry[] {
  if (!source) return [];
  ...
  const programScopeOnly = opts.programScopeOnly === true;
  const omitSameScope = opts.omitSameScope === true;
  ...
  traverse(ast, {
    Scopable(path) {
      ...
      if (programScopeOnly && block.type !== "Program") return;

      const scopeStart = block.start ?? 0;
      const scopeEnd = block.end ?? source.length;
      const scopeSize = scopeEnd - scopeStart;
      const sameScopeBindings = omitSameScope
        ? []
        : Object.keys(scope.bindings).sort();
...
/** Line-count threshold where full extract OOMs on typical 16GB hosts. */
export const HUGE_FILE_LEDGER_LINE_THRESHOLD = 100_000;
