...
var WindowsPtyAgent = /** @class */ (function () {
    function WindowsPtyAgent(file, args, env, cwd, cols, rows, debug, _useConpty, _useConptyDll, conptyInheritCursor, _getConsoleProcessListOverride) {
        var _this = this;
        if (_useConptyDll === void 0) { _useConptyDll = false; }
        if (conptyInheritCursor === void 0) { conptyInheritCursor = false; }
        if (_getConsoleProcessListOverride === void 0) { _getConsoleProcessListOverride = undefined; }
        this._useConpty = _useConpty;
        this._useConptyDll = _useConptyDll;
        this._getConsoleProcessListOverride = _getConsoleProcessListOverride;
        this._pid = 0;
        this._innerPid = 0;
    ...
    WindowsPtyAgent.prototype.kill = function () {
        ...
        if (this._useConpty) {
            if (!this._useConptyDll) {
                ...
                var consoleProcessList = this._getConsoleProcessList();
                if (this._getConsoleProcessListOverride) {
                    return consoleProcessList.then(function (processList) {
                        _this._killConpty();
                        _this._killConsoleProcesses(processList);
                    });
                }
                else {
                    consoleProcessList.then(function (processList) { return _this._killConsoleProcesses(processList); });
                    this._killConpty();
                }
            }
            else {
    ...
    WindowsPtyAgent.prototype._killConsoleProcesses = function (processList) {
        processList.forEach(function (pid) {
            try {
                process.kill(pid);
            }
    ...
    WindowsPtyAgent.prototype._killConpty = function () {
        this._ptyNative.kill(this._pty, this._useConptyDll);
    ...
    WindowsPtyAgent.prototype._getConsoleProcessList = function () {
        var _this = this;
        if (this._getConsoleProcessListOverride) {
            return this._getConsoleProcessListOverride(this._innerPid).catch(function () { return [_this._innerPid]; });
        }
